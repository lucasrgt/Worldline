...
/** Public fluid-dynamics provider backed by an unmodified Beta 1.7.3 server. */
public final class B173ServerFluidDynamicsTestRuntimeProvider implements TestRuntimeProvider {
    public static final String RUNTIME_ID = "b1.7.3-server-fluid-dynamics";
    ...
    @Override public TestRuntimeSession open(TestRuntimeRequest request) throws Exception {
        ...
        if (request.modPath() != null) throw new IllegalArgumentException(
                "fluid dynamics provider does not load server mods");
        if (request.seed() != B173LifecycleArena.SEED) throw new IllegalArgumentException(
                "fluid dynamics provider requires seed " + B173LifecycleArena.SEED);
        B173LifecycleLoadout loadout = B173LifecycleLoadout.from(request);
        ...
        Path workspace = Files.createTempDirectory(base, "b173-fluid-dynamics-");
        int port = freePort();
        ...
        try {
            server.boot();
            client = B173FluidDynamicsArena.open(
