...
/** Public collision TestKit provider backed by the unmodified b1.7.3 server. */
public final class B173ServerCollisionTestRuntimeProvider implements TestRuntimeProvider {
    public static final String RUNTIME_ID = "b1.7.3-server-collision";
    ...
    @Override public TestRuntimeSession open(TestRuntimeRequest request) throws Exception {
        ...
        if (request.modPath() != null) throw new IllegalArgumentException(
                "collision provider does not load server mods");
        if (request.seed() != B173CollisionArena.SEED) throw new IllegalArgumentException(
                "collision provider requires seed " + B173CollisionArena.SEED);
        B173CollisionLoadout loadout = B173CollisionLoadout.from(request);
        ...
        Path workspace = Files.createTempDirectory(base, "b173-collision-");
        int port = freePort();
        B173DedicatedServer server = new B173DedicatedServer(settings.serverJar, workspace,
                port, request.seed(), settings.timeout, 3, true);
        B173CollisionArena.Start start = null;
        try {
            server.boot(); start = B173CollisionArena.open(
                    server, workspace, port, settings.timeout, loadout);
            ...
            Supplier<B173WireClient> sessions = new Supplier<B173WireClient>() {
                @Override public B173WireClient get() {
                    return new B173WireClient("127.0.0.1", reconnectPort,
                            B173CollisionArena.USERNAME, settings.timeout);
                }
            };
            B173BlockCollisionDriver driver = new B173BlockCollisionDriver(
                    server, start.client, start.origin, sessions);
    ...
    private static final class Session implements TestRuntimeSession {
        private final B173BlockCollisionDriver driver;
        private final B173DedicatedServer server;
        private boolean closed;
        Session(B173BlockCollisionDriver driver, B173DedicatedServer server) {
            this.driver = driver; this.server = server;
        }
        @Override public <T> T capability(Class<T> type) {
            if (type == null) throw new NullPointerException("type");
            if (type == BlockCollisionDriver.class) return type.cast(driver);
