...
public final class B173GroundFloraPhysicalScenarioFactory {
    ...
    private B173GroundFloraPhysicalScenarioFactory() {
    }
    ...
    public static List<BlockStateDomainScenario> stateDomains() {
        ...
        for (B173GroundFloraPhysicalCatalog.Subject subject
                : B173GroundFloraPhysicalCatalog.singletonStateSubjects()) {
    ...
    public static List<BlockCollisionScenario> collisions() {
        ...
        for (B173GroundFloraPhysicalCatalog.Subject subject
                : B173GroundFloraPhysicalCatalog.subjects()) rows.add(collision(subject));
        return Collections.unmodifiableList(rows);
    }
    ...
    public static List<BlockLightScenario> lights() {
        ...
        for (B173GroundFloraPhysicalCatalog.Subject subject
                : B173GroundFloraPhysicalCatalog.subjects()) rows.add(light(subject));
        return Collections.unmodifiableList(rows);
    }
    ...
    private static BlockLightScenario light(B173GroundFloraPhysicalCatalog.Subject subject) {
        return new BlockLightScenario(subject.scenario(), claim(subject, "light-behavior"),
                ...
                Collections.singletonList(new BlockLightProbe("source", B173LightArena.SOURCE,
                        ...
                        new BlockLightExpectation(subject.state(), 0, subject.sky))),
                subject.support);
    }

    private static BlockPosition support(
            B173GroundFloraPhysicalCatalog.Subject subject, int index) {
        BlockPosition pad = B173StateDomainArena.SUPPORTS.get(index);
        return subject.support.equals(B173PhysicalSupport.STONE) ? pad : BlockFace.UP.adjacent(pad);
    }

    private static BlockLifecycleSlot slot(
            B173GroundFloraPhysicalCatalog.Subject subject, int count) {
