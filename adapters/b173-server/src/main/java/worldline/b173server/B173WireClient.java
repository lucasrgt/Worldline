...
public final class B173WireClient implements PaintingObservationSession {
    ...
    @Override public worldline.api.RemoteDroppedItem awaitDroppedItem(worldline.api.RemoteItemStack expected) { return B173ItemAccess.awaitDroppedItem(channel(), expected); } @Override public void useSelectedItemInAir() { B173ItemAccess.useSelectedItemInAir(channel()); } @Override public worldline.api.RemoteObjectSpawn awaitObjectSpawn(int type) { return B173ItemAccess.awaitObjectSpawn(channel(), type); } @Override public worldline.api.RemotePaintingSpawn awaitPaintingSpawn() { return B173PaintingAccess.await(this); } @Override public int awaitPaintingDestroy(int entity) { return B173PaintingAccess.awaitDestroy(this, entity); }
    public java.util.List<worldline.api.RemoteDroppedItem> droppedItems() {
        return channel().inbound().dropped().snapshot();
    }
    @Override public worldline.api.RemoteItemCollection awaitItemCollection(worldline.api.RemoteDroppedItem expected, String username) { return B173ItemAccess.awaitItemCollection(channel(), expected, username); } public B173EntityVelocity awaitVelocity(int entity) { try { return channel().inbound().awaitVelocity(entity); } catch (IOException error) { throw new IllegalStateException("entity velocity observation absent", error); } }
