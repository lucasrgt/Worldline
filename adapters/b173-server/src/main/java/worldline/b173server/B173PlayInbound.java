...
final class B173PlayInbound {
    ...
    void skip(int packet) throws IOException {
        ...
        if (packet == 53) { cache.singleBlock(input); return; } if(packet==24){mobs.spawn(input);return;} if(packet>=30&&packet<=34){mobs.update(packet,input);return;} if (packet == 60) { if (explosion != null) throw new IOException("unconsumed explosion observation"); explosion = cache.explosion(input); return; }
