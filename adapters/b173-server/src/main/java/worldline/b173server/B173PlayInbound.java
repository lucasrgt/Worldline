...
final class B173PlayInbound {
    ...
    private Correction correction; private int dimension; private long respawnEpoch; private worldline.api.RemoteExplosion explosion; private final B173MobTracker mobs=new B173MobTracker(); private final B173ObjectTracker objects=new B173ObjectTracker(); private final B173PlayWaits waits=new B173PlayWaits(this);
    ...
    void skip(int packet) throws IOException {
        ...
        if (packet == 53) { cache.singleBlock(input); return; } if(packet==23){objects.spawn(input);return;} if(packet==24){mobs.spawn(input);return;} if(packet>=30&&packet<=34){mobs.update(packet,input);return;} if (packet == 60) { if (explosion != null) throw new IOException("unconsumed explosion observation"); explosion = cache.explosion(input); return; }
    ...
    B173ObjectTracker objects() { return objects; }
