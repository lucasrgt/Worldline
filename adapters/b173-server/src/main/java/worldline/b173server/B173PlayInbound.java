...
final class B173PlayInbound {
    ...
    B173PlayInbound(DataInputStream input, DataOutputStream output, int timeoutMillis,
        ...
        this.items = new B173ItemInbound(localEntityId, localUsername, output, mobs, paintings); if (dimension != 0 && dimension != -1) throw new IOException("invalid initial dimension"); this.dimension = dimension; }

    void skip(int packet) throws IOException {
        int sequence = protocol.next();
        if (packet == 0) { protocol.keepAlive(); synchronized (output) {
    ...
    B173ObjectTracker objects() { return objects; } B173VelocityTracker velocities() { return velocities; } B173BedTracker beds() { return beds; } B173NoteEvent notes() { return notes; } B173SignTracker signs() { return signs; } B173Protocol14Tracker protocol() { return protocol; } B173PaintingTracker paintings() { return paintings; }
