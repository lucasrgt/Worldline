...
final class B173ItemInbound {
    ...
    worldline.api.RemoteIncomingHit awaitIncomingHit(int health, Pump pump) throws IOException { worldline.api.RemoteIncomingHit value = combat.takeIncoming(health); if (value != null) return value; for (int count = 0; count < 8192; count++) { pump.one(); value = combat.takeIncoming(health); if (value != null) return value; } throw new IOException("incoming combat health absent"); } worldline.api.RemotePeerSwing awaitPeerSwing(String username, Pump pump) throws IOException { combat.expectSwing(username); for (int count = 0; count < 8192; count++) { pump.one(); worldline.api.RemotePeerSwing value = combat.takeSwing(); if (value != null) return value; } throw new IOException("peer swing absent"); } int health() { return combat.health(); } int awaitHealth(int expected, Pump pump) throws IOException { if (expected < 0 || expected > 20) throw new IllegalArgumentException("invalid expected health"); for (int count=0;count<8192;count++){if(combat.health()==expected)return expected;pump.one();}throw new IOException("expected local health absent"); }
    ...
    boolean despawned(RemoteDroppedItem expected) { return dropped.despawned(expected); }
    boolean collected(RemoteDroppedItem expected) { return dropped.collected(expected); }
