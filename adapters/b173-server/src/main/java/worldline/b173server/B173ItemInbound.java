...
final class B173ItemInbound {
    ...
    worldline.api.RemoteIncomingHit awaitIncomingHit(int health, Pump pump) throws IOException { worldline.api.RemoteIncomingHit value = combat.takeIncoming(health); if (value != null) return value; for (int count = 0; count < 8192; count++) { pump.one(); value = combat.takeIncoming(health); if (value != null) return value; } throw new IOException("incoming combat health absent"); } worldline.api.RemotePeerSwing awaitPeerSwing(String username, Pump pump) throws IOException { combat.expectSwing(username); for (int count = 0; count < 8192; count++) { pump.one(); worldline.api.RemotePeerSwing value = combat.takeSwing(); if (value != null) return value; } throw new IOException("peer swing absent"); }
