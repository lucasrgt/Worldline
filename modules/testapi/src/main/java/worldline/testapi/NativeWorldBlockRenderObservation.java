...
/** Renderer-neutral evidence from one native world block-render invocation. */
public final class NativeWorldBlockRenderObservation {
    private final String subject, frameSha256, work;
    private final int legacyId, metadata, renderType, geometryPixels;

    public NativeWorldBlockRenderObservation(String subject, int legacyId, int metadata,
            int renderType, int geometryPixels, String frameSha256, String work) {
        new NativeWorldBlockRenderSubject(subject, legacyId, metadata, renderType);
        if (geometryPixels < 1 || geometryPixels > 128 * 128) {
            throw new IllegalArgumentException("invalid native world-render coverage");
        }
        if (frameSha256 == null || !frameSha256.matches("[0-9a-f]{64}")) {
            throw new IllegalArgumentException("invalid native world-render signature");
        }
        if (work == null || !work.matches("draw-calls=1,pixels=[1-9][0-9]*")) {
            throw new IllegalArgumentException("invalid native world-render work");
        }
