...
/** Public row for tick stability followed by causal removal of the supporting block. */
public final class BlockSupportLossScenario {
    private final String id;
    private final BlockConformanceCase tickPolicy, neighborResponse;
    private final BlockPosition support;
    private final BlockState supportState, targetState;
    private final BlockLifecycleSlot placementSlot, breakSlot;
    private final int tickWindow, breakTicks, observationTicks;

    public BlockSupportLossScenario(String id, BlockConformanceCase tickPolicy,
            BlockConformanceCase neighborResponse, BlockPosition support,
            BlockState supportState, BlockState targetState,
        ...
        if (!tickPolicy.profile().subject().equals(neighborResponse.profile().subject())) {
            throw new IllegalArgumentException("support-loss claims target different subjects");
        }
        if (id == null || !id.matches("[a-z0-9][a-z0-9._-]{0,127}")) {
            throw new IllegalArgumentException("invalid support-loss scenario id");
        }
        if (tickWindow < 1 || tickWindow > 24_000 || breakTicks < 0
                || observationTicks < 1 || observationTicks > 1_200) {
            throw new IllegalArgumentException("invalid support-loss tick bounds");
        }
        this.id = id;
        ...
        if (placementSlot.hotbarSlot() == breakSlot.hotbarSlot()) {
            throw new IllegalArgumentException("support-loss slots overlap");
        }
    }
