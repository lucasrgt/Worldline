...
/** Repository launcher for the stable Worldline replay CLI. */
public final class Replay {
    ...
    private int execute(String[] arguments) throws Exception {
        if (arguments.length != 2 || !arguments[0].equals("replay")) {
            System.err.println("usage: java tools/replay/Replay.java replay <bundle.wlrb>"); return 2;
        }
        int inputs = new ProcessBuilder("java", "tools/harness/RuntimeCheck.java", "--required")
        ...
        List<Path> paths = new ArrayList<>(Arrays.asList(classes.resolve("cli"),
                classes.resolve("reproduction"), classes.resolve("api"), classes.resolve("kernel"),
                classes.resolve("trace"), client.resolve("adapter-classes"),
        ...
        try (Stream<Path> libraries = Files.walk(workspace.resolve("libraries"))) {
