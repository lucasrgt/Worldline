...
/** Runs two fresh same-plan index-four sibling-cell recovery replicas. */
public final class SiblingCellMembershipRecoveryCycle {
  private static final String ID = "m89-sibling-cell-membership-recovery";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/SiblingCellMembershipRecoveryCycle.java " + ID);
      System.exit(2);
    }
    try {
      new SiblingCellMembershipRecoveryCycle().execute();
    } catch (Exception e) {
      System.err.println("Sibling-cell membership recovery failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    require(value("replicas").equals("2") && value("warmup.frames").equals("300")
            ...
            && value("minimum.millis").equals("12000")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m89.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 8908901;
    ...
    if (diagnostic) {
      System.out.println("M89 diagnostic replica passed; qualification not attempted");
    ...
    String trace =
        "v1|design=2-fresh-same-plan-nonce-index4-sibling-cell-membership-recovery-replicas|fixture=constant16-synced+client-marker+exact-camera|server-closure=aero-free|frame-limit=vanilla-max0+aero-pacer-off-runtime-gated|warm-path=enqueue16+flush2+cached4+pageCalls4+direct0+render0/list0+rebuild0|remove=request-after300+server-remove-index4+ack+client-air+membership16to15+pageCalls4to3+direct0to1+render/list0to1+rebuild0|restore=request-30-records-after-remove+server-restore-index4+buffered-state+ack+membership15to16+pageCalls3to4+direct1to0+render/list1to0+rebuild1|window=M74-min720intervals+12s|capture=M78-primitive-spans+page-counters+post-seal-52-byte-recovery-sidecar|per-record=identity16to15to16+state16/maskffff|stats=descriptive-transition-dynamic|arbitrary-addition-other-cells-repeat-stale-cleanup-uninstrumented-cost-causality-regression-historical-lag=not-claimed|shutdown=clean";
    String signature = sha256(trace);
    require(signature.equals(value("expected.signature")), "M89 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M89 sibling-cell membership recovery passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m89-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      require(marker(complete, "samples") == paged.count
              ...
              && marker(changed, "removeRequest") == change.removeRequest
              && marker(changed, "removeEvent") == change.removeEvent
              && marker(changed, "restoreRequest") == change.restoreRequest
              && marker(changed, "restoreEvent") == change.restoreEvent
      ...
      String scene = unique(serverText, "[WorldlineCensus] scene "),
             removed = unique(serverText, "[WorldlineRecovery] removed "),
             restored = unique(serverText, "[WorldlineRecovery] restored ");
      ...
      require(marker(removed, "x") == x && marker(removed, "y") == y
              && marker(removed, "z") == z + 1 && marker(removed, "nonce") == nonce
              && marker(restored, "x") == x && marker(restored, "y") == y
              && marker(restored, "z") == z + 1 && marker(restored, "nonce") == nonce
      ...
      require(serverText.indexOf("[WorldlineCensus] scene ")
                  < serverText.indexOf("[WorldlineRecovery] removed ")
              && serverText.indexOf("[WorldlineRecovery] removed ")
                  < serverText.indexOf("[WorldlineRecovery] restored ")
  ...
  private Recovery parseRecovery(Path file, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 52 && in.readInt() == 0x574c3839 && in.readInt() == 1
              && in.readInt() == 52 && in.readInt() == nonce,
          "recovery schema drift");
      int x = in.readInt(), y = in.readInt(), z = in.readInt(), removeRequest = in.readInt(),
          removeEvent = in.readInt(), restoreRequest = in.readInt(), restoreEvent = in.readInt(),
          removeRebuild = in.readInt(), restoreRebuild = in.readInt();
      require(in.read() == -1 && removeRequest >= 300 && removeEvent >= removeRequest
              && restoreRequest >= removeEvent + 30 && restoreEvent >= restoreRequest
              && removeRebuild == 0 && restoreRebuild == 1,
          "recovery event drift");
      return new Recovery(x, y, z, removeRequest, removeEvent, restoreRequest, restoreEvent);
    }
  }
  private Census parseCensus(Path file, Recovery change, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      require(x == change.x && y == change.y && z == change.z && change.restoreEvent < count
      ...
      for (int i = 0; i < count; i++) {
        ...
        int renders = in.readInt(), lists = in.readInt(), visible = in.readInt(),
            ...
            expected = i < change.removeEvent || i >= change.restoreEvent ? 16 : 15,
            direct = expected == 16 ? 0 : 1;
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        int rc = in.readUnsignedShort(), qc = in.readUnsignedShort(), fc = in.readUnsignedShort(),
            ...
            members = i < change.removeEvent || i >= change.restoreEvent ? 16 : 15,
            pages = members == 16 ? 4 : 3, expectedDirect = members == 16 ? 0 : 1,
            expectedRebuild = i == change.restoreEvent ? 1 : 0;
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                ...
                && cached == 4,
            "recovery paged record drift at " + i + " calls=" + rc + "/" + qc + "/" + fc + " state="
      ...
      return new Paged(
          count, renderer, queue, flush, zeroFlush, change.removeEvent, change.restoreEvent);
    }
  }
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m89-build-" + System.nanoTime());
    try {
  ...
  private void verifyBoundary() throws IOException {
    for (Path base :
        Arrays.asList(root.resolve("smokes/m74-complete-aero-census/runtime-src/worldline/m74"),
            smoke.resolve("runtime-src/worldline/m74"), smoke.resolve("runtime-src/worldline/m89")))
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M89 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Recovery {
    final int x, y, z, removeRequest, removeEvent, restoreRequest, restoreEvent;
    Recovery(int x, int y, int z, int rr, int re, int sr, int se) {
      this.x = x;
      this.y = y;
      this.z = z;
      removeRequest = rr;
  ...
  private static final class Paged {
    final int count, zeroFlush, removeEvent, restoreEvent;
    final long[] renderer, queue, flush;
    Paged(int n, long[] r, long[] q, long[] f, int zero, int remove, int restore) {
      count = n;
      renderer = r;
      queue = q;
      flush = f;
    ...
    String summary() {
      long[] wr = before(renderer, removeEvent), wq = before(queue, removeEvent),
             wf = before(flush, removeEvent);
      return "preRendererNs=" + q(wr, .5) + "/" + q(wr, .95) + "/" + q(wr, .99) + ",preEnqueueNs="
          + q(wq, .5) + "/" + q(wq, .95) + "/" + q(wq, .99) + ",preFlushNs=" + q(wf, .5) + "/"
          + q(wf, .95) + "/" + q(wf, .99) + ",eventNs=" + renderer[removeEvent] + "/"
          + renderer[restoreEvent] + ",flushZero=" + zeroFlush;
    }
  }
  private static final class Arm {
    ...
    String summary() {
      return "samples=" + census.count + ",requests=" + change.removeRequest + "/"
          + change.restoreRequest + ",events=" + change.removeEvent + "/" + change.restoreEvent
