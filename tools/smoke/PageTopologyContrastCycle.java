...
public final class PageTopologyContrastCycle {
  private static final String ID = "m83-page-topology-contrast";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/PageTopologyContrastCycle.java " + ID);
      System.exit(2);
    }
    try {
      new PageTopologyContrastCycle().execute();
    } catch (Exception e) {
      System.err.println("Page topology contrast failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("topologies").equals("same,cross") && value("plan.x").equals("10")
            && value("plan.y").equals("65") && value("plan.z").equals("31")
    ...
    recreate(build);
    buildAero(checkout);
    int diagnostic = Integer.getInteger("worldline.m83.diagnostic.topology", 0);
    require(
        diagnostic == 0 || diagnostic == 1 || diagnostic == 2, "invalid M83 diagnostic topology");
    int[] topologies = diagnostic == 0 ? new int[] {1, 2} : new int[] {diagnostic};
    int nonce = 8308301;
    ...
    for (int topology : topologies) {
      verifyCheckout(checkout);
      Arm arm =
          run(checkout, build.resolve(topology == 1 ? "same" : "cross"), topology, nonce, plan);
      arms.add(arm);
      verifyCheckout(checkout);
    }
    if (diagnostic != 0) {
      System.out.println("M83 diagnostic arm passed; qualification not attempted");
      System.out.println("  " + arms.get(0).summary());
      return;
    }
    require(arms.size() == 2, "M83 arm count drift");
    Arm same = arms.get(0), cross = arms.get(1);
    for (Arm a : arms)
      require(a.nonce == nonce && a.x == plan.x && a.y == plan.y && a.z == plan.z
              && a.raw == same.raw && a.paged.prePages == 2 && a.paged.postPages == 2,
          "arm fixture/page drift");
    require(same.topology == 1 && cross.topology == 2 && same.change.rebuilds == 1
            && cross.change.rebuilds == 2,
        "arm topology drift");
    String trace =
    ...
    require(signature.equals(value("expected.signature")), "M83 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\narms=2\nserver.jvm=2\nclient.jvm=2\nsame=" + same.summary()
            + "\ncross=" + cross.summary() + "\ntrace=" + trace + "\nsignature=" + signature + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M83 page topology contrast passed");
    System.out.println("  same: " + same.summary());
    System.out.println("  cross: " + cross.summary());
  ...
  private Arm run(Path checkout, Path workspace, int topology, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m83-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      require(marker(complete, "samples") == paged.count
              ...
              && marker(changed, "topology") == topology && marker(changed, "targets") == 2
      ...
      require(marker(removed, "topology") == topology && marker(removed, "targets") == 2
  ...
  private Topology parseTopology(Path file, int nonce, int expectedTopology) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 44 && in.readInt() == 0x574c3833 && in.readInt() == 1
  ...
  private Census parseCensus(Path file, Topology change, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && renders == 0 && lists == 0 && visible > 0
                && calls == (i < change.event ? 16 : 14) && state == 0x1010 && mask == 0xffff,
  ...
  private Paged parsePaged(Path file, Census census, Topology change, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      int prePages = 2, postPages = 2, zeroFlush = 0;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m83-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M83 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Arm {
    ...
    String summary() {
      return "topology=" + (topology == 1 ? "same" : "cross") + ",samples=" + census.count
