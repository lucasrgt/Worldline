...
/** Runs two fresh same-plan sequential two-cell recovery replicas. */
public final class ThirdMemberDepletionRecoveryCycle {
  private static final String ID = "m92-third-member-depletion-recovery";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/ThirdMemberDepletionRecoveryCycle.java " + ID);
      System.exit(2);
    }
    try {
      new ThirdMemberDepletionRecoveryCycle().execute();
    } catch (Exception e) {
      System.err.println("Larger-page depletion recovery failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("replicas").equals("2") && value("cycles").equals("3")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m92.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 9209201;
    ...
    if (diagnostic) {
      System.out.println("M92 diagnostic replica passed; qualification not attempted");
    ...
    String trace =
        "v1|design=2-fresh-same-plan-nonce-larger-page-three-cell-depletion-recovery-replicas|fixture=constant16-synced+client-marker+exact-camera|server-closure=aero-free|frame-limit=vanilla-max0+aero-pacer-off-runtime-gated|warm-path=enqueue16+flush2+cached4+pageCalls4+direct0+render0/list0+rebuild0|sequence=remove-index1+remove-index2+remove-index3+restore-index3+restore-index2+restore-index1-with-ordinal+operation+index-bound-ack/state|spacing=request1-after300+each-next-request-30-records-after-prior-event|membership=16to15to14to13to14to15to16|all-transitions=pageCalls4+direct0+render/list0+rebuild1|window=M74-min720intervals+12s|capture=M78-primitive-spans+page-counters+post-seal-100-byte-recovery-sidecar|per-record=state16/maskffff|stats=descriptive-transition-dynamic|page-emptying-threshold-crossing-concurrency-stale-cleanup-uninstrumented-cost-causality-regression-historical-lag=not-claimed|shutdown=clean";
    String signature = sha256(trace);
    require(signature.equals(value("expected.signature")), "M92 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M92 third-member depletion recovery passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m92-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      String scene = unique(serverText, "[WorldlineCensus] scene "),
             removed1 = unique(serverText, "[WorldlineRecovery] removed ordinal=1 "),
             removed2 = unique(serverText, "[WorldlineRecovery] removed ordinal=2 "),
             removed3 = unique(serverText, "[WorldlineRecovery] removed ordinal=3 "),
             restored3 = unique(serverText, "[WorldlineRecovery] restored ordinal=4 "),
             restored2 = unique(serverText, "[WorldlineRecovery] restored ordinal=5 "),
             restored1 = unique(serverText, "[WorldlineRecovery] restored ordinal=6 ");
      ...
      require(sameCell(removed1, x, y, z, nonce, 1) && sameCell(removed2, x, y, z, nonce, 2)
              && sameCell(removed3, x, y, z, nonce, 3) && sameCell(restored3, x, y, z, nonce, 3)
              && sameCell(restored2, x, y, z, nonce, 2) && sameCell(restored1, x, y, z, nonce, 1)
      ...
      require(serverText.indexOf("[WorldlineCensus] scene ")
                  < serverText.indexOf("[WorldlineRecovery] removed ordinal=1 ")
              && serverText.indexOf("[WorldlineRecovery] removed ordinal=1 ")
                  < serverText.indexOf("[WorldlineRecovery] removed ordinal=2 ")
              && serverText.indexOf("[WorldlineRecovery] removed ordinal=2 ")
                  < serverText.indexOf("[WorldlineRecovery] removed ordinal=3 ")
              && serverText.indexOf("[WorldlineRecovery] removed ordinal=3 ")
                  < serverText.indexOf("[WorldlineRecovery] restored ordinal=4 ")
              && serverText.indexOf("[WorldlineRecovery] restored ordinal=4 ")
                  < serverText.indexOf("[WorldlineRecovery] restored ordinal=5 ")
              && serverText.indexOf("[WorldlineRecovery] restored ordinal=5 ")
                  < serverText.indexOf("[WorldlineRecovery] restored ordinal=6 ")
  ...
  private Recovery parseRecovery(Path file, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 100 && in.readInt() == 0x574c3932 && in.readInt() == 1
              && in.readInt() == 100 && in.readInt() == nonce,
          "recovery schema drift");
      int x = in.readInt(), y = in.readInt(), z = in.readInt();
      int[] requests = new int[6], events = new int[6];
      for (int i = 0; i < 6; i++) {
        requests[i] = in.readInt();
        events[i] = in.readInt();
        int index = in.readInt();
        require(index == new int[] {1, 2, 3, 3, 2, 1}[i], "recovery cell index drift " + i);
      }
      for (int i = 0; i < 6; i++)
        require(requests[i] >= (i == 0 ? 300 : events[i - 1] + 30) && events[i] >= requests[i],
            "recovery event drift " + i);
      require(in.read() == -1, "recovery trailing bytes");
      return new Recovery(x, y, z, requests, events);
    }
  }
  private Census parseCensus(Path file, Recovery change, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      require(x == change.x && y == change.y && z == change.z && change.events[5] < count
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        int rc = in.readUnsignedShort(), qc = in.readUnsignedShort(), fc = in.readUnsignedShort(),
            ...
            members = change.members(i), expectedDirect = change.direct(i), pages = 4,
            expectedRebuild = i == change.events[0] || i == change.events[1]
                || i == change.events[2] || i == change.events[3] || i == change.events[4]
                || i == change.events[5]
            ? 1
            : 0;
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m92-build-" + System.nanoTime());
    try {
  ...
  private void verifyBoundary() throws IOException {
    for (Path base :
        Arrays.asList(root.resolve("smokes/m74-complete-aero-census/runtime-src/worldline/m74"),
            smoke.resolve("runtime-src/worldline/m74"), smoke.resolve("runtime-src/worldline/m92")))
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M92 cleanup failed " + t, e);
    }
  }
  ...
  private boolean sameCell(String r, int x, int y, int z, int nonce, int index) {
    return marker(r, "index") == index && marker(r, "x") == x && marker(r, "y") == y + index
        && marker(r, "z") == z && marker(r, "nonce") == nonce;
  }
  ...
  private static final class Recovery {
    ...
    Recovery(int x, int y, int z, int[] r, int[] e) {
      this.x = x;
      this.y = y;
      this.z = z;
      requests = r;
  ...
  private static final class Paged {
    ...
    String summary() {
      ...
      return "preRendererNs=" + q(wr, .5) + "/" + q(wr, .95) + "/" + q(wr, .99) + ",preEnqueueNs="
          ...
          + renderer[events[4]] + "/" + renderer[events[5]] + ",flushZero=" + zeroFlush;
    }
  }
