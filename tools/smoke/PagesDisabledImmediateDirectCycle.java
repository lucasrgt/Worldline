...
/** Runs two fresh, same-plan real Aero pages-disabled direct replicas. */
public final class PagesDisabledImmediateDirectCycle {
  private static final String ID = "m103-pages-disabled-immediate-direct";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/PagesDisabledImmediateDirectCycle.java " + ID);
      System.exit(2);
    }
    try {
      new PagesDisabledImmediateDirectCycle().execute();
    } catch (Exception e) {
      System.err.println("Pages-disabled immediate-direct failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    require(value("replicas").equals("2") && value("warmup.frames").equals("300")
            ...
            && value("aero.cell.pages").equals("false"),
        "M103 design drift");
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m103.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 10301001;
    ...
    if (diagnostic) {
      System.out.println("M103 diagnostic replica passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M103 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M103 pages-disabled immediate-direct passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m103-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      require(count(client, "[WorldlinePaged] armed ") == 1
              && unique(client, "[WorldlinePaged] armed ")
                  .equals("marker=true pages=false fpsLimit=0 aeroFramePacing=false")
  ...
  private Paged parsePaged(Path file, Census census, int nonce) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      require(in.readInt() == 0x574c6700 && in.readInt() == 1 && in.readInt() == 44
      ...
      int pages = 0, zeroFlush = 0, firstEvicted = -1, priorEvicted = -1, immediateRecords = 0;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                && rc == 16 && qc == 16 && fc == 2 && reserved == 0 && queued == 0 && pageCalls == 0
                && direct == 0 && rebuilds == 0 && cached == pages && evicted == 0
                && immediate == 16,
            "pages-disabled record drift at " + i + " time=" + renderer[i] + "/" + queue[i] + "/"
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m103-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M103 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Paged {
    ...
    Paged(
        int n, int p, long[] r, long[] q, long[] f, int zero, int first, int last, int immediate) {
      count = n;
      pages = p;
      renderer = r;
      queue = q;
    ...
    String summary() {
      return "pages=" + pages + ",immediateRecords=" + immediateRecords + ",evicted=" + firstEvicted
