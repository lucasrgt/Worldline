...
public final class PageCapacityThrashCycle {
  private static final String ID = "m95-page-capacity-thrash";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/PageCapacityThrashCycle.java " + ID);
      System.exit(2);
    }
    try {
      new PageCapacityThrashCycle().execute();
    } catch (Exception e) {
      System.err.println("Paged stage timing failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    require(value("replicas").equals("2") && value("warmup.frames").equals("300")
            ...
            && value("minimum.millis").equals("12000") && value("max.cached.pages").equals("3")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m95.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 9509501;
    ...
    if (diagnostic) {
      System.out.println("M95 diagnostic replica passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M95 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M95 page-capacity thrash passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m95-" + ProcessHandle.current().pid() + "-"
  ...
  private Paged parsePaged(Path file, Census census, int nonce) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      require(in.readInt() == 0x574c3935 && in.readInt() == 1 && in.readInt() == 44
      ...
      int pages = 3, zeroFlush = 0, firstEvicted = -1, priorEvicted = -1;
      int[] rebuildModes = new int[5];
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                ...
                && pageCalls == 4 && direct == 0 && rebuilds >= 1 && rebuilds <= 4
                && cached == pages && evicted > 0
                && (priorEvicted < 0 || evicted == priorEvicted + rebuilds),
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m95-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M95 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Paged {
    final int count, pages, zeroFlush, firstEvicted, lastEvicted;
    final long[] renderer, queue, flush;
    final int[] rebuildModes;
    Paged(int n, int p, long[] r, long[] q, long[] f, int zero, int first, int last, int[] modes) {
      count = n;
      pages = p;
      renderer = r;
      queue = q;
    ...
    String summary() {
      return "pages=" + pages + ",rebuildModes=1:" + rebuildModes[1] + "/2:" + rebuildModes[2]
          + "/3:" + rebuildModes[3] + "/4:" + rebuildModes[4] + ",evicted=" + firstEvicted + ".."
