...
/** Runs fresh same-plan, constant-three-member one/three-page arms. */
public final class FourPageTopologyContrastCycle {
  private static final String ID = "m84-four-page-topology-contrast";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/FourPageTopologyContrastCycle.java " + ID);
      System.exit(2);
    }
    try {
      new FourPageTopologyContrastCycle().execute();
    } catch (Exception e) {
      System.err.println("Four-page topology contrast failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("topologies").equals("one,three") && value("plan.x").equals("10")
            && value("plan.y").equals("77") && value("plan.z").equals("29")
            ...
            && value("aero.rebuilds.per.frame").equals("8") && value("fps.limit").equals("0")
    ...
    recreate(build);
    buildAero(checkout);
    int diagnostic = Integer.getInteger("worldline.m84.diagnostic.topology", 0);
    require(
        diagnostic == 0 || diagnostic == 1 || diagnostic == 3, "invalid M84 diagnostic topology");
    int[] topologies = diagnostic == 0 ? new int[] {1, 3} : new int[] {diagnostic};
    int nonce = 8408401;
    List<Arm> arms = new ArrayList<>();
    Plan plan = new Plan(Integer.parseInt(value("plan.x")), Integer.parseInt(value("plan.y")),
        Integer.parseInt(value("plan.z")));
    for (int topology : topologies) {
      verifyCheckout(checkout);
      Arm arm =
          run(checkout, build.resolve(topology == 1 ? "one" : "three"), topology, nonce, plan);
      arms.add(arm);
      verifyCheckout(checkout);
    }
    if (diagnostic != 0) {
      System.out.println("M84 diagnostic arm passed; qualification not attempted");
      System.out.println("  " + arms.get(0).summary());
      return;
    }
    require(arms.size() == 2, "M84 arm count drift");
    Arm same = arms.get(0), cross = arms.get(1);
    for (Arm a : arms)
      require(a.nonce == nonce && a.x == plan.x && a.y == plan.y && a.z == plan.z
              && a.raw == same.raw && a.paged.prePages == 3 && a.paged.postPages == 3,
          "arm fixture/page drift");
    require(same.topology == 1 && cross.topology == 3 && same.change.rebuilds == 1
            && cross.change.rebuilds == 3,
        "arm topology drift");
    String trace =
    ...
    require(signature.equals(value("expected.signature")), "M84 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\narms=2\nserver.jvm=2\nclient.jvm=2\nsame=" + same.summary()
            + "\nthree=" + cross.summary() + "\ntrace=" + trace + "\nsignature=" + signature + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M84 four-page topology contrast passed");
    System.out.println("  same: " + same.summary());
    System.out.println("  three: " + cross.summary());
    System.out.println("  signature: " + signature);
  }
  private Arm run(Path checkout, Path workspace, int topology, int nonce, Plan plan)
      throws Exception {
    Files.createDirectories(workspace);
    int port = freePort(), timeout = Integer.parseInt(value("timeout.seconds"));
    Path base = root.resolve(".worldline/worktrees/m84-" + ProcessHandle.current().pid() + "-"
             + topology + "-" + System.nanoTime()),
    ...
    try {
      ...
      Path init = root.resolve(value("runner")), serverGame = workspace.resolve("server"),
           ...
           topologyFile = clientGame.resolve("topology.bin");
      server = startServer(serverTree.resolve("stationapi/test-bare"),
          command(serverTree, init, "server", serverGame, port, nonce, topology, null, null, null,
              plan),
          timeout);
      String client = runGradle(clientTree.resolve("stationapi/test-bare"),
          command(clientTree, init, "client", clientGame, port, nonce, topology, censusFile,
              pagedFile, topologyFile, null),
          timeout);
      ...
      require(count(client, "[WorldlinePaged] armed ") == 1
              ...
              && count(client, "[WorldlineTopology] complete ") == 1,
          "client lifecycle drift");
      Topology change = parseTopology(topologyFile, nonce, topology);
      ...
      String complete = unique(client, "[WorldlinePaged] complete "),
             changed = unique(client, "[WorldlineTopology] complete ");
      require(marker(complete, "samples") == paged.count
              ...
              && marker(changed, "requestIndex") == change.request
              && marker(changed, "eventIndex") == change.event
              && marker(changed, "topology") == topology && marker(changed, "targets") == 3
              && marker(changed, "rebuilds") == change.rebuilds
              && marker(changed, "bytes") == Files.size(topologyFile)
              && token(changed, "sha256").equals(sha256(Files.readAllBytes(topologyFile))),
      ...
      String scene = unique(serverText, "[WorldlineCensus] scene "),
             removed = unique(serverText, "[WorldlineTopology] removed ");
      ...
      require(marker(removed, "topology") == topology && marker(removed, "targets") == 3
              && marker(removed, "x") == x && marker(removed, "y") == y && marker(removed, "z") == z
              && marker(removed, "nonce") == nonce && change.topology == topology && change.x == x
              && change.y == y && change.z == z && plan.x == x && plan.y == y && plan.z == z
              && census.x == x && census.y == y && census.z == z,
          "topology/plan drift");
      require(serverText.indexOf("[WorldlineCensus] scene ")
                  < serverText.indexOf("[WorldlineTopology] removed ")
      ...
      return new Arm(topology, nonce, x, y, z, raw, census, paged, change,
          sha256(Files.readAllBytes(censusFile)), sha256(Files.readAllBytes(pagedFile)),
          sha256(Files.readAllBytes(topologyFile)));
    } finally {
      if (server != null) {
        try {
          server.write("stop\n");
  ...
  private Topology parseTopology(Path file, int nonce, int expectedTopology) throws Exception {
    byte[] b = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 44 && in.readInt() == 0x574c3834 && in.readInt() == 1
              && in.readInt() == 44 && in.readInt() == nonce,
          "topology schema drift");
      int x = in.readInt(), y = in.readInt(), z = in.readInt(), request = in.readInt(),
          event = in.readInt(), topology = in.readInt(), rebuilds = in.readInt();
      require(in.read() == -1 && request >= 300 && event >= request && topology == expectedTopology
              && rebuilds == topology,
          "topology event drift");
      return new Topology(x, y, z, request, event, topology, rebuilds);
    }
  }
  private Census parseCensus(Path file, Topology change, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      require(x == change.x && y == change.y && z == change.z && change.event < count
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && renders == 1 && lists == 1 && visible > 0
                && calls == (i < change.event ? 16 : 13) && state == 0x1010 && mask == 0xffff,
            "topology census record drift at " + i + " values=" + renders + "/" + lists + "/"
                + calls);
        frames[i] = delta;
  ...
  private Paged parsePaged(Path file, Census census, Topology change, int nonce) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      int prePages = 3, postPages = 3, zeroFlush = 0;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        int rc = in.readUnsignedShort(), qc = in.readUnsignedShort(), fc = in.readUnsignedShort(),
            ...
            members = i < change.event ? 16 : 13, pages = i < change.event ? prePages : postPages;
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                && rc == members && qc == members && fc == 2 && reserved == 0 && queued == members
                && pageCalls == pages && direct == 1
                && rebuilds == (i == change.event ? change.rebuilds : 0) && cached == pages,
            "topology paged record drift at " + i + " calls=" + rc + "/" + qc + "/" + fc + " state="
                + queued + "/" + pageCalls + "/" + direct + "/" + rebuilds + "/" + cached);
        if (flush[i] == 0)
          zeroFlush++;
        flushSum = Math.addExact(flushSum, flush[i]);
      }
      require(in.read() == -1 && flushSum > 0, "paged aggregate drift");
      return new Paged(count, prePages, postPages, renderer, queue, flush, zeroFlush, change.event);
    }
  }
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
      int topology, Path census, Path paged, Path artifact, Plan plan) {
    String wrapper =
    ...
    List<String> r = new ArrayList<>(Arrays.asList(
        ...
        "-PworldlineNonce=" + nonce, "-PworldlineTopology=" + topology));
    if (plan != null)
    ...
    if (census != null)
      r.addAll(Arrays.asList("-PworldlineArtifact=" + census, "-PworldlinePagedArtifact=" + paged,
          "-PworldlineTopologyArtifact=" + artifact,
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m84-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M84 cleanup failed " + t, e);
    }
  }
    ...
    return s.lines()
        .filter(x
            -> x.contains("WorldlineCensus") || x.contains("WorldlinePaged")
                || x.contains("WorldlineTopology") || x.contains("Loading ")
  ...
  private static final class Topology {
    final int x, y, z, request, event, topology, rebuilds;
    Topology(int x, int y, int z, int r, int e, int t, int b) {
      this.x = x;
      this.y = y;
      this.z = z;
      request = r;
  ...
  private static final class Paged {
    final int count, prePages, postPages, zeroFlush, event;
    final long[] renderer, queue, flush;
    Paged(int n, int before, int after, long[] r, long[] q, long[] f, int zero, int event) {
      count = n;
      prePages = before;
      postPages = after;
      renderer = r;
    ...
    String summary() {
      long[] wr = before(renderer, event), wq = before(queue, event), wf = before(flush, event);
      return "pages=" + prePages + "to" + postPages + ",event=" + event + ",preRendererNs="
          + q(wr, .5) + "/" + q(wr, .95) + "/" + q(wr, .99) + ",preEnqueueNs=" + q(wq, .5) + "/"
          + q(wq, .95) + "/" + q(wq, .99) + ",preFlushNs=" + q(wf, .5) + "/" + q(wf, .95) + "/"
          + q(wf, .99) + ",eventNs=" + renderer[event] + "/" + queue[event] + "/" + flush[event]
          + ",flushZero=" + zeroFlush;
    }
  }
  private static final class Arm {
    final int topology, nonce, x, y, z, raw;
    final Census census;
    final Paged paged;
    final Topology change;
    final String censusHash, pagedHash, topologyHash;
    Arm(int topology, int n, int x, int y, int z, int raw, Census c, Paged p, Topology m, String ch,
        String ph, String mh) {
    ...
    String summary() {
      return "topology=" + (topology == 1 ? "one" : "three") + ",samples=" + census.count
          + ",request=" + change.request + ",event=" + change.event + ",rebuilds=" + change.rebuilds
          + "," + paged.summary() + ",census=" + censusHash.substring(0, 12) + ",paged="
          + pagedHash.substring(0, 12) + ",topologyArtifact=" + topologyHash.substring(0, 12);
    }
  }
