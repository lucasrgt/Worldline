...
public final class RebuildBudgetZeroDirectCycle {
  private static final String ID = "m101-rebuild-budget-zero-direct";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/RebuildBudgetZeroDirectCycle.java " + ID);
      System.exit(2);
    }
    try {
      new RebuildBudgetZeroDirectCycle().execute();
    } catch (Exception e) {
  ...
  private void execute() throws Exception {
    ...
    require(value("replicas").equals("2") && value("warmup.frames").equals("300")
            ...
            && value("aero.rebuilds.per.frame").equals("0")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m101.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 10101001;
    ...
    if (diagnostic) {
      System.out.println("M101 diagnostic replica passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M101 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M101 rebuild-budget-zero direct passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m101-" + ProcessHandle.current().pid() + "-"
  ...
  private Paged parsePaged(Path file, Census census, int nonce) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      require(in.readInt() == 0x574c6500 && in.readInt() == 1 && in.readInt() == 44
      ...
      int pages = 0, zeroFlush = 0, firstEvicted = 0, priorEvicted = 0, budgetRecords = 0;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                ...
                && pageCalls == 0 && direct == 16 && rebuilds == 0 && cached == pages
                && evicted == 0,
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m101-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M101 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Paged {
    ...
    String summary() {
      return "pages=" + pages + ",directOnlyRecords=" + budgetRecords + ",evicted=" + firstEvicted
