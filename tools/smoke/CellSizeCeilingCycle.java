...
/** Runs two balanced pairs of fresh raw-size-thirty-three and size-thirty-two clients. */
public final class CellSizeCeilingCycle {
  private static final String ID = "m110-cell-size-ceiling";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/CellSizeCeilingCycle.java " + ID);
      System.exit(2);
    }
    try {
      new CellSizeCeilingCycle().execute();
    } catch (Exception e) {
      System.err.println("Cell size ceiling failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("pairs").equals("2") && value("order").equals("raw33-raw32,raw32-raw33")
            ...
            && value("aero.cell.size.ceiling").equals("33")
            && value("aero.cell.size.exact").equals("32") && value("min.instances").equals("2")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m110.diagnostic");
    int pairCount = diagnostic ? 1 : 2;
    List<Arm> arms = new ArrayList<>();
    for (int p = 0; p < pairCount; p++) {
      int nonce = 11001001 + p;
      ...
      for (int phase = 0; phase < 2; phase++) {
        ...
        verifyCheckout(checkout);
        Arm arm =
            run(checkout, build.resolve("pair-" + (p + 1) + "-" + (small ? "raw33" : "raw32")),
    ...
    if (diagnostic) {
      System.out.println("M110 diagnostic pair passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M110 signature drift: " + signature);
    ...
    System.out.println("M110 cell size ceiling passed");
    for (Arm arm : arms)
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan, boolean small)
    ...
    Path base = root.resolve(".worldline/worktrees/m110-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      require(count(client, "[WorldlinePaged] armed ") == 1
              ...
              && count(client, "[WorldlineCellSizeCeiling] armed ") == 1
              && unique(client, "[WorldlineCellSizeCeiling] armed ")
                  .equals("raw=" + rawSize + " effective=32 minimum=2 skipIndividual=false")
  ...
  private Paged parsePaged(Path file, Census census, int nonce, boolean small) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      int effectiveSize = 32, zeroFlush = 0, firstEvicted = -1, priorEvicted = -1,
          immediateRecords = 0;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        boolean state = queued == 16 && pageCalls == 1 && direct == 0 && rebuilds == 0
  ...
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
    ...
    if (census != null)
      r.addAll(Arrays.asList("-PworldlineArtifact=" + census, "-PworldlinePagedArtifact=" + paged,
          ...
          "-PworldlineCellSize=" + value(small ? "aero.cell.size.ceiling" : "aero.cell.size.exact"),
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m110-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M110 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Paged {
    ...
    String summary() {
      return "mode=" + (small ? "raw33" : "raw32") + ",samples=" + count
