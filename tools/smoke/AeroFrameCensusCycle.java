...
public final class AeroFrameCensusCycle {
  ...
  public static void main(String[] args) {
    if (!Arrays.equals(args, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/AeroFrameCensusCycle.java " + ID);
      System.exit(2);
    }
    try {
      new AeroFrameCensusCycle().execute();
    } catch (Exception error) {
      System.err.println("Aero frame census failed: " + error.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(ID.equals(value("id")) && value("pairs").equals("2")
    ...
    int start = Integer.getInteger("worldline.m74.armStart", 0),
        limit = Integer.getInteger("worldline.m74.armLimit", order.length - start);
    require(start >= 0 && limit > 0 && start + limit <= order.length, "arm range drift");
    boolean diagnostic = start != 0 || limit != order.length;
    require(!diagnostic || Boolean.getBoolean("worldline.m74.diagnostic"),
        "partial M74 requires diagnostic opt-in");
    for (int index = start; index < start + limit; index++) {
      int pair = index / 2, nonce = 7407401 + pair;
      verifyCheckout(checkout);
      Arm arm = run(checkout, build.resolve("pair-" + (pair + 1) + "-" + order[index]),
          order[index], pair, nonce, plans[pair]);
      arms.add(arm);
      if (plans[pair] == null)
        plans[pair] = new Plan(arm.x, arm.y, arm.z);
      verifyCheckout(checkout);
    }
    if (diagnostic) {
      System.out.println("M74 diagnostic arm passed; qualification not attempted arms=" + limit);
      arms.forEach(arm -> System.out.println("  " + arm.summary()));
      return;
    }
    Pair first = pair(arms.get(0), arms.get(1)), second = pair(arms.get(2), arms.get(3));
    String trace =
        "v1|design=2-balanced-pairs-16/0+0/16|fixture=exact-plan+tracked-camera+explicit16"
    ...
    require(signature.equals(value("expected.signature")), "M74 signature drift: " + signature);
    String evidence = "id=" + ID
        + "\npairs=2\narms=4\nserver.jvm=4\nclient.jvm=4\nfirst=" + first.summary()
        + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature + "\n";
    Files.writeString(build.resolve("evidence.txt"), evidence, StandardCharsets.UTF_8);
    System.out.println("M74 complete Aero census passed");
  ...
  private Arm run(Path checkout, Path workspace, String mode, int pair, int nonce, Plan plan)
    ...
    int port = freePort(), timeout = Integer.parseInt(value("child.timeout.seconds"));
    Path base = root.resolve(".worldline/worktrees/m74-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      Path serverGame = workspace.resolve("server"), clientGame = workspace.resolve("client"),
           artifact = clientGame.resolve("census.bin");
      ...
      String client = runGradle(clientTree.resolve("stationapi/test-bare"),
          command(clientTree, init, "client", clientGame, port, nonce, mode, artifact, null),
          timeout);
      ...
      require(count(client, "[WorldlineCensus] trigger ") == 1
              && unique(client, "[WorldlineCensus] trigger ").contains("logger=false")
      ...
      Census census = parseArtifact(artifact, mode, nonce);
      ...
      require(marker(complete, "samples") == census.count
              ...
              && marker(complete, "mask") == census.mask,
      ...
      require(serverText.contains("Loading 39 mods:") || serverText.contains("Loading 40 mods:"),
          "server loader marker absent");
      require(serverText.contains("- worldline-m74-content 1.0.0")
      ...
      require(token(scene, "mode").equals(mode) && marker(scene, "planned") == 16
              && marker(scene, "placed") == expected && marker(scene, "yaw") == -90
      ...
      return new Arm(mode, nonce, x, y, z, raw, census, sha256(Files.readAllBytes(artifact)));
    } finally {
      if (server != null) {
        try {
          server.write("stop\n");
  ...
  private Census parseArtifact(Path file, String mode, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      require(density == (mode.equals("present") ? 16 : 0) && in.readInt() == nonce,
          "artifact arm drift");
      ...
      long sum = 0, renderSum = 0, listSum = 0, callSum = 0;
      int visible = 0, mask = density == 16 ? 0xffff : 0;
      long[] frames = new long[count];
      for (int i = 0; i < count; i++) {
        long delta = in.readLong();
        int renders = in.readInt(), lists = in.readInt(), chunks = in.readInt(),
            calls = in.readInt();
        int state = in.readUnsignedShort(), seen = in.readUnsignedShort();
        require(delta > 0 && renders >= 0 && lists >= 0 && chunks >= 0 && calls >= 0,
            "invalid census record");
        require(
            state == (density == 16 ? 0x1010 : 0) && seen == mask, "fixture state drift at " + i);
        if (density == 0)
          require(renders == 0 && lists == 0 && calls == 0, "absent work drift");
        frames[i] = delta;
      ...
      if (density == 16)
        require(renderSum > 0 && listSum > 0 && callSum > 0, "present work absent");
      return new Census(x, y, z, count, elapsed, mask, frames, renderSum, callSum);
    }
  }
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
      String mode, Path artifact, Plan plan) {
    String wrapper =
    ...
    if (artifact != null)
      result.addAll(Arrays.asList("-PworldlineArtifact=" + artifact,
  ...
  private void buildAero(Path checkout) throws Exception {
    Path tree = root.resolve(".worldline/worktrees/m74-build-" + System.nanoTime());
    try {
      ...
      String output = runGradle(stationapi,
          ...
          Integer.parseInt(value("child.timeout.seconds")));
  ...
  private Captured startServer(Path dir, List<String> command, int timeout) throws Exception {
    Captured server = Captured.start(dir, command);
    server.awaitText("Done (", timeout);
    return server;
  }
  private String runGradle(Path dir, List<String> command, int timeout) throws Exception {
    return Captured.run(dir, command, timeout);
  }
  private void verifyBoundary() throws IOException {
    try (Stream<Path> paths = Files.walk(smoke.resolve("runtime-src/worldline/m74"))) {
  ...
  private void verifyCheckout(Path checkout) throws Exception {
    require(git(checkout, "remote", "get-url", "origin").trim().equals(value("aero.repository"))
            && git(checkout, "rev-parse", "HEAD").trim().equals(value("aero.revision"))
            && git(checkout, "status", "--porcelain", "--untracked-files=all").trim().isEmpty(),
        "Aero checkout drift");
  }
  ...
  private void verifyWorktree(Path target) throws Exception {
    require(git(target, "rev-parse", "HEAD").trim().equals(value("aero.revision"))
            && git(target, "status", "--porcelain", "--untracked-files=all").trim().isEmpty(),
        "worktree drift");
  }
  private void removeWorktree(Path checkout, Path target) {
    try {
      ...
      throw new IllegalStateException("M74 cleanup failed " + target, error);
    }
  }
  private boolean registered(Path checkout, Path target) throws Exception {
    return Arrays.stream(git(checkout, "worktree", "list", "--porcelain").split("\\R"))
        .anyMatch(row
  ...
  private String unique(String text, String prefix) {
    ...
    require(rows.size() == 1, "marker drift " + prefix);
    return rows.get(0).substring(prefix.length());
  }
  private int marker(String row, String name) {
    return Integer.parseInt(token(row, name));
  }
  private long markerLong(String row, String name) {
    return Long.parseLong(token(row, name));
  }
  private String token(String row, String name) {
  ...
  private String diagnostic(String text) {
    return text.lines()
        .filter(row
            -> row.contains("WorldlineCensus") || row.contains("Loading ")
                || row.contains("Exception") || row.contains("BUILD "))
  ...
  private static long quantile(long[] values, double q) {
    long[] copy = values.clone();
    Arrays.sort(copy);
    return copy[(int) Math.ceil(copy.length * q) - 1];
  }
  ...
  private static final class Census {
    final int x, y, z, count, mask;
    final long elapsed, median, p95, p99, max, renderSum, callSum;
    Census(int x, int y, int z, int count, long elapsed, int mask, long[] frames, long renderSum,
        long callSum) {
      this.x = x;
      this.y = y;
      this.z = z;
      ...
      this.mask = mask;
      this.renderSum = renderSum;
      this.callSum = callSum;
      median = quantile(frames, .5);
      p95 = quantile(frames, .95);
      p99 = quantile(frames, .99);
      max = Arrays.stream(frames).max().orElseThrow();
    }
  }
  private static final class Arm {
    final String mode, hash;
    final int nonce, x, y, z, raw;
    final Census c;
    Arm(String mode, int nonce, int x, int y, int z, int raw, Census c, String hash) {
      this.mode = mode;
      this.nonce = nonce;
      this.x = x;
      this.y = y;
    ...
    String summary() {
      return mode + ":nonce=" + nonce + ",samples=" + c.count + ",intervalNs=" + c.median + "/"
          + c.p95 + "/" + c.p99 + "/" + c.max + ",renders=" + c.renderSum
          + ",artifact=" + hash.substring(0, 12);
    }
  }
  private static final class Pair {
    ...
    Pair(Arm a, Arm p) {
      absent = a;
      present = p;
      require(a.x == p.x && a.y == p.y && a.z == p.z && a.raw == p.raw, "pair plan drift");
    }
    String summary() {
      return absent.summary() + " | " + present.summary()
          + " | descriptiveDeltaNs(median/p95/p99/max)=" + (present.c.median - absent.c.median)
          + "/" + (present.c.p95 - absent.c.p95) + "/" + (present.c.p99 - absent.c.p99) + "/"
          + (present.c.max - absent.c.max);
    }
  }
  private static final class Captured {
    ...
    static Captured start(Path d, List<String> c) throws IOException {
      return new Captured(
          new ProcessBuilder(c).directory(d.toFile()).redirectErrorStream(true).start());
    }
    ...
    void write(String v) throws IOException {
      process.getOutputStream().write(v.getBytes(StandardCharsets.UTF_8));
    ...
    void awaitText(String v, int t) throws Exception {
      ...
      synchronized (text) {
        while (
            !text.toString().contains(v) && process.isAlive() && System.currentTimeMillis() < end)
          text.wait(100);
      }
      require(output().contains(v), "missing " + v + "\n" + output());
    }
    void finish(int t) throws Exception {
      if (!process.waitFor(t, TimeUnit.SECONDS)) {
        kill();
