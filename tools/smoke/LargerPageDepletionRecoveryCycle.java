...
public final class LargerPageDepletionRecoveryCycle {
  private static final String ID = "m91-larger-page-depletion-recovery";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/LargerPageDepletionRecoveryCycle.java " + ID);
      System.exit(2);
    }
    try {
      new LargerPageDepletionRecoveryCycle().execute();
    } catch (Exception e) {
  ...
  private void execute() throws Exception {
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m91.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 9109101;
    ...
    if (diagnostic) {
      System.out.println("M91 diagnostic replica passed; qualification not attempted");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m91-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      String scene = unique(serverText, "[WorldlineCensus] scene "),
             ...
             restored2 = unique(serverText, "[WorldlineRecovery] restored ordinal=3 "),
             restored1 = unique(serverText, "[WorldlineRecovery] restored ordinal=4 ");
      ...
      require(serverText.indexOf("[WorldlineCensus] scene ")
              ...
              && serverText.indexOf("[WorldlineRecovery] removed ordinal=2 ")
                  < serverText.indexOf("[WorldlineRecovery] restored ordinal=3 ")
              && serverText.indexOf("[WorldlineRecovery] restored ordinal=3 ")
  ...
  private Recovery parseRecovery(Path file, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 76 && in.readInt() == 0x574c3931 && in.readInt() == 1
      ...
      for (int i = 0; i < 4; i++) {
        ...
        require(index == new int[] {1, 2, 2, 1}[i], "recovery cell index drift " + i);
      }
      for (int i = 0; i < 4; i++)
      ...
      for (int i = 0; i < count; i++) {
        ...
        int rc = in.readUnsignedShort(), qc = in.readUnsignedShort(), fc = in.readUnsignedShort(),
            ...
            expectedRebuild = i == change.events[0] || i == change.events[1]
                || i == change.events[2] || i == change.events[3]
            ? 1
            : 0;
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m91-build-" + System.nanoTime());
    try {
  ...
  private void verifyBoundary() throws IOException {
    for (Path base :
        Arrays.asList(root.resolve("smokes/m74-complete-aero-census/runtime-src/worldline/m74"),
            smoke.resolve("runtime-src/worldline/m74"), smoke.resolve("runtime-src/worldline/m91")))
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M91 cleanup failed " + t, e);
    }
  }
