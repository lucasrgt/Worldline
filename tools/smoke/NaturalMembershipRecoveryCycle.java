...
/** Runs two fresh, same-plan single natural membership recovery replicas. */
public final class NaturalMembershipRecoveryCycle {
  private static final String ID = "m85-natural-membership-recovery";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/NaturalMembershipRecoveryCycle.java " + ID);
      System.exit(2);
    }
    try {
      new NaturalMembershipRecoveryCycle().execute();
    } catch (Exception e) {
      System.err.println("Natural membership recovery failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m85.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 8508501;
    ...
    if (diagnostic) {
      System.out.println("M85 diagnostic replica passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M85 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M85 natural membership recovery passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m85-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      require(marker(removed, "x") == x && marker(removed, "y") == y && marker(removed, "z") == z
              && marker(removed, "nonce") == nonce && marker(restored, "x") == x
              && marker(restored, "y") == y && marker(restored, "z") == z
              && marker(restored, "nonce") == nonce && change.x == x && change.y == y
  ...
  private Recovery parseRecovery(Path file, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 52 && in.readInt() == 0x574c3835 && in.readInt() == 1
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m85-build-" + System.nanoTime());
    try {
  ...
  private void verifyBoundary() throws IOException {
    for (Path base :
        Arrays.asList(root.resolve("smokes/m74-complete-aero-census/runtime-src/worldline/m74"),
            smoke.resolve("runtime-src/worldline/m74"), smoke.resolve("runtime-src/worldline/m85")))
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M85 cleanup failed " + t, e);
    }
  }
