...
/** Runs two balanced pairs of fresh pages-enabled and pages-disabled Aero clients. */
public final class PairedPagesControlCycle {
  private static final String ID = "m104-paired-pages-control";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/PairedPagesControlCycle.java " + ID);
      System.exit(2);
    }
    try {
      new PairedPagesControlCycle().execute();
    } catch (Exception e) {
      System.err.println("Paired pages control failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("pairs").equals("2") && value("order").equals("enabled-disabled,disabled-enabled")
            ...
            && value("max.cached.pages").equals("1")
            && value("aero.rebuilds.per.frame").equals("-1")
            && value("aero.page.ttl.frames").equals("100000")
            && value("aero.cell.pages.enabled").equals("true")
            && value("aero.cell.pages.disabled").equals("false") && value("fps.limit").equals("0")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m104.diagnostic");
    int pairCount = diagnostic ? 1 : 2;
    List<Arm> arms = new ArrayList<>();
    for (int p = 0; p < pairCount; p++) {
      int nonce = 10401001 + p;
      boolean enabledFirst = (p & 1) == 0;
      Plan plan = null;
      for (int phase = 0; phase < 2; phase++) {
        boolean enabled = phase == 0 ? enabledFirst : !enabledFirst;
        verifyCheckout(checkout);
        Arm arm = run(checkout,
            build.resolve("pair-" + (p + 1) + "-" + (enabled ? "enabled" : "disabled")),
            p * 2 + phase, nonce, plan, enabled);
        arms.add(arm);
        if (plan == null)
    ...
    if (diagnostic) {
      System.out.println("M104 diagnostic pair passed; qualification not attempted");
      System.out.println("  " + arms.get(0).summary());
      System.out.println("  " + arms.get(1).summary());
      return;
    }
    require(arms.size() == 4 && arms.stream().filter(x -> x.enabled).count() == 2,
    ...
    require(signature.equals(value("expected.signature")), "M104 signature drift: " + signature);
    ...
    System.out.println("M104 paired pages control passed");
    for (Arm arm : arms)
      System.out.println("  " + arm.summary());
    System.out.println("  signature: " + signature);
  }
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan, boolean enabled)
      throws Exception {
    Files.createDirectories(workspace);
    int port = freePort(), timeout = Integer.parseInt(value("timeout.seconds"));
    Path base = root.resolve(".worldline/worktrees/m104-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      server = startServer(serverTree.resolve("stationapi/test-bare"),
          command(serverTree, init, "server", serverGame, port, nonce, null, null, plan, enabled),
          timeout);
      ...
      require(count(client, "[WorldlinePaged] armed ") == 1
              && unique(client, "[WorldlinePaged] armed ")
                  .equals("marker=true pages=" + enabled + " fpsLimit=0 aeroFramePacing=false")
      ...
      Census census = parseCensus(censusFile, nonce, enabled);
      Paged paged = parsePaged(pagedFile, census, nonce, enabled);
      ...
      return new Arm(enabled, nonce, x, y, z, raw, census, paged,
  ...
  private Census parseCensus(Path file, int nonce, boolean enabled) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && renders == expected && lists == expected && visible > 0 && calls == 16
                && state == 0x1010 && mask == 0xffff,
            "paired census record drift at " + i + " mode=" + enabled + " values=" + renders + "/"
                + lists + "/" + calls);
        frames[i] = delta;
  ...
  private Paged parsePaged(Path file, Census census, int nonce, boolean enabled) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      int pages = enabled ? 1 : 0, zeroFlush = 0, firstEvicted = -1, priorEvicted = -1,
          immediateRecords = 0;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        boolean state = enabled ? queued == 16 && pageCalls == 4 && direct == 0 && rebuilds == 4
                && cached == 1 && evicted > 0 && (priorEvicted < 0 || evicted == priorEvicted + 4)
                && immediate == 0
                                : queued == 0 && pageCalls == 0 && direct == 0 && rebuilds == 0
                && cached == 0 && evicted == 0 && immediate == 16;
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                && rc == 16 && qc == 16 && fc == 2 && reserved == 0 && state,
            "paired page record drift at " + i + " mode=" + enabled + " time=" + renderer[i] + "/"
      ...
      return new Paged(enabled, count, pages, renderer, queue, flush, zeroFlush, firstEvicted,
          priorEvicted, immediateRecords);
    }
  }
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
      Path census, Path paged, Plan plan, boolean enabled) {
    String wrapper =
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m104-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M104 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Paged {
    ...
    final int count, pages, zeroFlush, firstEvicted, lastEvicted, immediateRecords;
    final long[] renderer, queue, flush;
    Paged(boolean enabled, int n, int p, long[] r, long[] q, long[] f, int zero, int first,
        int last, int immediate) {
    ...
    String summary() {
      return "mode=" + (enabled ? "enabled" : "disabled") + ",samples=" + count + ",pages=" + pages
  ...
  private static final class Arm {
    ...
    Arm(boolean enabled, int n, int x, int y, int z, int raw, Census c, Paged p, String ch,
        String ph) {
      this.enabled = enabled;
      nonce = n;
      this.x = x;
