...
public final class RebuildBudgetFallbackCycle {
  private static final String ID = "m99-rebuild-budget-fallback";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/RebuildBudgetFallbackCycle.java " + ID);
      System.exit(2);
    }
    try {
      new RebuildBudgetFallbackCycle().execute();
    } catch (Exception e) {
  ...
  private void execute() throws Exception {
    ...
    require(value("replicas").equals("2") && value("warmup.frames").equals("300")
            ...
            && value("aero.rebuilds.per.frame").equals("2")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m99.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 9909901;
    ...
    if (diagnostic) {
      System.out.println("M99 diagnostic replica passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M99 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M99 rebuild-budget fallback passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m99-" + ProcessHandle.current().pid() + "-"
  ...
  private Census parseCensus(Path file, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && renders == 4 && lists == 4 && visible > 0 && calls == 16
  ...
  private Paged parsePaged(Path file, Census census, int nonce) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      require(in.readInt() == 0x574c3939 && in.readInt() == 1 && in.readInt() == 44
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                ...
                && pageCalls == 2 && direct == 4 && rebuilds == 2 && cached == pages && evicted > 0
                && (priorEvicted < 0 || evicted == priorEvicted + 2),
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m99-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M99 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Paged {
    final int count, pages, zeroFlush, firstEvicted, lastEvicted, budgetRecords;
    ...
    String summary() {
      return "pages=" + pages + ",budgetPathRecords=" + budgetRecords + ",evicted=" + firstEvicted
