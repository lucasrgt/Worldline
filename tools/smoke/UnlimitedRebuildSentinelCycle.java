...
public final class UnlimitedRebuildSentinelCycle {
  private static final String ID = "m102-unlimited-rebuild-sentinel";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/UnlimitedRebuildSentinelCycle.java " + ID);
      System.exit(2);
    }
    try {
      new UnlimitedRebuildSentinelCycle().execute();
    } catch (Exception e) {
      System.err.println("Unlimited-rebuild sentinel failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m102.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 10201001;
    ...
    if (diagnostic) {
      System.out.println("M102 diagnostic replica passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M102 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M102 unlimited-rebuild sentinel passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m102-" + ProcessHandle.current().pid() + "-"
  ...
  private Paged parsePaged(Path file, Census census, int nonce) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      require(in.readInt() == 0x574c6600 && in.readInt() == 1 && in.readInt() == 44
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m102-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M102 cleanup failed " + t, e);
    }
  }
