...
/** Runs two fresh same-plan reverse-order two-cell recovery replicas. */
public final class ReverseTwoCellMembershipRecoveryCycle {
  private static final String ID = "m88-reverse-two-cell-membership-recovery";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println(
          "usage: java tools/smoke/ReverseTwoCellMembershipRecoveryCycle.java " + ID);
      System.exit(2);
    }
    try {
      new ReverseTwoCellMembershipRecoveryCycle().execute();
    } catch (Exception e) {
      System.err.println("Reverse two-cell membership recovery failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("replicas").equals("2") && value("cycles").equals("2")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m88.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 8808801;
    ...
    if (diagnostic) {
      System.out.println("M88 diagnostic replica passed; qualification not attempted");
    ...
    String trace =
        "v1|design=2-fresh-same-plan-nonce-reverse-two-distinct-cell-membership-recovery-replicas|fixture=constant16-synced+client-marker+exact-camera|server-closure=aero-free|frame-limit=vanilla-max0+aero-pacer-off-runtime-gated|warm-path=enqueue16+flush2+cached4+pageCalls4+direct0+render0/list0+rebuild0|sequence=remove-index1+restore-index1+remove-index0+restore-index0-with-generation+index-bound-ack/state|spacing=request1-after300+each-next-request-30-records-after-prior-event|membership=16to15to16to15to16|index1-first-removed=pageCalls4+direct0+render/list0+rebuild1|index1-first-restored=pageCalls4+direct0+render/list0+rebuild1|index0-second-removed=pageCalls3+direct1+render/list1+rebuild0|index0-second-restored=pageCalls4+direct0+render/list0+rebuild1|window=M74-min720intervals+12s|capture=M78-primitive-spans+page-counters+post-seal-76-byte-recovery-sidecar|per-record=state16/maskffff|stats=descriptive-transition-dynamic|arbitrary-addition-more-than2-cells-concurrency-stale-cleanup-uninstrumented-cost-causality-regression-historical-lag=not-claimed|shutdown=clean";
    String signature = sha256(trace);
    require(signature.equals(value("expected.signature")), "M88 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M88 reverse two-cell membership recovery passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m88-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      String scene = unique(serverText, "[WorldlineCensus] scene "),
             removed1 = unique(serverText, "[WorldlineRecovery] removed generation=1 "),
             restored1 = unique(serverText, "[WorldlineRecovery] restored generation=1 "),
             removed2 = unique(serverText, "[WorldlineRecovery] removed generation=2 "),
             restored2 = unique(serverText, "[WorldlineRecovery] restored generation=2 ");
      ...
      require(sameCell(removed1, x, y, z, nonce, 1) && sameCell(restored1, x, y, z, nonce, 1)
              && sameCell(removed2, x, y, z, nonce, 0) && sameCell(restored2, x, y, z, nonce, 0)
      ...
      require(serverText.indexOf("[WorldlineCensus] scene ")
                  < serverText.indexOf("[WorldlineRecovery] removed generation=1 ")
              && serverText.indexOf("[WorldlineRecovery] removed generation=1 ")
                  < serverText.indexOf("[WorldlineRecovery] restored generation=1 ")
              && serverText.indexOf("[WorldlineRecovery] restored generation=1 ")
                  < serverText.indexOf("[WorldlineRecovery] removed generation=2 ")
              && serverText.indexOf("[WorldlineRecovery] removed generation=2 ")
                  < serverText.indexOf("[WorldlineRecovery] restored generation=2 ")
  ...
  private Recovery parseRecovery(Path file, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 76 && in.readInt() == 0x574c3838 && in.readInt() == 1
              && in.readInt() == 76 && in.readInt() == nonce,
          "recovery schema drift");
      int x = in.readInt(), y = in.readInt(), z = in.readInt();
      int[] requests = new int[4], events = new int[4];
      for (int i = 0; i < 4; i++) {
        requests[i] = in.readInt();
        events[i] = in.readInt();
        int index = in.readInt();
        require(index == 1 - i / 2, "recovery cell index drift " + i);
      }
      for (int i = 0; i < 4; i++)
  ...
  private Census parseCensus(Path file, Recovery change, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      require(x == change.x && y == change.y && z == change.z && change.events[3] < count
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        int rc = in.readUnsignedShort(), qc = in.readUnsignedShort(), fc = in.readUnsignedShort(),
            ...
            pages = expectedDirect == 1 ? 3 : 4,
            expectedRebuild =
                i == change.events[0] || i == change.events[1] || i == change.events[3] ? 1 : 0;
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m88-build-" + System.nanoTime());
    try {
  ...
  private void verifyBoundary() throws IOException {
    for (Path base :
        Arrays.asList(root.resolve("smokes/m74-complete-aero-census/runtime-src/worldline/m74"),
            smoke.resolve("runtime-src/worldline/m74"), smoke.resolve("runtime-src/worldline/m88")))
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M88 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Recovery {
    ...
    int members(int i) {
      return i < events[0] || i >= events[1] && i < events[2] || i >= events[3] ? 16 : 15;
    }
    int direct(int i) {
      return i >= events[2] && i < events[3] ? 1 : 0;
    }
    String requests() {
  ...
  private static final class Paged {
    ...
    String summary() {
      ...
      return "preRendererNs=" + q(wr, .5) + "/" + q(wr, .95) + "/" + q(wr, .99)
          + ",preEnqueueNs=" + q(wq, .5) + "/" + q(wq, .95) + "/" + q(wq, .99)
          + ",preFlushNs=" + q(wf, .5) + "/" + q(wf, .95) + "/" + q(wf, .99)
          + ",eventNs=" + renderer[events[0]] + "/" + renderer[events[1]] + "/"
          + renderer[events[2]] + "/" + renderer[events[3]] + ",flushZero=" + zeroFlush;
    }
  }
