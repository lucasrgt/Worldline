...
/** Runs two mirrored fresh 0/1/4/16 Aero-call ladders on the M74 census. */
public final class AeroDensityLadderCycle {
  private static final String ID = "m75-aero-density-ladder";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/AeroDensityLadderCycle.java " + ID);
      System.exit(2);
    }
    try {
      new AeroDensityLadderCycle().execute();
    } catch (Exception e) {
      System.err.println("Aero density ladder failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("ladders").equals("2") && value("levels").equals("0,1,4,16")
    ...
    recreate(build);
    buildAero(checkout);
    int[] order = {0, 1, 4, 16, 16, 4, 1, 0};
    List<Arm> arms = new ArrayList<>();
    Plan[] plans = new Plan[2];
    int start = Integer.getInteger("worldline.m75.armStart", 0),
        limit = Integer.getInteger("worldline.m75.armLimit", order.length - start);
    require(start >= 0 && limit > 0 && start + limit <= order.length, "arm range drift");
    boolean diagnostic = start != 0 || limit != order.length;
    require(!diagnostic || Boolean.getBoolean("worldline.m75.diagnostic"),
        "partial M75 requires diagnostic opt-in");
    for (int i = start; i < start + limit; i++) {
      int ladder = i / 4, nonce = 7507501 + ladder;
      verifyCheckout(checkout);
      Path workspace = build.resolve("ladder-" + (ladder + 1) + "-level-" + order[i]);
      Arm arm;
      try {
        arm = run(checkout, workspace, order[i], ladder, nonce, plans[ladder]);
      } catch (IllegalStateException e) {
        if (!preCensusTimeout(e))
          throw e;
        System.out.println("M75 retrying one pre-census readiness timeout level=" + order[i]);
        arm = run(checkout, workspace.resolveSibling(workspace.getFileName() + "-retry"), order[i],
            ladder, nonce, plans[ladder]);
      }
      arms.add(arm);
      if (plans[ladder] == null)
        plans[ladder] = new Plan(arm.x, arm.y, arm.z);
      verifyCheckout(checkout);
    }
    if (diagnostic) {
      System.out.println("M75 diagnostic arm passed; qualification not attempted arms=" + limit);
      arms.forEach(x -> System.out.println("  " + x.summary()));
      return;
    }
    Ladder first = new Ladder(arms.subList(0, 4)), second = new Ladder(arms.subList(4, 8));
    String trace =
        "v1|design=2-mirrored-fresh-ladders-0/1/4/16+16/4/1/0|fixture=constant16-synced-dispatched+exact-plan-camera-nonce"
        + "|treatment=nested-firstN-aero-queueAtRest-calls|census=complete-head-to-head-after-fixture-ready|window=min720intervals+12s|capture=fixed65536-primitive+post-seal-binary"
        + "|per-record=dispatch16+aeroCalls-level|retry=one-pre-census-timeout-only|stats=descriptive-whole-census+level-minus-zero-dynamic|monotonicity-causality-regression-density-response-historical-lag=not-claimed|shutdown=clean";
    String signature = sha256(trace);
    require(signature.equals(value("expected.signature")), "M75 signature drift: " + signature);
    String evidence = "id=" + ID
        + "\nladders=2\narms=8\nserver.jvm=8\nclient.jvm=8\nfirst=" + first.summary()
        + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature + "\n";
    Files.writeString(build.resolve("evidence.txt"), evidence, StandardCharsets.UTF_8);
    System.out.println("M75 Aero density ladder passed");
    System.out.println("  ladder 1: " + first.summary());
    System.out.println("  ladder 2: " + second.summary());
    System.out.println("  signature: " + signature);
  }
  private Arm run(Path checkout, Path workspace, int level, int ladder, int nonce, Plan plan)
      throws Exception {
    Files.createDirectories(workspace);
    int port = freePort(), timeout = Integer.parseInt(value("timeout.seconds"));
    Path base = root.resolve(".worldline/worktrees/m75-" + ProcessHandle.current().pid() + "-"
             + ladder + "-" + level + "-" + System.nanoTime()),
    ...
    try {
      ...
      server = startServer(serverTree.resolve("stationapi/test-bare"),
          command(serverTree, init, "server", serverGame, port, nonce, level, null, plan), timeout);
      String client = runGradle(clientTree.resolve("stationapi/test-bare"),
          command(clientTree, init, "client", clientGame, port, nonce, level, artifact, null),
          timeout);
      ...
      require(count(client, "[WorldlineDensity] level=") == 1
              && unique(client, "[WorldlineDensity] level=").equals(level + " root=" + nonce)
      ...
      Census census = parseArtifact(artifact, level, nonce);
      ...
      return new Arm(level, nonce, x, y, z, raw, census, sha256(Files.readAllBytes(artifact)));
    } finally {
      if (server != null) {
        try {
          server.write("stop\n");
  ...
  private Census parseArtifact(Path file, int level, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && renders == level && lists == level && chunks >= 0 && calls == 16
                && state == 0x1010 && mask == 0xffff,
            "density record drift level=" + level + " at " + i);
        frames[i] = delta;
  ...
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
      int level, Path artifact, Plan plan) {
    String wrapper =
    ...
    if (artifact != null)
      r.addAll(Arrays.asList("-PworldlineArtifact=" + artifact,
          "-PworldlineUsername=" + value("username"), "-PworldlineDensity=" + level,
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m75-build-" + System.nanoTime());
    try {
  ...
  private void verifyBoundary() throws IOException {
    try (Stream<Path> p = Files.walk(
             root.resolve("smokes/m74-complete-aero-census/runtime-src/worldline/m74"))) {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M75 cleanup failed " + t, e);
    }
  }
  ...
  private String diagnostic(String s) {
    return s.lines()
        .filter(x
            -> x.contains("WorldlineCensus") || x.contains("WorldlineDensity")
  ...
  private static final class Census {
    ...
    Census(int x, int y, int z, int count, long elapsed, long[] f, long renders) {
      this.x = x;
      this.y = y;
      this.z = z;
      this.count = count;
  ...
  private static final class Arm {
    final int level, nonce, x, y, z, raw;
    final Census c;
    final String hash;
    Arm(int l, int n, int x, int y, int z, int r, Census c, String h) {
      level = l;
      nonce = n;
      this.x = x;
      this.y = y;
    ...
    String summary() {
      return level + ":samples=" + c.count + ",intervalNs=" + c.median + "/" + c.p95 + "/" + c.p99
          + "/" + c.max + ",renders=" + c.renders + ",artifact=" + hash.substring(0, 12);
    }
  }
  private static final class Ladder {
    final Arm[] by = new Arm[17];
    Ladder(List<Arm> a) {
      for (Arm x : a) {
        require(x.level == 0 || x.level == 1 || x.level == 4 || x.level == 16, "level drift");
        require(by[x.level] == null, "duplicate level");
        by[x.level] = x;
      }
      require(
          by[0] != null && by[1] != null && by[4] != null && by[16] != null, "ladder incomplete");
      for (Arm x : a)
        require(x.nonce == by[0].nonce && x.x == by[0].x && x.y == by[0].y && x.z == by[0].z
                && x.raw == by[0].raw,
            "ladder fixture drift");
    }
    String summary() {
      StringBuilder s = new StringBuilder();
      for (int n : new int[] {0, 1, 4, 16}) {
        if (s.length() > 0)
          s.append(" | ");
        Arm a = by[n];
        s.append(a.summary()).append(",deltaMedianFrom0=").append(a.c.median - by[0].c.median);
      }
      return s.toString();
    }
  }
