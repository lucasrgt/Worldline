...
public final class PageCapacityTwoThrashCycle {
  private static final String ID = "m96-page-capacity-two-thrash";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/PageCapacityTwoThrashCycle.java " + ID);
      System.exit(2);
    }
    try {
      new PageCapacityTwoThrashCycle().execute();
    } catch (Exception e) {
      System.err.println("Page-capacity-two thrash failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    require(value("replicas").equals("2") && value("warmup.frames").equals("300")
            ...
            && value("minimum.millis").equals("12000") && value("max.cached.pages").equals("2")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m96.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 9609601;
    ...
    if (diagnostic) {
      System.out.println("M96 diagnostic replica passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M96 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M96 page-capacity-two bounded thrash passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m96-" + ProcessHandle.current().pid() + "-"
  ...
  private Paged parsePaged(Path file, Census census, int nonce) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      require(in.readInt() == 0x574c3936 && in.readInt() == 1 && in.readInt() == 44
      ...
      int pages = 2, zeroFlush = 0, firstEvicted = -1, priorEvicted = -1, rebuild3 = 0,
          rebuild4 = 0;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                ...
                && pageCalls == 4 && direct == 0 && (rebuilds == 3 || rebuilds == 4)
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m96-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M96 cleanup failed " + t, e);
    }
  }
