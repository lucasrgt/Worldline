...
/** Runs two balanced pairs of fresh cache-one and unlimited-cache Aero clients. */
public final class PairedCacheCapacityCycle {
  private static final String ID = "m105-paired-cache-capacity";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/PairedCacheCapacityCycle.java " + ID);
      System.exit(2);
    }
    try {
      new PairedCacheCapacityCycle().execute();
    } catch (Exception e) {
      System.err.println("Paired cache capacity failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("pairs").equals("2") && value("order").equals("cache1-unlimited,unlimited-cache1")
            ...
            && value("max.cached.pages.limited").equals("1")
            && value("max.cached.pages.unlimited").equals("-1")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m105.diagnostic");
    int pairCount = diagnostic ? 1 : 2;
    List<Arm> arms = new ArrayList<>();
    for (int p = 0; p < pairCount; p++) {
      int nonce = 10501001 + p;
      boolean limitedFirst = (p & 1) == 0;
      Plan plan = null;
      for (int phase = 0; phase < 2; phase++) {
        boolean limited = phase == 0 ? limitedFirst : !limitedFirst;
        verifyCheckout(checkout);
        Arm arm = run(checkout,
            build.resolve("pair-" + (p + 1) + "-" + (limited ? "cache1" : "unlimited")),
            p * 2 + phase, nonce, plan, limited);
        arms.add(arm);
        if (plan == null)
    ...
    if (diagnostic) {
      System.out.println("M105 diagnostic pair passed; qualification not attempted");
      System.out.println("  " + arms.get(0).summary());
      System.out.println("  " + arms.get(1).summary());
      return;
    }
    require(arms.size() == 4 && arms.stream().filter(x -> x.limited).count() == 2,
    ...
    require(signature.equals(value("expected.signature")), "M105 signature drift: " + signature);
    ...
    System.out.println("M105 paired cache capacity passed");
    for (Arm arm : arms)
      System.out.println("  " + arm.summary());
    System.out.println("  signature: " + signature);
  }
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan, boolean limited)
      throws Exception {
    Files.createDirectories(workspace);
    int port = freePort(), timeout = Integer.parseInt(value("timeout.seconds"));
    Path base = root.resolve(".worldline/worktrees/m105-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      server = startServer(serverTree.resolve("stationapi/test-bare"),
          command(serverTree, init, "server", serverGame, port, nonce, null, null, plan, limited),
          timeout);
      ...
      Census census = parseCensus(censusFile, nonce, limited);
      Paged paged = parsePaged(pagedFile, census, nonce, limited);
      ...
      return new Arm(limited, nonce, x, y, z, raw, census, paged,
  ...
  private Census parseCensus(Path file, int nonce, boolean limited) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && renders == 0 && lists == 0 && visible > 0 && calls == 16
            ...
            "paired census record drift at " + i + " limited=" + limited + " values=" + renders
                + "/" + lists + "/" + calls);
        frames[i] = delta;
  ...
  private Paged parsePaged(Path file, Census census, int nonce, boolean limited) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      int capacity = limited ? 1 : -1, zeroFlush = 0, firstEvicted = -1, priorEvicted = -1,
          immediateRecords = 0;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        boolean state = limited ? queued == 16 && pageCalls == 4 && direct == 0 && rebuilds == 4
                && cached == 1 && evicted > 0 && (priorEvicted < 0 || evicted == priorEvicted + 4)
                && immediate == 0
                                : queued == 16 && pageCalls == 4 && direct == 0 && rebuilds == 0
        ...
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] > 0 && renderer[i] >= queue[i]
            ...
            "paired cache record drift at " + i + " limited=" + limited + " time=" + renderer[i]
      ...
      return new Paged(limited, count, capacity, renderer, queue, flush, zeroFlush, firstEvicted,
          priorEvicted, immediateRecords);
    }
  }
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
      Path census, Path paged, Plan plan, boolean limited) {
    String wrapper =
    ...
    if (census != null)
      r.addAll(Arrays.asList("-PworldlineArtifact=" + census, "-PworldlinePagedArtifact=" + paged,
          ...
          "-PworldlineMaxCachedPages="
              + value(limited ? "max.cached.pages.limited" : "max.cached.pages.unlimited"),
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m105-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M105 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Paged {
    ...
    final int count, capacity, zeroFlush, firstEvicted, lastEvicted, immediateRecords;
    final long[] renderer, queue, flush;
    Paged(boolean limited, int n, int p, long[] r, long[] q, long[] f, int zero, int first,
    ...
    String summary() {
      return "mode=" + (limited ? "cache1" : "unlimited") + ",samples=" + count
          + ",capacity=" + capacity + ",immediateRecords=" + immediateRecords
  ...
  private static final class Arm {
    ...
    Arm(boolean limited, int n, int x, int y, int z, int raw, Census c, Paged p, String ch,
        String ph) {
      this.limited = limited;
      nonce = n;
      this.x = x;
