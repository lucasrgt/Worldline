...
/** Runs two fresh, same-plan single natural membership rebuild replicas. */
public final class NaturalMembershipRebuildCycle {
  private static final String ID = "m80-natural-membership-rebuild";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/NaturalMembershipRebuildCycle.java " + ID);
      System.exit(2);
    }
    try {
      new NaturalMembershipRebuildCycle().execute();
    } catch (Exception e) {
      System.err.println("Natural membership rebuild failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m80.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 8008001;
    ...
    if (diagnostic) {
      System.out.println("M80 diagnostic replica passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M80 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M80 natural membership rebuild passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m80-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      Path init = root.resolve(value("runner")), serverGame = workspace.resolve("server"),
           ...
           membershipFile = clientGame.resolve("membership.bin");
      ...
      require(count(client, "[WorldlinePaged] armed ") == 1
              ...
              && count(client, "[WorldlineMembership] complete ") == 1,
          "client lifecycle drift");
      Membership change = parseMembership(membershipFile, nonce);
      ...
      String complete = unique(client, "[WorldlinePaged] complete "),
             changed = unique(client, "[WorldlineMembership] complete ");
      require(marker(complete, "samples") == paged.count
              ...
              && marker(changed, "bytes") == Files.size(membershipFile)
              && token(changed, "sha256").equals(sha256(Files.readAllBytes(membershipFile))),
      ...
      String scene = unique(serverText, "[WorldlineCensus] scene "),
             removed = unique(serverText, "[WorldlineMembership] removed ");
      ...
      require(marker(removed, "x") == x && marker(removed, "y") == y && marker(removed, "z") == z
              && marker(removed, "nonce") == nonce && change.x == x && change.y == y
              && change.z == z && (plan == null || plan.x == x && plan.y == y && plan.z == z)
              && census.x == x && census.y == y && census.z == z,
          "membership/plan drift");
      require(serverText.indexOf("[WorldlineCensus] scene ")
                  < serverText.indexOf("[WorldlineMembership] removed ")
      ...
      return new Arm(nonce, x, y, z, raw, census, paged, change,
          ...
          sha256(Files.readAllBytes(membershipFile)));
    } finally {
      if (server != null) {
        try {
          server.write("stop\n");
  ...
  private Membership parseMembership(Path file, int nonce) throws Exception {
    byte[] b = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 36 && in.readInt() == 0x574c3830 && in.readInt() == 1
              && in.readInt() == 36 && in.readInt() == nonce,
          "membership schema drift");
      int x = in.readInt(), y = in.readInt(), z = in.readInt(), request = in.readInt(),
          event = in.readInt();
      require(in.read() == -1 && request >= 300 && event >= request, "membership event drift");
      return new Membership(x, y, z, request, event);
    }
  }
  private Census parseCensus(Path file, Membership change, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && renders == 0 && lists == 0 && visible > 0
                && calls == (i < change.event ? 16 : 15) && state == 0x1010 && mask == 0xffff,
            "membership census record drift at " + i + " values=" + renders + "/" + lists + "/"
                + calls);
        frames[i] = delta;
  ...
  private Paged parsePaged(Path file, Census census, Membership change, int nonce)
      throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        int rc = in.readUnsignedShort(), qc = in.readUnsignedShort(), fc = in.readUnsignedShort(),
            ...
            members = i < change.event ? 16 : 15, pages = i < change.event ? prePages : postPages;
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                && rc == members && qc == members && fc == 2 && reserved == 0 && queued == members
                && pageCalls == pages && direct == 0 && rebuilds == (i == change.event ? 1 : 0)
                && cached == pages,
            "membership paged record drift at " + i + " calls=" + rc + "/" + qc + "/" + fc
  ...
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
    ...
    if (census != null)
      r.addAll(Arrays.asList("-PworldlineArtifact=" + census, "-PworldlinePagedArtifact=" + paged,
          "-PworldlineMembershipArtifact=" + membership,
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m80-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M80 cleanup failed " + t, e);
    }
  }
    ...
    return s.lines()
        .filter(x
            -> x.contains("WorldlineCensus") || x.contains("WorldlinePaged")
                || x.contains("WorldlineMembership") || x.contains("Loading ")
  ...
  private static final class Membership {
    final int x, y, z, request, event;
    Membership(int x, int y, int z, int r, int e) {
      this.x = x;
      this.y = y;
      this.z = z;
      request = r;
  ...
  private static final class Arm {
    ...
    final Census census;
    final Paged paged;
    final Membership change;
    final String censusHash, pagedHash, changeHash;
    Arm(int n, int x, int y, int z, int raw, Census c, Paged p, Membership m, String ch, String ph,
        String mh) {
      nonce = n;
      this.x = x;
      this.y = y;
    ...
    String summary() {
      return "samples=" + census.count + ",request=" + change.request + ",event=" + change.event
          ...
          + ",paged=" + pagedHash.substring(0, 12) + ",membership=" + changeHash.substring(0, 12);
    }
  }
