...
public final class PagedStageTimingCycle {
  private static final String ID = "m78-paged-stage-timing";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/PagedStageTimingCycle.java " + ID);
      System.exit(2);
    }
    try {
      new PagedStageTimingCycle().execute();
    } catch (Exception e) {
  ...
  private void execute() throws Exception {
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m78.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 7807801;
    ...
    if (diagnostic) {
      System.out.println("M78 diagnostic replica passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M78 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M78 paged stage timing passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m78-" + ProcessHandle.current().pid() + "-"
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                ...
                && pageCalls == pages && direct == 0 && rebuilds == 0 && cached == pages,
      ...
      return new Paged(count, pages, renderer, queue, flush, zeroFlush);
    }
  }
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m78-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M78 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Paged {
    final int count, pages, zeroFlush;
    final long[] renderer, queue, flush;
    Paged(int n, int p, long[] r, long[] q, long[] f, int zero) {
      count = n;
      pages = p;
      renderer = r;
      queue = q;
    ...
    String summary() {
      return "pages=" + pages + ",rendererNs=" + q(renderer, .5) + "/" + q(renderer, .95) + "/"
