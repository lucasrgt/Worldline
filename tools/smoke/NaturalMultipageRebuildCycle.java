...
/** Runs two fresh, same-plan natural two-page rebuild replicas. */
public final class NaturalMultipageRebuildCycle {
  private static final String ID = "m81-natural-multipage-rebuild";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/NaturalMultipageRebuildCycle.java " + ID);
      System.exit(2);
    }
    try {
      new NaturalMultipageRebuildCycle().execute();
    } catch (Exception e) {
      System.err.println("Natural multipage rebuild failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("replicas").equals("2") && value("plan.x").equals("10")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m81.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 8108101;
    ...
    if (diagnostic) {
      System.out.println("M81 diagnostic replica passed; qualification not attempted");
    ...
    require(first.nonce == second.nonce && first.x == second.x && first.y == second.y
            ...
            && first.paged.prePages == second.paged.prePages
            && first.paged.postPages == second.paged.postPages,
    ...
    require(signature.equals(value("expected.signature")), "M81 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M81 natural multipage rebuild passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m81-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      Path init = root.resolve(value("runner")), serverGame = workspace.resolve("server"),
           ...
           pagedFile = clientGame.resolve("paged.bin"), waveFile = clientGame.resolve("wave.bin");
      ...
      require(count(client, "[WorldlinePaged] armed ") == 1
              ...
              && count(client, "[WorldlineWave] complete ") == 1,
          "client lifecycle drift");
      Wave change = parseWave(waveFile, nonce);
      ...
      String complete = unique(client, "[WorldlinePaged] complete "),
             changed = unique(client, "[WorldlineWave] complete ");
      require(marker(complete, "samples") == paged.count
              ...
              && marker(changed, "eventIndex") == change.event && marker(changed, "targets") == 2
              && marker(changed, "bytes") == Files.size(waveFile)
              && token(changed, "sha256").equals(sha256(Files.readAllBytes(waveFile))),
      ...
      String scene = unique(serverText, "[WorldlineCensus] scene "),
             removed = unique(serverText, "[WorldlineWave] removed ");
      ...
      require(marker(removed, "targets") == 2 && marker(removed, "x") == x
              && marker(removed, "y") == y && marker(removed, "z") == z
              && marker(removed, "z2") == z + 2 && marker(removed, "nonce") == nonce
              && change.targets == 2 && change.x == x && change.y == y && change.z == z
              && (plan == null || plan.x == x && plan.y == y && plan.z == z) && census.x == x
              && census.y == y && census.z == z,
          "wave/plan drift");
      require(serverText.indexOf("[WorldlineCensus] scene ")
                  < serverText.indexOf("[WorldlineWave] removed ")
      ...
      return new Arm(nonce, x, y, z, raw, census, paged, change,
          ...
          sha256(Files.readAllBytes(waveFile)));
    } finally {
      if (server != null) {
        try {
          server.write("stop\n");
  ...
  private Wave parseWave(Path file, int nonce) throws Exception {
    byte[] b = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 40 && in.readInt() == 0x574c3831 && in.readInt() == 1
              && in.readInt() == 40 && in.readInt() == nonce,
          "wave schema drift");
      int x = in.readInt(), y = in.readInt(), z = in.readInt(), request = in.readInt(),
          event = in.readInt(), targets = in.readInt();
      require(in.read() == -1 && request >= 300 && event >= request && targets == 2,
          "wave event drift");
      return new Wave(x, y, z, request, event, targets);
    }
  }
  private Census parseCensus(Path file, Wave change, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && renders == 0 && lists == 0 && visible > 0
            ...
            "wave census record drift at " + i + " values=" + renders + "/" + lists + "/" + calls);
        frames[i] = delta;
  ...
  private Paged parsePaged(Path file, Census census, Wave change, int nonce) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                ...
                && pageCalls == pages && direct == 0 && rebuilds == (i == change.event ? 2 : 0)
                && cached == pages,
            "wave paged record drift at " + i + " calls=" + rc + "/" + qc + "/" + fc + " state="
  ...
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
      Path census, Path paged, Path membership, Plan plan) {
    String wrapper =
    ...
    if (census != null)
      r.addAll(Arrays.asList("-PworldlineArtifact=" + census, "-PworldlinePagedArtifact=" + paged,
          "-PworldlineWaveArtifact=" + membership,
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m81-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M81 cleanup failed " + t, e);
    }
  }
    ...
    return s.lines()
        .filter(x
            -> x.contains("WorldlineCensus") || x.contains("WorldlinePaged")
                || x.contains("WorldlineWave") || x.contains("Loading ") || x.contains("Exception")
                || x.contains("BUILD "))
  ...
  private static final class Wave {
    final int x, y, z, request, event, targets;
    Wave(int x, int y, int z, int r, int e, int t) {
      this.x = x;
      this.y = y;
      this.z = z;
      request = r;
  ...
  private static final class Arm {
    ...
    final Paged paged;
    final Wave change;
    final String censusHash, pagedHash, waveHash;
    Arm(int n, int x, int y, int z, int raw, Census c, Paged p, Wave m, String ch, String ph,
        String mh) {
      nonce = n;
      this.x = x;
      this.y = y;
    ...
    String summary() {
      return "samples=" + census.count + ",request=" + change.request + ",event=" + change.event
          + "," + paged.summary() + ",census=" + censusHash.substring(0, 12)
          + ",paged=" + pagedHash.substring(0, 12) + ",wave=" + waveHash.substring(0, 12);
    }
  }
