...
/** Runs two fresh same-plan index-two larger-page sibling replicas. */
public final class LargerPageSiblingRecoveryCycle {
  private static final String ID = "m90-larger-page-sibling-recovery";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/LargerPageSiblingRecoveryCycle.java " + ID);
      System.exit(2);
    }
    try {
      new LargerPageSiblingRecoveryCycle().execute();
    } catch (Exception e) {
      System.err.println("Larger-page sibling recovery failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m90.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 9009001;
    ...
    if (diagnostic) {
      System.out.println("M90 diagnostic replica passed; qualification not attempted");
    ...
    String trace =
        "v1|design=2-fresh-same-plan-nonce-index2-larger-page-sibling-recovery-replicas|fixture=constant16-synced+client-marker+exact-camera|server-closure=aero-free|frame-limit=vanilla-max0+aero-pacer-off-runtime-gated|warm-path=enqueue16+flush2+cached4+pageCalls4+direct0+render0/list0+rebuild0|remove=request-after300+server-remove-index2+ack+client-air+membership16to15+pageCalls4+direct0+render/list0+rebuild1|restore=request-30-records-after-remove+server-restore-index2+buffered-state+ack+membership15to16+pageCalls4+direct0+render/list0+rebuild1|window=M74-min720intervals+12s|capture=M78-primitive-spans+page-counters+post-seal-52-byte-recovery-sidecar|per-record=identity16to15to16+state16/maskffff|stats=descriptive-transition-dynamic|arbitrary-addition-other-cells-repeat-stale-cleanup-uninstrumented-cost-causality-regression-historical-lag=not-claimed|shutdown=clean";
    String signature = sha256(trace);
    require(signature.equals(value("expected.signature")), "M90 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M90 larger-page sibling recovery passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m90-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      require(marker(removed, "x") == x && marker(removed, "y") == y + 2
              && marker(removed, "z") == z && marker(removed, "nonce") == nonce
              && marker(restored, "x") == x && marker(restored, "y") == y + 2
              && marker(restored, "z") == z && marker(restored, "nonce") == nonce && change.x == x
  ...
  private Recovery parseRecovery(Path file, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 52 && in.readInt() == 0x574c3930 && in.readInt() == 1
      ...
      require(in.read() == -1 && removeRequest >= 300 && removeEvent >= removeRequest
              ...
              && removeRebuild == 1 && restoreRebuild == 1,
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        int renders = in.readInt(), lists = in.readInt(), visible = in.readInt(),
            ...
            expected = i < change.removeEvent || i >= change.restoreEvent ? 16 : 15, direct = 0;
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        int rc = in.readUnsignedShort(), qc = in.readUnsignedShort(), fc = in.readUnsignedShort(),
            ...
            members = i < change.removeEvent || i >= change.restoreEvent ? 16 : 15, pages = 4,
            expectedDirect = 0,
            expectedRebuild = i == change.removeEvent || i == change.restoreEvent ? 1 : 0;
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m90-build-" + System.nanoTime());
    try {
  ...
  private void verifyBoundary() throws IOException {
    for (Path base :
        Arrays.asList(root.resolve("smokes/m74-complete-aero-census/runtime-src/worldline/m74"),
            smoke.resolve("runtime-src/worldline/m74"), smoke.resolve("runtime-src/worldline/m90")))
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M90 cleanup failed " + t, e);
    }
  }
