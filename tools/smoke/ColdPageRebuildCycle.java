...
/** Runs two fresh, same-plan single cold page rebuild replicas. */
public final class ColdPageRebuildCycle {
  private static final String ID = "m79-cold-page-rebuild";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/ColdPageRebuildCycle.java " + ID);
      System.exit(2);
    }
    try {
      new ColdPageRebuildCycle().execute();
    } catch (Exception e) {
      System.err.println("Cold page rebuild failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    require(value("replicas").equals("2") && value("warmup.frames").equals("300")
            ...
            && value("invalidation.after.records").equals("300")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m79.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 7907901;
    ...
    if (diagnostic) {
      System.out.println("M79 diagnostic replica passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M79 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M79 cold page rebuild passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m79-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      Path init = root.resolve(value("runner")), serverGame = workspace.resolve("server"),
           ...
           pagedFile = clientGame.resolve("paged.bin"), coldFile = clientGame.resolve("cold.bin");
      ...
      require(count(client, "[WorldlinePaged] armed ") == 1
              ...
              && count(client, "[WorldlineCold] complete ") == 1,
          "client lifecycle drift");
      Census census = parseCensus(censusFile, nonce);
      Cold cold = parseCold(coldFile, census, nonce);
      Paged paged = parsePaged(pagedFile, census, cold, nonce);
      String complete = unique(client, "[WorldlinePaged] complete "),
             coldComplete = unique(client, "[WorldlineCold] complete ");
      require(marker(complete, "samples") == paged.count
              ...
              && marker(coldComplete, "eventIndex") == cold.index
              && marker(coldComplete, "disposeNs") == cold.dispose
              && marker(coldComplete, "bytes") == Files.size(coldFile)
              && token(coldComplete, "sha256").equals(sha256(Files.readAllBytes(coldFile))),
      ...
      return new Arm(nonce, x, y, z, raw, census, paged, cold,
          sha256(Files.readAllBytes(censusFile)), sha256(Files.readAllBytes(pagedFile)),
          sha256(Files.readAllBytes(coldFile)));
    } finally {
      if (server != null) {
        try {
          server.write("stop\n");
  ...
  private Cold parseCold(Path file, Census census, int nonce) throws Exception {
    byte[] b = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 68 && in.readInt() == 0x574c3739 && in.readInt() == 1
              && in.readInt() == 68 && in.readInt() == nonce && in.readInt() == census.x
              && in.readInt() == census.y && in.readInt() == census.z,
          "cold schema/identity drift");
      int index = in.readInt();
      long dispose = in.readLong();
      int before = in.readInt(), afterDispose = in.readInt(), afterRebuild = in.readInt(),
          compiledBefore = in.readInt(), compiledAfter = in.readInt(), deletedBefore = in.readInt(),
          deletedAfter = in.readInt();
      require(in.read() == -1 && index >= 300 && index < census.count && dispose > 0 && before == 4
              && afterDispose == 0 && afterRebuild == 4 && compiledBefore >= 4
              && compiledAfter - compiledBefore == 4 && deletedBefore >= 0
              && deletedAfter - deletedBefore == 4,
          "cold event drift");
      return new Cold(index, dispose);
    }
  }
  private Paged parsePaged(Path file, Census census, Cold cold, int nonce) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                ...
                && pageCalls == pages && direct == 0 && rebuilds == (i == cold.index ? 4 : 0)
                && cached == pages,
            "paged record drift at " + i + " time=" + renderer[i] + "/" + queue[i] + "/" + flush[i]
                + " calls=" + rc + "/" + qc + "/" + fc + " state=" + queued + "/" + pageCalls + "/"
                + direct + "/" + rebuilds + "/" + cached);
        if (flush[i] == 0)
          zeroFlush++;
        flushSum = Math.addExact(flushSum, flush[i]);
      }
      require(in.read() == -1 && flushSum > 0, "paged aggregate drift");
      return new Paged(count, pages, renderer, queue, flush, zeroFlush, cold.index);
    }
  }
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
      Path census, Path paged, Path cold, Plan plan) {
    String wrapper =
    ...
    if (census != null)
      r.addAll(Arrays.asList("-PworldlineArtifact=" + census, "-PworldlinePagedArtifact=" + paged,
          "-PworldlineColdArtifact=" + cold,
          "-PworldlineInvalidationAfter=" + value("invalidation.after.records"),
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m79-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M79 cleanup failed " + t, e);
    }
  }
    ...
    return s.lines()
        .filter(x
            -> x.contains("WorldlineCensus") || x.contains("WorldlinePaged")
                || x.contains("WorldlineCold") || x.contains("Loading ") || x.contains("Exception")
                || x.contains("BUILD "))
  ...
  private static long[] without(long[] v, int index) {
    long[] r = new long[v.length - 1];
    System.arraycopy(v, 0, r, 0, index);
    System.arraycopy(v, index + 1, r, index, v.length - index - 1);
    return r;
  }
  private static final class Paged {
    final int count, pages, zeroFlush, event;
    final long[] renderer, queue, flush;
    Paged(int n, int p, long[] r, long[] q, long[] f, int zero, int event) {
      count = n;
      pages = p;
      renderer = r;
      queue = q;
    ...
    String summary() {
      long[] wr = without(renderer, event), wq = without(queue, event), wf = without(flush, event);
      return "pages=" + pages + ",event=" + event + ",warmRendererNs=" + q(wr, .5) + "/"
          + q(wr, .95) + "/" + q(wr, .99) + ",warmEnqueueNs=" + q(wq, .5) + "/" + q(wq, .95) + "/"
          + q(wq, .99) + ",warmFlushNs=" + q(wf, .5) + "/" + q(wf, .95) + "/" + q(wf, .99)
          + ",eventNs=" + renderer[event] + "/" + queue[event] + "/" + flush[event]
          + ",flushZero=" + zeroFlush;
    }
  }
  private static final class Arm {
    ...
    final Paged paged;
    final Cold cold;
    final String censusHash, pagedHash, coldHash;
    Arm(int n, int x, int y, int z, int raw, Census c, Paged p, Cold cold, String ch, String ph,
        String eh) {
      nonce = n;
      this.x = x;
      this.y = y;
    ...
    String summary() {
      return "samples=" + census.count + ",disposeNs=" + cold.dispose + "," + paged.summary()
          + ",census=" + censusHash.substring(0, 12) + ",paged=" + pagedHash.substring(0, 12)
          + ",cold=" + coldHash.substring(0, 12);
    }
  }
