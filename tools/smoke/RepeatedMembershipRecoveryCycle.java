...
/** Runs two fresh, same-plan single repeated membership recovery replicas. */
public final class RepeatedMembershipRecoveryCycle {
  private static final String ID = "m86-repeated-membership-recovery";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/RepeatedMembershipRecoveryCycle.java " + ID);
      System.exit(2);
    }
    try {
      new RepeatedMembershipRecoveryCycle().execute();
    } catch (Exception e) {
      System.err.println("Repeated membership recovery failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m86.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 8608601;
    ...
    if (diagnostic) {
      System.out.println("M86 diagnostic replica passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M86 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M86 repeated membership recovery passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m86-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      require(sameCell(removed1, x, y, z, nonce) && sameCell(restored1, x, y, z, nonce)
              && sameCell(removed2, x, y, z, nonce) && sameCell(restored2, x, y, z, nonce)
  ...
  private Recovery parseRecovery(Path file, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 60 && in.readInt() == 0x574c3836 && in.readInt() == 1
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        int renders = in.readInt(), lists = in.readInt(), visible = in.readInt(),
            ...
            expected = change.members(i), direct = expected == 16 ? 0 : 1;
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        int rc = in.readUnsignedShort(), qc = in.readUnsignedShort(), fc = in.readUnsignedShort(),
            ...
            members = change.members(i), pages = members == 16 ? 4 : 3,
            expectedDirect = members == 16 ? 0 : 1,
            expectedRebuild = i == change.events[1] || i == change.events[3] ? 1 : 0;
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m86-build-" + System.nanoTime());
    try {
  ...
  private void verifyBoundary() throws IOException {
    for (Path base :
        Arrays.asList(root.resolve("smokes/m74-complete-aero-census/runtime-src/worldline/m74"),
            smoke.resolve("runtime-src/worldline/m74"), smoke.resolve("runtime-src/worldline/m86")))
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M86 cleanup failed " + t, e);
    }
  }
  ...
  private boolean sameCell(String r, int x, int y, int z, int nonce) {
    return marker(r, "x") == x && marker(r, "y") == y && marker(r, "z") == z
        && marker(r, "nonce") == nonce;
  }
