...
public final class RebuildBudgetOneFallbackCycle {
  private static final String ID = "m100-rebuild-budget-one-fallback";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/RebuildBudgetOneFallbackCycle.java " + ID);
      System.exit(2);
    }
    try {
      new RebuildBudgetOneFallbackCycle().execute();
    } catch (Exception e) {
      System.err.println("Rebuild-budget fallback failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m100.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 10001001;
    ...
    if (diagnostic) {
      System.out.println("M100 diagnostic replica passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M100 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M100 rebuild-budget fallback passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m100-" + ProcessHandle.current().pid() + "-"
  ...
  private Census parseCensus(Path file, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && (renders == 4 || renders == 10) && lists == renders && visible > 0
      ...
      return new Census(x, y, z, count, elapsed, frames, directs);
    }
  }
  private Paged parsePaged(Path file, Census census, int nonce) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      require(in.readInt() == 0x574c6400 && in.readInt() == 1 && in.readInt() == 44
      ...
      int pages = 1, zeroFlush = 0, firstEvicted = -1, priorEvicted = -1, budgetRecords = 0,
          mode4 = 0, mode10 = 0, priorDirect = -1;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                ...
                && ((direct == 4 && pageCalls == 2) || (direct == 10 && pageCalls == 1))
                && direct == census.direct[i] && rebuilds == 1 && cached == pages && evicted > 0
                && (priorEvicted < 0 || evicted == priorEvicted + 1)
                && (priorDirect < 0 || direct != priorDirect),
      ...
      require(in.read() == -1 && flushSum > 0 && mode4 > 0 && mode10 > 0
              && Math.abs(mode4 - mode10) <= 1,
          "paged aggregate drift");
      return new Paged(count, pages, renderer, queue, flush, zeroFlush, firstEvicted, priorEvicted,
          budgetRecords, mode4, mode10);
    }
  }
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m100-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M100 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Census {
    ...
    final long[] frames;
    final int[] direct;
    Census(int x, int y, int z, int n, long e, long[] f, int[] d) {
      this.x = x;
      this.y = y;
      this.z = z;
      count = n;
  ...
  private static final class Paged {
    final int count, pages, zeroFlush, firstEvicted, lastEvicted, budgetRecords, mode4, mode10;
    final long[] renderer, queue, flush;
    Paged(int n, int p, long[] r, long[] q, long[] f, int zero, int first, int last, int budget,
        int m4, int m10) {
      count = n;
      pages = p;
      renderer = r;
    ...
    String summary() {
      return "pages=" + pages + ",mode4/mode10=" + mode4 + "/" + mode10
          + ",budgetPathRecords=" + budgetRecords + ",evicted=" + firstEvicted + ".." + lastEvicted
          + ",rendererNs=" + q(renderer, .5) + "/" + q(renderer, .95) + "/" + q(renderer, .99) + "/"
