...
/** Runs two balanced pairs of fresh cell-size-two and cell-size-eight clients. */
public final class PairedCellSizeCycle {
  private static final String ID = "m108-paired-cell-size";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/PairedCellSizeCycle.java " + ID);
      System.exit(2);
    }
    try {
      new PairedCellSizeCycle().execute();
    } catch (Exception e) {
      System.err.println("Paired cell size failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("pairs").equals("2") && value("order").equals("size2-size8,size8-size2")
            ...
            && value("aero.cell.size.small").equals("2")
            && value("aero.cell.size.large").equals("8") && value("min.instances").equals("2")
            && value("skip.individual").equals("false")
            && value("aero.max.cached.pages").equals("-1")
            && value("aero.rebuilds.per.frame").equals("-1")
            && value("aero.page.ttl.frames").equals("100000")
            && value("aero.cell.pages").equals("true") && value("fps.limit").equals("0")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m108.diagnostic");
    int pairCount = diagnostic ? 1 : 2;
    List<Arm> arms = new ArrayList<>();
    for (int p = 0; p < pairCount; p++) {
      int nonce = 10801001 + p;
      boolean smallFirst = (p & 1) == 0;
      Plan plan = new Plan(2, 80, 0);
      for (int phase = 0; phase < 2; phase++) {
        boolean small = phase == 0 ? smallFirst : !smallFirst;
        verifyCheckout(checkout);
        Arm arm =
            run(checkout, build.resolve("pair-" + (p + 1) + "-" + (small ? "size2" : "size8")),
                p * 2 + phase, nonce, plan, small);
        arms.add(arm);
        verifyCheckout(checkout);
      }
      Arm a = arms.get(p * 2), b = arms.get(p * 2 + 1);
      require(a.nonce == b.nonce && a.x == b.x && a.y == b.y && a.z == b.z && a.raw == b.raw
              && a.small != b.small,
          "pair fixture/mode drift");
    }
    if (diagnostic) {
      System.out.println("M108 diagnostic pair passed; qualification not attempted");
      System.out.println("  " + arms.get(0).summary());
      System.out.println("  " + arms.get(1).summary());
      return;
    }
    require(arms.size() == 4 && arms.stream().filter(x -> x.small).count() == 2,
        "paired arm count drift");
    String trace =
    ...
    require(signature.equals(value("expected.signature")), "M108 signature drift: " + signature);
    StringBuilder evidence =
        new StringBuilder("id=" + ID + "\npairs=2\nserver.jvm=4\nclient.jvm=4\n");
    for (int i = 0; i < arms.size(); i++)
      evidence.append("arm-").append(i + 1).append("=").append(arms.get(i).summary()).append("\n");
    ...
    System.out.println("M108 paired cell size passed");
    for (Arm arm : arms)
      System.out.println("  " + arm.summary());
    System.out.println("  signature: " + signature);
  }
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan, boolean small)
      throws Exception {
    Files.createDirectories(workspace);
    int port = freePort(), timeout = Integer.parseInt(value("timeout.seconds"));
    Path base = root.resolve(".worldline/worktrees/m108-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      server = startServer(serverTree.resolve("stationapi/test-bare"),
          command(serverTree, init, "server", serverGame, port, nonce, null, null, plan, small),
          timeout);
      ...
      String cellSize = small ? "2" : "8";
      require(count(client, "[WorldlinePaged] armed ") == 1
              && unique(client, "[WorldlinePaged] armed ")
                  .equals(
                      "marker=true pages=true minimum=2 cache=-1 fpsLimit=0 aeroFramePacing=false")
              && count(client, "[WorldlineCellSize] armed ") == 1
              && unique(client, "[WorldlineCellSize] armed ")
                  .equals("size=" + cellSize + " minimum=2 skipIndividual=false")
      ...
      Census census = parseCensus(censusFile, nonce, small);
      Paged paged = parsePaged(pagedFile, census, nonce, small);
      ...
      return new Arm(small, nonce, x, y, z, raw, census, paged,
  ...
  private Census parseCensus(Path file, int nonce, boolean small) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && renders == 0 && lists == 0 && visible > 0 && calls == 16
            ...
            "paired census record drift at " + i + " small=" + small + " values=" + renders + "/"
                + lists + "/" + calls);
        frames[i] = delta;
  ...
  private Paged parsePaged(Path file, Census census, int nonce, boolean small) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      require(in.readInt() == 0x574c6800 && in.readInt() == 1 && in.readInt() == 44
              && in.readInt() == 60 && in.readInt() == nonce,
          "paged schema drift");
      int x = in.readInt(), y = in.readInt(), z = in.readInt(), count = in.readInt();
      long elapsed = in.readLong();
      require(x == census.x && y == census.y && z == census.z && count == census.count
              && elapsed == census.elapsed && bytes.length == 44L + count * 60L,
          "paged/census identity drift");
      long[] renderer = new long[count], queue = new long[count], flush = new long[count];
      int cellSize = small ? 2 : 8, zeroFlush = 0, firstEvicted = -1, priorEvicted = -1,
          immediateRecords = 0;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        int rc = in.readUnsignedShort(), qc = in.readUnsignedShort(), fc = in.readUnsignedShort(),
            ...
            evicted = in.readInt(), immediate = in.readInt();
        boolean state = small ? queued == 16 && pageCalls == 4 && direct == 0 && rebuilds == 0
                && cached == 4 && evicted == 0 && immediate == 0
                              : queued == 16 && pageCalls == 1 && direct == 0 && rebuilds == 0
                && cached == 1 && evicted == 0 && immediate == 0;
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] > 0 && renderer[i] >= queue[i]
                && rc == 16 && qc == 16 && fc == 2 && reserved == 0 && state,
            "paired cell-size record drift at " + i + " small=" + small + " time=" + renderer[i]
                + "/" + queue[i] + "/" + flush[i] + " calls=" + rc + "/" + qc + "/" + fc + " state="
                + queued + "/" + pageCalls + "/" + direct + "/" + rebuilds + "/" + cached
                + " evicted=" + evicted + " immediate=" + immediate + " prior=" + priorEvicted);
        if (immediate > 0)
          immediateRecords++;
        if (firstEvicted < 0)
      ...
      require(in.read() == -1 && flushSum > 0 && immediateRecords == 0, "paged aggregate drift");
      return new Paged(small, count, cellSize, renderer, queue, flush, zeroFlush, firstEvicted,
          priorEvicted, immediateRecords);
    }
  }
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
      Path census, Path paged, Plan plan, boolean small) {
    String wrapper =
    ...
    if (census != null)
      r.addAll(Arrays.asList("-PworldlineArtifact=" + census, "-PworldlinePagedArtifact=" + paged,
          "-PworldlineCellPages=" + value("aero.cell.pages"),
          "-PworldlineMinInstances=" + value("min.instances"),
          "-PworldlineSkipIndividual=" + value("skip.individual"),
          "-PworldlineCellSize=" + value(small ? "aero.cell.size.small" : "aero.cell.size.large"),
          "-PworldlineMaxCachedPages=" + value("aero.max.cached.pages"),
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m108-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M108 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Paged {
    final boolean small;
    final int count, cellSize, zeroFlush, firstEvicted, lastEvicted, immediateRecords;
    final long[] renderer, queue, flush;
    Paged(boolean small, int n, int p, long[] r, long[] q, long[] f, int zero, int first, int last,
        int immediate) {
      this.small = small;
      count = n;
      cellSize = p;
      ...
      firstEvicted = first;
      lastEvicted = last;
      immediateRecords = immediate;
    }
    String summary() {
      return "mode=" + (small ? "size2" : "size8") + ",samples=" + count + ",cellSize=" + cellSize
          + ",immediateRecords=" + immediateRecords + ",evicted=" + firstEvicted + ".."
          + lastEvicted + ",rendererNs=" + q(renderer, .5) + "/" + q(renderer, .95) + "/"
          + q(renderer, .99) + "/" + Arrays.stream(renderer).max().orElseThrow()
          + ",enqueueNs=" + q(queue, .5) + "/" + q(queue, .95) + "/" + q(queue, .99) + "/"
          + Arrays.stream(queue).max().orElseThrow() + ",flushNs=" + q(flush, .5) + "/"
          + q(flush, .95) + "/" + q(flush, .99) + "/" + Arrays.stream(flush).max().orElseThrow()
          + ",flushZero=" + zeroFlush;
    }
  }
  private static final class Arm {
    ...
    Arm(boolean small, int n, int x, int y, int z, int raw, Census c, Paged p, String ch,
        String ph) {
      this.small = small;
      nonce = n;
      this.x = x;
    ...
    String summary() {
      return "nonce=" + nonce + "," + paged.summary() + ",census=" + censusHash.substring(0, 12)
          + ",paged=" + pagedHash.substring(0, 12);
    }
  }
