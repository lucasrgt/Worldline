...
/** Runs fresh same-plan 1/2/4-member natural rebuild-wave arms. */
public final class NaturalWaveLadderCycle {
  private static final String ID = "m82-natural-wave-ladder";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/NaturalWaveLadderCycle.java " + ID);
      System.exit(2);
    }
    try {
      new NaturalWaveLadderCycle().execute();
    } catch (Exception e) {
      System.err.println("Natural wave ladder failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("modes").equals("1,2,4") && value("plan.x").equals("10")
    ...
    recreate(build);
    buildAero(checkout);
    int diagnostic = Integer.getInteger("worldline.m82.diagnostic.targets", 0);
    require(diagnostic == 0 || diagnostic == 1 || diagnostic == 2 || diagnostic == 4,
        "invalid M82 diagnostic target");
    int[] modes = diagnostic == 0 ? new int[] {1, 2, 4} : new int[] {diagnostic};
    int nonce = 8208201;
    List<Arm> arms = new ArrayList<>();
    Plan plan = new Plan(Integer.parseInt(value("plan.x")), Integer.parseInt(value("plan.y")),
        Integer.parseInt(value("plan.z")));
    for (int targets : modes) {
      verifyCheckout(checkout);
      Arm arm = run(checkout, build.resolve("targets-" + targets), targets, nonce, plan);
      arms.add(arm);
      verifyCheckout(checkout);
    }
    if (diagnostic != 0) {
      System.out.println("M82 diagnostic arm passed; qualification not attempted");
      System.out.println("  " + arms.get(0).summary());
      return;
    }
    require(arms.size() == 3, "M82 arm count drift");
    Arm one = arms.get(0), two = arms.get(1), four = arms.get(2);
    for (Arm a : arms)
      require(a.nonce == nonce && a.x == plan.x && a.y == plan.y && a.z == plan.z
              && a.raw == one.raw && a.paged.prePages == 2 && a.paged.postPages == 2,
          "arm fixture/page drift");
    require(one.targets == 1 && two.targets == 2 && four.targets == 4, "arm order drift");
    String trace =
    ...
    require(signature.equals(value("expected.signature")), "M82 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\narms=3\nserver.jvm=3\nclient.jvm=3\none=" + one.summary()
            + "\ntwo=" + two.summary() + "\nfour=" + four.summary() + "\ntrace=" + trace
            + "\nsignature=" + signature + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M82 natural wave ladder passed");
    System.out.println("  one: " + one.summary());
    System.out.println("  two: " + two.summary());
    System.out.println("  four: " + four.summary());
    System.out.println("  signature: " + signature);
  }
  private Arm run(Path checkout, Path workspace, int targets, int nonce, Plan plan)
      throws Exception {
    Files.createDirectories(workspace);
    int port = freePort(), timeout = Integer.parseInt(value("timeout.seconds"));
    Path base = root.resolve(".worldline/worktrees/m82-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      Path init = root.resolve(value("runner")), serverGame = workspace.resolve("server"),
           ...
           ladderFile = clientGame.resolve("ladder.bin");
      server = startServer(serverTree.resolve("stationapi/test-bare"),
          command(
              serverTree, init, "server", serverGame, port, nonce, targets, null, null, null, plan),
          timeout);
      String client = runGradle(clientTree.resolve("stationapi/test-bare"),
          command(clientTree, init, "client", clientGame, port, nonce, targets, censusFile,
              pagedFile, ladderFile, null),
          timeout);
      ...
      require(count(client, "[WorldlinePaged] armed ") == 1
              ...
              && count(client, "[WorldlineLadder] complete ") == 1,
          "client lifecycle drift");
      Ladder change = parseLadder(ladderFile, nonce, targets);
      ...
      String complete = unique(client, "[WorldlinePaged] complete "),
             changed = unique(client, "[WorldlineLadder] complete ");
      require(marker(complete, "samples") == paged.count
              ...
              && marker(changed, "targets") == targets
              && marker(changed, "rebuilds") == change.rebuilds
              && marker(changed, "bytes") == Files.size(ladderFile)
              && token(changed, "sha256").equals(sha256(Files.readAllBytes(ladderFile))),
      ...
      String scene = unique(serverText, "[WorldlineCensus] scene "),
             removed = unique(serverText, "[WorldlineLadder] removed ");
      ...
      require(marker(removed, "targets") == targets && marker(removed, "x") == x
              && marker(removed, "y") == y && marker(removed, "z") == z
              && marker(removed, "nonce") == nonce && change.targets == targets && change.x == x
      ...
      require(serverText.indexOf("[WorldlineCensus] scene ")
                  < serverText.indexOf("[WorldlineLadder] removed ")
      ...
      return new Arm(targets, nonce, x, y, z, raw, census, paged, change,
          sha256(Files.readAllBytes(censusFile)), sha256(Files.readAllBytes(pagedFile)),
          sha256(Files.readAllBytes(ladderFile)));
    } finally {
      if (server != null) {
        try {
          server.write("stop\n");
  ...
  private Ladder parseLadder(Path file, int nonce, int expectedTargets) throws Exception {
    byte[] b = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 44 && in.readInt() == 0x574c3832 && in.readInt() == 1
              && in.readInt() == 44 && in.readInt() == nonce,
          "ladder schema drift");
      int x = in.readInt(), y = in.readInt(), z = in.readInt(), request = in.readInt(),
          event = in.readInt(), targets = in.readInt(), rebuilds = in.readInt();
      require(in.read() == -1 && request >= 300 && event >= request && targets == expectedTargets
              && rebuilds == (targets == 1 ? 1 : 2),
          "ladder event drift");
      return new Ladder(x, y, z, request, event, targets, rebuilds);
    }
  }
  private Census parseCensus(Path file, Ladder change, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && renders == 0 && lists == 0 && visible > 0
                && calls == (i < change.event ? 16 : 16 - change.targets) && state == 0x1010
                && mask == 0xffff,
            "ladder census record drift at " + i + " values=" + renders + "/" + lists + "/"
                + calls);
        frames[i] = delta;
  ...
  private Paged parsePaged(Path file, Census census, Ladder change, int nonce) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        int rc = in.readUnsignedShort(), qc = in.readUnsignedShort(), fc = in.readUnsignedShort(),
            ...
            members = i < change.event ? 16 : 16 - change.targets,
            pages = i < change.event ? prePages : postPages;
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
            ...
            "ladder paged record drift at " + i + " calls=" + rc + "/" + qc + "/" + fc + " state="
  ...
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
      int targets, Path census, Path paged, Path ladder, Plan plan) {
    String wrapper =
    ...
    List<String> r = new ArrayList<>(Arrays.asList(
        ...
        "-PworldlineNonce=" + nonce, "-PworldlineTargets=" + targets));
    if (plan != null)
    ...
    if (census != null)
      r.addAll(Arrays.asList("-PworldlineArtifact=" + census, "-PworldlinePagedArtifact=" + paged,
          "-PworldlineLadderArtifact=" + ladder,
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m82-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M82 cleanup failed " + t, e);
    }
  }
    ...
    return s.lines()
        .filter(x
            -> x.contains("WorldlineCensus") || x.contains("WorldlinePaged")
                || x.contains("WorldlineLadder") || x.contains("Loading ")
  ...
  private static final class Ladder {
    final int x, y, z, request, event, targets, rebuilds;
    Ladder(int x, int y, int z, int r, int e, int t, int b) {
      this.x = x;
      this.y = y;
      this.z = z;
      request = r;
  ...
  private static final class Arm {
    final int targets, nonce, x, y, z, raw;
    final Census census;
    final Paged paged;
    final Ladder change;
    final String censusHash, pagedHash, ladderHash;
    Arm(int targets, int n, int x, int y, int z, int raw, Census c, Paged p, Ladder m, String ch,
        String ph, String mh) {
    ...
    String summary() {
      return "targets=" + targets + ",samples=" + census.count + ",request=" + change.request
          + ",event=" + change.event + ",rebuilds=" + change.rebuilds + "," + paged.summary()
          + ",census=" + censusHash.substring(0, 12) + ",paged=" + pagedHash.substring(0, 12)
          + ",ladder=" + ladderHash.substring(0, 12);
    }
  }
