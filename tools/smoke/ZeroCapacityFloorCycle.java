...
public final class ZeroCapacityFloorCycle {
  private static final String ID = "m98-zero-capacity-floor";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/ZeroCapacityFloorCycle.java " + ID);
      System.exit(2);
    }
    try {
      new ZeroCapacityFloorCycle().execute();
    } catch (Exception e) {
      System.err.println("Zero-capacity floor failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    require(value("replicas").equals("2") && value("warmup.frames").equals("300")
            ...
            && value("minimum.millis").equals("12000") && value("max.cached.pages").equals("0")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m98.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 9809801;
    ...
    if (diagnostic) {
      System.out.println("M98 diagnostic replica passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M98 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M98 zero-capacity protected floor passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m98-" + ProcessHandle.current().pid() + "-"
  ...
  private Paged parsePaged(Path file, Census census, int nonce) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      require(in.readInt() == 0x574c3938 && in.readInt() == 1 && in.readInt() == 44
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m98-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M98 cleanup failed " + t, e);
    }
  }
