...
/** Runs two balanced pairs of fresh cache-one and min5-cache Aero clients. */
public final class PairedMinimumInstancesCycle {
  private static final String ID = "m106-paired-min-instances";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/PairedMinimumInstancesCycle.java " + ID);
      System.exit(2);
    }
    try {
      new PairedMinimumInstancesCycle().execute();
    } catch (Exception e) {
      System.err.println("Paired minimum instances failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("pairs").equals("2") && value("order").equals("min2-min5,min5-min2")
            ...
            && value("min.instances.low").equals("2") && value("min.instances.high").equals("5")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m106.diagnostic");
    int pairCount = diagnostic ? 1 : 2;
    List<Arm> arms = new ArrayList<>();
    for (int p = 0; p < pairCount; p++) {
      int nonce = 10601001 + p;
      boolean lowFirst = (p & 1) == 0;
      Plan plan = null;
      for (int phase = 0; phase < 2; phase++) {
        boolean low = phase == 0 ? lowFirst : !lowFirst;
        verifyCheckout(checkout);
        Arm arm = run(checkout, build.resolve("pair-" + (p + 1) + "-" + (low ? "min2" : "min5")),
            p * 2 + phase, nonce, plan, low);
        arms.add(arm);
        if (plan == null)
    ...
    if (diagnostic) {
      System.out.println("M106 diagnostic pair passed; qualification not attempted");
      System.out.println("  " + arms.get(0).summary());
      System.out.println("  " + arms.get(1).summary());
      return;
    }
    require(arms.size() == 4 && arms.stream().filter(x -> x.low).count() == 2,
    ...
    require(signature.equals(value("expected.signature")), "M106 signature drift: " + signature);
    ...
    System.out.println("M106 paired minimum instances passed");
    for (Arm arm : arms)
      System.out.println("  " + arm.summary());
    System.out.println("  signature: " + signature);
  }
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan, boolean low)
      throws Exception {
    Files.createDirectories(workspace);
    int port = freePort(), timeout = Integer.parseInt(value("timeout.seconds"));
    Path base = root.resolve(".worldline/worktrees/m106-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      server = startServer(serverTree.resolve("stationapi/test-bare"),
          command(serverTree, init, "server", serverGame, port, nonce, null, null, plan, low),
          timeout);
      ...
      Census census = parseCensus(censusFile, nonce, low);
      Paged paged = parsePaged(pagedFile, census, nonce, low);
      ...
      return new Arm(low, nonce, x, y, z, raw, census, paged,
  ...
  private Census parseCensus(Path file, int nonce, boolean low) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && renders == (low ? 0 : 4) && lists == (low ? 0 : 4) && visible > 0
                && calls == 16 && state == 0x1010 && mask == 0xffff,
            "paired census record drift at " + i + " low=" + low + " values=" + renders + "/"
                + lists + "/" + calls);
        frames[i] = delta;
  ...
  private Paged parsePaged(Path file, Census census, int nonce, boolean low) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      int minimum = low ? 2 : 5, zeroFlush = 0, firstEvicted = -1, priorEvicted = -1,
          immediateRecords = 0;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        boolean state = low ? queued == 16 && pageCalls == 4 && direct == 0 && rebuilds == 0
                && cached == 4 && evicted == 0 && immediate == 0
                            : queued == 16 && pageCalls == 2 && direct == 4 && rebuilds == 0
                && cached == 2 && evicted == 0 && immediate == 0;
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] > 0 && renderer[i] >= queue[i]
                && rc == 16 && qc == 16 && fc == 2 && reserved == 0 && state,
            "paired minimum record drift at " + i + " low=" + low + " time=" + renderer[i] + "/"
      ...
      return new Paged(low, count, minimum, renderer, queue, flush, zeroFlush, firstEvicted,
          priorEvicted, immediateRecords);
    }
  }
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
      Path census, Path paged, Plan plan, boolean low) {
    String wrapper =
    ...
    if (census != null)
      r.addAll(Arrays.asList("-PworldlineArtifact=" + census, "-PworldlinePagedArtifact=" + paged,
          ...
          "-PworldlineMinInstances=" + value(low ? "min.instances.low" : "min.instances.high"),
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m106-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M106 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Paged {
    final boolean low;
    final int count, minimum, zeroFlush, firstEvicted, lastEvicted, immediateRecords;
    final long[] renderer, queue, flush;
    Paged(boolean low, int n, int p, long[] r, long[] q, long[] f, int zero, int first, int last,
        int immediate) {
      this.low = low;
      count = n;
      minimum = p;
    ...
    String summary() {
      return "mode=" + (low ? "min2" : "min5") + ",samples=" + count + ",minimum=" + minimum
  ...
  private static final class Arm {
    ...
    Arm(boolean low, int n, int x, int y, int z, int raw, Census c, Paged p, String ch, String ph) {
      this.low = low;
      nonce = n;
      this.x = x;
      this.y = y;
