...
public final class TwoCellMembershipRecoveryCycle {
  private static final String ID = "m87-two-cell-membership-recovery";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/TwoCellMembershipRecoveryCycle.java " + ID);
      System.exit(2);
    }
    try {
      new TwoCellMembershipRecoveryCycle().execute();
    } catch (Exception e) {
      System.err.println("Two-cell membership recovery failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m87.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 8708701;
    ...
    if (diagnostic) {
      System.out.println("M87 diagnostic replica passed; qualification not attempted");
    ...
    String trace =
        "v1|design=2-fresh-same-plan-nonce-two-distinct-cell-membership-recovery-replicas|fixture=constant16-synced+client-marker+exact-camera|server-closure=aero-free|frame-limit=vanilla-max0+aero-pacer-off-runtime-gated|warm-path=enqueue16+flush2+cached4+pageCalls4+direct0+render0/list0+rebuild0|sequence=remove-index0+restore-index0+remove-index1+restore-index1-with-generation+index-bound-ack/state|spacing=request1-after300+each-next-request-30-records-after-prior-event|membership=16to15to16to15to16|index0-removed=pageCalls3+direct1+render/list1+rebuild0|index0-restored=pageCalls4+direct0+render/list0+rebuild1|index1-removed=pageCalls4+direct0+render/list0+rebuild1|index1-restored=pageCalls4+direct0+render/list0+rebuild1|window=M74-min720intervals+12s|capture=M78-primitive-spans+page-counters+post-seal-76-byte-recovery-sidecar|per-record=state16/maskffff|stats=descriptive-transition-dynamic|arbitrary-addition-more-than2-cells-concurrency-stale-cleanup-uninstrumented-cost-causality-regression-historical-lag=not-claimed|shutdown=clean";
    String signature = sha256(trace);
    require(signature.equals(value("expected.signature")), "M87 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M87 two-cell membership recovery passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m87-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      require(sameCell(removed1, x, y, z, nonce, 0) && sameCell(restored1, x, y, z, nonce, 0)
              && sameCell(removed2, x, y, z, nonce, 1) && sameCell(restored2, x, y, z, nonce, 1)
  ...
  private Recovery parseRecovery(Path file, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 76 && in.readInt() == 0x574c3837 && in.readInt() == 1
      ...
      for (int i = 0; i < 4; i++) {
        ...
        require(index == i / 2, "recovery cell index drift " + i);
      }
      for (int i = 0; i < 4; i++)
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        int rc = in.readUnsignedShort(), qc = in.readUnsignedShort(), fc = in.readUnsignedShort(),
            ...
            expectedRebuild =
                i == change.events[1] || i == change.events[2] || i == change.events[3] ? 1 : 0;
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m87-build-" + System.nanoTime());
    try {
  ...
  private void verifyBoundary() throws IOException {
    for (Path base :
        Arrays.asList(root.resolve("smokes/m74-complete-aero-census/runtime-src/worldline/m74"),
            smoke.resolve("runtime-src/worldline/m74"), smoke.resolve("runtime-src/worldline/m87")))
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M87 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Recovery {
    ...
    int direct(int i) {
      return i >= events[0] && i < events[1] ? 1 : 0;
    }
    String requests() {
