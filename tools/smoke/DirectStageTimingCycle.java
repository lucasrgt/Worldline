...
/** Runs two fresh, same-plan direct renderer/Aero stage timing replicas. */
public final class DirectStageTimingCycle {
  private static final String ID = "m77-direct-stage-timing";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/DirectStageTimingCycle.java " + ID);
      System.exit(2);
    }
    try {
      new DirectStageTimingCycle().execute();
    } catch (Exception e) {
      System.err.println("Direct stage timing failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    ...
    require(value("replicas").equals("2") && value("warmup.frames").equals("300")
            ...
            && value("minimum.millis").equals("12000") && value("fps.limit").equals("0")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m77.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 7707701;
    List<Arm> arms = new ArrayList<>();
    Plan plan = null;
    for (int i = 0; i < runs; i++) {
      verifyCheckout(checkout);
      Path workspace = build.resolve("replica-" + (i + 1));
      Arm arm = run(checkout, workspace, i, nonce, plan);
      arms.add(arm);
      if (plan == null)
    ...
    if (diagnostic) {
      System.out.println("M77 diagnostic replica passed; qualification not attempted");
    ...
    String trace =
        "v1|design=2-fresh-same-plan-nonce-replicas|fixture=constant16-synced-aero+exact-camera|frame-limit=vanilla-max0+aero-pacer-off-runtime-gated"
        + "|timers=renderer+queueAtRest+flush-synchronous-nanoTime-primitive|window=M74-min720intervals+12s|capture=no-retained-allocation-io+post-seal-sidecar"
        + "|per-record=renderer16+queue16+flush2+state16/maskffff|clock=zero-flush-spans-preserved+series-positive|stats=descriptive-direct-spans-dynamic|uninstrumented-cost-causality-regression-historical-lag=not-claimed|shutdown=clean";
    String signature = sha256(trace);
    require(signature.equals(value("expected.signature")), "M77 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M77 direct stage timing passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m77-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      Path init = root.resolve(value("runner")), serverGame = workspace.resolve("server"),
           ...
           stageFile = clientGame.resolve("stages.bin");
      ...
      String client = runGradle(clientTree.resolve("stationapi/test-bare"),
          command(clientTree, init, "client", clientGame, port, nonce, censusFile, stageFile, null),
          timeout);
      ...
      require(count(client, "[WorldlineStage] armed ") == 1
              && unique(client, "[WorldlineStage] armed ")
                  .equals("fpsLimit=0 aeroFramePacing=false")
              ...
              && count(client, "[WorldlineStage] complete ") == 1,
          "client lifecycle drift");
      Census census = parseCensus(censusFile, nonce);
      Stage stage = parseStage(stageFile, census, nonce);
      String complete = unique(client, "[WorldlineStage] complete ");
      require(marker(complete, "samples") == stage.count
              && marker(complete, "bytes") == Files.size(stageFile)
              && token(complete, "sha256").equals(sha256(Files.readAllBytes(stageFile))),
          "stage marker drift");
      ...
      return new Arm(nonce, x, y, z, raw, census, stage, sha256(Files.readAllBytes(censusFile)),
          sha256(Files.readAllBytes(stageFile)));
    } finally {
      if (server != null) {
        try {
          server.write("stop\n");
  ...
  private Stage parseStage(Path file, Census census, int nonce) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      require(in.readInt() == 0x574c3737 && in.readInt() == 1 && in.readInt() == 44
              && in.readInt() == 32 && in.readInt() == nonce,
          "stage schema drift");
      int x = in.readInt(), y = in.readInt(), z = in.readInt(), count = in.readInt();
      long elapsed = in.readLong();
      require(x == census.x && y == census.y && z == census.z && count == census.count
              && elapsed == census.elapsed && bytes.length == 44L + count * 32L,
          "stage/census identity drift");
      long[] renderer = new long[count], queue = new long[count], flush = new long[count];
      int minFlush = Integer.MAX_VALUE, maxFlush = 0, zeroFlush = 0;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        require(renderer[i] > 0 && queue[i] > 0 && flush[i] >= 0 && renderer[i] >= queue[i]
                && rc == 16 && qc == 16 && fc == 2 && reserved == 0,
            "stage record drift at " + i + " values=" + renderer[i] + "/" + queue[i] + "/"
                + flush[i] + " calls=" + rc + "/" + qc + "/" + fc);
        if (flush[i] == 0)
          zeroFlush++;
        flushSum = Math.addExact(flushSum, flush[i]);
        minFlush = Math.min(minFlush, fc);
        maxFlush = Math.max(maxFlush, fc);
      }
      require(in.read() == -1 && flushSum > 0, "stage aggregate drift");
      return new Stage(count, renderer, queue, flush, minFlush, maxFlush, zeroFlush);
    }
  }
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
      Path census, Path stage, Plan plan) {
    String wrapper =
    ...
    if (census != null)
      r.addAll(Arrays.asList("-PworldlineArtifact=" + census, "-PworldlineStageArtifact=" + stage,
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m77-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M77 cleanup failed " + t, e);
    }
  }
  ...
  private String diagnostic(String s) {
    return s.lines()
        .filter(x
            -> x.contains("WorldlineCensus") || x.contains("WorldlineStage")
  ...
  private static final class Stage {
    final int count, minFlush, maxFlush, zeroFlush;
    final long[] renderer, queue, flush;
    Stage(int n, long[] r, long[] q, long[] f, int min, int max, int zero) {
      count = n;
      renderer = r;
      queue = q;
      flush = f;
    ...
    String summary() {
      return "rendererNs=" + q(renderer, .5) + "/" + q(renderer, .95) + "/" + q(renderer, .99) + "/"
          + Arrays.stream(renderer).max().orElseThrow() + ",queueNs=" + q(queue, .5) + "/"
          + q(queue, .95) + "/" + q(queue, .99) + "/" + Arrays.stream(queue).max().orElseThrow()
          + ",flushNs=" + q(flush, .5) + "/" + q(flush, .95) + "/" + q(flush, .99) + "/"
          + Arrays.stream(flush).max().orElseThrow() + ",flushCalls=" + minFlush + "-" + maxFlush
          + ",flushZero=" + zeroFlush;
    }
  }
  private static final class Arm {
    ...
    final Census census;
    final Stage stage;
    final String censusHash, stageHash;
    Arm(int n, int x, int y, int z, int raw, Census c, Stage s, String ch, String sh) {
      nonce = n;
      this.x = x;
      this.y = y;
      this.z = z;
    ...
    String summary() {
      return "samples=" + census.count + "," + stage.summary()
          + ",census=" + censusHash.substring(0, 12) + ",stages=" + stageHash.substring(0, 12);
    }
  }
