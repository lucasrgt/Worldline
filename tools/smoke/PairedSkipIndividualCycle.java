...
/** Runs two balanced pairs of fresh cache-one and skip-false-cache Aero clients. */
public final class PairedSkipIndividualCycle {
  private static final String ID = "m107-paired-skip-individual";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/PairedSkipIndividualCycle.java " + ID);
      System.exit(2);
    }
    try {
      new PairedSkipIndividualCycle().execute();
    } catch (Exception e) {
      System.err.println("Paired skip individual failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("pairs").equals("2") && value("order").equals("skip-true-false,false-true")
            ...
            && value("min.instances").equals("2") && value("skip.individual.true").equals("true")
            && value("skip.individual.false").equals("false")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m107.diagnostic");
    int pairCount = diagnostic ? 1 : 2;
    List<Arm> arms = new ArrayList<>();
    for (int p = 0; p < pairCount; p++) {
      int nonce = 10701001 + p;
      boolean skipFirst = (p & 1) == 0;
      Plan plan = null;
      for (int phase = 0; phase < 2; phase++) {
        boolean skip = phase == 0 ? skipFirst : !skipFirst;
        verifyCheckout(checkout);
        Arm arm = run(checkout,
            build.resolve("pair-" + (p + 1) + "-" + (skip ? "skip-true" : "skip-false")),
            p * 2 + phase, nonce, plan, skip);
        arms.add(arm);
        if (plan == null)
    ...
    if (diagnostic) {
      System.out.println("M107 diagnostic pair passed; qualification not attempted");
      System.out.println("  " + arms.get(0).summary());
      System.out.println("  " + arms.get(1).summary());
      return;
    }
    require(arms.size() == 4 && arms.stream().filter(x -> x.skip).count() == 2,
    ...
    require(signature.equals(value("expected.signature")), "M107 signature drift: " + signature);
    ...
    System.out.println("M107 paired skip individual passed");
    for (Arm arm : arms)
      System.out.println("  " + arm.summary());
    System.out.println("  signature: " + signature);
  }
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan, boolean skip)
      throws Exception {
    Files.createDirectories(workspace);
    int port = freePort(), timeout = Integer.parseInt(value("timeout.seconds"));
    Path base = root.resolve(".worldline/worktrees/m107-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      server = startServer(serverTree.resolve("stationapi/test-bare"),
          command(serverTree, init, "server", serverGame, port, nonce, null, null, plan, skip),
          timeout);
      ...
      Census census = parseCensus(censusFile, nonce, skip);
      Paged paged = parsePaged(pagedFile, census, nonce, skip);
      ...
      return new Arm(skip, nonce, x, y, z, raw, census, paged,
  ...
  private Census parseCensus(Path file, int nonce, boolean skip) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && renders == 0 && lists == 0 && visible > 0 && calls == 16
            ...
            "paired census record drift at " + i + " skip=" + skip + " values=" + renders + "/"
                + lists + "/" + calls);
        frames[i] = delta;
  ...
  private Paged parsePaged(Path file, Census census, int nonce, boolean skip) throws Exception {
    byte[] bytes = Files.readAllBytes(file);
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      int minimum = 2, zeroFlush = 0, firstEvicted = -1, priorEvicted = -1, immediateRecords = 0;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        boolean state = queued == 16 && pageCalls == 4 && direct == 0 && rebuilds == 0
            && cached == 4 && evicted == 0 && immediate == 0;
        require((skip ? renderer[i] == 0 && rc == 0
                      : renderer[i] > 0 && renderer[i] >= queue[i] && rc == 16)
                && queue[i] > 0 && flush[i] > 0 && qc == 16 && fc == 2 && reserved == 0 && state,
            "paired minimum record drift at " + i + " skip=" + skip + " time=" + renderer[i] + "/"
      ...
      return new Paged(skip, count, minimum, renderer, queue, flush, zeroFlush, firstEvicted,
          priorEvicted, immediateRecords);
    }
  }
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
      Path census, Path paged, Plan plan, boolean skip) {
    String wrapper =
    ...
    if (census != null)
      r.addAll(Arrays.asList("-PworldlineArtifact=" + census, "-PworldlinePagedArtifact=" + paged,
          ...
          "-PworldlineSkipIndividual="
              + value(skip ? "skip.individual.true" : "skip.individual.false"),
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m107-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      if (Files.exists(t)) {
        Path alskiped = root.resolve(".worldline/worktrees").normalize(),
             exact = t.toAbsolutePath().normalize();
        require(exact.startsWith(alskiped) && !exact.equals(alskiped), "unsafe remainder");
      ...
      throw new IllegalStateException("M107 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Paged {
    ...
    Paged(boolean skip, int n, int p, long[] r, long[] q, long[] f, int zero, int first, int last,
        int immediate) {
      this.skip = skip;
      count = n;
      minimum = p;
    ...
    String summary() {
      return "mode=" + (skip ? "skip-true" : "skip-false") + ",samples=" + count
          + ",minimum=" + minimum + ",immediateRecords=" + immediateRecords
  ...
  private static final class Arm {
    ...
    Arm(boolean skip, int n, int x, int y, int z, int raw, Census c, Paged p, String ch,
        String ph) {
      this.skip = skip;
      nonce = n;
      this.x = x;
