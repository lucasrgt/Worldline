...
public final class CellSizeFloorCycle {
  private static final String ID = "m109-cell-size-floor";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/CellSizeFloorCycle.java " + ID);
      System.exit(2);
    }
    try {
      new CellSizeFloorCycle().execute();
    } catch (Exception e) {
      System.err.println("Cell size floor failed: " + e.getMessage());
      System.exit(1);
    }
  }
  private void execute() throws Exception {
    load(smoke.resolve("smoke.properties"), config);
    require(value("pairs").equals("2") && value("order").equals("raw0-raw1,raw1-raw0")
            ...
            && value("aero.cell.size.floor").equals("0")
            && value("aero.cell.size.exact").equals("1") && value("min.instances").equals("2")
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m109.diagnostic");
    int pairCount = diagnostic ? 1 : 2;
    List<Arm> arms = new ArrayList<>();
    for (int p = 0; p < pairCount; p++) {
      int nonce = 10901001 + p;
      boolean smallFirst = (p & 1) == 0;
      Plan plan = new Plan(2, 80, 0);
      for (int phase = 0; phase < 2; phase++) {
        boolean small = phase == 0 ? smallFirst : !smallFirst;
        verifyCheckout(checkout);
        Arm arm = run(checkout, build.resolve("pair-" + (p + 1) + "-" + (small ? "raw0" : "raw1")),
    ...
    if (diagnostic) {
      System.out.println("M109 diagnostic pair passed; qualification not attempted");
    ...
    require(signature.equals(value("expected.signature")), "M109 signature drift: " + signature);
    ...
    System.out.println("M109 cell size floor passed");
    for (Arm arm : arms)
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan, boolean small)
    ...
    Path base = root.resolve(".worldline/worktrees/m109-" + ProcessHandle.current().pid() + "-"
    ...
    try {
      ...
      require(count(client, "[WorldlinePaged] armed ") == 1
              ...
              && count(client, "[WorldlineCellSizeFloor] armed ") == 1
              && unique(client, "[WorldlineCellSizeFloor] armed ")
                  .equals("raw=" + rawSize + " effective=1 minimum=2 skipIndividual=false")
  ...
  private Census parseCensus(Path file, int nonce, boolean small) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        require(delta > 0 && renders == 16 && lists == 16 && visible > 0 && calls == 16
  ...
  private Paged parsePaged(Path file, Census census, int nonce, boolean small) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
      ...
      int effectiveSize = 1, zeroFlush = 0, firstEvicted = -1, priorEvicted = -1,
          immediateRecords = 0;
      long flushSum = 0;
      for (int i = 0; i < count; i++) {
        ...
        boolean state = queued == 16 && pageCalls == 0 && direct == 16 && rebuilds == 0
            && cached == 0 && evicted == 0 && immediate == 0;
      ...
      return new Paged(small, count, effectiveSize, renderer, queue, flush, zeroFlush, firstEvicted,
  ...
  private List<String> command(Path tree, Path init, String role, Path game, int port, int nonce,
    ...
    if (census != null)
      r.addAll(Arrays.asList("-PworldlineArtifact=" + census, "-PworldlinePagedArtifact=" + paged,
          ...
          "-PworldlineCellSize=" + value(small ? "aero.cell.size.floor" : "aero.cell.size.exact"),
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m109-build-" + System.nanoTime());
    try {
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M109 cleanup failed " + t, e);
    }
  }
  ...
  private static final class Paged {
    final boolean small;
    final int count, effectiveSize, zeroFlush, firstEvicted, lastEvicted, immediateRecords;
    ...
    String summary() {
      return "mode=" + (small ? "raw0" : "raw1") + ",samples=" + count
          + ",effectiveSize=" + effectiveSize + ",immediateRecords=" + immediateRecords
