...
public final class FullPageDepletionRecoveryCycle {
  private static final String ID = "m93-full-page-depletion-recovery";
  ...
  public static void main(String[] a) {
    if (!Arrays.equals(a, new String[] {ID})) {
      System.err.println("usage: java tools/smoke/FullPageDepletionRecoveryCycle.java " + ID);
      System.exit(2);
    }
    try {
      new FullPageDepletionRecoveryCycle().execute();
    } catch (Exception e) {
  ...
  private void execute() throws Exception {
    ...
    recreate(build);
    buildAero(checkout);
    boolean diagnostic = Boolean.getBoolean("worldline.m93.diagnostic");
    int runs = diagnostic ? 1 : 2, nonce = 9309301;
    ...
    if (diagnostic) {
      System.out.println("M93 diagnostic replica passed; qualification not attempted");
    ...
    String trace =
        "v1|design=2-fresh-same-plan-nonce-full-six-member-page-depletion-recovery-replicas|fixture=constant16-synced+client-marker+exact-camera|server-closure=aero-free|frame-limit=vanilla-max0+aero-pacer-off-runtime-gated|page-ttl=100000-runtime-gated|warm-path=enqueue16+flush2+cached4+pageCalls4+direct0+render0/list0+rebuild0|sequence=remove-indices1-2-3-5-6-7+restore7-6-5-3-2-1-with-ordinal+operation+index-bound-ack/state|spacing=request1-after300+each-next-request-30-records-after-prior-event|membership=16to15to14to13to12to11to10to11to12to13to14to15to16|page-count-6to2-and-2to6=pageCalls4+direct0+rebuild1|page-count-1=pageCalls3+direct1+rebuild0|page-count-0=pageCalls3+direct0+rebuild0|window=M74-min720intervals+12s|capture=M78-primitive-spans+page-counters+post-seal-172-byte-recovery-sidecar|per-record=state16/maskffff|stats=descriptive-transition-dynamic|other-pages-concurrency-default-ttl-stale-cache-eviction-uninstrumented-cost-causality-regression-historical-lag=not-claimed|shutdown=clean";
    String signature = sha256(trace);
    require(signature.equals(value("expected.signature")), "M93 signature drift: " + signature);
    Files.writeString(build.resolve("evidence.txt"),
        "id=" + ID + "\nreplicas=2\nserver.jvm=2\nclient.jvm=2\nfirst=" + first.summary()
            + "\nsecond=" + second.summary() + "\ntrace=" + trace + "\nsignature=" + signature
            + "\n",
        StandardCharsets.UTF_8);
    System.out.println("M93 full-page depletion recovery passed");
  ...
  private Arm run(Path checkout, Path workspace, int replica, int nonce, Plan plan)
    ...
    Path base = root.resolve(".worldline/worktrees/m93-" + ProcessHandle.current().pid() + "-"
  ...
  private Recovery parseRecovery(Path file, int nonce) throws Exception {
    ...
    try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(b))) {
      require(b.length == 172 && in.readInt() == 0x574c3933 && in.readInt() == 1
              && in.readInt() == 172 && in.readInt() == nonce,
  ...
  private void buildAero(Path checkout) throws Exception {
    ...
    Path tree = root.resolve(".worldline/worktrees/m93-build-" + System.nanoTime());
    try {
  ...
  private void verifyBoundary() throws IOException {
    for (Path base :
        Arrays.asList(root.resolve("smokes/m74-complete-aero-census/runtime-src/worldline/m74"),
            smoke.resolve("runtime-src/worldline/m74"), smoke.resolve("runtime-src/worldline/m93")))
  ...
  private void removeWorktree(Path c, Path t) {
    try {
      ...
      throw new IllegalStateException("M93 cleanup failed " + t, e);
    }
  }
