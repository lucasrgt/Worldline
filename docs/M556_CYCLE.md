...
`76:3`, waits for RESET `75:4` plus `76:3`, arms the hold path, pulses SET
until the pair stays ON after the input drops, pulses RESET until it stays
OFF, and reloads that RESET pair after save plus a fresh login. The signal
...
Canonical evidence uses two official server JVMs and five client sessions.
