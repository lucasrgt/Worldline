...
## Immediate post-v1.73.0 direction

M85 qualifies exact cache recovery for one natural remove/restore cycle. It
does not qualify different cells, repeated cycles, concurrent mutations,
stale client block-entity cleanup, merge/repacking policy, or page-count cost.
The next boundary should repeat the now-qualified cycle on one identity with a
bounded generation protocol before expanding to multi-cell churn. M71-M85 do
not establish causal spike attribution.
