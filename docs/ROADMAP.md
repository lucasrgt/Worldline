...
## Immediate post-v1.30.0 direction

The next milestone can compose a bounded sequential batch of correlated route
executions while retaining per-route terminal reasons and stopping before an
unsent batch entry. Packet control still does not imply server tick control,
