...
## Immediate post-v1.91.0 direction

M103 separates explicit page disablement from budget-zero fallback: all
sixteen entries draw immediately and later empty flushes reset the public cell
counters. Other render entrypoints, other negative values, other membership
distributions, page-order independence, unlimited cache, skip-individual
controls, other topologies, concurrency, merge/repacking, and uninstrumented
cost remain outside scope. M71-M103 do not establish causal spike attribution.
