...
## Immediate post-v1.25.0 direction

The next milestone can add a bounded alternate-step policy whose fallback is
explicitly supplied rather than discovered, retaining zero-retry accounting.
Packet control still does not imply server tick control, world-scale
