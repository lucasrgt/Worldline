...
## Immediate post-v1.33.0 direction

The next milestone can preserve an explicit batch terminal event when an event
controller stops execution, binding the batch reason to the exact resolved
movement. Packet control still does not imply server tick control, world-scale
