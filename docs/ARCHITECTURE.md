...
only through official-runtime milestones. `modloader-forge` combines the
Java 8 profiler source-injection boundary with process-isolated ModLoader and
Forge TestKit providers. M767 qualifies fresh single-player boot, controlled
ticks, read-only player/world state, profiler capture, and clean shutdown for
both loaders. It does not imply broader gameplay or performance equivalence.
