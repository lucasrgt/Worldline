...
| MIG-07 | DONE | Version and normalize `smoke.properties`. | The migration baseline is sealed in `schema-migration.lock`; every current descriptor uses schema 1 and validates behavior plus TestKit identity. |
...
| ID | Status | Deliverable | Completion evidence |
| --- | --- | --- | --- |
| CACHE-01 | DONE | Separate behavioral identity from execution-lane identity and promote portable pins safely. | M114 produced the same `321ebc7d0725dc872602f632820b8fbbecff951e48208f5a5ce6a0f493c71653` semantic digest in clean Windows and Linux official-server lanes. The sealed lock promotes 498 headless qualifications to portable v5 identity while 77 GUI/tooling inputs remain lane-bound; execution observations retain OS, architecture, and JDK identity, and the canonical Gate validates all 575 proof envelopes with zero pending. |
