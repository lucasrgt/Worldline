...
| EXT-01 | ACTIVE | Release TestKit 0.3.0 with current public behavior APIs. | Versioned code/docs, the release-ready manifest, `worldline behaviors list`, and an end-to-end vanilla expectation are implemented; the candidate Gate passes, while the single changed `testkit-cycle` still requires its exclusive milestone qualification before packaging. Publication remains tag-authorized only. |
