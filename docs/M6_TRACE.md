...
`role=<ROLE>`. When it names a conservation rule, the CLI also prints
`invariant=<rule>`. The exact `TraceDiff` document is unchanged so
fingerprints stay stable. Usage errors exit 2; unreadable or invalid traces
exit 1.
