...
break, and drop for rail, powered rail, and detector rail. The public pressure-plate lifecycle
...
verified claims (32.95%) and leave 708 unknown. The public floor-mounted lifecycle subsystem then
...
390 verified claims (36.93%) and leaving 666 unknown. The deterministic harvest family then proves
...
observations produce 15 verified claims plus one partial snow-block tick-policy claim, bringing
the census to 538 verified claims (50.95%) and leaving 518 unresolved claims, including that
explicitly partial snow-block policy.
...
paintings, TNT, falling sand, vehicles, hostile and passive mobs. Forty-three of those proofs now
...
archetype routes. A separate universal Packet23 matrix proves arrow, snowball, primed TNT, falling
sand, minecart, and boat materialization while preserving their qualified thrower and velocity
constraints. The painting lifecycle fixture binds Packet25 materialization, two distinct anchored
facings, causal support removal, same-identity Packet29 destruction, and the exact item-321 drop as
...
tick-lifecycle claim. A complete wolf owner-state fixture composes controlled type-95
materialization, bone taming, red-collar observation, and the Packet40 sitting-standing-sitting-
standing sequence. Forty-three verified entity claims now have strict public bindings. The other
9 verified cells remain `SMOKE_ONLY`; the remaining 212 cells are explicit `UNKNOWN` Atlas claims.
...
(83.94%) and 1,099/1,320 public TestKit claims (83.26%). The entity family contributes 43 public
claims, 9 smoke-only claims, and all 212 unknown cells.
...
`ObjectMaterializationFixture` applies the universal spawn law to Packet23 entities without
pretending that every object has mob lifecycle semantics. `ObjectSpawnExpectation` retains the
qualified object type and optional exact thrower/zero-velocity constraints, while canonical
evidence normalizes fresh entity and actor IDs. Its public scenario adapter delegates directly to
`ObjectObservationSession#awaitObjectSpawn`.
...
The pressure-plate package similarly reuses one `pressure-plate` archetype across stone and wooden
plates on flat stone support. Player/entity activation, release timing, redstone propagation, and
unsupported-block removal remain separate from these lifecycle claims.
