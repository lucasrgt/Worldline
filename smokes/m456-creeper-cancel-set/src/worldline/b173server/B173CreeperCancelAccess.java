...
public final class B173CreeperCancelAccess {
    ...
    public static worldline.api.RemoteMobSpawn near(B173WireClient client, worldline.api.BlockPosition pad) {
        for (int n = 0; n < 32; n++) {
    ...
    public static PlayerPose stand(B173WireClient client, worldline.api.BlockPosition top) {
        ...
        if (Math.abs(pose.x() - (top.x() + 0.5D)) > 3D || Math.abs(pose.z() - (top.z() + 0.5D)) > 3D
                || pose.y() > top.y() + 2.5D)
            throw new IllegalStateException("actor missed creeper pad at "+pose.x()+","+pose.y()+","+pose.z());
        return pose;
    }
    ...
    public static void fleeWest(B173WireClient client) {
        ...
        for (int n = 0; n < 4; n++) {
            ...
            if (before.x() - after.x() >= 7.5D) clear = true;
        }
        PlayerPose after = client.moveAndObserve(0D, 0D, 0D, 1).resulting();
        if (!clear)
            throw new IllegalStateException("creeper-cancel flee stayed in range before="+before.x()+","+before.y()+","+before.z()
    ...
    public static int awaitState(B173WireClient client, int entity, int expected) {
        ...
        throw new IllegalStateException("creeper Packet40 index 16 state " + expected
                + " absent queued=" + client.channel().inbound().mobs().fuseQueued(entity)
                + " explosion=" + peekExplosion(client)
                + " pose=" + here.x() + "," + here.y() + "," + here.z());
    }
