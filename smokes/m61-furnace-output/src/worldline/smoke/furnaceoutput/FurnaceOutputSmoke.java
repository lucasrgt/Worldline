...
public final class FurnaceOutputSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        B173DedicatedServer first = null, second = null; FurnaceOutputSession actor = null, observer = null, reopened = null;
        RemoteFurnaceExtraction extraction; RemoteWindowClosure firstClose, secondClose; BlockPosition target;
        try {
            ...
            actor.look(0F, 90F); PlayerPose pose = acquireFurnace(actor, actorName);
            ...
            target = BlockFace.UP.adjacent(support); require(baseline.blockAt(support.x(), support.y(), support.z())
                    ...
                    "furnace output anchor drifted"); observer = client(port, observerName, timeout);
            observer.connect(); observer.synchronizePose(); observer.moveAndObserve(5D, 5D, 0D, 3);
            observer.moveAndObserve(5D, 5D, 0D, 3); requirePlayers(first.players(), actorName, observerName);
            actor.placeHeldBlock(support, BlockFace.UP); BlockState idle = worldline.test.WorldlineSmokeAwait.observe(actor,5)
                    ...
                    "placed output furnace diverged");
            ...
            actor.selectHeldSlot(0); RemoteContainerWindow opened = actor.openFurnace(target, BlockFace.UP);
            require(opened.inventory().slot(sandSlot - 6).item().equals(sand)
                    && opened.inventory().slot(coalSlot - 6).item().equals(coal), "output furnace mapping drifted");
            actor.selectHeldSlot(sandSlot - 36); observer.awaitPeerHeldItem(new RemoteHeldItem(actorName, 12, 0));
            actor.loadFurnace(sandSlot, coalSlot); worldline.test.WorldlineSmokeAwait.observe(actor,5);
            observer.awaitPeerHeldItem(RemoteHeldItem.empty(actorName)); RemoteFurnaceSmelt smelt = actor.awaitFurnaceSmelt();
            require(smelt.output().equals(new RemoteItemStack(20, 1, 0)), "output smelt drifted");
            actor.selectHeldSlot(0);
            extraction = actor.takeFurnaceOutput(36); require(extraction.takeAction() == 5
                    && extraction.storeAction() == 6 && extraction.craftedCount() == 1
                    && extraction.before().equals(smelt.window().inventory()) && extraction.after().slot(2).empty()
                    && extraction.after().slot(30).item().equals(extraction.stack())
                    && actor.inventory().slot(36).item().equals(extraction.stack()), "furnace extraction drifted");
            worldline.test.WorldlineSmokeAwait.observe(actor,5); observer.awaitPeerHeldItem(new RemoteHeldItem(actorName, 20, 0));
            firstClose = actor.closeWindow(); require(firstClose.closedWindow().inventory().equals(extraction.after())
                    && firstClose.proofAction() == 1, "extraction close proof drifted");
            actor.close(); observer.close(); awaitPlayers(first, 0); first.save();
            require(first.player(actorName).inventoryItems() == 1, "extracted player inventory did not persist");
            first.close(); first = null; second = server(jar, workspace, port, seed, timeout);
            second.boot(); second.operator(actorName); reopened = client(port, actorName, timeout);
            reopened.connect(); reopened.synchronizePose(); require(reopened.awaitInventory().occupiedSlots() == 1
                    && reopened.inventory().slot(36).item().equals(extraction.stack()), "restarted glass inventory drifted");
            reopened.selectHeldSlot(1); RemoteContainerWindow persisted = reopened.openFurnace(target, BlockFace.UP);
            require(persisted.inventory().slot(0).empty() && persisted.inventory().slot(1).empty()
                    && persisted.inventory().slot(2).empty() && persisted.inventory().slot(30).item()
                    .equals(extraction.stack()), "restarted furnace output removal drifted");
            secondClose = reopened.closeWindow(); require(secondClose.proofAction() == 1,
                    "restarted furnace close drifted"); reopened.close(); awaitPlayers(second, 0); second.save();
            require(second.player(actorName).inventoryItems() == 1, "restarted extracted inventory drifted");
        ...
        System.out.println("WORLDLINE_M61_OUTPUT=actions=" + extraction.takeAction() + ","
                + extraction.storeAction() + ";crafted=" + extraction.craftedCount() + ";stack="
                + extraction.stack() + ";close=" + firstClose.proofAction() + ";close2=" + secondClose.proofAction());
    ...
    private static PlayerPose acquireIngredients(FurnaceOutputSession client, String username) {
        ...
        worldline.test.WorldlineSmokeAwait.awaitEntity(client,()->{client.moveAndObserve(0D,-1D,0D,1);
            return client.inventory();},inventory->inventory.occupiedSlots()>=3,"furnace ingredients",100);
        worldline.test.WorldlineSmokeAwait.observe(client,10); MovementOutcome settled = null;
        for (int step = 0; step < 100; step++) { settled = client.moveAndObserve(0D, -1D, 0D, 2);
            if (settled.corrected()) break; } require(settled != null && settled.corrected(),
                "ingredient ground settlement absent"); return settled.resulting(); }
    ...
    private static void requirePlayers(List<String> players, String first, String second) { Set<String> expected = new HashSet<>();
        expected.add(first); expected.add(second); require(players.size() == 2 && new HashSet<>(players).equals(expected),
