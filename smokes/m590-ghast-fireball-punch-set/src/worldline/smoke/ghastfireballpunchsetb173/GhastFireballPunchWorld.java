...
final class GhastFireballPunchWorld {
  ...
  static void inventory(Path workspace, String user, BlockPosition land) {
    B173PlayerSeed.writeInventory(workspace, user, land.x() + 0.5D, land.y() + 1.0D, land.z() + 0.5D, -1,
        new int[] {0, 1, 2, 3}, new int[] {87, 4, 52, 268}, new int[] {64, 64, 1, 1}, new int[] {0, 0, 0, 0});
  }
  ...
  static BlockPosition site(RemoteWorldView view, BlockPosition top) {
    ...
    while (i < faces.length) {
      ...
      if (view.containsChunk(ccx, ccz) && !air(view.blockAt(support.x(), support.y(), support.z()).legacyId())
  ...
  static int fill(B173WireClient actor, RemoteWorldView view, BlockPosition top, BlockFace face, int id)
      throws Exception {
    BlockPosition placed = face.adjacent(top);
    int ccx = Math.floorDiv(placed.x(), 16);
    int ccz = Math.floorDiv(placed.z(), 16);
    if (!view.containsChunk(ccx, ccz) || !air(view.blockAt(placed.x(), placed.y(), placed.z()).legacyId())
      ...
      while (z <= 12) {
        ...
        while (y >= 1) {
          if (chunk.blockAt(x, y, z).legacyId() == 87 && air(chunk.blockAt(x, y + 1, z).legacyId())
              && !lava(chunk.blockAt(x, y + 1, z).legacyId()) && !lava(chunk.blockAt(x, y + 2, z).legacyId()))
  ...
  static BlockPosition cavern(RemoteWorldView view, int targetCx, int targetCz) {
      ...
      while (x <= 13) {
        ...
        while (z <= 13) {
          ...
          while (y < 128) {
            ...
            int start = y;
            while (y < 128 && air(chunk.blockAt(x, y, z).legacyId()) && !lava(chunk.blockAt(x, y, z).legacyId())) y++;
            int run = y - start;
            int floor = start - 1;
    ...
    if (best == null || bestRun < 6)
      throw new IllegalStateException(
          "no deterministic nether ghast cavern run=" + bestRun + " chunk=" + targetCx + ":" + targetCz);
    return best;
  }

  static boolean air(int id) {
    return id == 0;
  }
  static boolean lava(int id) {
    return id == 10 || id == 11;
  }
  ...
  static int count(RemoteChunkSnapshot chunk, int id) {
    ...
    while (x < 16) {
      ...
      while (z < 16) {
        int y = 0;
        while (y < 128) {
          if (chunk.blockAt(x, y, z).legacyId() == id)
            n++;
          y++;
        }
        z++;
