...
public final class GhastFireballPunchSetSmoke {
    ...
    public static void main(String[] args) throws Exception {
        ...
        GhastFireballPunchWorld.require(user.length() <= 16 && user.equals("GhastPunch590")
                && seed == 17320110707L, "username or seed drift");
        ...
        try {
            ...
            GhastFireballPunchWorld.require(scout.dimension() == -1 && scout.awaitDimension(-1) == -1,
                    "nether scout dimension drift");
            RemoteChunkSnapshot initial = scout.awaitRemoteChunk(cx, cz).chunkAt(cx, cz);
            GhastFireballPunchWorld.require(GhastFireballPunchWorld.count(initial, 87) > 0
                    && GhastFireballPunchWorld.sky(initial) == 0, "nether terrain identity drift");
            ...
            actor.connect();
            GhastFireballPunchWorld.require(actor.synchronizePose() != null && actor.dimension() == -1
                    && actor.awaitInventory().occupiedSlots() == 4,
                    "nether ghast-punch inventory or dimension drift");
            int cavernCx = cx + 2;
            int cavernCz = cz - 1;
            top = GhastFireballPunchWorld.cavern(GhastFireballPunchWorld.load(actor, land, cavernCx, cavernCz),
                    cavernCx, cavernCz);
            if (top.x() != land.x() || top.y() != land.y() || top.z() != land.z()) {
                ...
                actor.connect();
                GhastFireballPunchWorld.require(actor.synchronizePose() != null && actor.dimension() == -1
                        && actor.awaitInventory().occupiedSlots() == 4, "nether cavern relog drift");
                clients++;
            }
            RemoteWorldView live = actor.awaitRemoteChunk(Math.floorDiv(top.x(), 16),
                    Math.floorDiv(top.z(), 16));
            ...
            spawner = GhastFireballPunchWorld.place(actor, GhastFireballPunchWorld.site(live, top),
                    BlockFace.UP, 52);
            actor.close();
            GhastFireballPunchWorld.awaitPlayers(server, 0);
            server.save();
        ...
        try {
            ...
            actor.connect();
            GhastFireballPunchWorld.require(actor.synchronizePose() != null && actor.dimension() == -1,
                    "nether ghast-punch reload dimension drift");
            ...
            GhastFireballPunchWorld.require(ghast.legacyType() == 56
                    && ghast.entityId() != actor.state().entityId(), "ghast Packet24 identity drift");
            actor.selectHeldSlot(3);
            B173EntityVelocity punch = B173GhastFireballPunch.awaitRedirect(actor, ghast);
            GhastFireballPunchWorld.require(punch != null && B173GhastFireballPunch.redirectedUp(punch),
                    "ghast fireball punch Packet28 redirect drift");
            actor.close();
            GhastFireballPunchWorld.awaitPlayers(server, 0);
            server.save();
            ...
            String trace = "v1|server=official-b1.7.3|seed=" + seed
                    + "|profile=allow-nether-true+spawn-monsters-true"
                    ...
                    + "|oracle=nether-ghast-fireball-punch-not-m410-spawn-only-not-m459-hit|"
                    + evidence;
