...
public final class PortalScaleSetSmoke {
  ...
  public static void main(String[] a) throws Exception {
    ...
    PortalScaleSetSupport.require(seed == 17320110707L && user.equals("PortalScale560")
            && user.length() <= 16 && cx == 20 && cz == 20,
        "portal-scale-set identity drift");
    ...
    try {
      ...
      PortalScaleSetSupport.require(
          actor.awaitInventory().occupiedSlots() == 3 && actor.dimension() == 0,
      ...
      server.save();
      B173PlayerSeed.writeInventory(workspace, user, anchor.x() + 0.5D, anchor.y() + 4.5D,
          anchor.z() + 0.5D, new int[] {0, 1, 2}, new int[] {1, 49, 259}, new int[] {64, 14, 1},
          new int[] {0, 0, 0});
      builder = new B173WireClient("127.0.0.1", port, user, timeout);
      builder.connect();
      PlayerPose pose = builder.synchronizePose();
      PortalScaleSetSupport.require(
          builder.dimension() == 0 && builder.awaitInventory().occupiedSlots() == 3,
          "relocated portal-scale inventory or dimension drift");
      builder.awaitRemoteChunk(cx, cz);
      PortalScaleSetFrame frame =
          PortalScaleSetFrame.build(builder, initial, cx, cz, anchor, pose, portalTicks);
      pose = frame.pose;
      pose = builder
                 .moveAndObserve(frame.bottom.x() + 1.5D - pose.x(),
                     frame.bottom.y() + 1D - pose.y(), frame.bottom.z() + 0.5D - pose.z(), 1)
                 .resulting();
      PortalScaleSetSupport.require(
          builder.dimension() == 0, "dimension changed before portal residence");
      PlayerPose entry = pose;
      worldline.test.WorldlineSmokeAwait.observe(builder, travelTicks);
      PortalScaleSetSupport.require(
          builder.awaitDimension(-1) == -1, "official Packet9 0->-1 absent");
      ...
      int dcx = PortalScaleSetSupport.floor(dest.x()) >> 4,
          dcz = PortalScaleSetSupport.floor(dest.z()) >> 4;
      RemoteChunkSnapshot nether = world.chunkAt(dcx, dcz);
      evidence =
          PortalScaleSetScale.prove(entry, dest, frame.column, PortalScaleSetSupport.sky(nether));
      builder.close();
      PortalScaleSetSupport.awaitPlayers(server, 0);
      server.save();
      PortalScaleSetSupport.require(
          server.player(user).dimension() == -1, "scaled player dimension was not persisted");
    } finally {
      actor.close();
      if (builder != null)
        builder.close();
    ...
    String trace = "v1|server=official-b1.7.3|seed=" + seed
        ...
        + "|oracle=packet13-pose-quantized-8-to-1-not-nether-exists-not-m132-m133-m134-m382|"
