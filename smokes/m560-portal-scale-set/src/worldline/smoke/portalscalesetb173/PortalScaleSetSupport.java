...
final class PortalScaleSetSupport {
 ...
 static BlockPosition foundation(RemoteChunkSnapshot chunk, int cx, int cz) {
  ...
  throw new IllegalStateException("no deterministic portal-scale foundation in chunk " + cx + ":" + cz);
 }

 private static boolean clear(RemoteChunkSnapshot chunk, int x, int y, int z) {
  int water = 0;
  while (water < 15 && y + 1 + water < 128 && water(chunk.blockAt(x, y + 1 + water, z).legacyId())) water++;
  int height = water + 6;
 ...
 static int local(int v, int c) { return v - c * 16; }
 ...
 static void awaitPlayers(B173DedicatedServer server, int n) throws Exception {
  ...
  while (System.currentTimeMillis() < end) { if (server.players().size() == n) return; Thread.sleep(100); }
