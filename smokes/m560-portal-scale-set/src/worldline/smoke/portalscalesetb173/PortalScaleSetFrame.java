...
final class PortalScaleSetFrame {
  ...
  static PortalScaleSetFrame build(B173WireClient actor, RemoteChunkSnapshot initial, int cx,
      int cz, BlockPosition anchor, PlayerPose pose, int portalTicks) throws Exception {
    int column = 0;
    actor.selectHeldSlot(0);
    while (PortalScaleSetSupport.water(initial
            .blockAt(PortalScaleSetSupport.local(anchor.x(), cx), anchor.y() + 1,
                PortalScaleSetSupport.local(anchor.z(), cz))
            .legacyId())) {
    ...
    for (int i = 0; i < 3; i++)
      p = PortalScaleSetSupport.place(actor, p, BlockFace.EAST, 49);
    ...
    for (int y = 1; y <= 3; y++)
      for (int x = 1; x <= 2; x++)
        if (active.blockAt(bottom.x() + x, bottom.y() + y, bottom.z()).legacyId() == 90)
          portals++;
    PortalScaleSetSupport.require(
        portals == 6 && actor.dimension() == 0, "portal-scale frame did not ignite in dimension 0");
