package worldline.smoke.irondoorplaceb173;
...
public final class IronDoorPlaceSmoke {
  private IronDoorPlaceSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: IronDoorPlaceSmoke server.jar workspace port seed username chunkX chunkZ");
    ...
    try {
      ...
      while (
        ...
        require(++column <= 15, "water column exceeded iron door fixture");
      }
      ...
      require(initial.blockAt(local(lower.x(), cx), lower.y(), local(lower.z(), cz)).legacyId() == 0
              && initial.blockAt(local(upper.x(), cx), upper.y(), local(upper.z(), cz)).legacyId()
                  == 0,
          "iron door cells were not initial air");
      actor.look(-90F, 0F);
      ...
      String evidence = "column=" + column + ",support=" + top.x() + ":" + top.y() + ":" + top.z()
          ...
          + ":71:8,persisted=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=raised-stone+irondoor71|cause=packet15-item330-place|wire=packet53-door71:0/8|oracle=irondoor-place+fresh-login|"
          + evidence;
      System.out.println("WORLDLINE_M241_DOOR=" + evidence);
      System.out.println("WORLDLINE_M241_TRACE=" + trace);
      System.out.println("WORLDLINE_M241_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
  ...
  private static BlockPosition foundation(RemoteChunkSnapshot q, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic iron door foundation");
  }
