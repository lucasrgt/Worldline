...
public final class BatchTerminalSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        RemoteWorldView after; ServerPlayerState player;
        try {
            ...
            CorrelatedMovementRoutePlan eventPlan = new CorrelatedMovementRoutePlan(new Object(), Arrays.asList(
                    alternative(.125D, 0D, 0D), alternative(0D, 0D, .125D)));
            event = client.moveCorrelatedRouteBatchExecutionUntilEvent(
                    Collections.singletonList(eventPlan), value -> MovementRouteDirective.CONTINUE,
            ...
            afterRoute = client.moveCorrelatedRouteBatchExecutionUntilEvent(
                    Collections.singletonList(plan(0D, 0D, .125D)), value -> MovementRouteDirective.CONTINUE,
            ...
            exhausted = client.moveCorrelatedRouteBatchExecutionUntilEvent(
                    Collections.singletonList(plan(-.125D, 0D, 0D)), value -> MovementRouteDirective.CONTINUE,
            ...
            after = worldline.test.WorldlineSmokeAwait.observe(client,1); require(after.containsChunk(chunkX, chunkZ), "terminal batches lost cache");
            ...
            require(close(player.x(), finalPose.x()) && close(player.y(), finalPose.y())
                    && close(player.z(), finalPose.z()), "terminal batch pose was not persisted");
        ...
        System.out.println("WORLDLINE_M46_PERSISTED=" + player.x() + "," + player.y() + "," + player.z());
    ...
    private static CorrelatedMovementRoutePlan plan(double x, double y, double z) {
        return new CorrelatedMovementRoutePlan(new Object(), Collections.singletonList(alternative(x, y, z))); }
    ...
    private static boolean terminal(CorrelatedMovementRouteBatchExecution value,
            MovementRouteBatchTerminalKind kind) { return value.terminalKind() == kind
    ...
    private static String describe(CorrelatedMovementRouteBatchExecution value) { return value.terminalKind()
            + "@" + value.terminalEvent().routeIndex() + "/"
