...
public final class GuiActionsOracle {
    ...
    public static void main(String[] arguments) {
        ...
        player.a = new uo(); player.c.a[0] = new iz(1, 4, 0);
        ...
        CanonicalStateTrace trace = trace(); record(trace, "opened", screen);
        click(screen, client, 36, 0); click(screen, client, 37, 0); tick(client);
        record(trace, "dragged", screen);
        click(screen, client, 37, 1); tick(client);
        click(screen, client, 38, 1); tick(client);
        record(trace, "split", screen);
    ...
    private static OracleClientBoundaries.Client client() {
        ...
        client.d = 854; client.e = 480; client.k = new gr("Worldline", "offline");
    ...
    private static CanonicalStateTrace trace() {
        return new CanonicalStateTrace(SEED, "viewport_w", "viewport_h", "slot_x", "slot_y",
                "slot_w", "slot_h", "item36", "count36", "item37", "count37", "item38", "count38");
    }
    ...
    private static void record(CanonicalStateTrace trace, String label, id screen) {
        ...
        trace.record(label, screen.c, screen.d, x, y, 16, 16,
                item(s36), count(s36), item(s37), count(s37), item(s38), count(s38));
    }

    private static void click(id screen, net.minecraft.client.Minecraft client, int index, int button) {
    ...
    private static iz stack(id screen, int index) { return ((gp) screen.j.e.get(index)).a(); }
    private static int item(iz stack) { return stack == null ? -1 : stack.c; }
    private static int count(iz stack) { return stack == null ? 0 : stack.a; }
    private static void tap(int key) {
        Keyboard.worldlinePush(key, true, (char) 0); Keyboard.worldlinePush(key, false, (char) 0);
    }
