...
import aero.modellib.test.WorldlineM770Rehydrator;
...
/** Deterministic restored-world lifecycle shared by all three M770 arms. */
public final class PresentState {
    public static final boolean ENABLED = Boolean.getBoolean("worldline.m770.enabled");
    private static final boolean PREPARE = Boolean.getBoolean("worldline.m770.prepare");
    private static final String ARM = System.getProperty("worldline.m770.arm", "prepare");
    private static final int TICKS = Integer.getInteger("worldline.m770.ticks", 1200);
    private static final long MINIMUM_MILLIS = Long.getLong("worldline.m770.minimumMillis", 60000L);
    ...
    public static void drive(Minecraft game) {
        ...
        if (stage == 0 && game.world == null) {
            ...
            System.out.println("[WorldlineM770] start prepare=" + PREPARE + " arm=" + ARM);
    ...
    private static void warm(Minecraft game) {
        ...
        if (!PREPARE && warmup == 2 && machines == 0) {
            WorldlineM770Rehydrator.rehydrate(world);
            machines = machineCount(world);
            System.out.println("[WorldlineM770] restored synthetic machines=" + machines);
        }
        if (machines < 576 && warmup < LOAD_TIMEOUT_TICKS) return;
        require(machines == 576, "M770 fixture machine count drift: " + machines);
        world.entities.retainAll(Collections.singleton(game.player));
        world.globalEntities.clear();
        place(game.player, 0);
        int backlog = ((PresentRendererStats) game.worldRenderer).worldlineCompileBacklog();
        stable = backlog == 0 ? stable + 1 : 0;
        if (stable < DRAIN_FRAMES) return;
        if (PREPARE) {
            world.saveWithLoadingDisplay(true, null);
            System.out.println("[WorldlineM770] template-ready machines=576 backlog=" + backlog);
            stage = 4;
            game.scheduleStop();
            return;
        }
        GpuQueryCapture.start();
        ClientProfilerRuntime.startCapture();
        retainedStarted = System.currentTimeMillis();
        stage = 2;
        System.out.println("[WorldlineM770] retained-start arm=" + ARM + " machines=576");
    }

    private static void retain(Minecraft game) {
        place(game.player, retained);
        retained++;
        long elapsed = System.currentTimeMillis() - retainedStarted;
        if (retained < TICKS || elapsed < MINIMUM_MILLIS) return;
        GpuQueryCapture.finish();
        ClientProfilerRuntime.finish("retained-complete");
        System.out.println("[WorldlineM770] retained-complete arm=" + ARM
    ...
    private static void place(ClientPlayerEntity player, int tick) {
        ...
        float pitch = 4.0F;
        if (tick >= TICKS / 2) {
            int route = tick - TICKS / 2;
            yaw += route * 7.0F;
            pitch = (float) (Math.sin(route * 0.08D) * 20.0D);
        }
