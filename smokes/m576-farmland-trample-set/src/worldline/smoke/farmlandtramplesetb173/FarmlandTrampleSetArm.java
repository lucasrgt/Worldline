...
public final class FarmlandTrampleSetArm {
  ...
  static BlockPosition raise(B173WireClient a, RemoteChunkSnapshot initial, int cx, int cz, int[] column)
    ...
    while (water(at(initial, BlockFace.UP.adjacent(top), cx, cz).legacyId())) {
      ...
      require(++column[0] <= 15, "water column exceeded farmland-trample fixture");
    }
  ...
  static void trample(B173WireClient a, BlockPosition farm) throws Exception {
    int[] step = {0};
  ...
  static void persist(
      RemoteChunkSnapshot after, int cx, int cz, BlockPosition top, BlockPosition dirt, BlockState trampled) {
    require(at(after, top, cx, cz).equals(new BlockState(1, 0)) && at(after, dirt, cx, cz).equals(trampled),
        "persisted farmland-trample dirt 3:0 drift");
  }
  static BlockPosition foundation(RemoteChunkSnapshot c, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic farmland-trample foundation");
  }
