...
public final class WorldlinePagedTimer {
  ...
  static void arm() {
    if (armed)
      throw new IllegalStateException("duplicate M105 arm");
    ...
    if (!"true".equals(System.getProperty("aero.becell.pages"))
      ...
      throw new IllegalStateException("M105 runtime drift");
    armed = true;
  }
  public static void head() {
    if (!armed || WorldlinePagedBridge.sealed() || !WorldlinePagedBridge.running())
      return;
    if (capture)
      throw new IllegalStateException("M105 HEAD without TAIL");
  ...
  public static void tail() {
    ...
    if (rendererStart != 0 || queueStart != 0 || flushStart != 0)
      throw new IllegalStateException("unterminated M105 span");
    int i = WorldlinePagedBridge.count();
    if (i >= CAP || rendererCount > 0xffff || queueCount > 0xffff || flushCount > 0xffff)
      throw new IllegalStateException("M105 counter overflow");
  ...
  private static long begin(long old, String label) {
    if (old != 0)
      throw new IllegalStateException("recursive M105 " + label);
    return System.nanoTime();
  }
  private static long end(long start, String label) {
    if (start == 0)
      throw new IllegalStateException("unstarted M105 " + label);
    long v = System.nanoTime() - start;
    if (v < 0)
      throw new IllegalStateException("negative M105 " + label);
    return v;
  }
