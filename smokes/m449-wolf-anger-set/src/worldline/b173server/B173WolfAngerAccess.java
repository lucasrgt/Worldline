...
public final class B173WolfAngerAccess {
  ...
  public static RemoteMobSpawn anger(
      B173WireClient client, BlockPosition spawner, BlockPosition top) {
    ...
    throw new IllegalStateException(
        "wolf Packet8 hostility absent after bounded Packet7 strikes" + why);
  }
  ...
  private static RemoteMobSpawn near(
    ...
    for (int n = 0; n < 32; n++) {
      ...
      if (Math.abs(dx) <= 2.5D && Math.abs(dz) <= 2.5D && Math.abs(spawn.y() - spawner.y()) <= 2D)
        return spawn;
    }
  ...
  private static String strikeAndWait(B173WireClient client, RemoteMobSpawn spawn) {
    ...
    if (dead(client, entity))
      return "dead-after-strike";
    if (client.health() < before && client.health() > 0)
      return null;
    for (int n = 0; n < 16; n++) {
      ...
      if (dead(client, entity))
        return "dead-wait-h" + client.health();
  ...
  private static void strike(B173WireClient client, int entity) {
    ...
    if (sword < 36)
      throw new IllegalStateException("wood sword 268 absent from hotbar");
