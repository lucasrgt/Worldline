...
public final class RemainingOrePlaceSetSmoke{
 ...
 public static void main(String[]a)throws Exception{
  ...
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1,2,3},new int[]{1,16,21,73},new int[]{32,1,1,1},new int[]{0,0,0,0});actor.connect();actor.synchronizePose();
   ...
   RemoteWorldView live=worldline.test.WorldlineSmokeAwait.observe(actor,5);require(live.blockAt(coal.x(),coal.y(),coal.z()).equals(coalPlaced)&&live.blockAt(lapis.x(),lapis.y(),lapis.z()).equals(lapisPlaced)&&live.blockAt(redstone.x(),redstone.y(),redstone.z()).equals(redstonePlaced),"live remaining-ore-place-set drift");
