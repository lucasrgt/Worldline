  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      ...
      route = client.moveRouteWithFallback(Arrays.asList(safe, blocked), event -> {
        ...
        require(event.outcomeIndex() == events.size(), "route event was not immediate");
        events.add(event);
      });
      require(events.size() == 3 && event(events.get(0), 0, 0, MovementAttemptKind.PRIMARY)
              ...
              && event(events.get(2), 1, 2, MovementAttemptKind.FALLBACK),
          "route event order drifted");
      ...
      require(after.containsChunk(chunkX, chunkZ), "observed route lost cache");
      client.close();
      ...
      require(close(player.x(), route.finalPose().x()) && close(player.y(), route.finalPose().y())
          ...
          "observed route pose was not persisted");
    } finally {
      client.close();
      server.close();
    }
    System.out.println(
        "WORLDLINE_M39_API=route-observer,synchronous,indexed,identity-bound,cache,persistence");
    System.out.println(
        "WORLDLINE_M39_EVENTS=" + describe(events) + ";final=" + pose(route.finalPose()));
    System.out.println("WORLDLINE_M39_CACHE=chunks=" + after.chunks().size());
    System.out.println(
        "WORLDLINE_M39_PERSISTED=" + player.x() + "," + player.y() + "," + player.z());
