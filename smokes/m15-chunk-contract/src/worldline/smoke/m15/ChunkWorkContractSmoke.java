...
public final class ChunkWorkContractSmoke {
  ...
  public static void main(String[] arguments) {
    ...
    require(accepted.status == Status.ACCEPTED_DEFERRED && accepted.accepted == 2
        && accepted.remaining == 10);
    require(stalled.status == Status.STALLED_DEFERRED && stalled.accepted == 0);
    require(complete.endCurrentFrame() && accepted.endCurrentFrame() && stalled.endCurrentFrame());
  ...
  private static void require(boolean condition) {
    if (!condition)
      throw new IllegalStateException("contract state drifted");
  }
}
