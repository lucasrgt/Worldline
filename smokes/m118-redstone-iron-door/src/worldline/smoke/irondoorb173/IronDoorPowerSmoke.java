...
public final class IronDoorPowerSmoke {
    ...
    public static void main(String[]arguments)throws Exception{
        if(arguments.length!=9)throw new IllegalArgumentException("usage: IronDoorPowerSmoke server.jar workspace port seed username chunkX chunkZ fixtureTicks signalTicks");
        ...
        RemoteChunkSnapshot before,after;BlockPosition foundation,stone,lever,bottom,top;BlockState leverOff,leverOn,bottomOff,bottomOn,topOff,topOn;int column;
        try{server.boot();B173PlayerSeed.writeInventory(workspace,username,4.5D,60D,4.5D,new int[]{0,1,2},
                    new int[]{1,69,330},new int[]{16,1,1},new int[]{0,0,0});actor.connect();actor.synchronizePose();actor.look(-90F,0F);
            RemoteInventoryView inventory=actor.awaitInventory();require(inventory.occupiedSlots()==3,"door fixture inventory drift");
            RemoteChunkSnapshot initial=actor.awaitRemoteChunk(chunkX,chunkZ).chunkAt(chunkX,chunkZ);
            foundation=foundation(initial,chunkX,chunkZ);stone=foundation;column=0;actor.selectHeldSlot(0);
            while(water(initial.blockAt(local(stone.x(),chunkX),stone.y()+1,local(stone.z(),chunkZ)).legacyId())){
                actor.placeHeldBlock(stone,BlockFace.UP);stone=BlockFace.UP.adjacent(stone);actor.awaitBlock(stone,new BlockState(1,0));
                actor.moveAndObserve(0D,1D,0D,1);column++;require(column<=15,"water column exceeded fixture stack");}
            actor.placeHeldBlock(stone,BlockFace.UP);stone=BlockFace.UP.adjacent(stone);actor.awaitBlock(stone,new BlockState(1,0));
            actor.moveAndObserve(0D,1D,0D,1);column++;bottom=BlockFace.UP.adjacent(stone);top=BlockFace.UP.adjacent(bottom);lever=BlockFace.EAST.adjacent(stone);
            ...
            require(after.blockAt(local(lever.x(),chunkX),lever.y(),local(lever.z(),chunkZ)).equals(leverOn)
                    &&after.blockAt(local(bottom.x(),chunkX),bottom.y(),local(bottom.z(),chunkZ)).equals(bottomOn)
                    &&after.blockAt(local(top.x(),chunkX),top.y(),local(top.z(),chunkZ)).equals(topOn),"fresh powered door drift");
        }finally{actor.close();if(reader!=null)reader.close();server.close();}
        StateDelta delta=delta(before,after);require(delta.changed==3,"door activation changed unrelated states: "+delta);
        String evidence="column="+column+",lever="+lever.x()+":"+lever.y()+":"+lever.z()+":"+leverOff.metadata()+"->"+leverOn.metadata()
                +",door="+bottom.x()+":"+bottom.y()+":"+bottom.z()+":"+bottomOff.metadata()+"->"+bottomOn.metadata()+",top="+topOff.metadata()+"->"+topOn.metadata()+",states="+delta;
        String trace="v1|server=official-b1.7.3|seed="+seed+"|fixture=stone-column+lever69+iron-door-item330-block71|settle="+fixtureTicks
                +"+"+signalTicks+"ticks|cause=packet15-lever-activate|effect=packet53-iron-door-open|observation=fresh-login-packet51|"+evidence+"|disconnect=clean";
        System.out.println("WORLDLINE_M118_DOOR="+evidence);System.out.println("WORLDLINE_M118_TRACE="+trace);System.out.println("WORLDLINE_M118_SIGNATURE="+sha256(trace));
    }
    private static BlockPosition foundation(RemoteChunkSnapshot chunk,int chunkX,int chunkZ){for(int x=4;x<=11;x++)for(int z=4;z<=11;z++)for(int y=126;y>=1;y--)
        if(chunk.blockAt(x,y,z).legacyId()==3&&water(chunk.blockAt(x,y+1,z).legacyId()))return new BlockPosition(chunkX*16+x,y,chunkZ*16+z);throw new IllegalStateException("no deterministic door foundation");}
