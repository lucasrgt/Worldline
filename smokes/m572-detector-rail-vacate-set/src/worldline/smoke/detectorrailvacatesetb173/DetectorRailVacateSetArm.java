...
public final class DetectorRailVacateSetArm {
    ...
    private DetectorRailVacateSetArm(BlockPosition support, BlockPosition high, BlockPosition detector,
        ...
        this.support = support; this.high = high; this.detector = detector;
        this.landing = landing; this.landing2 = landing2;
    }

    static DetectorRailVacateSetArm place(B173WireClient actor, RemoteChunkSnapshot initial,
        ...
        BlockState idle = worldline.test.WorldlineSmokeAwait.awaitBlockMatching(actor, detector,
                state -> state.legacyId() == 28 && slope(state.metadata()), "sloped detector 28", 20);
        require(idle.legacyId() == 28 && slope(idle.metadata()) && (idle.metadata() & 8) == 0,
                "idle sloped detector drift " + idle + " at " + cell(detector));
        require(id(actor, landing) == 66 && id(actor, landing2) == 66,
                "landing rail 66 absent before occupy");
    ...
    BlockState occupy(B173WireClient actor) {
        return worldline.test.WorldlineSmokeAwait.awaitBlockMatching(actor, detector,
                state -> state.legacyId() == 28 && (state.metadata() & 8) != 0,
                "occupied detector 28", 40);
    }

    BlockState vacate(B173WireClient actor) {
        return worldline.test.WorldlineSmokeAwait.awaitBlockMatching(actor, detector,
                state -> state.legacyId() == 28 && (state.metadata() & 8) == 0,
                "vacated detector 28", 200);
    }
    ...
    void persist(RemoteChunkSnapshot after, int cx, int cz, BlockState vacated) {
        ...
        require(at(after, landing, cx, cz).legacyId() == 66
                && at(after, landing2, cx, cz).legacyId() == 66, "landing persist drift");
    }
    ...
    static BlockPosition placeId(B173WireClient actor, BlockPosition support, BlockFace face, int id) {
        BlockPosition target = face.adjacent(support);
        actor.placeHeldBlock(support, face);
        worldline.test.WorldlineSmokeAwait.awaitBlockMatching(actor, target,
                state -> state.legacyId() == id, "block " + id, 10);
        return target;
    }
