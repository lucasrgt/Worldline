...
public final class DetectorRailVacateSetSmoke {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DetectorRailVacateSetArm.require(seed == 17320110707L && user.equals("DetVacate572")
                && user.length() <= 16, "detector-rail-vacate identity drift");
        ...
        try {
            ...
            B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D,
                    new int[] {0, 1, 2, 3}, new int[] {1, 28, 66, 328},
                    new int[] {32, 1, 4, 1}, new int[] {0, 0, 0, 0});
            actor.connect();
            actor.synchronizePose();
            DetectorRailVacateSetArm.require(actor.awaitInventory().occupiedSlots() == 4,
                    "detector-rail-vacate inventory drift");
            RemoteChunkSnapshot initial = actor.awaitRemoteChunk(cx, cz).chunkAt(cx, cz);
            arm = DetectorRailVacateSetArm.place(actor, initial, cx, cz, column);
            BlockState idle = actor.awaitRemoteChunk(cx, cz).blockAt(
                    arm.detector.x(), arm.detector.y(), arm.detector.z());
            actor.selectHeldSlot(3);
            actor.useHeldItemOnBlock(arm.detector, BlockFace.UP);
            RemoteObjectSpawn cart = actor.awaitObjectSpawn(10);
            DetectorRailVacateSetArm.require(cart.type() == 10 && cart.throwerId() == 0
                    && cart.velocityX() == 0 && cart.velocityY() == 0 && cart.velocityZ() == 0
                    && cart.fixedX() == arm.detector.x() * 32 + 16
                    && cart.fixedZ() == arm.detector.z() * 32 + 16,
                    "minecart packet23 type10 spawn bounds drift");
            BlockState occupied = arm.occupy(actor);
            DetectorRailVacateSetArm.require(occupied.legacyId() == 28 && (occupied.metadata() & 8) != 0
                    && !occupied.equals(idle), "occupied detector power absent");
            BlockState vacated = arm.vacate(actor);
            DetectorRailVacateSetArm.require(vacated.legacyId() == 28 && (vacated.metadata() & 8) == 0
                    && vacated.equals(idle) && !vacated.equals(occupied),
            ...
            String evidence = "column=" + column[0]
                    + ",support=" + DetectorRailVacateSetArm.token(arm.support, 1, 0)
                    + ",high=" + DetectorRailVacateSetArm.token(arm.high, 1, 0)
                    + ",detector=" + DetectorRailVacateSetArm.cell(arm.detector) + ":28:"
                    + idle.metadata() + "->" + occupied.metadata() + "->" + vacated.metadata()
                    + ",landing=" + DetectorRailVacateSetArm.token(arm.landing, 66,
                    after.blockAt(arm.landing.x() - cx * 16, arm.landing.y(),
                            arm.landing.z() - cz * 16).metadata())
                    + ",cart=type10+thrower0+fixed" + cart.fixedX() + ":" + cart.fixedY()
                    + ":" + cart.fixedZ()
                    + ",occupy=28:" + occupied.metadata() + ",vacate=28:" + vacated.metadata()
