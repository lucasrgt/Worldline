...
public final class ChestRetrievalSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    ChestRetrievalSession actor = null, restored = null;
    RemoteChestRetrieval retrieval;
    RemoteWindowClosure storeClose, retrievalClose, restartClose;
    ...
    try {
      ...
      actor.look(0F, 90F);
      PlayerPose pose = acquireChest(actor, name);
      ...
      BlockState chest = worldline.test.WorldlineSmokeAwait.observe(actor, 5).blockAt(
          target.x(), target.y(), target.z());
      require(chest.legacyId() == 54, "retrieval chest placement drifted");
      ...
      RemoteContainerWindow opened = actor.openChest(target, BlockFace.UP);
      require(
          opened.inventory().slot(54).item().equals(stone) && opened.inventory().slot(0).empty(),
          "initial chest mapping drifted");
      RemoteChestTransfer stored = actor.storeInOpenChest(36, 0);
      require(stored.takeAction() == 1 && stored.storeAction() == 2
              && stored.after().slot(0).item().equals(stone),
          "retrieval fixture store drifted");
      storeClose = actor.closeWindow();
      require(storeClose.proofAction() == 1, "fixture store close proof drifted");
      RemoteContainerWindow reopened = actor.openChest(target, BlockFace.UP);
      require(reopened.inventory().slot(0).item().equals(stone)
              && reopened.inventory().slot(54).empty(),
          "same-server chest reopen drifted");
      retrieval = actor.retrieveFromOpenChest(0, 36);
      require(retrieval.takeAction() == 1 && retrieval.storeAction() == 2
              && retrieval.before().equals(reopened.inventory())
              && retrieval.after().slot(0).empty()
              && retrieval.after().slot(54).item().equals(stone)
              && actor.inventory().slot(36).item().equals(stone),
          "accepted chest retrieval drifted");
      retrievalClose = actor.closeWindow();
      require(retrievalClose.proofAction() == 2
      ...
      restored = client(port, name, timeout);
      restored.connect();
      restored.synchronizePose();
      require(
          restored.awaitInventory().slot(36).item().equals(stone), "restored player stone drifted");
      restored.selectHeldSlot(1);
      RemoteContainerWindow finalChest = restored.openChest(target, BlockFace.UP);
      require(finalChest.inventory().slot(0).empty()
              && finalChest.inventory().slot(54).item().equals(stone)
              && finalChest.inventory().occupiedSlots() == 1,
          "restored chest/player state drifted");
      restartClose = restored.closeWindow();
      require(restartClose.proofAction() == 1, "restart close proof drifted");
      restored.close();
      restored = null;
      awaitPlayers(second, 0);
      second.save();
      require(second.player(name).inventoryItems() == 1, "restart inventory count drifted");
    } finally {
      if (actor != null)
        actor.close();
      if (restored != null)
  ...
  private static PlayerPose acquireChest(ChestRetrievalSession client, String name) {
    ...
    }, inventory -> inventory.occupiedSlots() >= 1, "retrieval chest grant", 100);
