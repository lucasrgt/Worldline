...
public final class FishingCatchSetSmoke{
 ...
 public static void main(String[]a)throws Exception{
  if(a.length!=7)throw new IllegalArgumentException("usage: FishingCatchSetSmoke server.jar workspace port seed username chunkX chunkZ");
  Path jar=Paths.get(a[0]),workspace=Paths.get(a[1]);int port=Integer.parseInt(a[2]);long seed=Long.parseLong(a[3]);String user=a[4];int cx=Integer.parseInt(a[5]),cz=Integer.parseInt(a[6]);Duration timeout=Duration.ofSeconds(300);
  ...
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1,2},new int[]{1,9,346},new int[]{48,16,1},new int[]{0,0,0});actor.connect();actor.synchronizePose();require(actor.awaitInventory().occupiedSlots()==3,"fishing-catch inventory drift");RemoteChunkSnapshot initial=actor.awaitRemoteChunk(cx,cz).chunkAt(cx,cz);top=foundation(initial,cx,cz);column=0;actor.selectHeldSlot(0);
   ...
   String evidence="column="+column+",support="+top.x()+":"+top.y()+":"+top.z()+":1:0,water="+pool.x()+":"+pool.y()+":"+pool.z()+":9:0,hook=type90,catch=349,rod=346,clients=1,disconnect=clean";String trace="v1|server=official-b1.7.3|seed="+seed+"|fixture=raised-stone+still-water-dock9|cause=packet15-dir255-rod346+reel|wire=packet23-type90+packet21-349|oracle=hook-then-official-catch|"+evidence;System.out.println("WORLDLINE_M360_CATCH="+evidence);System.out.println("WORLDLINE_M360_TRACE="+trace);System.out.println("WORLDLINE_M360_SIGNATURE="+sha(trace));
 ...
 private static BlockPosition fill(B173WireClient a,BlockPosition top)throws Exception{BlockPosition center=null;for(int dz=1;dz<=3;dz++)for(int dx=-1;dx<=1;dx++){BlockPosition cell=place(a,new BlockPosition(top.x()+dx,top.y(),top.z()+dz),BlockFace.UP,9);if(dx==0&&dz==2)center=cell;}require(center!=null,"fishing pool center absent");return center;}
 ...
 private static BlockPosition foundation(RemoteChunkSnapshot q,int cx,int cz){for(int x=4;x<=11;x++)for(int z=4;z<=11;z++)for(int y=126;y>=1;y--)if(q.blockAt(x,y,z).legacyId()==3&&fluid(q.blockAt(x,y+1,z).legacyId()))return new BlockPosition(cx*16+x,y,cz*16+z);throw new IllegalStateException("no deterministic fishing-catch foundation");}
 private static boolean fluid(int id){return id==8||id==9;}private static int local(int v,int c){return v-c*16;}private static void awaitPlayers(B173DedicatedServer s,int n)throws Exception{long e=System.currentTimeMillis()+5000;while(System.currentTimeMillis()<e){if(s.players().size()==n)return;Thread.sleep(100);}throw new IllegalStateException("player count drift");}private static String sha(String s)throws Exception{byte[]b=MessageDigest.getInstance("SHA-256").digest(s.getBytes(StandardCharsets.UTF_8));StringBuilder v=new StringBuilder();for(byte x:b)v.append(String.format("%02x",x&255));return v.toString();}private static void require(boolean v,String m){if(!v)throw new IllegalStateException(m);}
}
