...
public final class B173BowArrowSetCrafts {
    public static final int BOW = 261, ARROW = 262, STICK = 280, STRING = 287, FLINT = 318, FEATHER = 288;
    private B173BowArrowSetCrafts() {}

    public static int apply(B173WireClient actor) throws Exception {
        require(BOW == 261 && ARROW == 262 && STICK == 280 && STRING == 287 && FLINT == 318 && FEATHER == 288,
                "bow-arrow craft identities drifted");
        int[] n = new int[] {0};
        take(actor, n, 38);
        put(actor, n, 2, 0, 0); put(actor, n, 4, 0, 0); put(actor, n, 8, 0, 0);
        store(actor, n, 38);
        take(actor, n, 39);
        put(actor, n, 3, 0, 0); put(actor, n, 6, 0, 0); put(actor, n, 9, BOW, 1);
        store(actor, n, 39);
        ...
        store(actor, n, 38);
        require(has(actor, 37, BOW, 1) && has(actor, 38, ARROW, 4), "crafted bow 261 and arrows 262 drifted");
        return n[0];
    }
    ...
    private static void put(B173WireClient a, int[] n, int slot, int result, int count) throws Exception {
        B173PlayInbound in = a.channel().inbound();
        ...
        if (result > 0) after = set(after, 0, item(result, count, 0));
    ...
    private static void takeResult(B173WireClient a, int[] n, int result, int count, int[] slots) throws Exception {
        B173PlayInbound in = a.channel().inbound();
        RemoteInventoryView w = in.activeWindow().inventory(), p = in.inventory();
        ...
        for (int s : slots) after = set(after, s, null);
        B173ContainerStep step = new B173ContainerStep(w.windowId(), ++n[0], 0, tool, w, after, p, p, null, tool,
                16842752 + result, count);
    ...
    private static void click(B173WireClient a, int action, int slot, int button, RemoteItemStack predicted,
            RemoteInventoryView after, RemoteInventoryView personalAfter, RemoteItemStack cursorAfter)
            throws Exception {
        B173PlayInbound in = a.channel().inbound();
        RemoteInventoryView w = in.activeWindow().inventory(), p = in.inventory();
        B173ContainerStep step = new B173ContainerStep(w.windowId(), action, slot, button, predicted, w, after, p,
                personalAfter, in.cursor(), cursorAfter);
        in.beginContainerTransaction(step);
