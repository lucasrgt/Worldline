...
public final class MushroomSpreadSetArm {
  static BlockPosition raise(B173WireClient a, RemoteChunkSnapshot initial, int cx, int cz, int[] column, int[] used)
      throws Exception {
  ...
  static BlockPosition glassFloor(B173WireClient a, BlockPosition top, int[] used) throws Exception {
  ...
  static BlockPosition ringCell(B173WireClient a, BlockPosition top, int[] used, BlockPosition glass, int dx, int dz)
      throws Exception {
    ...
    while (dx <= 3) {
      ...
      while (dz <= 3) {
        if (Math.max(Math.abs(dx), Math.abs(dz)) == 3) {
          stone(a, used, new BlockPosition(top.x() + dx, top.y(), top.z() + dz), BlockFace.UP);
        }
        dz++;
      }
      dx++;
  ...
  static BlockPosition roof(B173WireClient a, BlockPosition top, int[] used) throws Exception {
    ...
    while (dx <= 3) {
      ...
      while (dz <= 3) {
        if (Math.max(Math.abs(dx), Math.abs(dz)) == 3) {
          stone(a, used, new BlockPosition(top.x() + dx, top.y() + 1, top.z() + dz), BlockFace.UP);
        }
        dz++;
      }
      dx++;
  ...
  static void fillRoofRing(B173WireClient a, BlockPosition top, int[] used, int r) throws Exception {
    int dx = -r;
    while (dx <= r) {
      int dz = -r;
      while (dz <= r) {
  ...
  static void roofCell(B173WireClient a, BlockPosition top, int[] used, int dx, int dz, int r) throws Exception {
    BlockPosition support;
    BlockFace face;
    if (Math.abs(dx) == r) {
  ...
  static void plant(B173WireClient a, BlockPosition top, BlockPosition[] sources, BlockPosition[] targets)
      throws Exception {
    int s = 0;
    int t = 0;
    int dx = -2;
  ...
  static int[] plantCell(B173WireClient a, BlockPosition top, BlockPosition[] sources, BlockPosition[] targets, int s,
      int t, int dx, int dz) throws Exception {
  ...
  static BlockPosition stone(B173WireClient a, int[] used, BlockPosition support, BlockFace face) throws Exception {
