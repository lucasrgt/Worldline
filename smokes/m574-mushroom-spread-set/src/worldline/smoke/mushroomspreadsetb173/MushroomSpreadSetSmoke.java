...
public final class MushroomSpreadSetSmoke {
    ...
    public static void main(String[] a) throws Exception {
        ...
        MushroomSpreadSetArm.require(seed == 17320110707L && user.equals("MushSprd574")
                && user.length() <= 16 && window >= 1 && window <= 1200 && windows >= 1
                && windows <= 80, "mushroom-spread-set identity drift");
        ...
        try {
            ...
            B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D,
                    new int[] {0, 1, 2, 3, 4, 5, 6}, new int[] {1, 1, 1, 1, 39, 40, 20},
                    new int[] {64, 64, 64, 64, 32, 32, 8}, new int[] {0, 0, 0, 0, 0, 0, 0});
            actor.connect();
            actor.synchronizePose();
            MushroomSpreadSetArm.require(actor.awaitInventory().occupiedSlots() == 7,
                    "mushroom-spread inventory drift");
            ...
            MushroomSpreadSetArm.require(MushroomSpreadSetArm.id(placed, sources[0]) == 39
                    && MushroomSpreadSetArm.id(placed, glassAir) == 0
                    && MushroomSpreadSetArm.id(placed, cover) == 1,
            ...
            String evidence = "column=" + column[0] + ",support="
                    + MushroomSpreadSetArm.token(top, 1, 0) + ",pad=7x7,sources=15,brown=9,"
