...
public final class MushroomSpreadSetWait {
  static void waitSpread(B173WireClient a, BlockPosition[] sources, BlockPosition[] targets, BlockPosition glassAir,
      int window, int windows) throws Exception {
    RemoteWorldView spread = worldline.test.WorldlineSmokeAwait.awaitWorld(
        a, v -> anySpread(v, targets), "dark opaque mushroom spread", window * windows);
    int i = 0;
    ...
    MushroomSpreadSetArm.require(MushroomSpreadSetArm.id(spread, glassAir) == 0, "glass cell converted during wait");
  }

  static void persist(RemoteChunkSnapshot after, int cx, int cz, BlockPosition[] sources, BlockPosition[] targets,
      BlockPosition glass, BlockPosition glassAir, BlockPosition cover) {
    int i = 0;
    while (i < sources.length) {
    ...
    MushroomSpreadSetArm.require(
        MushroomSpreadSetArm.at(after, glass, cx, cz).equals(new BlockState(20, 0)), "glass persist drift");
    MushroomSpreadSetArm.require(
        MushroomSpreadSetArm.at(after, glassAir, cx, cz).legacyId() == 0, "glass air persist drift");
    MushroomSpreadSetArm.require(
        MushroomSpreadSetArm.at(after, cover, cx, cz).equals(new BlockState(1, 0)), "roof persist drift");
