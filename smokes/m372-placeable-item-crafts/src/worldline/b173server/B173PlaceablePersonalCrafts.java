...
public final class B173PlaceablePersonalCrafts {
  ...
  private static int pair(B173PlayChannel channel, B173PlayInbound inbound, int action, int planks,
    ...
    action =
        step(channel, inbound, 38, 0, all, replace(inbound.inventory(), 38, null), all, action);
    ...
    action =
        step(channel, inbound, 38, 0, null, replace(inbound.inventory(), 38, rest), null, action);
    action = step(channel, inbound, 0, 0, STICKS4,
        replace(replace(replace(inbound.inventory(), 0, null), 1, null), 3, null), STICKS4, action);
    if (sticks == 0)
      return step(
          channel, inbound, 37, 0, null, replace(inbound.inventory(), 37, STICKS4), null, action);
    return step(
        channel, inbound, 37, 0, STICKS4, replace(inbound.inventory(), 37, STICKS8), null, action);
  }
