...
public final class B173PlaceableWorkbenchCrafts {
  ...
  public static void apply(B173WireClient actor) {
    try {
      new B173PlaceableWorkbenchCrafts(actor).run();
    } catch (IOException error) {
      throw new IllegalStateException("placeable workbench crafts failed", error);
    }
  }
  ...
  private void run() throws IOException {
    require(PAINTING.legacyId() == 321 && SIGN.legacyId() == 323 && BOWL.legacyId() == 281
            && BOWL.count() == 4,
        "placeable result identity drifted");
  ...
  private void fill(int personal, int id, int[] cells, RemoteItemStack complete)
      throws IOException {
