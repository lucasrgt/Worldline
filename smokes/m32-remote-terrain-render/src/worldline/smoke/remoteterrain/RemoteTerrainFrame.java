...
final class RemoteTerrainFrame {
    ...
    static Frame render(RemoteWorldView world, BlockPosition target) throws Exception {
        ...
        try {
            ...
            buffer.makeCurrent(); require(buffer.isCurrent() && !Display.isCreated(),
                    "render context is not offscreen");
            configure(); Tessellator renderer = Tessellator.instance;
            renderer.startDrawingQuads(); int nonAir = 0;
            for (int row = 0; row < CELLS; row++) for (int column = 0; column < CELLS; column++) {
                BlockState state = world.blockAt(startX + column, startY + row, target.z());
                if (state.legacyId() == 0) continue;
                int color = color(state); renderer.setColorRGBA(color >>> 24, color >>> 16 & 255,
                        color >>> 8 & 255, 255); quad(renderer, column, row); nonAir++;
            }
            renderer.draw(); GL11.glFinish(); ByteBuffer pixels = BufferUtils.createByteBuffer(SIZE * SIZE * 4);
            ...
            int targetPixel = pixel(pixels, (target.x() - startX) * CELL + 4,
                    (target.y() - startY) * CELL + 4);
            require(targetPixel == (targetState.legacyId() == 0 ? BACKGROUND : color(targetState)),
                    "target pixel did not represent cached state");
            int coverage = countChanged(pixels); require(coverage == nonAir * CELL * CELL,
                    "slice coverage mismatch: " + coverage + "/" + nonAir);
            return new Frame(sha256(pixels), targetPixel, coverage, nonAir);
        } finally { if (buffer != null) buffer.destroy(); }
    }

    static String provenance() { return Tessellator.class.getProtectionDomain()
            .getCodeSource().getLocation().toString(); }

    private static void quad(Tessellator renderer, int column, int row) {
        double x = column * CELL, y = row * CELL;
        renderer.addVertex(x, y, 0); renderer.addVertex(x + CELL, y, 0);
        renderer.addVertex(x + CELL, y + CELL, 0); renderer.addVertex(x, y + CELL, 0);
    }
    ...
    private static void configure() {
        GL11.glViewport(0, 0, SIZE, SIZE); GL11.glDisable(GL11.GL_TEXTURE_2D);
        GL11.glDisable(GL11.GL_DEPTH_TEST); GL11.glDisable(GL11.GL_BLEND);
        GL11.glDisable(GL11.GL_CULL_FACE); GL11.glDisable(GL11.GL_DITHER);
    ...
    private static int pixel(ByteBuffer pixels, int x, int y) { int offset = (y * SIZE + x) * 4;
        return (pixels.get(offset) & 255) << 24 | (pixels.get(offset + 1) & 255) << 16
                | (pixels.get(offset + 2) & 255) << 8 | pixels.get(offset + 3) & 255; }
    private static int countChanged(ByteBuffer pixels) { int result = 0;
        for (int y = 0; y < SIZE; y++) for (int x = 0; x < SIZE; x++)
            if (pixel(pixels, x, y) != BACKGROUND) result++; return result; }
    private static String sha256(ByteBuffer pixels) throws Exception { MessageDigest digest = MessageDigest.getInstance("SHA-256");
        ByteBuffer copy = pixels.duplicate(); copy.clear(); digest.update(copy); StringBuilder result = new StringBuilder();
        for (byte value : digest.digest()) result.append(String.format("%02x", value & 255)); return result.toString(); }
    private static void require(boolean condition, String message) { if (!condition) throw new IllegalStateException(message); }
    static final class Frame { final String hash; final int targetPixel, coverage, nonAir;
        Frame(String hash, int targetPixel, int coverage, int nonAir) {
            this.hash = hash; this.targetPixel = targetPixel; this.coverage = coverage; this.nonAir = nonAir; } }
}
