...
public final class RemoteTerrainSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        int minimumChunks = Integer.parseInt(arguments[6]); long seed = Long.parseLong(arguments[3]);
        String username = arguments[4]; Duration timeout = Duration.ofSeconds(90);
        B173DedicatedServer server = new B173DedicatedServer(jar, workspace, port, seed, timeout, 3, true);
        SustainedRemoteWorldMultiplayerSession client = new B173WireClient(
                "127.0.0.1", port, username, timeout);
        BlockPosition target; BlockState state; RemoteTerrainFrame.Frame before, after;
        int chunks;
        try {
            ...
            Thread.sleep(1000L); client.beginBreak(target); Thread.sleep(3000L); client.finishBreak(target);
    ...
    private static BlockPosition target(RemoteWorldView world, PlayerPose pose) {
        BlockPosition soft = target(world, pose, false), any = soft == null ? target(world, pose, true) : soft;
        if (any != null) return any; throw new IllegalStateException("no nearby breakable block");
    }
    private static BlockPosition target(RemoteWorldView world, PlayerPose pose, boolean broad) {
        ...
        for (int y = centerY; y >= centerY - 6; y--) for (int radius = 0; radius <= 4; radius++)
            for (int x = centerX - radius; x <= centerX + radius; x++)
                for (int z = centerZ - radius; z <= centerZ + radius; z++) {
                    ...
                    if (dx * dx + dy * dy + dz * dz > 35D) continue;
