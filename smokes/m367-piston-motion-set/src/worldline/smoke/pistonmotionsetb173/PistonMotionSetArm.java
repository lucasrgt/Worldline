...
public final class PistonMotionSetArm {
  ...
  static PistonMotionSetArm place(B173WireClient a, RemoteChunkSnapshot initial,
      BlockPosition support, int cx, int cz, int slot, int id) throws Exception {
    ...
    require(at(initial, piston, cx, cz).legacyId() == 0 && at(initial, head, cx, cz).legacyId() == 0
        ...
        "piston " + id + " targets were not initial air");
    a.look(-90F, 0F);
    a.selectHeldSlot(slot);
    a.placeHeldBlock(support, BlockFace.UP);
    BlockState placed = worldline.test.WorldlineSmokeAwait.observe(a, 5).blockAt(
        piston.x(), piston.y(), piston.z());
    require(placed.equals(new BlockState(id, 4)),
        "west piston " + id + " absent: " + placed + " at " + piston.x() + ":" + piston.y() + ":"
            + piston.z());
    a.selectHeldSlot(0);
  ...
  static BlockPosition raise(B173WireClient a, RemoteChunkSnapshot initial, int cx, int cz,
    ...
    while (water(at(initial, BlockFace.UP.adjacent(top), cx, cz).legacyId())) {
      ...
      require(++column[0] <= 15, "water column exceeded piston-motion fixture");
    }
    top = place(a, top, BlockFace.UP, 1);
  ...
  static BlockPosition foundation(RemoteChunkSnapshot c, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic piston-motion foundation");
  }
