...
public final class ObserverControlSmoke {
  private static final String TRACE =
      "v1|server=official-b1.7.3|controller=synchronous-caller-thread"
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    List<MovementRouteEvent> events = new ArrayList<>();
    ...
    try {
      ...
      MovementAlternative blocked =
          new MovementAlternative(new MovementStep(block.x() + .5D - initial.x(),
                                      block.y() - initial.y(), block.z() + .5D - initial.z(), 10),
              new MovementStep(.125D, 0D, 0D, 5));
      MovementAlternative later = new MovementAlternative(
          new MovementStep(0D, 0D, .125D, 5), new MovementStep(-.125D, 0D, 0D, 5));
      route = client.moveRouteWithFallbackUntil(Arrays.asList(blocked, later), event -> {
        ...
        events.add(event);
        return event.kind() == MovementAttemptKind.FALLBACK ? MovementRouteDirective.STOP
                                                            : MovementRouteDirective.CONTINUE;
      });
      require(events.size() == 2 && event(events.get(0), 0, 0, MovementAttemptKind.PRIMARY)
              && event(events.get(1), 0, 1, MovementAttemptKind.FALLBACK),
          "controlled event order drifted");
      require(route.outcomes().size() == 2 && route.corrections() == 1,
          "observer stop did not exclude later alternative");
      for (int index = 0; index < events.size(); index++)
        require(events.get(index).outcome() == route.outcomes().get(index),
            "event outcome identity drifted");
      after = worldline.test.WorldlineSmokeAwait.observe(client, 1);
      require(after.containsChunk(chunkX, chunkZ), "controlled route lost cache");
      client.close();
      ...
      require(close(player.x(), route.finalPose().x()) && close(player.y(), route.finalPose().y())
          ...
          "controlled route pose was not persisted");
    } finally {
      client.close();
      server.close();
    }
    System.out.println(
        "WORLDLINE_M40_API=route-controller,synchronous,directive,stop,identity-bound,cache,persistence");
    System.out.println(
        "WORLDLINE_M40_EVENTS=" + describe(events) + ";outcomes=" + route.outcomes().size());
    System.out.println("WORLDLINE_M40_CACHE=chunks=" + after.chunks().size());
    System.out.println(
        "WORLDLINE_M40_PERSISTED=" + player.x() + "," + player.y() + "," + player.z());
  ...
  private static boolean event(
      MovementRouteEvent event, int alternative, int outcome, MovementAttemptKind kind) {
    return event.alternativeIndex() == alternative && event.outcomeIndex() == outcome
        && event.kind() == kind;
  }
  private static String describe(List<MovementRouteEvent> events) {
    StringBuilder value = new StringBuilder();
    for (MovementRouteEvent event : events) {
      if (value.length() > 0)
        value.append(',');
      value.append(event.alternativeIndex())
          .append(':')
          .append(event.outcomeIndex())
          .append(':')
          .append(event.kind());
    }
