...
public final class LavaDamageSmoke {
  ...
  public static void main(String[] args) throws Exception {
    ...
    require(seed == 17320110707L && user.equals("LavaDmg605") && user.length() <= 16, "lava-damage identity drift");
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(
          workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1}, new int[] {1, 327}, new int[] {48, 1}, new int[] {0, 0});
      actor.connect();
      ...
      BlockFace[] walls = new BlockFace[] {BlockFace.NORTH, BlockFace.SOUTH, BlockFace.EAST, BlockFace.WEST};
      int wall = 0;
      ...
      require(WorldlineSmokeAwait.observe(actor, 5).blockAt(lava.x(), lava.y(), lava.z()).equals(new BlockState(0, 0))
              && actor.health() == 20,
          "empty basin drift");
      ...
      require(WorldlineSmokeAwait.observe(actor, 5).blockAt(lava.x(), lava.y(), lava.z()).equals(new BlockState(11, 0))
              && actor.health() == 20,
          "pre-lava still-lava drift");
      pose = actor.moveAndObserve(lava.x() + 0.5D - pose.x(), lava.y() - pose.y(), lava.z() + 0.5D - pose.z(), 2)
                 .resulting();
      int first = dropTo(actor, 20, 16, "lava-first");
      int second = dropTo(actor, first, 13, "lava-repeat");
      actor.moveAndObserve(0D, 2D, 1D, 3);
      require(
          actor.health() == second && second == 13 && second > 0, "post-leave lava health drift: " + actor.health());
      require(WorldlineSmokeAwait.observe(actor, 1).blockAt(lava.x(), lava.y(), lava.z()).equals(new BlockState(11, 0)),
          "still lava 11:0 drifted after leave");
      actor.close();
      ...
      String evidence = "column=" + column + ",floor=" + cell(top, 1) + ",lava=" + cell(lava, 11)
          + ",health=20->16->13,damage=4+3,hits=2,status=2,alive=true,"
          + "persisted=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed + "|fixture=raised-stone-basin+still-lava11"
          + "|cause=packet15-lava-bucket327+stand-in-lava"
          + "|wire=packet53-lava11+packet38-status2+packet8-health20->16->13"
          + "|oracle=repeated-lava-damage-alive-not-m138-flow-not-m465-death-typically-fire|" + evidence;
  ...
  private static int dropTo(B173WireClient actor, int before, int expect, String name) throws Exception {
    int after = WorldlineSmokeAwait.awaitEntity(actor, actor::health, value -> value < before, name + " health", 200);
    require(after == expect, name + " Packet8 health drift: " + before + "->" + after);
    RemoteIncomingHit hit = actor.awaitIncomingHit(after);
    require(hit.healthBefore() == before && hit.healthAfter() == after && hit.damage() == before - after,
  ...
  private static BlockPosition foundation(RemoteChunkSnapshot snapshot, int cx, int cz) {
    ...
    while (x <= 11) {
      ...
      while (z <= 11) {
        int y = 126;
        while (y >= 1) {
          if (snapshot.blockAt(x, y, z).legacyId() == 3 && water(snapshot.blockAt(x, y + 1, z).legacyId())) {
