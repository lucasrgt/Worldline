...
final class OracleFluidFrozenMatterProbe {
    ...
    private OracleFluidFrozenMatterProbe(int[][] rows) {
        this.rows = rows;
    }

    static OracleFluidFrozenMatterProbe execute(dj world) {
        ...
        OracleFluidFrozenMatterProbe result = new OracleFluidFrozenMatterProbe(rows);
        result.validate();
        return result;
    }
    ...
    private static int[] inspect(dj world, em player, int x, int id) {
    ...
    private static int fluidPlacement(dj world, em player, int x, int id) {
        if (id != 8 && id != 10)
            return -1;
        fy stack = new fy(id, 1, 0);
        require(ej.c[id] != null, "fluid item route missing: " + id);
        boolean placed = ej.c[id].a(stack, player, world, x, 64, 8, 1);
        int code = placed && world.a(x, 65, 8) == id && stack.a == 0 ? 1 : 0;
        world.e(x, 65, 8, 0);
        return code;
    }

    private static int metadataDomain(dj world, int x, int id) {
        require(world.b(x, 65, 8, id, 0), "domain cell failed");
        int mask = 0;
        for (int metadata = 0; metadata <= 15; metadata++) {
            rawMetadata(world, x, 65, 8, metadata);
            require(world.c(x, 65, 8) == metadata,
    ...
    private static int fluidBreak(dj world, em player, int x, int id) {
        require(world.b(x, 65, 8, id, 0), "break cell failed: " + id);
        int entities = world.b.size();
        require(world.e(x, 65, 8, 0), "break removal failed: " + id);
        na.m[id].a(world, player, x, 65, 8, 0);
        return world.a(x, 65, 8) == 0 && world.b.size() == entities ? 1 : 0;
    }

    private static int[] sponge(dj world, int x, int placement, int domain, int lifecycle) {
        require(world.b(x, 65, 8, 19, 0), "sponge cell failed");
        int before = state(world, x);
        na.m[19].a(world, x, 65, 8, new Random(17320110891L));
        int tick = before == state(world, x) && na.m[19].c() == 10 ? 1 : 0;
        na.m[19].b(world, x, 65, 8, 1);
        int neighbor = before == state(world, x) ? 1 : 0;
        return new int[] {19, placement, domain, lifecycle, tick, neighbor, -1};
    }

    private static int[] snowLayer(dj world, int x, int placement, int domain, int lifecycle) {
        require(world.b(x, 65, 8, 78, 0), "snow-layer cell failed");
        int before = state(world, x);
        na.m[78].a(world, x, 65, 8, new Random(17320110892L));
        int tick = before == state(world, x) && na.m[78].c() == 10 ? 1 : 0;
        na.m[78].b(world, x, 65, 8, 1);
        int supported = before == state(world, x) ? 1 : 0;
        world.e(x, 64, 8, 0);
        if (world.a(x, 65, 8) != 0)
            na.m[78].b(world, x, 65, 8, 0);
        int loss = world.a(x, 65, 8) == 0 ? 1 : 0;
        return new int[] {78, placement, domain, lifecycle, tick, supported, loss};
    }

    private static int[] ice(dj world, em player, int x,
            int placement, int domain, int lifecycle) {
        ensure(world, x, 64, 8, 1, "ice support failed");
        require(world.b(x, 65, 8, 79, 0), "ice cell failed");
        cz box = na.m[79].e(world, x, 65, 8);
        int collision = full(box, x) ? 1 : 0;
        na.m[79].b(world, x, 65, 8, 1);
        int neighbor = state(world, x) == 7900 ? 1 : 0;
        int entities = world.b.size();
        world.e(x, 65, 8, 0);
        na.m[79].a(world, player, x, 65, 8, 0);
        int broken = world.a(x, 65, 8);
        int iceBreak = (broken == 8 || broken == 9) && world.b.size() == entities ? 1 : 0;
        return new int[] {79, placement, domain, lifecycle, collision, neighbor, iceBreak};
    }

    private static int[] snowBlock(dj world, int x,
            int placement, int domain, int lifecycle) {
        require(world.b(x, 65, 8, 80, 0), "snow-block cell failed");
        int before = state(world, x);
        na.m[80].a(world, x, 65, 8, new Random(17320110893L));
        int tick = before == state(world, x) && na.n[80] ? 1 : 0;
    ...
    private static int state(dj world, int x) {
        return world.a(x, 65, 8) * 100 + world.c(x, 65, 8);
    }
    ...
    private static void ensure(dj world, int x, int y, int z, int id, String message) {
        if (world.a(x, y, z) != id)
            require(world.b(x, y, z, id, 0), message);
    }
    private static boolean full(cz box, int x) {
        return box != null && box.a == x && box.b == 65D && box.c == 8D
                && box.d == x + 1D && box.e == 66D && box.f == 9D;
    }
