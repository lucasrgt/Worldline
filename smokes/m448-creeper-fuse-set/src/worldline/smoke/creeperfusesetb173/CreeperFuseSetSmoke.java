...
public final class CreeperFuseSetSmoke{
 ...
 public static void main(String[]a)throws Exception{
  ...
  try{server.boot();actor.connect();pose=stand(actor,actor.synchronizePose(),top);require(actor.awaitInventory().occupiedSlots()>=1&&Math.abs(pose.y()-(top.y()+1.0D))<2D,"creeper-fuse-set reload pose drift");server.setTime(14000L);
   ...
   explosion=B173CreeperFuseAccess.stayThenExplode(actor,creeper);require(explosion!=null&&explosion.strength()>0F,"Packet60 did not follow creeper proximity fuse");
   ...
   System.out.println("WORLDLINE_M448_SET="+evidence);System.out.println("WORLDLINE_M448_TRACE="+trace);System.out.println("WORLDLINE_M448_SIGNATURE="+sha(trace));
  }finally{actor.close();server.close();}
 }
