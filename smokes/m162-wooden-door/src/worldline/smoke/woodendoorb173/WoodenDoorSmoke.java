package worldline.smoke.woodendoorb173;
...
/** Places official wooden door item 324 and toggles BlockDoor 64 open then closed. */
public final class WoodenDoorSmoke {
  private WoodenDoorSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: WoodenDoorSmoke server.jar workspace port seed username chunkX chunkZ");
    ...
    try {
      ...
      require(after.blockAt(local(lower.x(), cx), lower.y(), local(lower.z(), cz))
          ...
          "persisted wooden door drift");
      String evidence = "column=" + column + ",support=" + top.x() + ":" + top.y() + ":" + top.z()
          + ":1:0,lower=" + lower.x() + ":" + lower.y() + ":" + lower.z()
          + ":64:0->4->0,upper=" + upper.x() + ":" + upper.y() + ":" + upper.z()
          + ":64:8->12->8,persisted=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=raised-stone+woodendoor64|cause=packet15-item324-place+empty-hand-packet15-toggle|wire=packet53-door64:0/8->4/12->0/8|oracle=woodendoor-open-close+fresh-login|"
          + evidence;
      System.out.println("WORLDLINE_M162_DOOR=" + evidence);
      System.out.println("WORLDLINE_M162_TRACE=" + trace);
      System.out.println("WORLDLINE_M162_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
