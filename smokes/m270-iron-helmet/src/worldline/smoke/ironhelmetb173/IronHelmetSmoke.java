...
public final class IronHelmetSmoke {
    ...
    private static final String SIGNAL = "window=5:306,packet5=4:306,item!=298,actions=1,2,persisted=true,clients=4,disconnect=clean";
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        require(HELMET.legacyId() == 306 && HELMET.legacyId() != 298 && HELMET.count() == 1
                && HELMET.damage() == 0, "iron helmet identity drifted");
        ...
        try {
            server.boot();
            B173PlayerSeed.writeInventory(workspace, actorName, 4.5D, 72D, 4.5D,
                    new int[] {0}, new int[] {306}, new int[] {1}, new int[] {0});
            B173PlayerSeed.write(workspace, observerName, 4.5D, 72D, 4.5D);
            actor.connect(); actor.synchronizePose();
            require(actor.awaitInventory().occupiedSlots() == 1, "actor inventory was not the iron helmet");
            ...
            RemoteArmorPiece peer = observer.awaitPeerArmor(
                    new RemoteArmorPiece(actorName, RemoteArmorSlot.HELMET, 306, 0));
            require(peer.slot() == RemoteArmorSlot.HELMET && peer.legacyId() == 306 && peer.legacyId() != 298
                    && peer.slot().containerSlot() == 5 && peer.slot().equipmentSlot() == 4
                    && peer.damage() == 0, "peer iron helmet drifted");
            actor.close(); observer.close(); awaitPlayers(server, 0); server.save();
            require(server.player(actorName).inventoryItems() == 1, "iron helmet persistence count drifted");
            restored = client(port, actorName, timeout); witness = client(port, observerName, timeout);
            restored.connect(); restored.synchronizePose(); requireArmor(restored.awaitInventory());
            ...
            if (restored != null) restored.close(); if (witness != null) witness.close(); server.close();
        }
    ...
    private static int find(RemoteInventoryView view, RemoteItemStack expected) {
        for (int slot = 9; slot <= 44; slot++)
            if (!view.slot(slot).empty() && view.slot(slot).item().equals(expected)) return slot;
        return -1; }
    private static void requireArmor(RemoteInventoryView view) {
        require(!view.slot(5).empty() && view.slot(5).item().equals(HELMET)
                && view.slot(36).empty() && view.occupiedSlots() == 1, "iron helmet window drifted"); }
