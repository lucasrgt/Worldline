...
final class MetadataRecipeContract {
    ...
    static void verify(List<ItemStackRecipe> recipes) {
        ...
        wool(recipes, 1, 14); wool(recipes, 2, 13); wool(recipes, 4, 11);
        ...
        slab(recipes, 24, 1); slab(recipes, 5, 2); slab(recipes, 4, 3);
        dye(recipes, 1, 11, 14); dye(recipes, 1, 4, 5); dye(recipes, 2, 15, 10);
        wool(recipes, 11, 4); wool(recipes, 14, 1); wool(recipes, 9, 6);
        dye(recipes, 2, 4, 6); dye(recipes, 1, 15, 9); dye(recipes, 4, 15, 12);
        wool(recipes, 13, 2); wool(recipes, 12, 3); wool(recipes, 10, 5);
        expect(recipes, out(351, 3, 7), in(351, 1, 0), in(351, 2, 15));
        dye(recipes, 8, 15, 7); dye(recipes, 5, 9, 13);
        try { recipes.clear(); throw new IllegalStateException("metadata recipes are mutable"); }
    ...
    private static void expect(List<ItemStackRecipe> recipes, RemoteItemStack output,
