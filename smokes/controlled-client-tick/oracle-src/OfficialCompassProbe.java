...
final class OfficialCompassProbe {
    ...
    static String trace(net.minecraft.client.Minecraft client, fd world, dc player) {
        br spawn = world.u(); double y = player.aN;
        ...
        return "spawn=" + spawn.a + ":" + spawn.b + ":" + spawn.c
                + ",east0=" + east0 + ",east180=" + east180
                + ",west0=" + west0 + ",west180=" + west180;
    }

    private static String sample(net.minecraft.client.Minecraft client, dc player,
            double x, double y, double z, float yaw) {
        player.c(x, y, z, yaw, 0F); av texture = new av(client); texture.a();
        return "i" + bits(texture, "i") + "j" + bits(texture, "j")
                + "p" + digest(bytes(texture));
    }

    private static String bits(av texture, String name) {
        try { Field field = av.class.getDeclaredField(name); field.setAccessible(true);
    ...
    private static byte[] bytes(av texture) {
        try { Field field = aw.class.getDeclaredField("a"); field.setAccessible(true);
