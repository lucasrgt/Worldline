...
final class OfficialPhysicsProbe {
  ...
  static String trace(fd world, dc player) {
    ...
    corridor(world, 1, 0);
    int air = horizontal(player, slowTicks);
    corridor(world, 1, 30);
    int web = horizontal(player, slowTicks);
    corridor(world, 88, 0);
    int soul = horizontal(player, slowTicks);
    require(web < air && soul < air && web >= 0 && soul >= 0,
        "official slow-block ordering absent " + air + "/" + web + "/" + soul);
    ladderFixture(world, false);
    int ladderAir = vertical(player, ladderTicks);
    ladderFixture(world, true);
    int climb = vertical(player, ladderTicks);
    require(climb > ladderAir, "official ladder climb ordering absent " + ladderAir + "/" + climb);
    return "v1|slow=ticks=" + slowTicks + ",air=" + air + ",web=" + web + ",soul=" + soul
        + "|ladder=ticks=" + ladderTicks + ",air=" + ladderAir + ",climb=" + climb;
  }
