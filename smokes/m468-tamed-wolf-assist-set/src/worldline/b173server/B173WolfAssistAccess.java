...
public final class B173WolfAssistAccess {
  ...
  public static int tame(B173WireClient client, RemoteMobSpawn spawn) {
    ...
    for (int n = 0; n < 32; n++) {
      ...
      if (slot < 0)
        throw new IllegalStateException("bone 352 exhausted before Packet38 status 7");
  ...
  public static void unsit(B173WireClient client, int entity) {
    ...
    if (slot < 0)
      throw new IllegalStateException("diamond sword 276 absent before unsit");
    client.selectHeldSlot(slot);
  ...
  public static RemoteMobDeath strikeOnceAndAssist(B173WireClient client, RemoteMobSpawn spawn) {
    ...
    int slot = find(client.inventory(), SWORD, -1);
    if (slot < 0)
      throw new IllegalStateException("diamond sword 276 absent before Packet7");
    int hurts = 0;
    ...
    if (hurts < 1)
      throw new IllegalStateException("target Packet38 status 2 absent after player Packet7");
    for (int n = 0; n < 32; n++) {
      RemoteMobMovement move = client.channel().inbound().mobs().takeMovement(entity);
      if (move != null) {
        x = move.toX();
  ...
  private static void use(B173WireClient client, int entity, int item, int button) {
    try {
      ...
      if (entity < 0 || entity == local)
        throw new IllegalArgumentException("invalid wolf-assist target");
      ...
      if (slot < 0)
        throw new IllegalStateException("item " + item + " absent from hotbar");
      if (button != 0 && button != 1)
        throw new IllegalArgumentException("invalid Packet7 button");
      synchronized (channel.output) {
        ...
        channel.output.writeByte(button);
        channel.output.flush();
      }
    } catch (IOException error) {
      throw new IllegalStateException("wolf-assist Packet7 failed", error);
    }
  }
