...
public final class B173MachineBlockCrafts {
  ...
  private static final int SAND = 12, GUNPOWDER = 289, PLANKS = 5, COBBLE = 4, IRON = 265,
                           REDSTONE = 331, SLIME = 341, STAT = 16842752;
  ...
  public static int[] apply(B173WireClient actor) {
    try {
      return new B173MachineBlockCrafts(actor).run();
    } catch (IOException error) {
      throw new IllegalStateException("machine-block crafts failed", error);
    }
  }
  ...
  private void spread(int ingredientId, int[] cells, RemoteItemStack[] mid) throws IOException {
    ...
    for (int index = 0; index < cells.length; index++)
      placeOne(cells[index], mid[index]);
    if (inbound.cursor() != null)
      store(source, inbound.cursor());
  }
  ...
  private void store(int combined, RemoteItemStack stack) throws IOException {
    step(combined, 0, null, replace(window(), combined, stack),
        replace(personal(), combined - 1, stack), null);
  }
  ...
  private static RemoteItemStack[] none(int n) {
    return new RemoteItemStack[n];
  }
  private static RemoteItemStack[] last(int n, RemoteItemStack result) {
    RemoteItemStack[] mid = new RemoteItemStack[n];
    mid[n - 1] = result;
    return mid;
  }
}
