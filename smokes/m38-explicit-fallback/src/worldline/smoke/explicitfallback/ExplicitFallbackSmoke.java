    ...
    public static void main(String[] arguments) throws Exception {
        ...
        try {
            ...
            require(outcomes.get(0).disposition() == MovementDisposition.UNCHALLENGED
                    && outcomes.get(0).resulting().equals(accepted), "safe primary drifted");
            ...
            require(outcomes.get(2).disposition() == MovementDisposition.UNCHALLENGED
                    ...
                    && close(route.finalPose().z(), accepted.z()), "explicit fallback did not execute once");
            after = worldline.test.WorldlineSmokeAwait.observe(client,1); require(after.containsChunk(chunkX, chunkZ), "fallback route lost cache");
            ...
            require(close(player.x(), route.finalPose().x()) && close(player.y(), route.finalPose().y())
                    && close(player.z(), route.finalPose().z()), "explicit fallback pose was not persisted");
        } finally { client.close(); server.close(); }
        System.out.println("WORLDLINE_M38_API=movement-alternative,conditional-fallback,no-retry,cache,persistence");
        ...
        System.out.println("WORLDLINE_M38_PERSISTED=" + player.x() + "," + player.y() + "," + player.z());
