...
public final class ItemCollectionSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    RemoteDroppedItem dropped;
    RemoteItemCollection collection;
    RemoteInventoryView restored;
    RemoteHeldItem peer;
    ...
    try {
      ...
      require(
          actor.inventory().occupiedSlots() == 1 && actor.inventory().slot(36).item().equals(stone),
          "held seed drifted");
      observer.connect();
      ...
      actor.look(0F, 90F);
      actor.dropHeldItem();
      dropped = observer.awaitDroppedItem(stone);
      worldline.test.WorldlineSmokeAwait.awaitEntity(
          actor, actor::inventory, view -> view.occupiedSlots() == 0, "dropped inventory", 10);
      ...
      worldline.test.WorldlineSmokeAwait.awaitEntity(
          actor, actor::inventory, view -> view.occupiedSlots() == 1, "collected inventory", 10);
      require(actor.inventory().occupiedSlots() == 1, "actor did not descend through dropped item");
      collection = observer.awaitItemCollection(dropped, actorName);
      require(collection.collectorEntityId() == actor.state().entityId()
              && collection.droppedItem().equals(dropped),
          "named collection correlation drifted");
      restored = actor.inventory();
      require(restored.occupiedSlots() == 1 && restored.slot(36).item().equals(stone),
          "collected stone did not restore actor inventory");
