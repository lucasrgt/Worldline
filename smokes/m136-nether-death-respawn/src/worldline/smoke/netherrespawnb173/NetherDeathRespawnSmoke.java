...
public final class NetherDeathRespawnSmoke {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 6) throw new IllegalArgumentException("usage: NetherDeathRespawnSmoke server.jar workspace port seed username fixtureTicks");
        Path jar = Paths.get(args[0]), workspace = Paths.get(args[1]); int port = Integer.parseInt(args[2]);
        long seed = Long.parseLong(args[3]); String username = args[4]; int fixtureTicks = Integer.parseInt(args[5]);
        Duration timeout = Duration.ofSeconds(90); B173DedicatedServer server = new B173DedicatedServer(jar, workspace, port, seed, timeout, 3, true, true);
        RespawnSession actor = new B173WireClient("127.0.0.1", port, username, timeout); RemoteRespawn respawn; RemoteChunkSnapshot nether, overworld;
        try {
            server.boot(); B173PlayerSeed.writeDimension(workspace, username, 8.5D, -80D, 8.5D, -1);
            ...
            require(saved.dimension() == 0 && saved.health() == 20 && saved.inventoryItems() == 0, "Nether respawn persistence drift");
        } finally { actor.close(); server.close(); }
        String evidence = "dimensions=-1->0,health=20->nonpositive->20,request=09:ff,nether=netherrack+sky0,overworld=sky-positive,cache=overworld-only,inventory=empty,persisted=0:20";
        String trace = "v1|server=official-b1.7.3|seed=" + seed + "|profile=allow-nether-true|entry=player-nbt-dimension-minus-one-y-minus-80|death=vanilla-void-damage|request=packet9-signed-minus-one|response=fresh-packet9-zero+packet8-health20|cache=dimension-change-reset+corrected-overworld-chunks|" + evidence + "|disconnect=clean";
        System.out.println("WORLDLINE_M136_RESPAWN=" + evidence); System.out.println("WORLDLINE_M136_TRACE=" + trace); System.out.println("WORLDLINE_M136_SIGNATURE=" + sha(trace));
    }
    private static int count(RemoteChunkSnapshot q, int id) { int n = 0; for (int x = 0; x < 16; x++) for (int z = 0; z < 16; z++) for (int y = 0; y < 128; y++) if (q.blockAt(x, y, z).legacyId() == id) n++; return n; }
    private static int sky(RemoteChunkSnapshot q) { int n = 0; for (int x = 0; x < 16; x++) for (int z = 0; z < 16; z++) for (int y = 0; y < 128; y++) if (q.skyLightAt(x, y, z) > 0) n++; return n; }
