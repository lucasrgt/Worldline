...
public final class M11ModApiSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        if ("run".equals(arguments[0])) run(jar);
        else if ("reject-schedule".equals(arguments[0])) reject(jar, "SCHEDULE");
        else if ("reject-spawn".equals(arguments[0])) reject(jar, "SPAWN");
        else throw new IllegalArgumentException("invalid M11 smoke mode");
    }
    ...
    private static void run(Path jar) throws Exception {
        try (LoadedMod<B173Mod> loaded = ModLoader.load(jar, RUNTIME, API, B173Mod.class)) {
            ...
            try {
                ...
                for (int index = 0; index < 4; index++) runtime.tick();
                GameWorld world = runtime.world();
                require(world.block(new BlockPosition(8, 65, 8)).legacyId() == 20,
                        "onLoad setBlock missing");
                ...
                require(world.itemsAt(new BlockPosition(10, 65, 10)).total() == 0,
                        "container census failed");
                B173Observation state = runtime.observe();
                CanonicalStateTrace trace = new CanonicalStateTrace(SEED,
                        "tick", "block65", "block9659", "pigs", "iron");
                trace.record("final", state.clientTick(),
                        state.blockColumn()[1],
                        world.block(new BlockPosition(9, 65, 9)).legacyId(),
                        countAlive(world, "minecraft:pig"),
    ...
    private static GameEntity find(GameWorld world, String type) {
        for (GameEntity entity : world.entities()) {
            if (type.equals(entity.type()) && entity.alive()) return entity;
        }
        return null;
    }
