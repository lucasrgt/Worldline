...
public final class AeroCombatWireSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        try {
            server.boot(); server.save(); server.operator(victimName); server.operator(attackerName);
            ...
            B173PlayerSeed.writeInventory(workspace, victimName, x, y, z,
                    new int[] {0, 1, 2, 3}, new int[] {298, 299, 300, 301},
                    new int[] {1, 1, 1, 1}, new int[] {0, 0, 0, 0});
            B173PlayerSeed.writeInventory(workspace, attackerName, x + 3D, y, z,
                    new int[] {0}, new int[] {276}, new int[] {1}, new int[] {0});
            B173PlayerSeed.write(workspace, observerName, x + 6D, y, z);
            victim.connect(); PlayerPose victimAir = victim.synchronizePose();
            require(victim.awaitInventory().occupiedSlots() == 4, "victim inventory seed drifted"); victim.look(0F, 90F);
            ...
            attacker.connect(); PlayerPose aligned = attacker.synchronizePose(); require(attacker.awaitInventory().occupiedSlots() == 1,
                    "attacker inventory seed drifted"); attacker.look(0F, 90F);
