...
final class RepeaterWorldBackend implements GameBackend {
  ...
  void placeCircuit() {
    ...
    require(current.setBlockAndMetadataWithNotify(
                REPEATER_X, Y, Z, Block.redstoneRepeaterIdle.blockID, 1),
  ...
  void snapshot(CanonicalTrace trace, String label) {
    ...
    trace.record(label, current.getWorldTime(), current.loadedEntityList.size(), repeater,
        current.getBlockId(WIRE_X, Y, Z), current.getBlockMetadata(WIRE_X, Y, Z), powering,
        powered);
  }
  ...
  void assertFinalState() {
    ...
    require(
        poweringTo(current, current.getBlockId(REPEATER_X, Y, Z)) == 1, "repeater output is dark");
