  ...
  public static synchronized void remove(PlayerEntity player, int[] v) {
    ...
    for (int i : ids) {
      ...
      if (player.world.getBlockId(x, cy, cz) != WorldlineCensusMod.block.id
        ...
        throw new IllegalStateException("M82 target drift");
    }
    for (int i : ids)
      if (!player.world.setBlock(x, y + i % 4, z + i / 4, 0))
        throw new IllegalStateException("M82 removal rejected");
    for (int i : ids)
      if (player.world.getBlockId(x, y + i % 4, z + i / 4) != 0)
        throw new IllegalStateException("M82 removal absent");
    done = true;
    MessagePacket ack = new MessagePacket(WorldlineLadderMod.CHANGE);
    ack.ints = v.clone();
    server.networkHandler.sendPacket(ack);
    System.out.println("[WorldlineLadder] removed targets=" + n + " x=" + x + " y=" + y + " z=" + z
        + " nonce=" + root);
  }
}
