...
public final class WorldlineLadderMod {
    public static final Identifier CHANGE=Identifier.of(WorldlineCensusMod.NAMESPACE,"membership_wave");static{EntrypointManager.registerLookup(MethodHandles.lookup());}public WorldlineLadderMod(){}
    @EventListener private static void messages(MessageListenerRegistryEvent event){event.register(CHANGE,(player,packet)->{if(player.world.isRemote)WorldlineLadderState.ack(packet.ints);else WorldlineLadderServer.remove(player,packet.ints);});}
}
