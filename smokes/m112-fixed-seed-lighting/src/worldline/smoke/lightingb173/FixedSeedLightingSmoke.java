...
public final class FixedSeedLightingSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    String trace = "v1|server=official-b1.7.3|seed=" + seed + "|target=absolute-chunk|origin="
        + chunkX + "," + chunkZ + "|samples=32768|block=" + block.hash
    ...
    System.out.println("WORLDLINE_M112_LIGHT=chunk=" + chunkX + ":" + chunkZ
        + ",block=" + block.hash + ",sky=" + sky.hash + ",blockHist=" + block.histogram
        + ",skyHist=" + sky.histogram);
    System.out.println("WORLDLINE_M112_TRACE=" + trace);
    System.out.println(
        "WORLDLINE_M112_SIGNATURE=" + sha256(trace.getBytes(StandardCharsets.UTF_8)));
  }
  ...
  private static Plane plane(RemoteChunkSnapshot chunk, boolean sky) throws Exception {
    ...
    for (int x = 0; x < 16; x++)
      for (int z = 0; z < 16; z++)
        for (int y = 0; y < 128; y++) {
          int value = sky ? chunk.skyLightAt(x, y, z) : chunk.blockLightAt(x, y, z);
  ...
  private static void verify(RemoteChunkSnapshot chunk, int chunkX, int chunkZ) {
    require(chunk.observation().x() == chunkX * 16 && chunk.observation().z() == chunkZ * 16
            ...
            && chunk.blockCount() == 32768,
        "lighting target shape drift");
  }
  ...
  private static final class Plane {
    ...
    Plane(String hash, String histogram) {
      this.hash = hash;
