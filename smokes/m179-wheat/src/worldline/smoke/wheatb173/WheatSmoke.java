package worldline.smoke.wheatb173;
...
/** Hoes official dirt, hydrates farmland, and plants seeds 295 as wheat 59:0 across restart. */
public final class WheatSmoke {
  private WheatSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: WheatSmoke server.jar workspace port seed username chunkX chunkZ");
    ...
    BlockPosition top, north, south, west, basin, water;
    BlockPosition[] plots;
    int column;
    try {
      server.boot();
      ...
      BlockPosition[] crops = new BlockPosition[plots.length];
      for (int i = 0; i < plots.length; i++)
        crops[i] = sow(actor, plots[i]);
      RemoteWorldView held = worldline.test.WorldlineSmokeAwait.observe(actor, 5);
      for (int i = 0; i < crops.length; i++)
        require(held.blockAt(crops[i].x(), crops[i].y(), crops[i].z()).equals(new BlockState(59, 0))
                && held.blockAt(plots[i].x(), plots[i].y(), plots[i].z()).legacyId() == 60,
            "live wheat drift");
      actor.close();
      ...
      require(moist(after, plots, cx, cz) && wheat(after, plots, cx, cz)
              && after.blockAt(local(water.x(), cx), water.y(), local(water.z(), cz))
                  .equals(new BlockState(9, 0)),
          "persisted wheat drift: " + dump(after, plots, cx, cz)
              + ",water=" + after.blockAt(local(water.x(), cx), water.y(), local(water.z(), cz)));
      String evidence = "column=" + column + ",plots=4,water=" + water.x() + ":" + water.y() + ":"
          + water.z()
          + ":9:0,hoe=290,seeds=295,wheat=59:0,hydrated=7,persisted=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=raised-dirt3+still-water9|cause=packet15-wooden-hoe290+seeds295|wire=packet53-farmland60+crops59:0|oracle=live-hold+fresh-login-wheat59:0|"
          + evidence;
      System.out.println("WORLDLINE_M179_WHEAT=" + evidence);
      System.out.println("WORLDLINE_M179_TRACE=" + trace);
      System.out.println("WORLDLINE_M179_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
  ...
  private static boolean wheat(RemoteChunkSnapshot q, BlockPosition[] plots, int cx, int cz) {
    for (BlockPosition p : plots)
      if (!q.blockAt(local(p.x(), cx), p.y() + 1, local(p.z(), cz)).equals(new BlockState(59, 0)))
        return false;
    return true;
  }
  private static String dump(RemoteChunkSnapshot q, BlockPosition[] plots, int cx, int cz) {
    ...
    for (BlockPosition p : plots)
      s.append(p.x())
          ...
          .append('/')
          .append(q.blockAt(local(p.x(), cx), p.y() + 1, local(p.z(), cz)))
          .append(' ');
    return s.toString();
  }
