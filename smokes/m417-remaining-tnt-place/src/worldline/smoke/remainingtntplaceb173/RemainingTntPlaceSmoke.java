...
public final class RemainingTntPlaceSmoke{
 ...
 public static void main(String[]a)throws Exception{
  if(a.length!=9)throw new IllegalArgumentException("usage: RemainingTntPlaceSmoke server.jar workspace port seed username chunkX chunkZ fixtureTicks fuseTicks");
  ...
  require(seed==17320110707L&&user.equals("TntChain417")&&user.length()<=16,"remaining-tnt-place identity drift");
  ...
  BlockPosition top,tnt1,tnt2;int column;RemoteObjectSpawn primed,chained;RemoteExplosion first,second;
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1,2},new int[]{1,46,259},new int[]{32,2,1},new int[]{0,0,0});actor.connect();actor.synchronizePose();require(actor.awaitInventory().occupiedSlots()==3,"remaining-tnt-place inventory drift");
   ...
   String trace="v1|server=official-b1.7.3|seed="+seed+"|fixture=raised-stone+tnt46+tnt46+flint259|cause=packet15-item46+packet15-item46+packet15-item259-prime-one|fuse="+fuseTicks+"ticks|wire=packet60-strength4+packet60-strength4+packet23-type50-chain|oracle=live-two-tnt-chain+crater-air+fresh-login|"+evidence;
   System.out.println("WORLDLINE_M417_SET="+evidence);System.out.println("WORLDLINE_M417_TRACE="+trace);System.out.println("WORLDLINE_M417_SIGNATURE="+sha(trace));
 ...
 private static BlockPosition foundation(RemoteChunkSnapshot q,int cx,int cz){for(int x=4;x<=11;x++)for(int z=4;z<=11;z++)for(int y=126;y>=1;y--)if(q.blockAt(x,y,z).legacyId()==3&&water(q.blockAt(x,y+1,z).legacyId()))return new BlockPosition(cx*16+x,y,cz*16+z);throw new IllegalStateException("no deterministic remaining-tnt-place foundation");}
