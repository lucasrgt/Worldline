...
public final class WorldlinePagedTimer {
  ...
  private static int rendererCount, queueCount, flushCount, immediateCount;
  static {
    for (int i = 0; i < CAP; i += 256) {
      renderer[i] = queue[i] = flush[i] = 0;
      rendererCalls[i] = queueCalls[i] = flushCalls[i] = 0;
      queued[i] = pageCalls[i] = direct[i] = rebuilds[i] = cached[i] = evicted[i] = immediate[i] =
          0;
    }
  }
  ...
  static void arm() {
    if (armed)
      throw new IllegalStateException("duplicate M110 arm");
    String size = System.getProperty("aero.becell.size");
    if (!"true".equals(System.getProperty("aero.becell.pages"))
        || !"2".equals(System.getProperty("aero.becell.minInstances"))
        || !"false".equals(System.getProperty("aero.becell.skipIndividual"))
        || !("33".equals(size) || "32".equals(size))
        || !"-1".equals(System.getProperty("aero.becell.maxCachedPages"))
        || !"-1".equals(System.getProperty("aero.becell.rebuildsPerFrame"))
        || !"100000".equals(System.getProperty("aero.becell.pageTtlFrames")))
      throw new IllegalStateException("M110 runtime drift");
    armed = true;
  }
  public static void head() {
    if (!armed || WorldlinePagedBridge.sealed() || !WorldlinePagedBridge.running())
      return;
    if (capture)
      throw new IllegalStateException("M110 HEAD without TAIL");
    rendererNow = queueNow = flushNow = rendererStart = queueStart = flushStart = 0;
    rendererCount = queueCount = flushCount = immediateCount = 0;
    capture = true;
  }
  public static void tail() {
    ...
    if (rendererStart != 0 || queueStart != 0 || flushStart != 0)
      throw new IllegalStateException("unterminated M110 span");
    int i = WorldlinePagedBridge.count();
    if (i >= CAP || rendererCount > 0xffff || queueCount > 0xffff || flushCount > 0xffff)
      throw new IllegalStateException("M110 counter overflow");
  ...
  public static void immediateDirect() {
    if (capture)
      immediateCount++;
  }
  private static long begin(long old, String label) {
    if (old != 0)
      throw new IllegalStateException("recursive M110 " + label);
    return System.nanoTime();
  }
  private static long end(long start, String label) {
    if (start == 0)
      throw new IllegalStateException("unstarted M110 " + label);
    long v = System.nanoTime() - start;
    if (v < 0)
      throw new IllegalStateException("negative M110 " + label);
    return v;
  }
