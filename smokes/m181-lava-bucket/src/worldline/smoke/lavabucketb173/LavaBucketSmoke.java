package worldline.smoke.lavabucketb173;
...
/** Places still lava 11:0 on a raised stone basin and picks it up with empty bucket 325. */
public final class LavaBucketSmoke {
  private LavaBucketSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: LavaBucketSmoke server.jar workspace port seed username chunkX chunkZ");
    ...
    int column;
    RemoteItemStack filled = new RemoteItemStack(327, 1, 0);
    PlayerPose pose;
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2},
          new int[] {1, 11, 325}, new int[] {48, 1, 1}, new int[] {0, 0, 0});
      actor.connect();
      ...
      require(
          actor.sustainTicks(5).blockAt(cell.x(), cell.y(), cell.z()).equals(new BlockState(0, 0))
              && actor.inventory().slot(38).item().equals(filled) && actor.health() == 20,
      ...
      require(after.blockAt(local(cell.x(), cx), cell.y(), local(cell.z(), cz))
          ...
          "persisted empty-basin lava-bucket drift");
      String evidence = "column=" + column + ",floor=" + top.x() + ":" + top.y() + ":" + top.z()
          + ":1:0,lava=" + cell.x() + ":" + cell.y() + ":" + cell.z()
          + ":11:0->0:0,held=325:1:0->327:1:0,persisted=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=raised-stone-basin+still-lava11|cause=packet15-lava-cell+packet15-dir255-bucket325|wire=packet53-air0+packet103-bucket327|oracle=live-pickup+fresh-login-empty-basin+lava-bucket|"
          + evidence;
      System.out.println("WORLDLINE_M181_BUCKET=" + evidence);
      System.out.println("WORLDLINE_M181_TRACE=" + trace);
      System.out.println("WORLDLINE_M181_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
