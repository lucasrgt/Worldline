...
public final class B173ChunkCodecFixture {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    RemoteChunkSnapshot snapshot =
        B173ChunkCodec.read(new DataInputStream(new ByteArrayInputStream(encoded)));
    for (int x = 0; x < WIDTH; x++)
      for (int z = 0; z < DEPTH; z++) {
        for (int y = 0; y < HEIGHT; y++) {
          int index = (x * DEPTH + z) * HEIGHT + y;
          require(snapshot.blockAt(x, y, z).equals(
                      new BlockState(ids[index] & 255, metadata.getNibble(x, y, z))),
              "block layout drift");
  ...
  private static byte[] packet(byte[] raw) throws IOException {
    ...
    output.writeShort(0);
    output.writeInt(-16);
    output.writeByte(WIDTH - 1);
    output.writeByte(HEIGHT - 1);
    output.writeByte(DEPTH - 1);
