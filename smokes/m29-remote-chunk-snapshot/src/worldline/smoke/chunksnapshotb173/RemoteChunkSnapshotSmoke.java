...
public final class RemoteChunkSnapshotSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        RemoteWorldMultiplayerSession client = new B173WireClient(
                "127.0.0.1", port, arguments[4], timeout);
        RemoteChunkSnapshot snapshot;
        try {
            server.boot(); client.connect(); client.synchronizePose();
            snapshot = client.awaitChunkSnapshot(); verify(snapshot);
    ...
    private static void verify(RemoteChunkSnapshot snapshot) {
        ...
        require(snapshot.blockCount() == 32768 && snapshot.nonAirBlocks() > 0
                && snapshot.nonAirBlocks() < snapshot.blockCount(), "implausible decoded block census");
        for (int x = 0; x < 16; x++) for (int z = 0; z < 16; z++) {
            BlockState floor = snapshot.blockAt(x, 0, z);
            require(floor.legacyId() >= 0 && floor.metadata() >= 0, "invalid floor block");
            require(snapshot.blockLightAt(x, 64, z) <= 15
                    && snapshot.skyLightAt(x, 127, z) <= 15, "invalid light nibble");
        }
    }
    ...
    private static String contentHash(RemoteChunkSnapshot snapshot) throws Exception {
        ...
        for (int x = 0; x < 16; x++) for (int z = 0; z < 16; z++) for (int y = 0; y < 128; y++) {
            BlockState block = snapshot.blockAt(x, y, z); sample.clear();
            sample.put((byte) block.legacyId()).put((byte) block.metadata())
                    .put((byte) snapshot.blockLightAt(x, y, z))
                    .put((byte) snapshot.skyLightAt(x, y, z)); digest.update(sample.array());
        }
