...
public final class LadderClimbPoseSmoke {
  ...
  public static void main(String[] args) throws Exception {
    ...
    require(seed == 17320110707L && user.equals("LadderClb600") && user.length() <= 16, "ladder-climb identity drift");
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(
          workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1}, new int[] {1, 65}, new int[] {32, 8}, new int[] {0, 0});
      actor.connect();
      ...
      pose = actor.moveAndObserve(low.x() + 1.5D - pose.x(), upper.y() + 1.5D - pose.y(), low.z() + 0.5D - pose.z(), 4)
                 .resulting();
      require((int) Math.floor(pose.x()) == low.x() + 1 && pose.y() >= upper.y() + 1.0D, "air pose drift");
      ...
      pose = actor.moveAndObserve(low.x() + 0.5D - pose.x(), low.y() + 1.0D - pose.y(), low.z() + 0.5D - pose.z(), 4)
                 .resulting();
      actor.look(90F, 0F);
      ...
      require(air > 0 && climb >= 0, "ladder pose y did not hold or climb vs air fall " + air + "/" + climb);
      actor.close();
      ...
      require(after.blockAt(local(top.x(), cx), top.y(), local(top.z(), cz)).equals(new BlockState(1, 0))
              && after.blockAt(local(upper.x(), cx), upper.y(), local(upper.z(), cz)).equals(new BlockState(1, 0))
      ...
      String evidence = "column=" + column + ",support=" + cell(top, 1, 0) + ",upper=" + cell(upper, 1, 0)
          + ",ladder=" + cell(low, 65, 5) + "+" + cell(high, 65, 5) + ",face=east,ticks=" + ticks
          + ",air-fall=true,ladder-hold=true,climbed=true,persisted=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed + "|fixture=raised-stone+ladder65-east-column2"
          + "|cause=packet15-item65+packet13-air-fall+packet13-ladder-pose"
          + "|wire=packet13-pose-air-vs-ladder|oracle=ladder-climb-or-hold-vs-air-fall|" + evidence;
