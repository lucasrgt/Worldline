...
public abstract class WorldlineStageFrameMixin {
    ...
    @Inject(method = "onFrameUpdate(F)V", at = @At("HEAD")) private void head(float delta, CallbackInfo callback) {
        if (!WorldlineStageGate.prepare(client)) WorldlineStageTimer.head();
    }
    @Inject(method = "onFrameUpdate(F)V", at = @At("TAIL")) private void tail(float delta, CallbackInfo callback) {
        WorldlineStageTimer.tail(); if (WorldlineFrameCensus.sealed() && !WorldlineStageFile.written()) WorldlineStageFile.write();
    }
}
