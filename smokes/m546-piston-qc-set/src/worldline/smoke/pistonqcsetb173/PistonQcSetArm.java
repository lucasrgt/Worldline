...
public final class PistonQcSetArm{
 ...
 private PistonQcSetArm(BlockPosition s,BlockPosition p,BlockPosition h,BlockPosition a,BlockPosition l){support=s;piston=p;head=h;above=a;lever=l;}
 static PistonQcSetArm place(B173WireClient a,RemoteChunkSnapshot initial,BlockPosition support,int cx,int cz)throws Exception{
 ...
 void persist(RemoteChunkSnapshot after,int cx,int cz,BlockState pistonWant,BlockState headWant,String label){
  require(at(after,lever,cx,cz).equals(new BlockState(69,1))&&at(after,above,cx,cz).equals(new BlockState(1,0))&&at(after,piston,cx,cz).equals(pistonWant)&&at(after,head,cx,cz).equals(headWant)&&!directPower(after,piston,cx,cz),label);
 }
 static BlockPosition raise(B173WireClient a,RemoteChunkSnapshot initial,int cx,int cz,int[] column)throws Exception{
  ...
  while(water(at(initial,BlockFace.UP.adjacent(top),cx,cz).legacyId())){top=place(a,top,BlockFace.UP,1);a.moveAndObserve(0D,1D,0D,1);require(++column[0]<=15,"water column exceeded piston-qc fixture");}
 ...
 static BlockPosition foundation(RemoteChunkSnapshot c,int cx,int cz){for(int x=4;x<=11;x++)for(int z=4;z<=11;z++)for(int y=126;y>=1;y--)if(c.blockAt(x,y,z).legacyId()==3&&water(c.blockAt(x,y+1,z).legacyId()))return new BlockPosition(cx*16+x,y,cz*16+z);throw new IllegalStateException("no deterministic piston-qc foundation");}
