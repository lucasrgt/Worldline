...
/** Proves bounded state, physical, tick, and causal consumer boundaries. */
final class RedstoneSignalConsumersProbe {
    private static final int[] IDS = {23, 25, 46, 50, 55, 84};
    final int[][] rows;

    private RedstoneSignalConsumersProbe(int[][] rows) {
        this.rows = rows;
    }

    static RedstoneSignalConsumersProbe execute(World world) {
        ...
        for (int index = 0; index < IDS.length; index++) {
            ...
            require(world.setBlockAndMetadataWithNotify(x, 65, 8, id, 0),
                    "consumer placement failed: " + id);
            int state = stateDomain(world, x, id);
            if (state >= 0)
                rawMetadata(world, x, 65, 8, 0);
            int shape = shape(Block.blocksList[id], world, x, id);
            int light = claimedPhysical(id)
                    ? Block.lightOpacity[id] * 100 + Block.lightValue[id] : -1;
            int tickRate = Block.blocksList[id].tickRate();
        ...
        RedstoneSignalConsumersProbe result = new RedstoneSignalConsumersProbe(rows);
        result.validate();
        return result;
    }
    ...
    private static int stateDomain(World world, int x, int id) {
    ...
    private static int shape(Block block, World world, int x, int id) {
        ...
        return scaled(box.minX - x) == 0 && scaled(box.minY - 65D) == 0
                ...
                && scaled(box.maxY - 65D) == 160 && scaled(box.maxZ - 8D) == 160
                ? 1 : -1;
    }
    ...
    private static int directTickStable(World world, int x, int id) {
        int beforeId = world.getBlockId(x, 65, 8);
        int beforeMeta = world.getBlockMetadata(x, 65, 8);
        int beforeEntities = world.loadedEntityList.size();
        int beforeScheduled = scheduledCount(world);
        Block.blocksList[id].updateTick(world, x, 65, 8, new Random(17320110855L + id));
        return world.getBlockId(x, 65, 8) == beforeId
                && world.getBlockMetadata(x, 65, 8) == beforeMeta
                && world.loadedEntityList.size() == beforeEntities
    ...
    private static int neighbor(World world, int x, int id) {
        ...
        if (id == 55) {
            ...
            require(world.setBlockWithNotify(x, 64, 8, 0), "wire support removal failed");
            if (world.getBlockId(x, 65, 8) != 0)
                Block.blocksList[id].onNeighborBlockChange(world, x, 65, 8, 0);
            return world.getBlockId(x, 65, 8) == 0
                    && world.loadedEntityList.size() == before + 1 ? 1 : 0;
        }
        if (id == 84) {
            int beforeId = world.getBlockId(x, 65, 8);
            int beforeMeta = world.getBlockMetadata(x, 65, 8);
            int beforeEntities = world.loadedEntityList.size();
            Block.blocksList[id].onNeighborBlockChange(world, x, 65, 8, 1);
            return world.getBlockId(x, 65, 8) == beforeId
                    && world.getBlockMetadata(x, 65, 8) == beforeMeta
                    && world.loadedEntityList.size() == beforeEntities ? 1 : 0;
        }

        preparePoweredLever(world, x);
        if (id == 23) {
            int before = scheduledCount(world);
            Block.blocksList[id].onNeighborBlockChange(world, x, 65, 8, 69);
            return scheduledCount(world) == before + 1 ? 1 : 0;
        }
        if (id == 25) {
            Object tile = world.getBlockTileEntity(x, 65, 8);
        ...
        Block.blocksList[id].onNeighborBlockChange(world, x, 65, 8, 69);
        return world.getBlockId(x, 65, 8) == 0
                && containsPrimedTnt(world, before) ? 1 : 0;
    }

    private static void preparePoweredLever(World world, int x) {
        ensureBlock(world, x + 1, 64, 8, 1, "lever support failed");
        require(world.setBlockAndMetadataWithNotify(x + 1, 65, 8, 69, 1),
                "unpowered lever placement failed");
        rawMetadata(world, x + 1, 65, 8, 9);
        require(world.getBlockMetadata(x + 1, 65, 8) == 9,
                "power source transition was not committed");
        require(world.isBlockGettingPowered(x, 65, 8),
                "consumer was not directly powered after source transition");
        require(world.isBlockIndirectlyGettingPowered(x, 65, 8),
                "consumer was not powered after source transition");
    }
    ...
    private static void ensureBlock(World world, int x, int y, int z, int id, String message) {
        if (world.getBlockId(x, y, z) != id)
            require(world.setBlockWithNotify(x, y, z, id), message);
    }

    private static int scheduledCount(World world) {
        try {
            TreeSet<?> ticks = null;
    ...
    private static boolean containsPrimedTnt(World world, int first) {
        for (int index = first; index < world.loadedEntityList.size(); index++)
            if (world.loadedEntityList.get(index) instanceof EntityTNTPrimed)
                return true;
        return false;
    }
