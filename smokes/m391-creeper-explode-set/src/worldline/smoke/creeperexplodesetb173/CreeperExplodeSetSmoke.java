...
public final class CreeperExplodeSetSmoke{
 ...
 public static void main(String[]a)throws Exception{
  ...
  BlockPosition top,dirt,wool,dirtProbe,woolProbe,spawner;int column;RemoteExplosion explosion;PlayerPose pose;
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1,2,3},new int[]{1,3,35,52},new int[]{32,48,48,1},new int[]{0,0,0,0});actor.connect();pose=actor.synchronizePose();require(actor.awaitInventory().occupiedSlots()==4,"creeper-explode-set inventory drift");
   ...
   pad(actor,top);dirtProbe=fixture(top,3);woolProbe=fixture(top,35);
   require(worldline.test.WorldlineSmokeAwait.observe(actor,2).blockAt(dirtProbe.x(),dirtProbe.y(),dirtProbe.z()).equals(new BlockState(3,0))&&worldline.test.WorldlineSmokeAwait.observe(actor,1).blockAt(woolProbe.x(),woolProbe.y(),woolProbe.z()).equals(new BlockState(35,0)),"checkerboard wool/dirt pad drift");
  ...
  try{server.boot();actor.connect();pose=stand(actor,actor.synchronizePose(),top);require(actor.awaitInventory().occupiedSlots()>=1&&Math.abs(pose.y()-(top.y()+1.0D))<2D,"creeper-explode-set reload pose drift");server.setTime(14000L);
   ...
   worldline.test.WorldlineSmokeAwait.observe(actor,40);explosion=actor.awaitExplosion();require(explosion.strength()==3F&&explosion.destroyed().size()>1&&Math.abs(explosion.x()-(top.x()+0.5D))<6D&&Math.abs(explosion.z()-(top.z()+0.5D))<6D,"Packet60 creeper center/strength drift: "+explosion.x()+":"+explosion.y()+":"+explosion.z()+":"+explosion.strength());
   RemoteWorldView after=worldline.test.WorldlineSmokeAwait.observe(actor,2);dirt=destroyed(explosion,top,3);wool=destroyed(explosion,top,35);require(dirt!=null&&wool!=null&&after.blockAt(dirt.x(),dirt.y(),dirt.z()).equals(new BlockState(0,0))&&after.blockAt(wool.x(),wool.y(),wool.z()).equals(new BlockState(0,0)),"creeper Packet60 wool/dirt destruction drift");
   ...
   require(persisted.blockAt(local(dirt.x(),cx),dirt.y(),local(dirt.z(),cz)).equals(new BlockState(0,0))&&persisted.blockAt(local(wool.x(),cx),wool.y(),local(wool.z(),cz)).equals(new BlockState(0,0)),"persisted creeper crater drift");
   String evidence="column="+column+",support="+top.x()+":"+top.y()+":"+top.z()+":1:0,pad=7x7-checkerboard-3+35,dirt=3:0->0:0,wool=35:0->0:0,spawner="+spawner.x()+":"+spawner.y()+":"+spawner.z()+":52:0,mob=type50,packet60=strength3,destroyed=multiple+wool+dirt,night=14000,persisted=air,clients=3,disconnect=clean";
   String trace="v2|server=official-b1.7.3|seed="+seed+"|fixture=raised-7x7-checkerboard-dirt+wool-pad+creeper-spawner52|cause=nbt-entityid-creeper+time-14000+proximity-fuse|wire=packet24-type50+packet60-strength3|oracle=creeper-explode-wool+dirt-set-not-tnt4-not-bed5|"+evidence;
 ...
 private static void pad(B173WireClient a,BlockPosition t)throws Exception{for(int r=1;r<=3;r++){for(int z=-r+1;z<r;z++){cell(a,new BlockPosition(t.x()-r+1,t.y(),t.z()+z),BlockFace.WEST);cell(a,new BlockPosition(t.x()+r-1,t.y(),t.z()+z),BlockFace.EAST);}for(int x=-r+1;x<r;x++){cell(a,new BlockPosition(t.x()+x,t.y(),t.z()-r+1),BlockFace.NORTH);cell(a,new BlockPosition(t.x()+x,t.y(),t.z()+r-1),BlockFace.SOUTH);}cell(a,new BlockPosition(t.x()-r,t.y(),t.z()-r+1),BlockFace.NORTH);cell(a,new BlockPosition(t.x()-r,t.y(),t.z()+r-1),BlockFace.SOUTH);cell(a,new BlockPosition(t.x()+r,t.y(),t.z()-r+1),BlockFace.NORTH);cell(a,new BlockPosition(t.x()+r,t.y(),t.z()+r-1),BlockFace.SOUTH);}}
 private static void cell(B173WireClient a,BlockPosition s,BlockFace f)throws Exception{BlockPosition p=f.adjacent(s);int id=material(p);a.selectHeldSlot(id==3?1:2);place(a,s,f,id);}
 ...
 private static BlockPosition fixture(BlockPosition t,int id){for(int dx=-3;dx<=3;dx++)for(int dz=-3;dz<=3;dz++)if((dx!=0||dz!=0)&&material(new BlockPosition(t.x()+dx,t.y(),t.z()+dz))==id)return new BlockPosition(t.x()+dx,t.y(),t.z()+dz);throw new IllegalStateException("missing pad material "+id);}
 private static BlockPosition destroyed(RemoteExplosion e,BlockPosition t,int id){for(BlockPosition p:e.destroyed())if(p.y()==t.y()&&Math.abs(p.x()-t.x())<=3&&Math.abs(p.z()-t.z())<=3&&(p.x()!=t.x()||p.z()!=t.z())&&material(p)==id)return p;return null;}
 private static int material(BlockPosition p){return ((p.x()+p.z())&1)==0?35:3;}
