...
public final class CreeperExplodeSetSmoke{
 ...
 public static void main(String[]a)throws Exception{
  ...
  try{server.boot();actor.connect();pose=stand(actor,actor.synchronizePose(),top);require(actor.awaitInventory().occupiedSlots()>=1&&Math.abs(pose.y()-(top.y()+1.0D))<2D,"creeper-explode-set reload pose drift");server.setTime(14000L);
   ...
   require(crater(persisted,top,cx,cz,true)>0&&crater(persisted,top,cx,cz,false)>0,"persisted creeper crater drift");
   String evidence="column="+column+",support="+top.x()+":"+top.y()+":"+top.z()+":1:0,pad="+dirt.x()+":"+dirt.y()+":"+dirt.z()+"+"+wool.x()+":"+wool.y()+":"+wool.z()+",dirt=3:0->0:0,wool=35:0->0:0,spawner="+spawner.x()+":"+spawner.y()+":"+spawner.z()+":52:0,mob=type50,packet60=strength3,destroyed=multiple+wool+dirt,night=14000,persisted=air,clients=3,disconnect=clean";
   String trace="v1|server=official-b1.7.3|seed="+seed+"|fixture=raised-7x7-dirt+wool-pad+creeper-spawner52|cause=nbt-entityid-creeper+time-14000+proximity-fuse|wire=packet24-type50+packet60-strength3|oracle=creeper-explode-wool+dirt-set-not-tnt4-not-bed5|"+evidence;
 ...
 private static int hits(RemoteExplosion e,BlockPosition t,boolean west){int n=0;for(BlockPosition p:e.destroyed()){int dx=p.x()-t.x(),dz=p.z()-t.z();if(Math.abs(dx)>4||Math.abs(p.y()-t.y())>3||Math.abs(dz)>4)continue;if(west?dx<=0:dx>0)n++;}return n;}
