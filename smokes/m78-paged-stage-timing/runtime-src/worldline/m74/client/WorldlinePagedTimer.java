...
public final class WorldlinePagedTimer {
  ...
  static final int[] queued = new int[CAP], pageCalls = new int[CAP], direct = new int[CAP],
                     rebuilds = new int[CAP], cached = new int[CAP];
  ...
  static {
    for (int i = 0; i < CAP; i += 256) {
      ...
      queued[i] = pageCalls[i] = direct[i] = rebuilds[i] = cached[i] = 0;
    }
  }
  ...
  static void arm() {
    if (armed)
      throw new IllegalStateException("duplicate M78 arm");
    armed = true;
  }
  public static void head() {
    if (!armed || WorldlinePagedBridge.sealed() || !WorldlinePagedBridge.running())
      return;
    if (capture)
      throw new IllegalStateException("M78 HEAD without TAIL");
  ...
  public static void tail() {
    ...
    if (rendererStart != 0 || queueStart != 0 || flushStart != 0)
      throw new IllegalStateException("unterminated M78 span");
    int i = WorldlinePagedBridge.count();
    if (i >= CAP || rendererCount > 0xffff || queueCount > 0xffff || flushCount > 0xffff)
      throw new IllegalStateException("M78 counter overflow");
  ...
  private static long begin(long old, String label) {
    if (old != 0)
      throw new IllegalStateException("recursive M78 " + label);
    return System.nanoTime();
  }
  private static long end(long start, String label) {
    if (start == 0)
      throw new IllegalStateException("unstarted M78 " + label);
    long v = System.nanoTime() - start;
    if (v < 0)
      throw new IllegalStateException("negative M78 " + label);
    return v;
  }
