...
public final class WorldlineRecoveryServer {
  ...
  public static synchronized void change(PlayerEntity player, int[] values) {
    ...
    if (!(player instanceof ServerPlayerEntity server) || values == null || values.length != 6
        || values[3] != expected || expected <= 0 || values[4] != step / 2 + 1
        || values[5] != step % 2 + 1)
      throw new IllegalStateException("invalid M86 request");
    if (values[5] == 1)
  ...
  private static void remove(PlayerEntity player, int[] values) {
    ...
    if (player.world.getBlockId(x, y, z) != WorldlineCensusMod.block.id
        ...
        || be.nonce() != root * 100 + 1)
      throw new IllegalStateException("M86 remove target drift");
    ...
    System.out.println("[WorldlineRecovery] removed generation=" + values[4] + " x=" + x + " y=" + y
        + " z=" + z + " nonce=" + root);
  }
  private static void restore(PlayerEntity player, ServerPlayerEntity server, int[] values) {
    if (values[0] != x || values[1] != y || values[2] != z || values[3] != root
      ...
      throw new IllegalStateException("M86 restore target drift");
    ...
    state.ints = new int[] {x, y, z, nonce, values[4]};
    server.networkHandler.sendPacket(state);
    System.out.println("[WorldlineRecovery] restored generation=" + values[4] + " x=" + x
        + " y=" + y + " z=" + z + " nonce=" + root);
  }
}
