...
public final class WorldlineRecoveryEvent {
  ...
  public static void head(Minecraft client) {
    ...
    if (step < 4 && readyToSend()) {
      if (AFTER != 300 || GAP != 30
        ...
        throw new IllegalStateException("M86 runtime drift");
      int generation = step / 2 + 1, operation = step % 2 + 1;
      MessagePacket packet = new MessagePacket(WorldlineRecoveryMod.CHANGE);
      packet.ints = new int[] {x, y, z, root, generation, operation};
      client.getNetworkHandler().sendPacket(packet);
      requests[step] = WorldlinePagedBridge.count();
      step++;
    }
    int generation = (step + 1) / 2;
    if (step % 2 == 1)
      air[generation - 1] |= client.world.getBlockId(x, y, z) == 0;
    else if (step > 0) {
      WorldlineRecoveryState.apply(client.world);
      restored[generation - 1] = client.world.getBlockId(x, y, z) == WorldlineCensusMod.block.id
          && client.world.getBlockEntity(x, y, z) instanceof WorldlineCensusBlockEntity be
          && be.nonce() == root * 100 + 1;
    }
  }
  ...
  public static void tail() {
    ...
    if (events[at] >= 0)
      return;
    int generation = at / 2 + 1, operation = at % 2 + 1, x = WorldlineCensusSync.x(),
        y = WorldlineCensusSync.y(), z = WorldlineCensusSync.z(),
        root = WorldlineCensusProbe.nonce(), queued = Aero_BECellRenderer.queuedLastFrame();
    boolean accepted = operation == 1 ? WorldlineRecoveryState.removed(generation, x, y, z, root)
            && air[generation - 1] && queued == 15
                                      : WorldlineRecoveryState.restored(generation, x, y, z, root)
            && restored[generation - 1] && queued == 16;
    if (!accepted)
      return;
