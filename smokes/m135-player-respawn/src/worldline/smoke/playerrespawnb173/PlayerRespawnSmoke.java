...
public final class PlayerRespawnSmoke {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 6) throw new IllegalArgumentException("usage: PlayerRespawnSmoke server.jar workspace port seed username fixtureTicks");
        ...
        Duration timeout = Duration.ofSeconds(90); B173DedicatedServer server = new B173DedicatedServer(jar, workspace, port, seed, timeout, 3, true);
        RespawnSession actor = new B173WireClient("127.0.0.1", port, username, timeout); RemoteRespawn respawn; PlayerPose afterPose; RemoteChunkSnapshot chunk;
        try {
        ...
        String evidence = "health=20->nonpositive->20,dimension=0,packet9=09:00,chunk=loaded,sky=positive,inventory=empty,persisted=20";
        String trace = "v1|server=official-b1.7.3|seed=" + seed + "|death=seeded-below-world+vanilla-void-damage+packet8-nonpositive|request=packet9-signed-dimension-zero|response=fresh-same-dimension-packet9-epoch+packet8-health20|world=corrected-pose+packet51-overworld|" + evidence + "|disconnect=clean";
        System.out.println("WORLDLINE_M135_RESPAWN=" + evidence); System.out.println("WORLDLINE_M135_TRACE=" + trace); System.out.println("WORLDLINE_M135_SIGNATURE=" + sha(trace));
    }
