package worldline.smoke.dispenserejectb173;
...
/** Loads one cobblestone into an official dispenser and proves a side lever ejects Packet21. */
public final class DispenserEjectSmoke {
  private static final RemoteItemStack COBBLE = new RemoteItemStack(4, 1, 0);
  private DispenserEjectSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: DispenserEjectSmoke server.jar workspace port seed username chunkX chunkZ");
    ...
    try {
      ...
      actor.look(-90F, 0F);
      require(actor.awaitInventory().occupiedSlots() == 4, "dispenser inventory drift");
      ...
      while (
        ...
        require(++column <= 15, "water column exceeded dispenser fixture");
      }
      ...
      require(opened.descriptor().kind() == RemoteWindowKind.DISPENSER
              ...
              && opened.inventory().slot(0).empty() && !opened.inventory().slot(39).empty()
              && opened.inventory().slot(39).item().equals(COBBLE),
          "dispenser open mapping drifted");
      ...
      require(load.takeAction() == 1 && load.storeAction() == 2 && load.stack().equals(COBBLE)
          ...
          "accepted dispenser load drifted");
      actor.closeWindow();
      actor.selectHeldSlot(1);
      ...
      require(worldline.test.WorldlineSmokeAwait.observe(actor, 5)
          ...
          "powered lever drift");
      drop = actor.awaitDroppedItem(COBBLE);
      require(drop.item().equals(COBBLE) && drop.item().count() == 1,
          "dispenser Packet21 cobblestone absent");
      remain = B173DispenserWindows.open(actor, disp, BlockFace.UP);
      require(remain.inventory().slot(0).empty(), "official ejected stack drift");
      actor.closeWindow();
      actor.close();
      ...
      String evidence = "column=" + column + ",disp=" + disp.x() + ":" + disp.y() + ":" + disp.z()
          ...
          + ":1->9,load=4x1,drop=packet21-4x1,remain=empty,clients=1,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=raised-dispenser23-west+side-lever69|cause=packet15-lever-activate|wire=packet100-trap+packet102-load+packet21-cobble4|oracle=official-dispenser-eject|"
          + evidence;
      System.out.println("WORLDLINE_M153_EJECT=" + evidence);
      System.out.println("WORLDLINE_M153_TRACE=" + trace);
      System.out.println("WORLDLINE_M153_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      server.close();
    }
  ...
  private static BlockPosition foundation(RemoteChunkSnapshot q, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic dispenser foundation");
  }
