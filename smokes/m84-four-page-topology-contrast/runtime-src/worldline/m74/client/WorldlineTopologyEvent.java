...
import worldline.m84.*;

/** Binds one one-page/three-page three-member transition record. */
public final class WorldlineTopologyEvent {
  ...
  public static void head(Minecraft client) {
    ...
    if (!requested && WorldlinePagedBridge.count() >= AFTER) {
      if (AFTER != 300 || !"8".equals(System.getProperty("aero.becell.rebuildsPerFrame")))
        throw new IllegalStateException("M84 runtime drift");
  ...
  public static void tail() {
    if (!requested || !acked || !allAir || eventIndex >= 0
        || Aero_BECellRenderer.queuedLastFrame() != 13)
      return;
  ...
  static boolean valid() {
    return eventIndex >= requestIndex && allAir && eventPages == 3
        && eventRebuilds == expectedRebuilds() && eventDirect == 1 && eventCache == 3;
  }
