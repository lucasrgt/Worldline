...
public final class FallWaterCancelSetSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    require(
        seed == 17320110707L && user.equals("FallWater586") && user.length() <= 16, "fall-water-cancel identity drift");
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(
          workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1}, new int[] {1, 9}, new int[] {64, 8}, new int[] {0, 0});
      actor.connect();
      ...
      require(water(WorldlineSmokeAwait.observe(actor, 5).blockAt(pool.x(), pool.y(), pool.z()).legacyId()),
          "still-water pool drift");
      actor.selectHeldSlot(0);
      ...
      require(drop >= 6 && over(pose, pad) && actor.health() == 20, "water-fall height drift drop=" + drop);
      pose = land(actor, pose, waterY);
      require(water(WorldlineSmokeAwait.observe(actor, 1).blockAt(pool.x(), pool.y(), pool.z()).legacyId())
      ...
      String evidence = "column=" + column + ",support=" + top.x() + ":" + top.y() + ":" + top.z()
          + ":1:0,pool=" + pool.x() + ":" + pool.y() + ":" + pool.z() + ":9:0,drop=" + drop
          + ",water=20->20,packet8=absent,status=0,persisted=true,"
          + "clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed + "|fixture=raised-stone+east-still-water9+drop" + drop
  ...
  private static PlayerPose walkOff(B173WireClient actor, PlayerPose pose, BlockPosition top, BlockPosition pad)
      throws Exception {
    pose = actor.moveAndObserve(top.x() + 0.5D - pose.x(), (top.y() + 1.0D) - pose.y(), top.z() + 0.5D - pose.z(), 20)
               .resulting();
    return actor.moveAndObserve(pad.x() + 0.5D - pose.x(), 0D, pad.z() + 0.5D - pose.z(), 1).resulting();
  }

  private static PlayerPose land(B173WireClient actor, PlayerPose pose, double y) throws Exception {
  ...
  private static boolean over(PlayerPose pose, BlockPosition block) {
    return Math.abs(pose.x() - (block.x() + 0.5D)) < 0.2D && Math.abs(pose.z() - (block.z() + 0.5D)) < 0.2D;
  }
