package worldline.smoke.bedb173;
...
/** Places one official bed, proves the night-time refusal, then occupies and skips on SMP. */
public final class BedSmoke {
  private BedSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: BedSmoke server.jar workspace port seed username chunkX chunkZ");
    ...
    try {
      ...
      require(actor.awaitChat().equals("You can only sleep at night"), "daytime bed chat drift");
      RemoteWorldView day = worldline.test.WorldlineSmokeAwait.observe(actor, 5);
      require(day.blockAt(foot.x(), foot.y(), foot.z()).equals(new BlockState(26, 0))
              && day.blockAt(head.x(), head.y(), head.z()).equals(new BlockState(26, 8)),
          "daytime bed occupy drift");
      ...
      require(sleep.entityId() == actor.state().entityId() && sleep.unused() == 0
          ...
          "Packet17 sleep drift");
      actor.awaitBlock(head, new BlockState(26, 12));
      require(worldline.test.WorldlineSmokeAwait.observe(actor, 120)
                  .blockAt(head.x(), head.y(), head.z())
                  .equals(new BlockState(26, 8)),
          "SMP bed skip did not clear occupied head");
      actor.close();
      ...
      String evidence = "column=" + column + ",foot=" + foot.x() + ":" + foot.y() + ":" + foot.z()
          ...
          + ":26:8,day=noSleep,occupy=" + head.x() + ":" + head.y() + ":" + head.z()
          + ":26:12,packet17=head,packet70=-1,skip=true,persisted=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=raised-stone+item355-block26|cause=packet15-empty-hand-bed|wire=packet3-noSleep+packet17-sleep+packet53-occupied|oracle=night-occupy+smp-skip|"
          + evidence;
      System.out.println("WORLDLINE_M158_BED=" + evidence);
      System.out.println("WORLDLINE_M158_TRACE=" + trace);
      System.out.println("WORLDLINE_M158_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
