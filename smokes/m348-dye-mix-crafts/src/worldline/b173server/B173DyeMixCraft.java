...
/** Eighteen accepted window-0 clicks that mix secondary dyes in the personal 2x2 grid. */
public final class B173DyeMixCraft {
  private static final int[] LEFT = {36, 38, 40}, RIGHT = {37, 39, 41};
  private static final int[] LEFT_DMG = {1, 1, 2}, RIGHT_DMG = {11, 4, 15},
                             RESULT_DMG = {14, 5, 10};
  private static final int[] CLICK_SLOTS = {
      36, 1, 37, 2, 0, 36, 38, 1, 39, 2, 0, 38, 40, 1, 41, 2, 0, 40};
  ...
  private static final int[] CLICK_DAMAGE = {
      1, 0, 11, 0, 14, 0, 1, 0, 4, 0, 5, 0, 2, 0, 15, 0, 10, 0};

  private B173DyeMixCraft() {
  }
  ...
  public static void verify() throws Exception {
    ...
    if (input.available() != 0 || bytes.size() != 207)
      throw new AssertionError("dye-mix Packet102 sequence size drifted");
  }
  ...
  public static void apply(B173WireClient actor) {
    try {
      ...
      if (inbound.windowActive() || !inbound.cursorObserved() || inbound.cursor() != null)
        throw new IllegalStateException("dye-mix craft preflight failed");
      int action = 1;
      ...
      if (action != 19 || inbound.cursor() != null)
        throw new IllegalStateException("dye-mix action drift");
      RemoteInventoryView view = inbound.inventory();
      if (view.occupiedSlots() != 3 || !same(view, 36, 351, 2, 14) || !same(view, 38, 351, 2, 5)
          || !same(view, 40, 351, 2, 10) || !emptyCraft(view) || !view.slot(37).empty()
          || !view.slot(39).empty() || !view.slot(41).empty())
        throw new IllegalStateException("dye-mix results drifted");
    } catch (IOException error) {
      throw new IllegalStateException("dye-mix craft failed", error);
    }
  }

  private static int mix(B173PlayChannel channel, B173PlayInbound inbound, int leftSlot,
      int rightSlot, int leftDamage, int rightDamage, int resultDamage, int action)
      throws IOException {
    ...
    if (leftSlot < 9 || rightSlot < 9 || leftSlot > 44 || rightSlot > 44 || inbound.cursor() != null
      ...
      throw new IllegalStateException("dye-mix ingredients drifted");
    ...
    step(channel, inbound, rightSlot, right, replace(inbound.inventory(), rightSlot, null), right,
        action++);
    ...
    step(channel, inbound, leftSlot, null, replace(inbound.inventory(), leftSlot, result), null,
        action++);
    if (!same(inbound.inventory(), leftSlot, result) || !emptyCraft(inbound.inventory())
      ...
      throw new IllegalStateException("dye-mix store drifted");
    return action;
  }
  ...
  private static void packet(DataInputStream input, int slot, int action, int item, int count,
    ...
    if (input.readUnsignedByte() != 102 || input.readUnsignedByte() != 0
      ...
      throw new AssertionError("dye-mix Packet102 fields drifted");
    if (item >= 0 && (input.readUnsignedByte() != count || input.readShort() != damage))
      throw new AssertionError("dye-mix Packet102 stack drifted");
  }
