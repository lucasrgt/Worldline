...
public final class DispenserQcSetArm{
 ...
 final BlockPosition support,disp,qc,lever;final int leverOff,leverOn;
 private DispenserQcSetArm(BlockPosition s,BlockPosition d,BlockPosition q,BlockPosition l,int off,int on){support=s;disp=d;qc=q;lever=l;leverOff=off;leverOn=on;}
 static BlockPosition raise(B173WireClient a,RemoteChunkSnapshot initial,int cx,int cz,int[] column)throws Exception{
  ...
  while(water(at(initial,BlockFace.UP.adjacent(top),cx,cz).legacyId())){top=place(a,top,BlockFace.UP,1);a.moveAndObserve(0D,1D,0D,1);require(++column[0]<=15,"water column exceeded dispenser-qc fixture");}
 ...
 void remain(B173WireClient a)throws Exception{
  RemoteContainerWindow remain=B173DispenserWindows.open(a,disp,BlockFace.WEST);require(remain.inventory().slot(0).empty(),"official qc ejected stack drift");a.closeWindow();
 }
 static BlockPosition place(B173WireClient a,BlockPosition support,BlockFace face,int id)throws Exception{BlockPosition target=face.adjacent(support);a.placeHeldBlock(support,face);a.awaitBlock(target,id==23?new BlockState(23,4):new BlockState(id,0));return target;}
 static BlockPosition foundation(RemoteChunkSnapshot q,int cx,int cz){for(int x=4;x<=11;x++)for(int z=4;z<=11;z++)for(int y=126;y>=1;y--)if(q.blockAt(x,y,z).legacyId()==3&&water(q.blockAt(x,y+1,z).legacyId()))return new BlockPosition(cx*16+x,y,cz*16+z);throw new IllegalStateException("no deterministic dispenser-qc foundation");}
