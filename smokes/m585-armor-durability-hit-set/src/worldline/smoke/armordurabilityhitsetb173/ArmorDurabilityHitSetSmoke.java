...
public final class ArmorDurabilityHitSetSmoke {
    ...
    public static void main(String[] a) throws Exception {
        ...
        require(seed == 17320110707L && user.equals("ArmorDur585") && user.length() <= 16,
        ...
        B173PlayerSeed.writeInventory(workspace, user, pad.top.x() + 0.5D, pad.top.y() + 1.0D,
                pad.top.z() + 0.5D, new int[] {0, 1, 102}, new int[] {322, 320, 307},
                new int[] {8, 8, 1}, new int[] {0, 0, 0});
        ...
        try {
            ...
            require(zombie.legacyType() == 54 && zombie.legacyType() != 90,
            ...
            RemoteInventoryView worn = WorldlineSmokeAwait.awaitEntity(actor, actor::inventory,
                    view -> damaged(view, before.damage()), "iron chestplate durability", 40);
            RemoteItemStack after = worn.slot(B173ArmorDurabilityAccess.SLOT).item();
            require(after.legacyId() == 307 && after.damage() > before.damage(),
                    "worn iron chestplate did not lose durability");
            actor.close();
            awaitPlayers(server, 0);
            server.save();
            String evidence = "column=" + pad.column + ",platform=7x7-48grass,spawner="
                    + B173ArmorDurabilityAccess.cell(pad.spawner)
                    + ",entityid=Zombie,mob=type54,night=14000,armor=307,slot=6,before="
                    + before.damage() + ",after=" + after.damage() + ",hit=20->" + hit.healthAfter()
                    + ":" + hit.damage()
            ...
            String trace = "v1|server=official-b1.7.3|seed=" + seed
                    ...
                    + "|oracle=zombie-melee-armor-durability-hit-not-reduction-not-pvp-not-craft-not-equip-only|"
                    + evidence;
