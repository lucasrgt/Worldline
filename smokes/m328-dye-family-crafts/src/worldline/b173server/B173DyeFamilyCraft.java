...
public final class B173DyeFamilyCraft {
    ...
    private static final int[] CLICK_SLOTS = {36, 1, 0, 36, 37, 1, 0, 37, 38, 1, 0, 38, 39, 1, 40, 2, 0, 39};
    private static final int[] CLICK_ITEMS = {352, -1, 351, -1, 38, -1, 351, -1, 37, -1, 351, -1, 351, -1, 351, -1, 351, -1};
    private static final int[] CLICK_COUNTS = {1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 2, 0, 1, 0, 1, 0, 2, 0};
    private static final int[] CLICK_DAMAGE = {0, 0, 15, 0, 0, 0, 1, 0, 0, 0, 11, 0, 0, 0, 15, 0, 8, 0};
    ...
    public static void apply(B173WireClient actor) {
        try {
            ...
            for (int index = 0; index < SOURCE.length; index++)
                action = mill(channel, inbound, SOURCE[index],
                        new RemoteItemStack(INPUT_IDS[index], 1, 0),
                        new RemoteItemStack(351, RESULT_COUNTS[index], RESULT_DAMAGES[index]), action);
            action = mix(channel, inbound, action);
            if (action != 19 || inbound.cursor() != null) throw new IllegalStateException("dye-family action drift");
    ...
    private static int mill(B173PlayChannel channel, B173PlayInbound inbound, int slot, RemoteItemStack input,
            RemoteItemStack result, int action) throws IOException {
        RemoteInventoryView before = inbound.inventory();
        if (slot < 9 || slot > 44 || inbound.cursor() != null || !same(before, slot, input) || !emptyCraft(before)
                || result.legacyId() != 351)
        ...
        if (!same(inbound.inventory(), slot, result) || !emptyCraft(inbound.inventory()) || inbound.cursor() != null)
            throw new IllegalStateException("dye-family store drifted");
        return action;
    }

    private static int mix(B173PlayChannel channel, B173PlayInbound inbound, int action) throws IOException {
        ...
        RemoteInventoryView before = inbound.inventory();
        if (inbound.cursor() != null || !same(before, 39, ink) || !same(before, 40, meal) || !emptyCraft(before)
                || !same(before, 36, 351, 3, 15) || !same(before, 37, 351, 2, 1) || !same(before, 38, 351, 2, 11))
        ...
        if (!same(inbound.inventory(), 39, gray) || !emptyCraft(inbound.inventory()) || inbound.cursor() != null
                || !inbound.inventory().slot(40).empty())
