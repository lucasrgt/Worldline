...
public final class B173RemainingDyeRest {
  private static final int[] CLICK_SLOTS = {
      36, 1, 37, 2, 38, 3, 0, 36, 39, 1, 40, 2, 0, 39, 41, 1, 42, 2, 0, 41};
  private static final int[] CLICK_ITEMS = {
      351, -1, 351, -1, 351, -1, 351, -1, 351, -1, 351, -1, 351, -1, 351, -1, 351, -1, 351, -1};
  private static final int[] CLICK_COUNTS = {
      1, 0, 1, 0, 1, 0, 3, 0, 1, 0, 1, 0, 2, 0, 1, 0, 1, 0, 2, 0};
  private static final int[] CLICK_DAMAGE = {
      0, 0, 15, 0, 15, 0, 7, 0, 8, 0, 15, 0, 7, 0, 5, 0, 9, 0, 13, 0};
  ...
  private static int mix(B173PlayChannel channel, B173PlayInbound inbound, int[] slots,
      int[] damages, int resultCount, int resultDamage, int action) throws IOException {
    ...
    for (int index = 0; index < slots.length; index++)
      if (slots[index] < 9 || slots[index] > 44
          || !same(before, slots[index], 351, 1, damages[index]))
        throw new IllegalStateException("remaining-dye-rest ingredients drifted");
    for (int index = 0; index < slots.length; index++) {
      RemoteItemStack item = new RemoteItemStack(351, 1, damages[index]);
      step(channel, inbound, slots[index], item, replace(inbound.inventory(), slots[index], null),
          item, action++);
      RemoteInventoryView placed = replace(inbound.inventory(), index + 1, item);
      if (index == slots.length - 1)
        placed = replace(placed, 0, result);
      step(channel, inbound, index + 1, null, placed, null, action++);
    }
    RemoteInventoryView crafted = inbound.inventory();
    for (int slot = 0; slot <= slots.length; slot++)
      crafted = replace(crafted, slot, null);
    step(channel, inbound, 0, result, crafted, result, action++);
    step(channel, inbound, slots[0], null, replace(inbound.inventory(), slots[0], result), null,
        action++);
