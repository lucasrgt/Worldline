...
public final class B173StoneToolCrafts {
    public static final int COBBLE = 4, STICK = 280, SWORD = 272, SHOVEL = 273, PICK = 274, AXE = 275, HOE = 291;
    ...
    private final int windowId; private int action;

    private B173StoneToolCrafts(B173PlayChannel channel) {
        this.channel = channel; this.inbound = channel.inbound();
    ...
    private void recipe(int[] grid, int cobble, int stick, int result, int store) throws IOException {
        int lastStick = -1;
        for (int cell = 0; cell < 9; cell++) if (grid[cell] == 2) lastStick = cell + 1;
        pick(cobble);
        for (int cell = 0; cell < 9; cell++) if (grid[cell] == 1) place(cell + 1, null);
        put(cobble); pick(stick);
        for (int cell = 0; cell < 9; cell++) if (grid[cell] == 2)
            place(cell + 1, cell + 1 == lastStick ? item(result, 1) : null);
        put(stick); take(result); store(store);
    }
    ...
    private void pick(int combined) throws IOException {
        ...
        step(combined, 0, stack, replace(before, combined, null), replace(personal, combined - 1, null), stack, -1, 0);
    }

    private void put(int combined) throws IOException {
        RemoteItemStack cursor = inbound.cursor(); if (cursor == null) return;
        RemoteInventoryView before = window(), personal = inbound.inventory();
        if (!before.slot(combined).empty()) throw new IllegalStateException("stone-tool put target occupied");
        step(combined, 0, null, replace(before, combined, cursor), replace(personal, combined - 1, cursor), null, -1, 0);
    }
    ...
    private void take(int result) throws IOException {
        ...
        RemoteItemStack tool = before.slot(0).item(); RemoteInventoryView after = before;
        for (int slot = 0; slot < 10; slot++) after = replace(after, slot, null);
        step(0, 0, tool, after, inbound.inventory(), tool, STAT + result, 1);
    }

    private void store(int personal) throws IOException {
        RemoteItemStack tool = inbound.cursor(); int combined = personal + 1;
        ...
        step(combined, 0, null, replace(before, combined, tool), replace(owned, personal, tool), null, -1, 0);
    }
    ...
    private void step(int slot, int button, RemoteItemStack predicted, RemoteInventoryView after,
        ...
        RemoteItemStack cursorBefore = inbound.cursor(); int next = action + 1;
        ...
        channel.output.flush(); action = next; inbound.awaitContainerTransaction();
    }
