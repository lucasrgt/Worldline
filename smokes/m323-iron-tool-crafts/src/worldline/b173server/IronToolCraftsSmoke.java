...
public final class IronToolCraftsSmoke {
  ...
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: IronToolCraftsSmoke server.jar workspace port seed username chunkX chunkZ");
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2, 3},
          new int[] {1, 58, 265, 280}, new int[] {32, 1, 11, 9}, new int[] {0, 0, 0, 0});
      actor.connect();
      actor.synchronizePose();
      require(actor.awaitInventory().occupiedSlots() == 4, "iron-tool inventory seed drift");
      ...
      while (
        ...
        require(++column <= 15, "water column exceeded iron-tool fixture");
      }
      ...
      craft(actor, n, 38, 39, 37, new int[] {2, 5}, new int[] {8}, 267);
      craft(actor, n, 38, 39, 40, new int[] {1, 2, 3}, new int[] {5, 8}, 257);
      craft(actor, n, 38, 39, 41, new int[] {1, 2, 4}, new int[] {5, 8}, 258);
      craft(actor, n, 38, 39, 42, new int[] {2}, new int[] {5, 8}, 256);
      craft(actor, n, 38, 39, 43, new int[] {1, 2}, new int[] {5, 8}, 292);
      require(has(actor, 37, 267) && has(actor, 40, 257) && has(actor, 41, 258)
              && has(actor, 42, 256) && has(actor, 43, 292),
          "iron tool results drifted");
      actor.closeWindow();
      actor.close();
      awaitPlayers(server, 0);
      server.save();
      require(server.player(user).inventoryItems() == 6, "crafted iron tool persistence drift");
      String evidence = "iron=267+257+258+256+292,column=" + column + ",support=" + top.x() + ":"
          + top.y() + ":" + top.z() + ":1:0,workbench=" + bench.x() + ":" + bench.y() + ":"
          + bench.z() + ":58:0,actions=" + n[0] + ",persisted=true,clients=1,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=workbench58+iron265x11+stick280x9|cause=packet102-workbench-matrix+result-take|wire=packet106-accepted+packet200-craft-stat|oracle=iron-family-267-257-258-256-292|"
          + evidence;
      System.out.println("WORLDLINE_M323_CRAFT=" + evidence);
      System.out.println("WORLDLINE_M323_TRACE=" + trace);
      System.out.println("WORLDLINE_M323_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      server.close();
    }
  ...
  private static void takeResult(B173WireClient a, int[] n, int result, int[] mats, int[] stks)
    ...
    require(!w.slot(0).empty() && w.slot(0).item().equals(tool) && in.cursor() == null,
        "modeled iron tool result absent");
  ...
  private static BlockPosition foundation(RemoteChunkSnapshot q, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic iron-tool foundation");
  }
