...
public final class FallingSandEntitySetSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, username, 4.5D, 60D, 4.5D, new int[] {0, 1}, new int[] {1, 12},
      ...
      require(initial.blockAt(local(sand.x(), chunkX), sand.y(), local(sand.z(), chunkZ)).legacyId() == 0,
      ...
      require(before.blockAt(local(support.x(), chunkX), support.y(), local(support.z(), chunkZ))
          ...
          "stable sand support fixture drift");
      require(before.blockAt(local(sand.x(), chunkX), sand.y(), local(sand.z(), chunkZ)).equals(new BlockState(12, 0)),
      ...
      require(opened.equals(new BlockState(0, 0)) && fall.type() == 70 && fall.entityId() != actor.state().entityId(),
      ...
      require(after.blockAt(local(support.x(), chunkX), support.y(), local(support.z(), chunkZ))
          ...
          "fresh landed sand 12:0 drift");
      require(after.blockAt(local(sand.x(), chunkX), sand.y(), local(sand.z(), chunkZ)).equals(new BlockState(0, 0)),
          "fresh cleared upper sand drift");
    } finally {
      actor.close();
      if (reader != null) {
        reader.close();
    ...
    String evidence = "column=" + column + ",lower=" + cell(support) + ":1:0->12:0,upper=" + cell(sand)
        + ":12:0->0:0,entity-type=" + fall.type() + ",packet23=" + fall.type()
        + ",persisted=true,clients=2,disconnect=clean";
    String trace = "v1|server=official-b1.7.3|seed=" + seed + "|fixture=stone-column+supported-sand12|settle="
