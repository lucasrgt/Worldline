...
public final class PeerArmorSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    ArmorEquipmentSession actor = client(port, actorName, timeout),
                          observer = client(port, observerName, timeout);
    try {
      B173ArmorPacketFixture.verify();
      first.boot();
      ...
      for (RemoteArmorSlot slot : RemoteArmorSlot.values()) {
        ...
        int source = find(actor.inventory(), stack);
        require(source == 36 + slot.ordinal(), "leather source drifted");
        ...
        require(equipped.takeAction() == action && equipped.placeAction() == action + 1
                && equipped.after().slot(slot.containerSlot()).item().equals(stack),
            "armor equip drifted");
        RemoteArmorPiece peer =
            observer.awaitPeerArmor(new RemoteArmorPiece(actorName, slot, slot.leatherItemId(), 0));
        require(
            peer.slot() == slot && peer.legacyId() == slot.leatherItemId(), "peer armor drifted");
        action += 2;
      }
      ...
      first.save();
      persisted = first.player(actorName);
      require(persisted.inventoryItems() == 4, "leather armor persistence count drifted");
    } finally {
      actor.close();
      observer.close();
      first.close();
    ...
    ArmorEquipmentSession restored = client(port, actorName, timeout),
    ...
    try {
      ...
      for (RemoteArmorSlot slot : RemoteArmorSlot.values())
        witness.awaitPeerArmor(new RemoteArmorPiece(actorName, slot, slot.leatherItemId(), 0));
      restored.close();
      witness.close();
      awaitPlayers(second, 0);
      second.save();
      require(
          second.player(actorName).inventoryItems() == 4, "restored leather armor count drifted");
    } finally {
      restored.close();
      witness.close();
      second.close();
    ...
    System.out.println(
        "WORLDLINE_M65_API=leather-armor,personal-transactions,peer-packet5,restart");
    System.out.println(
        "WORLDLINE_M65_ARMOR=actions=1..8;window=5:298,6:299,7:300,8:301;packet5=4:298,3:299,2:300,1:301;items="
  ...
  private static void acquire(ArmorEquipmentSession client, String username, int item) {
    ...
    client.sendChat("/give " + username + " " + item + " 1");
  ...
  private static void requireArmor(RemoteInventoryView view) {
    for (RemoteArmorSlot slot : RemoteArmorSlot.values())
      require(!view.slot(slot.containerSlot()).empty()
              && view.slot(slot.containerSlot())
                  .item()
                  .equals(new RemoteItemStack(slot.leatherItemId(), 1, 0)),
          "leather armor window drifted");
  }
