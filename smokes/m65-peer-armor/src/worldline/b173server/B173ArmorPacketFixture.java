...
public final class B173ArmorPacketFixture {
    ...
    public static void verify() throws Exception {
        ...
        int source = 36, action = 1; for (RemoteArmorSlot slot : RemoteArmorSlot.values()) {
            B173ContainerPacket.write(output, 0, source++, 0, action++,
                    new RemoteItemStack(slot.leatherItemId(), 1, 0));
            B173ContainerPacket.write(output, 0, slot.containerSlot(), 0, action++, null); }
        DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes.toByteArray()));
        source = 36; action = 1; for (RemoteArmorSlot slot : RemoteArmorSlot.values()) {
            packet(input, source++, action++, slot.leatherItemId()); packet(input, slot.containerSlot(), action++, -1); }
    ...
    private static void packet(DataInputStream input, int slot, int action, int item) throws Exception {
        if (input.readUnsignedByte() != 102 || input.readUnsignedByte() != 0 || input.readShort() != slot
                ...
                || input.readShort() != item) throw new AssertionError("armor Packet102 fields drifted");
