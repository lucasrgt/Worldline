...
public final class PersonalSlotSwapSmoke {
  ...
  public static void main(String[] a) throws Exception {
    ...
    RemoteItemStack stone = item(1), dirt = item(3);
    B173DedicatedServer server =
        new B173DedicatedServer(jar, workspace, port, seed, timeout, 3, true);
    B173WireClient actor = client(port, user, timeout), reader = null;
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 80D, 4.5D, new int[] {0, 1},
          new int[] {1, 3}, new int[] {1, 1}, new int[] {0, 0});
      actor.connect();
      actor.synchronizePose();
      RemoteInventoryView initial = actor.awaitInventory();
      require(stack(initial, 36).equals(stone) && stack(initial, 37).equals(dirt),
          "seed slots drifted");
      ...
      require(first.actionId() == 1 && first.after().slot(36).empty()
              && first.cursorAfter().equals(stone),
          "take transition drifted");
      require(second.actionId() == 2 && stack(second.after(), 37).equals(stone)
              && second.cursorAfter().equals(dirt),
          "occupied swap transition drifted");
      require(third.actionId() == 3 && stack(third.after(), 36).equals(dirt)
              && stack(third.after(), 37).equals(stone) && third.cursorAfterEmpty(),
          "swap completion drifted");
      ...
      require(rejected.actionId() == 4 && rejected.stalePredictionEmpty()
              ...
              && rejected.cursorAfter().equals(dirt),
          "stale rejection recovery drifted");
      ...
      require(restored.actionId() == 5 && stack(restored.after(), 36).equals(dirt)
              && stack(restored.after(), 37).equals(stone) && restored.cursorAfterEmpty(),
          "restore drifted");
      ...
      require(stack(persisted, 36).equals(dirt) && stack(persisted, 37).equals(stone)
              && persisted.occupiedSlots() == 2,
          "relogin swap persistence drifted");
      reader.close();
      reader = null;
      awaitPlayers(server, 0);
      server.save();
    ...
    String trace = "v1|server=official-b1.7.3|seed=" + seed + "|window=0|button=left"
        + "|positive=occupied-slot-exchange|negative=stale-prediction-authoritative-recovery|"
        + signal;
  ...
  private static RemoteItemStack stack(RemoteInventoryView view, int slot) {
    require(!view.slot(slot).empty(), "slot " + slot + " is empty");
