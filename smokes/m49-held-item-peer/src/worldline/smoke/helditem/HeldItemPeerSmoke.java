...
public final class HeldItemPeerSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      ...
      inventory = actor.awaitInventory();
      require(inventory.occupiedSlots() == 2
          ...
          "hotbar seed drifted: occupied=" + inventory.occupiedSlots()
              + ",slot36=" + slot(inventory, 36) + ",slot37=" + slot(inventory, 37));
      observer.connect();
      observer.synchronizePose();
      requirePlayers(server.players(), actorName, observerName);
      actor.selectHeldSlot(1);
      RemoteHeldItem expected = new RemoteHeldItem(actorName, 3, 0);
      held = observer.awaitPeerHeldItem(expected);
      require(held.equals(expected), "peer held item drifted");
      actor.close();
      observer.close();
    ...
    System.out.println(
        "WORLDLINE_M49_API=held-slot,packet16,named-peer,packet5,server-authoritative");
