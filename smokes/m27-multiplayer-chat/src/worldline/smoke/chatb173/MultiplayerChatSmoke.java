...
public final class MultiplayerChatSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        ChatMultiplayerSession sender = new B173WireClient(
                "127.0.0.1", port, senderName, timeout);
        ChatMultiplayerSession receiver = new B173WireClient(
                "127.0.0.1", port, receiverName, timeout);
        String broadcast;
        try {
            server.boot(); sender.connect(); sender.synchronizePose();
            receiver.connect(); receiver.synchronizePose();
            requirePlayers(server.players(), senderName, receiverName);
            sender.sendChat(message); broadcast = receiver.awaitChat();
            require(broadcast.equals("<" + senderName + "> " + message),
                    "native chat broadcast drifted: " + broadcast);
        } finally { sender.close(); receiver.close(); server.close(); }
        String trace = "v1|server=official-b1.7.3|clients=" + senderName + "," + receiverName
                + "|presence=2|sender=" + senderName + "|message=" + message + "|receiver="
                + receiverName + "|broadcast=" + broadcast + "|disconnect=clean";
        System.out.println("WORLDLINE_M27_API=server,two-sessions,pose,send-chat,inbound-pump,receive-chat");
