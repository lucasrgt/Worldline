...
public final class FixedSeedTerrainSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      server.boot();
      B173PlayerSeed.write(workspace, arguments[4], 8.5D, 120.0D, 8.5D);
      client.connect();
      pose = client.synchronizePose();
      RemoteWorldView world = client.awaitRemoteChunk(chunkX, chunkZ);
      chunk = world.chunkAt(chunkX, chunkZ);
      verify(chunk, chunkX, chunkZ);
    } finally {
      client.close();
      server.close();
    }
    ...
    String trace = "v2|server=official-b1.7.3|seed=" + seed + "|target=absolute-chunk|origin="
        + chunkX + "," + chunkZ + "|blocks=32768|nonair=" + chunk.nonAirBlocks()
        + "|surface=" + surface + "|full-id-plane=diagnostic"
        + "|decode=packet50+packet51-xzy|disconnect=clean";
    System.out.println("WORLDLINE_M111_TERRAIN=chunk=" + chunkX + ":" + chunkZ + ",nonair="
        + chunk.nonAirBlocks() + ",solid=" + solid + ",terrain=" + terrain + ",surface=" + surface);
    System.out.println("WORLDLINE_M111_RAW_DIAGNOSTIC=" + raw + ",nonair=" + chunk.nonAirBlocks());
    System.out.println("WORLDLINE_M111_METADATA_DIAGNOSTIC=" + metadata);
    System.out.println(
        "WORLDLINE_M111_SPAWN_DIAGNOSTIC=" + pose.x() + ":" + pose.y() + ":" + pose.z());
    System.out.println("WORLDLINE_M111_TRACE=" + trace);
    System.out.println(
        "WORLDLINE_M111_SIGNATURE=" + sha256(trace.getBytes(StandardCharsets.UTF_8)));
  }
  ...
  private static void verify(RemoteChunkSnapshot chunk, int chunkX, int chunkZ) {
    ...
    require(chunk.blockCount() == 32768 && chunk.nonAirBlocks() > 0 && chunk.nonAirBlocks() < 32768,
        "implausible terrain census");
    for (int x = 0; x < 16; x++)
      for (int z = 0; z < 16; z++)
  ...
  private static String terrainHash(RemoteChunkSnapshot chunk) throws Exception {
    ...
    for (int x = 0; x < 16; x++)
      for (int z = 0; z < 16; z++)
        for (int y = 0; y < 128; y++)
          digest.update((byte) (solid(chunk.blockAt(x, y, z).legacyId()) ? 1 : 0));
  ...
  private static String rawHash(RemoteChunkSnapshot chunk) throws Exception {
    ...
    for (int x = 0; x < 16; x++)
      for (int z = 0; z < 16; z++)
        for (int y = 0; y < 128; y++)
          digest.update((byte) chunk.blockAt(x, y, z).legacyId());
    return hex(digest.digest());
  }

  private static int solidCount(RemoteChunkSnapshot chunk) {
    int result = 0;
    for (int x = 0; x < 16; x++)
      for (int z = 0; z < 16; z++)
        for (int y = 0; y < 128; y++)
          if (solid(chunk.blockAt(x, y, z).legacyId()))
            result++;
    return result;
  }
  ...
  private static String metadataHash(RemoteChunkSnapshot chunk) throws Exception {
    ...
    for (int x = 0; x < 16; x++)
      for (int z = 0; z < 16; z++)
        for (int y = 0; y < 128; y++)
          digest.update((byte) chunk.blockAt(x, y, z).metadata());
    return hex(digest.digest());
  }
