...
/** Complete pixel comparison for the two generated M16 framebuffers. */
final class FrameDiff {
    ...
    static Result compare(Path left, Path right) throws Exception {
        ...
        long changed = 0; int maximum = 0;
        for (int y = 0; y < first.getHeight(); y++) for (int x = 0; x < first.getWidth(); x++) {
            int a = first.getRGB(x, y), b = second.getRGB(x, y); boolean pixelChanged = false;
            for (int shift : new int[] {0, 8, 16, 24}) {
                int delta = Math.abs((a >> shift & 255) - (b >> shift & 255));
                maximum = Math.max(maximum, delta); pixelChanged |= delta != 0;
            }
    ...
    static final class Result {
        final long changedPixels; final int maxChannelDelta;
        Result(long changedPixels, int maxChannelDelta) {
            this.changedPixels = changedPixels; this.maxChannelDelta = maxChannelDelta;
        }
    }
}
