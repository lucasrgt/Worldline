  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D,
          new int[] {0, 1, 2, 3, 4, 5, 6, 7}, new int[] {1, 61, 4, 4, 4, 263, 5, 327},
      ...
      FurnaceFuelSetSupport.require(inventory.occupiedSlots() == 8
              ...
              && inventory.slot(40).item().equals(cobble) && inventory.slot(41).item().equals(coal)
              && inventory.slot(42).item().equals(planks) && inventory.slot(43).item().equals(lava)
              && !inventory.slot(38).item().equals(new RemoteItemStack(15, 1, 0)),
          "furnace-fuel-set inventory drift");
      FurnaceFuelSetSupport.Raised raised = FurnaceFuelSetSupport.raise(
      ...
      RemoteFurnaceSmelt coalSmelt =
          FurnaceFuelSetClicks.smelt(actor, coalFurnace, 38, 41, coal, 1600);
      RemoteFurnaceSmelt plankSmelt =
          FurnaceFuelSetClicks.smelt(actor, plankFurnace, 39, 42, planks, 300);
      RemoteFurnaceSmelt lavaSmelt =
          FurnaceFuelSetClicks.smelt(actor, lavaFurnace, 40, 43, lava, 20000);
      FurnaceFuelSetSupport.require(coalSmelt.output().equals(stone)
              && plankSmelt.output().equals(stone) && lavaSmelt.output().equals(stone)
              && coalSmelt.maximumBurn() == 1600 && plankSmelt.maximumBurn() == 300
              && lavaSmelt.maximumBurn() == 20000,
          "compound furnace fuels drifted");
      actor.close();
      ...
      String evidence = "column=" + raised.column
          ...
          + ",out=" + FurnaceFuelSetSupport.item(stone) + ",cook=199,clients=1,disconnect=clean";
