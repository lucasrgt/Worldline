...
public final class HeldBlockPlacementSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      ...
      actorAfter = actor.awaitBlock(target, STONE);
      observerAfter = observer.awaitBlock(target, STONE);
      worldline.test.WorldlineSmokeAwait.awaitEntity(actor, actor::inventory,
          view -> view.occupiedSlots() == 0, "placed stack consumption", 5);
      require(actor.inventory().occupiedSlots() == 0 && actor.inventory().slot(36).empty(),
          "placed stack did not leave actor inventory");
    ...
    System.out.println(
        "WORLDLINE_M53_API=held-block,packet15,packet53,two-caches,server-authoritative");
    System.out.println(
        "WORLDLINE_M53_TARGET=" + target + ";support=" + support + ";before=" + beforeState);
    System.out.println(
        "WORLDLINE_M53_WORLD=actor=" + actorAfter.blockAt(target.x(), target.y(), target.z())
  ...
  private static PlayerPose acquire(BlockPlacementMultiplayerSession client, String username) {
    ...
    }, inventory -> inventory.occupiedSlots() >= 1, "placement grant", 100);
