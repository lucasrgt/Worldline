...
public final class BedSpawnSetSmoke {
  ...
  public static void main(String[] args) throws Exception {
    ...
    BedSpawnSupport.require(
        seed == 17320110707L && user.equals("BedSpawn567") && user.length() <= 16,
    ...
    int column;
    RemoteBedUse sleep;
    PlayerPose pose, wake, respawned;
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2, 3},
          new int[] {1, 355, 12, 81}, new int[] {32, 1, 1, 1}, new int[] {0, 0, 0, 0});
      actor.connect();
      ...
      top = BedSpawnSupport.foundation(initial, chunkX, chunkZ);
      column = 0;
      actor.selectHeldSlot(0);
      while (BedSpawnSupport.water(initial
              .blockAt(BedSpawnSupport.local(top.x(), chunkX), top.y() + 1,
                  BedSpawnSupport.local(top.z(), chunkZ))
              .legacyId())) {
      ...
      BedSpawnSupport.require(worldline.test.WorldlineSmokeAwait.observe(actor, 240)
      ...
      BlockPosition sand =
          BedSpawnSupport.place(actor, BlockFace.EAST.adjacent(top), BlockFace.UP, 12);
      ...
      actor.close();
      BedSpawnSupport.awaitPlayers(server, 0);
      server.save();
      ...
      BedSpawnSupport.require(
          after.blockAt(BedSpawnSupport.local(foot.x(), chunkX), foot.y(),
                   BedSpawnSupport.local(foot.z(), chunkZ))
                  .equals(new BlockState(26, 0))
              && after
                  .blockAt(BedSpawnSupport.local(head.x(), chunkX), head.y(),
                      BedSpawnSupport.local(head.z(), chunkZ))
                  .equals(new BlockState(26, 8)),
          "post-respawn bed halves drift");
      BedSpawnSupport.require(
          world.atBed(pose, foot, head) && !world.atWorld(pose) && reader.awaitHealth(20) == 20,
          "persisted Packet9 is not bed spawn");
