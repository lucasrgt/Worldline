...
public final class RemainingCartBreakSmoke{
 private static final RemoteItemStack CART=new RemoteItemStack(328,1,0),CHEST=new RemoteItemStack(54,1,0);
 private RemainingCartBreakSmoke(){}
 public static void main(String[]a)throws Exception{
  if(a.length!=7)throw new IllegalArgumentException("usage: RemainingCartBreakSmoke server.jar workspace port seed username chunkX chunkZ");
  ...
  require(seed==17320110707L&&user.equals("CartBrk404")&&user.length()<=16,"remaining-cart-break identity drift");
  Duration timeout=Duration.ofSeconds(90);B173DedicatedServer server=new B173DedicatedServer(jar,workspace,port,seed,timeout,3,true);B173WireClient actor=new B173WireClient("127.0.0.1",port,user,timeout);BlockPosition top,rail,east1,east2,chestRail;int column;
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1,2,3,4},new int[]{1,66,328,342,276},new int[]{32,2,1,1,1},new int[]{0,0,0,0,0});actor.connect();actor.synchronizePose();require(actor.awaitInventory().occupiedSlots()==5,"remaining-cart-break inventory drift");RemoteChunkSnapshot initial=actor.awaitRemoteChunk(cx,cz).chunkAt(cx,cz);top=foundation(initial,cx,cz);column=0;actor.selectHeldSlot(0);
   while(water(initial.blockAt(local(top.x(),cx),top.y()+1,local(top.z(),cz)).legacyId())){top=place(actor,top,BlockFace.UP,1);actor.moveAndObserve(0D,1D,0D,1);require(++column<=15,"water column exceeded remaining-cart-break fixture");}for(int lift=0;lift<8;lift++){top=place(actor,top,BlockFace.UP,1);actor.moveAndObserve(0D,1D,0D,1);column++;}
   actor.selectHeldSlot(1);rail=place(actor,top,BlockFace.UP,66);actor.selectHeldSlot(2);actor.useHeldItemOnBlock(rail,BlockFace.UP);RemoteObjectSpawn empty=actor.awaitObjectSpawn(10);
   ...
   String evidence="column="+column+",rail="+rail.x()+":"+rail.y()+":"+rail.z()+":66:0,cart=type"+empty.type()+"+thrower0+fixed"+empty.fixedX()+":"+empty.fixedY()+":"+empty.fixedZ()+",emptyDrop=packet21-328,chestRail="+chestRail.x()+":"+chestRail.y()+":"+chestRail.z()+":66:0,chest=type"+chest.type()+"+thrower0+fixed"+chest.fixedX()+":"+chest.fixedY()+":"+chest.fixedZ()+",chestDrops=packet21-328+packet21-54,sword=276,button=1,clients=1,disconnect=clean";
   String trace="v1|server=official-b1.7.3|seed="+seed+"|fixture=raised-rail66-pair|cause=packet7-attack-type10+packet7-attack-type11|wire=packet23-type10+packet21-328+packet23-type11+packet21-328+packet21-54|oracle=remaining-cart-break-type10-328+type11-328-54|"+evidence;
   System.out.println("WORLDLINE_M404_SET="+evidence);System.out.println("WORLDLINE_M404_TRACE="+trace);System.out.println("WORLDLINE_M404_SIGNATURE="+sha(trace));
 ...
 private static BlockPosition foundation(RemoteChunkSnapshot q,int cx,int cz){for(int x=4;x<=11;x++)for(int z=4;z<=11;z++)for(int y=126;y>=1;y--)if(q.blockAt(x,y,z).legacyId()==3&&water(q.blockAt(x,y+1,z).legacyId()))return new BlockPosition(cx*16+x,y,cz*16+z);throw new IllegalStateException("no deterministic remaining-cart-break foundation");}
