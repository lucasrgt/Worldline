package worldline.smoke.portalobsidiansetb173;
...
/** Places fourteen obsidian 49 frame cells and ignites six portal 90 interior cells as one SET. */
public final class PortalObsidianSetSmoke {
  private PortalObsidianSetSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: PortalObsidianSetSmoke server.jar workspace port seed username chunkX chunkZ");
    Path jar = Paths.get(a[0]), workspace = Paths.get(a[1]);
    int port = Integer.parseInt(a[2]);
    long seed = Long.parseLong(a[3]);
    String user = a[4];
    int cx = Integer.parseInt(a[5]), cz = Integer.parseInt(a[6]);
    require(seed == 17320110707L && user.equals("Portal382") && user.length() <= 16,
        "portal-obsidian-set identity drift");
    ...
    try {
      ...
      require(actor.awaitInventory().occupiedSlots() == 3 && actor.dimension() == 0,
          "portal-obsidian-set inventory or dimension drift");
      ...
      while (water(initial.blockAt(local(anchor.x(), cx), anchor.y() + 1, local(anchor.z(), cz))
        ...
        require(++column <= 15, "water column exceeded portal-obsidian-set fixture");
      }
      anchor = place(actor, anchor, BlockFace.UP, 1);
      actor.moveAndObserve(0D, 1D, 0D, 1);
      column++;
      ...
      require(frame.size() == 14 && inside.size() == 6, "portal-obsidian-set geometry drift");
      RemoteWorldView framed = worldline.test.WorldlineSmokeAwait.observe(actor, 20);
      for (BlockPosition q : frame)
        require(framed.blockAt(q.x(), q.y(), q.z()).equals(new BlockState(49, 0)),
            "obsidian 49 frame drift at " + q);
      ...
      RemoteWorldView active = worldline.test.WorldlineSmokeAwait.observe(actor, 20);
      for (BlockPosition q : inside) {
        BlockState s = active.blockAt(q.x(), q.y(), q.z());
        require(s.legacyId() == 90, "portal 90 activation absent at " + q + ": " + s);
        if (metadata < 0)
          metadata = s.metadata();
        else
          require(metadata == s.metadata(), "portal metadata drift");
      }
      require(actor.dimension() == 0, "portal-obsidian-set traveled during activation");
      actor.close();
      ...
      require(reader.dimension() == 0, "fresh login dimension drift");
      RemoteChunkSnapshot after = reader.awaitRemoteChunk(cx, cz).chunkAt(cx, cz);
      for (BlockPosition q : frame)
        require(
            after.blockAt(local(q.x(), cx), q.y(), local(q.z(), cz)).equals(new BlockState(49, 0)),
            "fresh obsidian 49 drift at " + q);
      for (BlockPosition q : inside)
        require(after.blockAt(local(q.x(), cx), q.y(), local(q.z(), cz))
                    .equals(new BlockState(90, metadata)),
            "fresh portal 90 drift at " + q);
      String evidence = "column=" + column + ",frame=" + bottom.x() + ":" + bottom.y() + ":"
          + bottom.z() + "-" + (bottom.x() + 3) + ":" + (bottom.y() + 4) + ":" + bottom.z()
          + ",obsidian=14x49,interior=6x90:" + metadata
          + ",flint=259,dimension=0,persisted=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|profile=allow-nether-true|fixture=stone-column+obsidian49-frame4x5+flintsteel259|construction=packet15-fourteen-obsidian49|baseline=six-air-cells|cause=packet15-flint-and-steel259|effect=official-portal-block90-six-cells|observation=live-packet53+fresh-login-packet51|oracle=fourteen-obsidian49+six-portal90+dimension-zero-no-travel|"
          + evidence;
      System.out.println("WORLDLINE_M382_SET=" + evidence);
      System.out.println("WORLDLINE_M382_TRACE=" + trace);
      System.out.println("WORLDLINE_M382_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
  ...
  private static BlockPosition foundation(RemoteChunkSnapshot q, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic portal-obsidian-set foundation");
  }
