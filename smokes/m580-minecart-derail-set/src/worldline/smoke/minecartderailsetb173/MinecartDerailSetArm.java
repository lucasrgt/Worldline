...
public final class MinecartDerailSetArm {
  ...
  private MinecartDerailSetArm(BlockPosition s, BlockPosition w, BlockPosition b, BlockPosition p, BlockPosition t,
      BlockPosition l, BlockPosition o) {
    support = s;
    wall = w;
    bumper = b;
    powered = p;
  ...
  static MinecartDerailSetArm place(B173WireClient a, BlockPosition top) throws Exception {
    ...
    require(powered.x() == track.x() && track.x() == plate.x() && track.z() == powered.z() + 1
            && plate.z() == track.z() + 1 && wall.z() == powered.z() - 1 && bumper.z() == plate.z() + 1,
        "dead-end north-south derail track drift");
    return new MinecartDerailSetArm(top, wall, bumper, powered, track, plate, BlockFace.UP.adjacent(eastPad));
  }
  ...
  RemoteObjectSpawn idleCart(B173WireClient a) throws Exception {
    ...
    require(cart.type() == 10 && cart.throwerId() == 0 && cart.velocityX() == 0 && cart.velocityY() == 0
            && cart.velocityZ() == 0 && cart.fixedX() == powered.x() * 32 + 16 && cart.fixedY() == powered.y() * 32 + 27
            && cart.fixedZ() == powered.z() * 32 + 16 && cart.fixedZ() != plate.z() * 32 + 16,
    ...
    require(idle.blockAt(plate.x(), plate.y(), plate.z()).equals(new BlockState(72, 0))
            && idle.blockAt(powered.x(), powered.y(), powered.z()).equals(new BlockState(27, 0))
  ...
  BlockState launch(B173WireClient a) throws Exception {
    ...
    require(worldline.test.WorldlineSmokeAwait.awaitBlock(a, plate, new BlockState(72, 1), 80)
  ...
  void persist(RemoteChunkSnapshot after, int cx, int cz, BlockState torchState) {
    require(at(after, powered, cx, cz).equals(new BlockState(27, 8))
            ...
            && at(after, plate, cx, cz).equals(new BlockState(72, 1)) && at(after, torch, cx, cz).equals(torchState)
