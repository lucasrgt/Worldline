...
public final class MinecartDerailSetSmoke {
  ...
  public static void main(String[] a) throws Exception {
    ...
    MinecartDerailSetArm.require(seed == 17320110707L && user.equals("CartDerail580") && user.length() <= 16,
        "minecart-derail-set identity drift");
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2, 3, 4, 5},
          new int[] {1, 27, 66, 72, 328, 76}, new int[] {32, 1, 1, 1, 1, 1}, new int[] {0, 0, 0, 0, 0, 0});
      actor.connect();
      actor.synchronizePose();
      MinecartDerailSetArm.require(actor.awaitInventory().occupiedSlots() == 6, "minecart-derail inventory drift");
      ...
      String evidence = "column=" + column[0] + ",support=" + MinecartDerailSetArm.cell(arm.support) + ":1:0,wall="
          + MinecartDerailSetArm.cell(arm.wall) + ":1:0,bumper=" + MinecartDerailSetArm.cell(arm.bumper) + ":1:0,rail="
          + MinecartDerailSetArm.cell(arm.powered) + ":27:0->8,track=" + MinecartDerailSetArm.cell(arm.track)
          + ":66:0,plate=" + MinecartDerailSetArm.cell(arm.plate) + ":72:0->1,cart=type10+thrower0+fixed"
          + cart.fixedX() + ":" + cart.fixedY() + ":" + cart.fixedZ()
          + ",unpowered-hold=idle,derail=1,torch=" + MinecartDerailSetArm.cell(arm.torch) + ":76:" + torch.metadata()
      ...
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          ...
          + "|oracle=unpowered-hold-idle+derail-off-rail66-onto-plate+fresh-login|" + evidence;
