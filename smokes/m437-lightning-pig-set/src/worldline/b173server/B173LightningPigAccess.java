...
public final class B173LightningPigAccess {
    ...
    public static B173DedicatedServer overworld(Path jar, Path directory, int port, long seed, Duration timeout) {
        return B173DedicatedServer.animals(jar, directory, port, seed, timeout, 3, true);
    }

    public static B173DedicatedServer nether(Path jar, Path directory, int port, long seed, Duration timeout) {
    ...
    public static void pad(B173WireClient actor, BlockPosition center) throws Exception {
        ...
        for (int i = 0; i < faces.length; i++) {
            ...
            if (air(actor.awaitRemoteChunk(next.x() >> 4, next.z() >> 4).blockAt(next.x(), next.y(), next.z()).legacyId()))
    ...
    public static int count(RemoteChunkSnapshot q, int id) {
        ...
        for (int x = 0; x < 16; x++)
            for (int z = 0; z < 16; z++)
                for (int y = 0; y < 128; y++)
                    if (q.blockAt(x, y, z).legacyId() == id) n++;
        return n;
    }
