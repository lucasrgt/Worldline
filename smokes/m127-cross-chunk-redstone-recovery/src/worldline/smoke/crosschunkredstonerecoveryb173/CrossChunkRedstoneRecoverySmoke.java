package worldline.smoke.crosschunkredstonerecoveryb173;
...
/** Depowers official dust through a lever placed across an east chunk seam. */
public final class CrossChunkRedstoneRecoverySmoke {
  private CrossChunkRedstoneRecoverySmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 10)
      throw new IllegalArgumentException(
          "usage: CrossChunkRedstoneRecoverySmoke server.jar workspace port seed username wireX leverX chunkZ fixtureTicks signalTicks");
    ...
    String user = a[4];
    require(leverX == wireX + 1, "M127 requires east seam");
    ...
    BlockPosition top, lever, wire;
    BlockState leverOff, leverOn, leverFinal, wireOff, wireOn, wireFinal;
    int column;
    try {
      server.boot();
      ...
      actor.look(-90F, 0F);
      require(actor.awaitInventory().occupiedSlots() == 3, "seam reset inventory drift");
      ...
      top = foundation(initial, wireX, cz);
      column = 0;
      ...
      require(wire.x() == wireX * 16 + 15 && lever.x() == leverX * 16, "reset fixture missed seam");
      ...
      leftOff = baseline.chunkAt(wireX, cz);
      ...
      require(wireOn.equals(new BlockState(55, 15)) && leverOn.equals(new BlockState(69, 9)),
          "cross-chunk powered precondition absent");
      actor.close();
      ...
      RemoteWorldView powered = witness.awaitRemoteChunk(wireX, cz);
      powered = witness.awaitRemoteChunk(leverX, cz);
      leftOn = powered.chunkAt(wireX, cz);
      rightOn = powered.chunkAt(leverX, cz);
      require(leftOn.blockAt(15, wire.y(), wire.z() - cz * 16).equals(wireOn)
              && rightOn.blockAt(0, lever.y(), lever.z() - cz * 16).equals(leverOn),
          "fresh powered precondition drift");
      witness.selectHeldSlot(3);
      witness.activateBlock(lever, BlockFace.UP);
      RemoteWorldView reset = worldline.test.WorldlineSmokeAwait.observe(witness, signalTicks);
      wireFinal = reset.blockAt(wire.x(), wire.y(), wire.z());
      leverFinal = reset.blockAt(lever.x(), lever.y(), lever.z());
      require(wireFinal.equals(wireOff) && leverFinal.equals(leverOff),
          "cross-chunk recovery absent: " + wireFinal + " / " + leverFinal);
      witness.close();
      awaitPlayers(server, 0);
      server.save();
      ...
      leftFinal = after.chunkAt(wireX, cz);
      rightFinal = after.chunkAt(leverX, cz);
      require(leftFinal.blockAt(15, wire.y(), wire.z() - cz * 16).equals(wireFinal)
              && rightFinal.blockAt(0, lever.y(), lever.z() - cz * 16).equals(leverFinal),
          "fresh recovered seam drift");
    } finally {
      actor.close();
      if (witness != null)
        witness.close();
    ...
    require(left.changed == 1 && right.changed == 1,
        "seam reset changed unrelated states " + left + " / " + right);
    require(leftResidual.changed == 0 && rightResidual.changed == 0,
        "seam reset did not restore baseline " + leftResidual + " / " + rightResidual);
    String evidence = "column=" + column + ",lever=" + lever.x() + ":" + lever.y() + ":" + lever.z()
        + ":" + leverOn.metadata() + "->" + leverFinal.metadata() + ",wire=" + wire.x() + ":"
        + wire.y() + ":" + wire.z() + ":" + wireOn.metadata() + "->" + wireFinal.metadata()
        + ",left=" + left + ",right=" + right + ",residual=" + leftResidual.changed + ":"
        + rightResidual.changed;
    String trace = "v1|server=official-b1.7.3|seed=" + seed
        ...
        + "ticks|precondition=fresh-lever69:9+wire55:15|cause=packet15-lever-deactivate-neighbor|effect=packet53-wire-depower-source-chunk|observation=fresh-packet51-both-chunks+exact-baseline-recovery|"
        + evidence + "|disconnect=clean";
    System.out.println("WORLDLINE_M127_SIGNAL=" + evidence);
    System.out.println("WORLDLINE_M127_TRACE=" + trace);
    System.out.println("WORLDLINE_M127_SIGNATURE=" + sha(trace));
  }
