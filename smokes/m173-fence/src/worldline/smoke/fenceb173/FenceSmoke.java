package worldline.smoke.fenceb173;
...
/** Places two adjacent official fence 85:0 blocks on raised stone and reloads both. */
public final class FenceSmoke {
  private FenceSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: FenceSmoke server.jar workspace port seed username chunkX chunkZ");
    ...
    BlockPosition top, east, westFence, eastFence;
    int column;
    ...
    try {
      ...
      require(actor.awaitInventory().occupiedSlots() == 2, "fence inventory drift");
      ...
      while (
        ...
        require(++column <= 15, "water column exceeded fence fixture");
      }
      ...
      require(after.blockAt(local(top.x(), cx), top.y(), local(top.z(), cz))
              ...
              && after.blockAt(local(eastFence.x(), cx), eastFence.y(), local(eastFence.z(), cz))
                  .equals(new BlockState(85, 0)),
          "persisted fence drift");
      String evidence = "column=" + column + ",support=" + top.x() + ":" + top.y() + ":" + top.z()
          + ":1:0,west=" + westFence.x() + ":" + westFence.y() + ":" + westFence.z()
          + ":85:0,east=" + eastFence.x() + ":" + eastFence.y() + ":" + eastFence.z()
          + ":85:0,persisted=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=raised-stone+adjacent-fence85|cause=packet15-item85|wire=packet53-fence85:0|oracle=adjacent-placement+fresh-login|"
          + evidence;
      System.out.println("WORLDLINE_M173_FENCE=" + evidence);
      System.out.println("WORLDLINE_M173_TRACE=" + trace);
      System.out.println("WORLDLINE_M173_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
  ...
  private static BlockPosition foundation(RemoteChunkSnapshot q, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic fence foundation");
  }
