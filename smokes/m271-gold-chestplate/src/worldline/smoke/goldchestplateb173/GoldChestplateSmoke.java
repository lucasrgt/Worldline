...
public final class GoldChestplateSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(workspace, actorName, 4.5D, 60D, 4.5D, new int[] {0},
          new int[] {315}, new int[] {1}, new int[] {0});
      B173PlayerSeed.write(workspace, observerName, 4.5D, 60D, 4.5D);
      actor.connect();
      actor.synchronizePose();
      require(actor.awaitInventory().occupiedSlots() == 1
              && actor.inventory().slot(36).item().equals(GOLD)
              && actor.inventory().slot(6).empty(),
          "gold chestplate seed drifted");
      observer.connect();
      observer.synchronizePose();
      observer.moveAndObserve(0D, 0D, 0D, 3);
      B173GoldChestplateEquip.apply(actor);
      requireChest(actor.inventory());
      ...
      require(peer.legacyId() == 315 && peer.slot().containerSlot() == 6
              && peer.slot().equipmentSlot() == 3,
          "peer gold chestplate drifted");
      actor.close();
      observer.close();
      ...
      requireChest(restored.awaitInventory());
      witness.connect();
      ...
      server.save();
      String evidence =
          "window=6:315,packet5=3:315,distinct-from=299,307,persisted=true,clients=4,disconnect=clean";
  ...
  private static void requireChest(RemoteInventoryView view) {
    require(!view.slot(6).empty() && view.slot(6).item().equals(GOLD)
            ...
            && view.slot(36).empty() && view.occupiedSlots() == 1,
        "gold chestplate window drifted");
  }
