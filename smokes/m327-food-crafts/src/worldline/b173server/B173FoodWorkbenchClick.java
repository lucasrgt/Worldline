...
public final class B173FoodWorkbenchClick {
    ...
    public static void apply(B173WireClient actor) {
        try { new B173FoodWorkbenchClick(actor).run(); }
        catch (IOException error) { throw new IllegalStateException("food workbench crafts failed", error); }
    }
    ...
    private void run() throws IOException {
        ...
        shaped(38, new int[] {7, 8, 9}, CAKE, new int[] {4, 5, 6, 7, 8, 9}, new int[] {1, 2, 3}, BUCKET, 40);
        leftover(1, 9); leftover(2, 10); leftover(3, 11);
    ...
    private void shaped(int personal, int[] cells, RemoteItemStack result, int[] consumed, int[] keep,
        ...
        for (int index = 0; index < cells.length; index++) {
            ...
            if (index == cells.length - 1 && result != null) after = replace(after, 0, result);
            step(cells[index], 1, null, after, personal(), personal(), cursor);
        }
        if (cursor != null) step(combined, 0, null, replace(window(), combined, cursor), personal(),
                replace(personal(), personal, cursor), null);
        if (result == null) return;
        if (!same(window(), 0, result)) throw new IllegalStateException("food result " + result.legacyId() + " absent");
        RemoteInventoryView taken = replace(window(), 0, null);
        for (int slot : consumed) taken = replace(taken, slot, null);
        for (int slot : keep) taken = replace(taken, slot, leftover);
    ...
    private void takeResult(RemoteItemStack result, RemoteInventoryView taken) throws IOException {
        send(new B173ContainerStep(windowId(), action + 1, 0, result, window(), taken,
                personal(), personal(), inbound.cursor(), result, STAT + result.legacyId(), result.count()));
