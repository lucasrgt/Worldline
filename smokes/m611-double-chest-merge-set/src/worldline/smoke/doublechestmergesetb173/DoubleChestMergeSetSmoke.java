...
public final class DoubleChestMergeSetSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        require(seed == 17320110707L && user.equals("DblChest611") && user.length() <= 16
                && cx == 0 && cz == 0, "double-chest-merge identity drift");
        ...
        try {
            ...
            B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D,
                    new int[] {0, 1}, new int[] {1, 54}, new int[] {32, 2}, new int[] {0, 0});
            actor.connect();
            actor.synchronizePose();
            require(actor.awaitInventory().occupiedSlots() == 2,
                    "double-chest-merge inventory drift");
            ...
            require(first.blockAt(left.x(), left.y(), left.z()).equals(chest),
                    "live first chest 54:0 drift");
            ...
            require(window(merged, "Large chest", 54, 90),
                    "live Packet100 large-chest window absent");
            actor.close();
            ...
            require(window(again, "Large chest", 54, 90),
                    "fresh-login Packet100 large-chest window absent");
            String evidence = "column=" + column + ",support=" + cell(top, 1, 0)
                    + ",east=" + cell(east, 1, 0) + ",left=" + cell(left, 54, 0)
            ...
            String trace = "v1|server=official-b1.7.3|seed=" + seed
                    + "|fixture=raised-stone+two-adjacent-chest54"
    ...
    private static boolean window(RemoteContainerWindow view, String title, int owned, int total) {
        ...
        for (int slot = 0; slot < owned; slot++)
            if (!view.inventory().slot(slot).empty()) return false;
        return true;
    }
    ...
    private static BlockPosition place(B173WireClient actor, BlockPosition support,
        ...
        WorldlineSmokeAwait.awaitBlock(actor, target, new BlockState(id, 0), 20);
        return target;
    }
