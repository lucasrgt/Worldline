...
final class ItemWorldBackend implements GameBackend {
    ...
    @Override public void close() { world = null; item = null; }
    ...
    void assertOutcome(int ticks) {
        ...
        require(item.age == ticks && sawGround,
                "item did not complete the grounding/age path: age=" + item.age
                        + " ground=" + sawGround
        ...
        if (airborne) require(initialY - minimumY >= 2_500L, "airborne item did not descend");
        else require(Math.abs(finalY - initialY) <= 125L, "supported item drifted vertically");
    }
