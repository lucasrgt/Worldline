...
public final class MovementOutcomeSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    PlayerPose initial;
    MovementOutcome small, rollback;
    ...
    try {
      ...
      require(small.disposition() == MovementDisposition.UNCHALLENGED
              && close(small.resulting().x(), initial.x() + .125D),
          "small move was challenged");
      RemoteWorldView before = worldline.test.WorldlineSmokeAwait.observe(client, 1);
      BlockPosition block = solid(before, small.resulting());
      ...
      require(after.containsChunk(chunkX, chunkZ), "rollback lost cached chunk");
      client.close();
      ...
      require(close(player.x(), small.resulting().x()) && close(player.y(), small.resulting().y())
              && close(player.z(), small.resulting().z()),
          "unchallenged move was not persisted");
    } finally {
      client.close();
      server.close();
    }
    System.out.println(
        "WORLDLINE_M35_API=movement-outcome,unchallenged,corrected,pose,cache,persistence");
    ...
    System.out.println(
        "WORLDLINE_M35_PERSISTED=" + player.x() + "," + player.y() + "," + player.z());
