...
final class OracleBedrockScenario implements BedrockSubsystemScenario {
    private final dj world;
    private final CanonicalTrace trace;
    OracleBedrockScenario(dj world, long seed) {
        this.world = world;
        trace = new CanonicalTrace(seed);
    }
    @Override public BedrockSubsystemObservation observe() {
        OracleBedrockProbe probe = OracleBedrockProbe.execute(world);
