...
public final class WorldlineRecoveryState {
    ...
    public static synchronized void ack(int[] values) {
        if (values == null || values.length != 5 || values[4] < 1 || values[4] > 2) throw new IllegalStateException("invalid M85 ack");
        if (values[4] == 1) { if (removeAck) throw new IllegalStateException("duplicate M85 remove ack");
        ...
        if (!removeAck || restoreAck || values[0] != x || values[1] != y || values[2] != z || values[3] != root)
            throw new IllegalStateException("invalid M85 restore ack"); restoreAck = true;
    }
    public static synchronized void restore(int[] values) {
        if (values == null || values.length != 4 || values[3] <= 0) throw new IllegalStateException("invalid M85 restore state");
        if (restoreReceived) { if (values[0] != x || values[1] != y || values[2] != z || values[3] != restoreNonce)
            throw new IllegalStateException("conflicting M85 restore state"); return; }
        if (!removeAck || values[0] != x || values[1] != y || values[2] != z || values[3] != root * 100 + 1)
            throw new IllegalStateException("unbound M85 restore state"); restoreNonce = values[3]; restoreReceived = true;
    }
    public static synchronized void apply(World world) {
        ...
        if (be.nonce() != 0 && be.nonce() != restoreNonce) throw new IllegalStateException("M85 restored nonce drift");
