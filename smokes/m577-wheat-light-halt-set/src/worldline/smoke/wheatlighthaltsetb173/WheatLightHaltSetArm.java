...
public final class WheatLightHaltSetArm {
 ...
 static void waitAge(B173WireClient a, BlockPosition[] lit, BlockPosition covered, int window,
   int windows) throws Exception {
  RemoteWorldView grown = worldline.test.WorldlineSmokeAwait.awaitWorld(a,
    v -> anyAged(v, lit) && crop(v, covered, 0), "lit wheat age", window * windows);
  require(crop(grown, covered, 0), "covered wheat 59 aged during wait");
  for (int i = 0; i < lit.length; i++)
   require(id(grown, lit[i]) == 59, "lit wheat 59 died during wait");
 }

 static void persist(RemoteChunkSnapshot after, int cx, int cz, BlockPosition[] plots,
   BlockPosition[] lit, BlockPosition covered, BlockPosition cover, BlockPosition water) {
  for (int i = 0; i < plots.length; i++)
  ...
  for (int i = 0; i < lit.length; i++) {
   ...
   if (s.metadata() >= 1) aged = true;
  }
  require(aged, "lit wheat age did not persist");
 }

 static boolean crops(RemoteWorldView v, BlockPosition[] lit, BlockPosition covered,
   BlockPosition cover) {
  if (id(v, cover) != 1 || !crop(v, covered, 0)) return false;
  for (int i = 0; i < lit.length; i++) if (id(v, lit[i]) != 59) return false;
  return true;
 }

 static String dump(RemoteWorldView v, BlockPosition[] lit, BlockPosition covered,
   BlockPosition cover) {
  StringBuilder s = new StringBuilder();
  s.append("cover=").append(id(v, cover)).append(",covered=").append(id(v, covered))
    .append(':').append(meta(v, covered));
  for (int i = 0; i < lit.length; i++)
   s.append(",lit").append(i).append('=').append(id(v, lit[i])).append(':')
     .append(meta(v, lit[i]));
  return s.toString();
 }
 ...
 static boolean anyAged(RemoteWorldView v, BlockPosition[] lit) {
  for (int i = 0; i < lit.length; i++)
   if (id(v, lit[i]) == 59 && meta(v, lit[i]) >= 1) return true;
  return false;
 }
