...
public final class B173SpawnerDelay {
    ...
    public static void delay(Path serverDirectory, BlockPosition spawner, short value) {
        ...
        try (RandomAccessFile file = new RandomAccessFile(region.toFile(), "rw")) {
            ...
            byte[] raw = inflate(compressed, type), patched = replace(raw, value), out = deflate(patched, type);
        ...
        } catch (IOException error) { throw new IllegalStateException("could not rewrite spawner delay", error); }
    }

    private static byte[] replace(byte[] raw, short value) {
        int at = indexOf(raw), extra = -1;
        if (at < 0) throw new IllegalStateException("MobSpawner Delay absent");
        outer: for (int i = at + NAME.length + 2; i + NAME.length + 2 <= raw.length; i++) {
            for (int j = 0; j < NAME.length; j++) if (raw[i + j] != NAME[j]) continue outer;
            extra = i; break;
        }
        if (extra >= 0) throw new IllegalStateException("duplicate MobSpawner Delay");
        byte[] next = raw.clone();
    ...
    private static int indexOf(byte[] raw) {
        outer: for (int i = 0; i + NAME.length + 2 <= raw.length; i++) {
            for (int j = 0; j < NAME.length; j++) if (raw[i + j] != NAME[j]) continue outer;
            return i;
        }
        return -1;
