...
public final class FurnaceSmeltSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        B173DedicatedServer server = null; FurnaceSession actor = null, observer = null;
        RemoteFurnaceLoad load; RemoteFurnaceSmelt smelt; RemoteWindowClosure closure; BlockPosition target;
        try {
            server = new B173DedicatedServer(jar, workspace, port, seed, timeout, 3, true);
            server.boot(); server.operator(actorName); actor = client(port, actorName, timeout);
            actor.connect(); actor.synchronizePose(); require(actor.awaitInventory().occupiedSlots() == 0,
                    "actor inventory was not empty"); actor.look(0F, 90F); PlayerPose pose = acquireFurnace(actor, actorName);
            ...
            target = BlockFace.UP.adjacent(support); require(baseline.blockAt(support.x(), support.y(), support.z())
                    ...
                    "furnace anchor drifted"); require(!actor.moveAndObserve(0D, 3D, 0D, 3).corrected(),
                    "furnace clearance failed"); observer = client(port, observerName, timeout);
            ...
            require(actor.inventory().slot(36).item().equals(sand)
                    && actor.inventory().slot(37).item().equals(coal), "furnace ingredient seed drifted");
            actor.selectHeldSlot(2); RemoteContainerWindow opened = actor.openFurnace(target, BlockFace.UP);
            require(opened.descriptor().kind() == RemoteWindowKind.FURNACE
                    ...
                    && opened.inventory().slot(31).item().equals(coal) && opened.inventory().slot(0).empty()
                    && opened.inventory().slot(1).empty() && opened.inventory().slot(2).empty(),
                    "furnace open mapping drifted"); actor.selectHeldSlot(0);
            observer.awaitPeerHeldItem(new RemoteHeldItem(actorName, 12, 0));
            load = actor.loadFurnace(36, 37); require(load.inputTakeAction() == 1
            ...
            server.save(); require(server.player(actorName).inventoryItems() == 0,
