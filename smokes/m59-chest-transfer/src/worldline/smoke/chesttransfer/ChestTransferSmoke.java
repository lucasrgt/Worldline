...
public final class ChestTransferSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    ChestTransferSession actor = null, observer = null, reopened = null;
    RemoteChestTransfer transfer;
    RemoteContainerWindow persisted;
    RemoteWindowClosure firstClose, secondClose;
    BlockPosition target;
    WorldlineAwait waits = new WorldlineAwait(100);
    try {
      ...
      actor.look(0F, 90F);
      PlayerPose pose = acquireChest(actor, actorName);
      ...
      awaitSlot(waits, actor, 36, new RemoteItemStack(54, 1, 0));
      ...
      transfer = actor.storeInOpenChest(36, 0);
      require(transfer.takeAction() == 1 && transfer.storeAction() == 2
              && transfer.stack().equals(stone) && transfer.after().slot(0).item().equals(stone)
              && actor.inventory().slot(36).empty(),
      ...
      require(firstClose.closedWindow().inventory().equals(transfer.after())
              && firstClose.proofAction() == 1,
          "post-transfer close proof drifted");
      actor.close();
      observer.close();
      awaitPlayers(first, 0);
      first.save();
      require(first.player(actorName).inventoryItems() == 0,
          "transferred player inventory persisted items");
      first.close();
      first = null;
      ...
      require(reopened.awaitInventory().occupiedSlots() == 0,
          "restarted player inventory was not empty");
      persisted = reopened.openChest(target, BlockFace.UP);
      require(persisted.inventory().slot(0).item().equals(stone)
              && persisted.inventory().occupiedSlots() == 1,
          "persisted chest contents drifted");
      secondClose = reopened.closeWindow();
      require(secondClose.proofAction() == 1, "restarted close proof drifted");
      reopened.close();
    ...
    System.out.println("WORLDLINE_M59_TRANSFER=actions=" + transfer.takeAction() + ","
        + transfer.storeAction() + ";close=" + firstClose.proofAction()
        + ";reopen=" + persisted.inventory().slot(0).item() + ";close2=" + secondClose.proofAction()
        + ";player-items=0");
  ...
  private static PlayerPose acquireChest(ChestTransferSession client, String username) {
    ...
    }, inventory -> inventory.occupiedSlots() >= 1, "transfer chest grant", 100);
  ...
  private static void awaitSlot(
      WorldlineAwait waits, ChestTransferSession client, int slot, RemoteItemStack expected) {
    int[] passivePolls = {0};
    waits.awaitSlot(() -> {
      RemoteInventoryView view = client.inventory();
      if (!view.slot(slot).empty() && view.slot(slot).item().equals(expected))
        return view;
      if (passivePolls[0]++ < 40)
