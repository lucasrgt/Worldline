...
final class NetherExitPortalScan {
  ...
  static PlayerPose leave(B173WireClient actor, Portal portal, PlayerPose pose, int cooldown)
      throws Exception {
    ...
    NetherExitPortalKit.require(
        actor.dimension() == portal.stayDimension, "left portal changed dimension early");
  ...
  static PlayerPose hoverUntilChunk(B173WireClient actor, PlayerPose pose, int chunkX, int chunkZ)
    ...
    for (int n = 0; n < 80; n++) {
      ...
      if (worldline.test.WorldlineSmokeAwait.observe(actor, 1).containsChunk(chunkX, chunkZ))
        return pose;
    }
  ...
  static int frame(RemoteWorldView view, Portal portal) {
    ...
    if (portal.maxX > portal.minX) {
      for (int x = portal.minX - 1; x <= portal.maxX + 1; x++) {
        if (view.blockAt(x, portal.minY - 1, portal.minZ).legacyId() == 49)
          count++;
        if (view.blockAt(x, portal.minY + 3, portal.minZ).legacyId() == 49)
          count++;
      }
      for (int y = portal.minY; y <= portal.minY + 2; y++) {
        if (view.blockAt(portal.minX - 1, y, portal.minZ).legacyId() == 49)
          count++;
        if (view.blockAt(portal.maxX + 1, y, portal.minZ).legacyId() == 49)
          count++;
      }
    } else {
      for (int z = portal.minZ - 1; z <= portal.maxZ + 1; z++) {
        if (view.blockAt(portal.minX, portal.minY - 1, z).legacyId() == 49)
          count++;
        if (view.blockAt(portal.minX, portal.minY + 3, z).legacyId() == 49)
          count++;
      }
      for (int y = portal.minY; y <= portal.minY + 2; y++) {
        if (view.blockAt(portal.minX, y, portal.minZ - 1).legacyId() == 49)
          count++;
        if (view.blockAt(portal.minX, y, portal.maxZ + 1).legacyId() == 49)
          count++;
      }
    }
    return count;
