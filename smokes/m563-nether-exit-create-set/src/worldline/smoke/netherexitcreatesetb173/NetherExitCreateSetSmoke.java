...
public final class NetherExitCreateSetSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    NetherExitPortalKit.require(seed == 17320110707L && user.equals("NetherExit563")
            && user.length() <= 16 && shift >= 24 && shift <= 48 && cooldownTicks >= 200,
        "nether-exit-create-set identity drift");
    ...
    try {
      ...
      NetherExitPortalKit.require(
          actor.dimension() == 0 && actor.awaitInventory().occupiedSlots() == 3,
      ...
      NetherExitPortalKit.require(
          actor.dimension() == 0, "source portal traveled during activation");
      pose = NetherExitPortalScan.enter(
          actor, pose, source.enterX(), source.enterY(), source.enterZ(), travelTicks, -1);
      ...
      NetherExitPortalKit.require(NetherExitPortalScan.frame(nether, generated) == 14
              && NetherExitPortalScan.sky(netherChunk) == 0,
      ...
      NetherExitPortalKit.require(
          actor.dimension() == -1 && actor.awaitInventory().occupiedSlots() == 3,
      ...
      actor = relog(actor, server, workspace, user, timeout, land.x() + 0.5D, land.y() + 1D,
          land.z() - 1.5D, -1);
      pose = actor.synchronizePose();
      NetherExitPortalKit.require(
          actor.dimension() == -1 && actor.awaitInventory().occupiedSlots() == 3,
      ...
      int up = worldline.test.WorldlineSmokeAwait.observe(actor, 1)
                   .blockAt(land.x(), land.y() + 1, land.z())
                   .legacyId();
      ...
      NetherExitPortalKit.require(
          actor.dimension() == -1, "far Nether portal traveled during activation");
      pose = actor.moveAndObserve(0D, 0D, 0D, 1).resulting();
      pose = NetherExitPortalScan.enter(
          actor, pose, far.enterX(), far.enterY(), far.enterZ(), travelTicks, 0);
      ...
      RemoteChunkSnapshot over =
          returned.chunkAt(((int) Math.floor(pose.x())) >> 4, ((int) Math.floor(pose.z())) >> 4);
      NetherExitPortalKit.require(NetherExitPortalScan.sky(over) > 0
              && NetherExitPortalScan.frame(returned, created) == 14 && created.count == 6
      ...
      server.save();
      NetherExitPortalKit.require(
          server.player(user).dimension() == 0, "created-exit player dimension was not persisted");
      ...
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          ...
          + "|outbound=packet9-0-to-minus1|nether=leave+cooldown+relog-shift-east-" + shift
          + "+m382-frame"
  ...
  private static void seedItems(
      Path workspace, String user, double x, double y, double z, int dimension) {
    B173PlayerSeed.writeInventory(workspace, user, x, y, z, dimension, new int[] {0, 1, 2},
        new int[] {1, 49, 259}, new int[] {64, 14, 1}, new int[] {0, 0, 0});
  }

  private static B173WireClient relog(B173WireClient actor, B173DedicatedServer server,
      Path workspace, String user, Duration timeout, double x, double y, double z, int dimension)
      throws Exception {
    actor.close();
    NetherExitPortalScan.awaitPlayers(server, 0);
    server.save();
