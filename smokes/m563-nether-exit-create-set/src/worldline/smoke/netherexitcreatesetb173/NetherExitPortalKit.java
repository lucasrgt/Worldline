...
final class NetherExitPortalKit {
  ...
  static Raised raise(B173WireClient actor, RemoteChunkSnapshot chunk, int chunkX, int chunkZ)
    ...
    while (water(chunk.blockAt(local(anchor.x(), chunkX), anchor.y() + 1, local(anchor.z(), chunkZ))
            .legacyId())) {
  ...
  static void light(B173WireClient actor, Frame frame, int settle) throws Exception {
    ...
    actor.useHeldItemOnBlock(
        new BlockPosition(frame.bottom.x() + 1, frame.bottom.y(), frame.bottom.z()), BlockFace.UP);
    RemoteWorldView active = actor.sustainTicks(settle);
    int portals = 0;
    for (int y = 1; y <= 3; y++)
      for (int x = 1; x <= 2; x++) {
        BlockState state =
            active.blockAt(frame.bottom.x() + x, frame.bottom.y() + y, frame.bottom.z());
