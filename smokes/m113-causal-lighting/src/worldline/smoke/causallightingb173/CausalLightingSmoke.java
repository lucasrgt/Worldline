...
public final class CausalLightingSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      server.boot();
      B173PlayerSeed.writeHolding(workspace, username, 8.5D, 120D, 8.5D, sourceId, 1, 0);
      actor.connect();
      ...
      require(inventory.occupiedSlots() == 1
          ...
          "seeded source inventory drift");
      before = actor.awaitRemoteChunk(chunkX, chunkZ).chunkAt(chunkX, chunkZ);
      support = support(before, chunkX, chunkZ);
      ...
      require(before.blockAt(local(target.x(), chunkX), target.y(), local(target.z(), chunkZ))
                      .legacyId()
                  != sourceId
              && after.blockAt(local(target.x(), chunkX), target.y(), local(target.z(), chunkZ))
                      .legacyId()
                  == sourceId,
          "source block transition drift");
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
    ...
    require(block.changed > 0 && block.increased > 0 && before.blockLightAt(lx, target.y(), lz) == 0
            && after.blockLightAt(lx, target.y(), lz) == 15,
        "causal block-light source absent");
    String evidence = "target=" + target.x() + ":" + target.y() + ":" + target.z() + ",support="
        + before.blockAt(lx, support.y(), lz).legacyId() + ",block=" + block + ",sky=" + sky;
      ...
      for (int z = 4; z <= 11; z++)
        for (int y = 126; y >= 1; y--)
          if (solid(chunk.blockAt(x, y, z).legacyId())
              && replaceable(chunk.blockAt(x, y + 1, z).legacyId()))
