...
final class SpawnLightCapProbe {
  ...
  static boolean inside(RemoteMobSpawn spawn, BlockPosition position) {
    return Math.abs(spawn.x() - (position.x() + 0.5D)) <= 4.5D && Math.abs(spawn.y() - position.y()) <= 2D
  ...
  static RemoteMobSpawn awaitDark(B173WireClient actor, BlockPosition first, BlockPosition second) {
    ...
    for (int n = 0; n < 64; n++) {
      ...
      SpawnLightCapPad.require(
          spawn.entityId() != player && type != 90 && (type == 50 || type == 51 || type == 52 || type == 54),
          "dark Packet24 identity drift");
      if ((type == 50 || type == 54) && near(spawn, first, second))
        return spawn;
    }
    throw new IllegalStateException("dark Packet24 type 50 or 54 absent near pad");
  }

  static void requireTorchReject(B173WireClient actor, BlockPosition first, BlockPosition second, int ticks) {
    ...
    while (peek != null) {
      ...
      SpawnLightCapPad.require(!((spawn.legacyType() == 50 || spawn.legacyType() == 54) && near(spawn, first, second)),
