...
public final class B173UtilityPersonalCrafts {
    ...
    private static int shears(B173PlayChannel channel, B173PlayInbound inbound, int action)
            throws IOException {
        action = step(channel, inbound, 38, 0, IRON6, replace(inbound.inventory(), 38, null), IRON6, action);
        action = step(channel, inbound, 2, 1, null, replace(inbound.inventory(), 2, item(1)), item(5), action);
        ...
        action = step(channel, inbound, 38, 0, null, replace(inbound.inventory(), 38, item(4)), null, action);
        action = step(channel, inbound, 0, 0, SHEARS,
                replace(replace(replace(inbound.inventory(), 0, null), 2, null), 3, null), SHEARS, action);
        return step(channel, inbound, 37, 0, null, replace(inbound.inventory(), 37, SHEARS), null, action);
    }

    private static int flintSteel(B173PlayChannel channel, B173PlayInbound inbound, int action)
            throws IOException {
        RemoteItemStack iron = item(4);
        action = step(channel, inbound, 38, 0, iron, replace(inbound.inventory(), 38, null), iron, action);
        action = step(channel, inbound, 1, 1, null, replace(inbound.inventory(), 1, item(1)), item(3), action);
        action = step(channel, inbound, 38, 0, null, replace(inbound.inventory(), 38, item(3)), null, action);
        action = step(channel, inbound, 39, 0, FLINT, replace(inbound.inventory(), 39, null), FLINT, action);
        ...
        action = step(channel, inbound, 0, 0, FLINT_STEEL,
                replace(replace(replace(inbound.inventory(), 0, null), 1, null), 4, null), FLINT_STEEL, action);
        return step(channel, inbound, 39, 0, null, replace(inbound.inventory(), 39, FLINT_STEEL), null, action);
    }
