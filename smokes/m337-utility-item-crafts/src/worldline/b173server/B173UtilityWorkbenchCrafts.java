...
public final class B173UtilityWorkbenchCrafts {
  ...
  public static void apply(B173WireClient actor) {
    try {
      ...
      if (window.descriptor().kind() != RemoteWindowKind.WORKBENCH
          || window.inventory().size() != 46 || !inbound.cursorObserved()
          || inbound.cursor() != null || !same(inbound.inventory(), 38, item(3))
          || !B173UtilityPersonalCrafts.stored(inbound.inventory()) || BUCKET.legacyId() != 325)
      ...
      for (int index = 0; index < cells.length; index++) {
        RemoteInventoryView after =
            replace(inbound.activeWindow().inventory(), cells[index], item(1));
        if (index == cells.length - 1)
          after = replace(after, 0, BUCKET);
  ...
  private static int takeResult(B173PlayChannel channel, B173PlayInbound inbound, int windowId,
      int action, RemoteInventoryView cleared) throws IOException {
    int next = action + 1;
  ...
  private static RemoteItemStack dec(RemoteItemStack stack) {
    if (stack == null || stack.legacyId() != IRON)
      throw new IllegalStateException("utility iron cursor drifted");
