...
/** Raised-stone pads and idle furnace 61:2 placement for remaining smelts. */
final class FurnaceRestSmeltsSupport {
  private FurnaceRestSmeltsSupport() {
  }
  ...
  static Raised raise(B173WireClient actor, RemoteChunkSnapshot chunk, int cx, int cz)
    ...
    while (water(chunk.blockAt(local(top.x(), cx), top.y() + 1, local(top.z(), cz)).legacyId())) {
      ...
      require(++column <= 15, "water column exceeded furnace-rest-smelts fixture");
    }
  ...
  static BlockPosition foundation(RemoteChunkSnapshot chunk, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic furnace-rest-smelts foundation");
  }
