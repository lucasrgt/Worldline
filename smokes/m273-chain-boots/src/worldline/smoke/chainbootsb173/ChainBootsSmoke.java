...
public final class ChainBootsSmoke {
    ...
    private static final String TRACE = "v1|server=official-b1.7.3|seed=17320110707|fixture=chain-boots305"
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        try {
            ...
            B173PlayerSeed.writeInventory(workspace, actorName, 4.5D, 70D, 4.5D,
                    new int[] {0}, new int[] {305}, new int[] {1}, new int[] {0});
            ...
            require(actor.awaitInventory().occupiedSlots() == 1
                    && actor.inventory().slot(36).item().equals(BOOTS), "chain boots source drifted");
            ...
            RemoteArmorPiece peer = observer.awaitPeerArmor(
                    new RemoteArmorPiece(actorName, RemoteArmorSlot.BOOTS, 305, 0));
            require(peer.slot() == RemoteArmorSlot.BOOTS && peer.legacyId() == 305
                    && peer.legacyId() != 301, "peer Packet5 chain boots drifted");
            ...
            String evidence = "actions=1,2;window=8:305;packet5=1:305;distinct=301;items=" + persisted.inventoryItems();
    ...
    private static void requireArmor(RemoteInventoryView view) {
        require(!view.slot(8).empty() && view.slot(8).item().equals(BOOTS) && view.slot(36).empty()
                && view.slot(8).item().legacyId() != 301, "chain boots window drifted");
    }
