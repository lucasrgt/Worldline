...
public final class SaplingDarkHaltSetSmoke {
 ...
 public static void main(String[] a) throws Exception {
  if (a.length != 9) throw new IllegalArgumentException(
    "usage: SaplingDarkHaltSetSmoke server.jar workspace port seed username chunkX chunkZ windowTicks haltWindows");
  ...
  SaplingDarkHaltSetArm.require(seed == 17320110707L && user.equals("SapDark609")
    && user.length() <= 16 && window >= 1 && window <= 1200 && windows >= 1 && windows <= 40,
  ...
  try {
   ...
   B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2},
     new int[] {1, 3, 6}, new int[] {64, 16, 16}, new int[] {0, 0, 0});
   actor.connect();
   actor.synchronizePose();
   SaplingDarkHaltSetArm.require(actor.awaitInventory().occupiedSlots() == 3,
     "sapling-dark-halt inventory drift");
   ...
   BlockPosition[] lit = new BlockPosition[] {saplings[0], saplings[1], saplings[2], saplings[3],
     saplings[4], saplings[5], saplings[6]};
   ...
   String trace = "v1|server=official-b1.7.3|seed=" + seed
     + "|fixture=raised-stone+lit-sapling6+covered-sapling6"
