...
public final class SaplingDarkHaltSetArm {
 ...
 static void waitStage(B173WireClient a, BlockPosition[] lit, BlockPosition covered, int window,
   int windows) throws Exception {
  RemoteWorldView grown = worldline.test.WorldlineSmokeAwait.awaitWorld(a,
    v -> anyStaged(v, lit) && sapling(v, covered, 0), "lit sapling stage", window * windows);
 ...
 static void persist(RemoteChunkSnapshot after, int cx, int cz, BlockPosition[] dirt,
   BlockPosition[] lit, BlockPosition covered, BlockPosition cover) {
  for (int i = 0; i < dirt.length; i++)
   require(at(after, dirt[i], cx, cz).legacyId() == 3, "dirt persist drift");
  ...
  for (int i = 0; i < lit.length; i++) {
   ...
   if (s.legacyId() == 17 || (s.legacyId() == 6 && (s.metadata() & 8) != 0)) staged = true;
  }
  require(staged, "lit sapling stage did not persist");
 }

 static void requirePlanted(RemoteWorldView v, BlockPosition[] lit, BlockPosition covered,
   BlockPosition cover) {
  StringBuilder s = new StringBuilder("cover=" + id(v, cover) + ":" + meta(v, cover)
    + ",covered=" + id(v, covered) + ":" + meta(v, covered));
  for (int i = 0; i < lit.length; i++)
 ...
 static boolean anyStaged(RemoteWorldView v, BlockPosition[] lit) {
  for (int i = 0; i < lit.length; i++) {
   int n = id(v, lit[i]);
   if (n == 17 || (n == 6 && (meta(v, lit[i]) & 8) != 0)) return true;
  }
  return false;
 }
