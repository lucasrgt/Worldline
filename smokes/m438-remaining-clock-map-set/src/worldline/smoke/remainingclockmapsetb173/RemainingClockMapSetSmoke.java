...
public final class RemainingClockMapSetSmoke{
 ...
 public static void main(String[]a)throws Exception{
  ...
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1,2,3,4},new int[]{1,58,266,331,358},new int[]{32,1,4,1,1},new int[]{0,0,0,0,0});actor.connect();actor.synchronizePose();
   ...
   actor.selectHeldSlot(1);bench=place(actor,top,BlockFace.UP,58);worldline.test.WorldlineSmokeAwait.observe(actor,5);actor.selectHeldSlot(1);actor.openWorkbench(bench,BlockFace.UP);B173RemainingClockCraft.apply(actor);requireCraft(actor.inventory());actor.closeWindow();
   ...
   worldline.test.WorldlineSmokeAwait.observe(actor,40);filled=stack(actor.inventory().slot(40));
