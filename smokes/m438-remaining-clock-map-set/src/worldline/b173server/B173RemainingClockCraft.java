...
public final class B173RemainingClockCraft {
    ...
    public static void apply(B173WireClient actor) {
        try { new B173RemainingClockCraft(actor).run(); }
        catch (IOException error) { throw new IllegalStateException("remaining-clock craft failed", error); }
    }
    ...
    private void run() throws IOException {
        ...
        pickup(39, 331, 1); placeOne(5, null); storeLeftover(39);
        pickup(38, 266, 4);
        ...
        require(!personal().slot(37).empty() && personal().slot(37).item().equals(CLOCK)
                && inbound.cursor() == null, "remaining-clock crafted inventory drifted");
    }
