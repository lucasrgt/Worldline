...
public final class ChunkTraversalSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        int steps, removedX, removedZ, addedX, addedZ; RemoteWorldView before, after;
        ChunkLifecycleFrame.Frame first, second; PlayerPose pose;
        try {
            ...
            removedX = x(removedKey); removedZ = z(removedKey); addedX = x(addedKey); addedZ = z(addedKey);
            int originX = Math.min(minX(before), minX(after)), originZ = Math.min(minZ(before), minZ(after));
            require(Math.max(maxX(before), maxX(after)) - originX < 12
                    && Math.max(maxZ(before), maxZ(after)) - originZ < 12, "chunk-map span exceeded");
        ...
        System.out.println("WORLDLINE_M33_API=server,session,movement,heartbeat,cache-lifecycle,native-render");
        System.out.println("WORLDLINE_M33_TRAVERSAL=steps=" + steps + ",chunks="
                + before.chunks().size() + "->" + after.chunks().size());
    ...
    private static Set<Long> difference(RemoteWorldView left, RemoteWorldView right) {
        Set<Long> result = keys(left); result.removeAll(keys(right)); return result;
    }
    private static Set<Long> keys(RemoteWorldView world) { Set<Long> result = new LinkedHashSet<>();
        for (RemoteChunkSnapshot chunk : world.chunks()) result.add(key(chunk.observation().x() >> 4,
                chunk.observation().z() >> 4)); return result; }
    private static int minX(RemoteWorldView world) { return world.chunks().stream()
            .mapToInt(chunk -> chunk.observation().x() >> 4).min().getAsInt(); }
    private static int maxX(RemoteWorldView world) { return world.chunks().stream()
            .mapToInt(chunk -> chunk.observation().x() >> 4).max().getAsInt(); }
    private static int minZ(RemoteWorldView world) { return world.chunks().stream()
            .mapToInt(chunk -> chunk.observation().z() >> 4).min().getAsInt(); }
    private static int maxZ(RemoteWorldView world) { return world.chunks().stream()
            .mapToInt(chunk -> chunk.observation().z() >> 4).max().getAsInt(); }
    private static long key(int x, int z) { return (long) x << 32 ^ z & 0xffffffffL; }
    private static int x(long key) { return (int) (key >> 32); }
    private static int z(long key) { return (int) key; }
