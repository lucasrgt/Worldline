...
import worldline.m93.*;

/** Binds complete depletion and reverse restoration of one six-member page. */
public final class WorldlineRecoveryEvent {
    ...
    private static final boolean[] air=new boolean[8],restored=new boolean[8];private static int step;private WorldlineRecoveryEvent(){}private static int[]fill(){int[]v=new int[12];java.util.Arrays.fill(v,-1);return v;}
    public static void head(Minecraft client){if(!WorldlinePagedBridge.running()||WorldlinePagedBridge.sealed())return;int x=WorldlineCensusSync.x(),y=WorldlineCensusSync.y(),z=WorldlineCensusSync.z(),root=WorldlineCensusProbe.nonce();WorldlineRecoveryState.apply(client.world);
        if(step<12&&readyToSend()){if(AFTER!=300||GAP!=30||!"8".equals(System.getProperty("aero.becell.rebuildsPerFrame"))||!"100000".equals(System.getProperty("aero.becell.pageTtlFrames")))throw new IllegalStateException("M93 runtime drift");int ordinal=step+1,index=indices[step],operation=operations[step],dy=index&3,dz=index>>2;MessagePacket packet=new MessagePacket(WorldlineRecoveryMod.CHANGE);packet.ints=new int[]{x,y+dy,z+dz,root,ordinal,operation,index};client.getNetworkHandler().sendPacket(packet);requests[step]=WorldlinePagedBridge.count();step++;}
    ...
    static boolean valid(){for(int i=0;i<12;i++)if(requests[i]<0||events[i]<requests[i]||i>0&&requests[i]<events[i-1]+GAP)return false;for(int i: new int[]{1,2,3,5,6,7})if(!air[i]||!restored[i])return false;return true;}
    static String diagnostic(){return "requests="+join(requests)+" events="+join(events)+" depleted="+validCells(air)+" restored="+validCells(restored);}
