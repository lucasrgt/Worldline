...
public final class WorldlineRecoveryState {
  ...
  public static synchronized void ack(int[] values) {
    if (values == null || values.length != 7 || values[4] < 1 || values[4] > 12)
      throw new IllegalStateException("invalid M93 ack");
    ...
    if (values[5] != operation || values[6] != index || ack[at] || at > 0 && !ack[at - 1])
      throw new IllegalStateException("out-of-order M93 ack");
    if (at == 0) {
      baseX = values[0];
      baseY = values[1] - dy;
      baseZ = values[2] - dz;
    ...
    if (values[0] != baseX || values[1] != baseY + dy || values[2] != baseZ + dz
      ...
      throw new IllegalStateException("conflicting M93 ack");
    ack[at] = true;
    if (operation == 1)
      removed[index] = true;
  }
  public static synchronized void restore(int[] values) {
    if (values == null || values.length != 6 || values[4] < 7 || values[4] > 12)
      throw new IllegalStateException("invalid M93 restore state");
    ...
    if (values[5] != index || !ack[removeAt] || values[0] != baseX || values[1] != baseY + dy
      ...
      throw new IllegalStateException("unbound M93 restore state");
    if (restoreReceived[index]) {
      if (restoreNonce[index] != values[3])
        throw new IllegalStateException("conflicting M93 restore state");
      return;
    }
  ...
  public static synchronized void apply(World world) {
    ...
    for (int index : new int[] {1, 2, 3, 5, 6, 7}) {
      ...
      if (raw != null && !(raw instanceof WorldlineCensusBlockEntity))
        throw new IllegalStateException("M93 restored BE type drift");
      ...
      if (be.nonce() != 0 && be.nonce() != restoreNonce[index])
        throw new IllegalStateException("M93 restored nonce drift");
