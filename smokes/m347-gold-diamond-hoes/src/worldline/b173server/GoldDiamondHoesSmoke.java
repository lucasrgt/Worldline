...
public final class GoldDiamondHoesSmoke {
  ...
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: GoldDiamondHoesSmoke server.jar workspace port seed username chunkX chunkZ");
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2, 3, 4},
          new int[] {1, 58, 266, 264, 280}, new int[] {32, 1, 2, 2, 4}, new int[] {0, 0, 0, 0, 0});
      actor.connect();
      actor.synchronizePose();
      require(actor.awaitInventory().occupiedSlots() == 5, "hoe-craft inventory seed drift");
      ...
      while (
        ...
        require(++column <= 15, "water column exceeded hoe-craft fixture");
      }
      ...
      craft(actor, n, 38, 40, 37, new int[] {1, 2}, new int[] {5, 8}, 294);
      craft(actor, n, 39, 40, 41, new int[] {1, 2}, new int[] {5, 8}, 293);
      require(has(actor, 37, 294) && has(actor, 41, 293), "gold/diamond hoe results drifted");
      actor.closeWindow();
      actor.close();
      awaitPlayers(server, 0);
      server.save();
      require(server.player(user).inventoryItems() == 3, "crafted hoe persistence drift");
      String evidence = "gold=294,diamond=293,column=" + column + ",support=" + top.x() + ":"
          + top.y() + ":" + top.z() + ":1:0,workbench=" + bench.x() + ":" + bench.y() + ":"
          + bench.z() + ":58:0,actions=" + n[0] + ",persisted=true,clients=1,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=workbench58+gold266x2+diamond264x2+stick280x4|cause=packet102-workbench-matrix+result-take|wire=packet106-accepted+packet200-craft-stat|oracle=gold-hoe-294+diamond-hoe-293|"
          + evidence;
      System.out.println("WORLDLINE_M347_CRAFT=" + evidence);
      System.out.println("WORLDLINE_M347_TRACE=" + trace);
      System.out.println("WORLDLINE_M347_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      server.close();
    }
  ...
  private static BlockPosition foundation(RemoteChunkSnapshot q, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic hoe-craft foundation");
  }
