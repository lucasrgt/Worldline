...
public final class BatchObservationSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        try {
            ...
            require(close(player.x(), finalPose.x()) && close(player.y(), finalPose.y())
                    && close(player.z(), finalPose.z()), "observed batch pose was not persisted");
        } finally { client.close(); server.close(); }
        System.out.println("WORLDLINE_M44_API=batch-observer,synchronous,route-index,embedded-index,identity");
        System.out.println("WORLDLINE_M44_EVENTS=" + describe(events) + ";batch=" + batch.termination());
        System.out.println("WORLDLINE_M44_CACHE=chunks=" + after.chunks().size());
        System.out.println("WORLDLINE_M44_PERSISTED=" + player.x() + "," + player.y() + "," + player.z());
        System.out.println("WORLDLINE_M44_TRACE=" + TRACE);
        System.out.println("WORLDLINE_M44_SIGNATURE=" + sha256(TRACE));
    }

    private static CorrelatedMovementRoutePlan plan(Object correlation, double x, double y, double z) {
        MovementAlternative alternative = new MovementAlternative(
                new MovementStep(x, y, z, 5), new MovementStep(-x, -y, -z, 5));
        return new CorrelatedMovementRoutePlan(correlation, Collections.singletonList(alternative)); }
    private static boolean indexed(CorrelatedMovementRouteBatchEvent value, int index, Object correlation) {
    ...
    private static String describe(List<CorrelatedMovementRouteBatchEvent> events) { StringBuilder value = new StringBuilder();
        for (CorrelatedMovementRouteBatchEvent event : events) { if (value.length() > 0) value.append(',');
            value.append(event.routeIndex()).append('/').append(event.event().event().alternativeIndex()).append(':')
                    .append(event.event().event().outcomeIndex()).append(':').append(event.event().event().kind()); }
