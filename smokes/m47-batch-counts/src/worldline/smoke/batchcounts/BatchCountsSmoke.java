...
public final class BatchCountsSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        try {
            ...
            CorrelatedMovementRoutePlan first = new CorrelatedMovementRoutePlan(new Object(), Arrays.asList(
                    alternative(.125D, 0D, 0D), alternative(0D, 0D, .125D)));
            CorrelatedMovementRoutePlan second = new CorrelatedMovementRoutePlan(new Object(),
                    Collections.singletonList(alternative(0D, 0D, .125D)));
            batch = client.moveCorrelatedRouteBatch(Arrays.asList(first, second),
                    event -> MovementRouteDirective.CONTINUE,
                    execution -> MovementRouteDirective.CONTINUE);
            MovementRouteBatchCounts counts = batch.counts();
            require(batch.termination() == MovementRouteBatchTermination.EXHAUSTED
                    && counts.routes() == 2 && counts.outcomes() == 3 && counts.corrections() == 0,
                    "batch aggregate counts drifted");
            require(batch.executions().size() == counts.routes()
                    && batch.finalExecution() == batch.executions().get(1), "counts replaced result identity");
            after = worldline.test.WorldlineSmokeAwait.observe(client,1); require(after.containsChunk(chunkX, chunkZ), "counted batch lost cache");
            ...
            require(close(player.x(), finalPose.x()) && close(player.y(), finalPose.y())
                    && close(player.z(), finalPose.z()), "counted batch pose was not persisted");
        } finally { client.close(); server.close(); }
        System.out.println("WORLDLINE_M47_API=batch-counts,immutable,routes,outcomes,corrections,no-replay");
        ...
        System.out.println("WORLDLINE_M47_PERSISTED=" + player.x() + "," + player.y() + "," + player.z());
