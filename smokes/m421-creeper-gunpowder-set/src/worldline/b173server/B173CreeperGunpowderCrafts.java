...
public final class B173CreeperGunpowderCrafts {
  ...
  public static int apply(B173WireClient actor) {
    try {
      return new B173CreeperGunpowderCrafts(actor).run();
    } catch (IOException error) {
      throw new IllegalStateException("creeper-gunpowder TNT craft failed", error);
    }
  }
  ...
  private int run() throws IOException {
    ...
    finish(item(TNT, 1));
    if (!has(TNT))
      throw new IllegalStateException("creeper-gunpowder TNT 46 absent");
    return TNT;
  }
