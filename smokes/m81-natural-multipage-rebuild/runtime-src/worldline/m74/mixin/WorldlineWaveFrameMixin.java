...
public abstract class WorldlineWaveFrameMixin {
    @Shadow @Final private Minecraft client;@Inject(method="onFrameUpdate(F)V",at=@At("HEAD"))private void head(float d,CallbackInfo c){WorldlineWaveEvent.head(client);}
    @Inject(method="onFrameUpdate(F)V",at=@At("TAIL"))private void tail(float d,CallbackInfo c){WorldlineWaveEvent.tail();if(WorldlineFrameCensus.sealed()&&!WorldlineWaveFile.written())WorldlineWaveFile.write();}
}
