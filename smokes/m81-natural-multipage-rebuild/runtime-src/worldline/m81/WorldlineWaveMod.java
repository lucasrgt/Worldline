...
public final class WorldlineWaveMod {
    public static final Identifier CHANGE=Identifier.of(WorldlineCensusMod.NAMESPACE,"multipage_change");static{EntrypointManager.registerLookup(MethodHandles.lookup());}public WorldlineWaveMod(){}
    @EventListener private static void messages(MessageListenerRegistryEvent event){event.register(CHANGE,(player,packet)->{if(player.world.isRemote)WorldlineWaveState.ack(packet.ints);else WorldlineWaveServer.remove(player,packet.ints);});}
}
