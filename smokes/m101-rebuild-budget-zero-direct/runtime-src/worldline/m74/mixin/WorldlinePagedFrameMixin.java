...
public abstract class WorldlinePagedFrameMixin {
  ...
  private void head(float delta, CallbackInfo ci) {
    if (!WorldlinePagedGate.prepare(client))
      WorldlinePagedTimer.head();
  }
  @Inject(method = "onFrameUpdate(F)V", at = @At("TAIL"))
  private void tail(float delta, CallbackInfo ci) {
    WorldlinePagedTimer.tail();
    if (WorldlineFrameCensus.sealed() && !WorldlinePagedFile.written())
      WorldlinePagedFile.write();
  }
}
