...
public final class WorldlinePagedTimer {
  ...
  static void arm() {
    if (armed)
      throw new IllegalStateException("duplicate M101 arm");
    if (!"1".equals(System.getProperty("aero.becell.maxCachedPages"))
        || !"0".equals(System.getProperty("aero.becell.rebuildsPerFrame"))
        || !"100000".equals(System.getProperty("aero.becell.pageTtlFrames")))
      throw new IllegalStateException("M101 runtime drift");
    armed = true;
  }
  public static void head() {
    if (!armed || WorldlinePagedBridge.sealed() || !WorldlinePagedBridge.running())
      return;
    if (capture)
      throw new IllegalStateException("M101 HEAD without TAIL");
  ...
  public static void tail() {
    ...
    if (rendererStart != 0 || queueStart != 0 || flushStart != 0)
      throw new IllegalStateException("unterminated M101 span");
    int i = WorldlinePagedBridge.count();
    if (i >= CAP || rendererCount > 0xffff || queueCount > 0xffff || flushCount > 0xffff)
      throw new IllegalStateException("M101 counter overflow");
  ...
  private static long begin(long old, String label) {
    if (old != 0)
      throw new IllegalStateException("recursive M101 " + label);
    return System.nanoTime();
  }
  private static long end(long start, String label) {
    if (start == 0)
      throw new IllegalStateException("unstarted M101 " + label);
    long v = System.nanoTime() - start;
    if (v < 0)
      throw new IllegalStateException("negative M101 " + label);
    return v;
  }
