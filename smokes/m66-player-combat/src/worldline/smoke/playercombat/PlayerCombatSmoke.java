...
public final class PlayerCombatSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    String attackerName = arguments[4], victimName = arguments[5];
    Duration timeout = Duration.ofSeconds(90);
    B173DedicatedServer server =
        new B173DedicatedServer(jar, workspace, port, seed, timeout, 3, true);
    CombatHealthSession victim = client(port, victimName, timeout),
                        attacker = client(port, attackerName, timeout);
    RemoteCombatStrike strike;
    RemoteIncomingHit hit;
    ServerPlayerState saved;
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, victimName, 8.5D, 70D, 8.5D, new int[] {0, 1, 2, 3},
          new int[] {298, 299, 300, 301}, new int[] {1, 1, 1, 1}, new int[] {0, 0, 0, 0});
      B173PlayerSeed.writeInventory(workspace, attackerName, 10.5D, 70D, 8.5D, new int[] {0},
          new int[] {276}, new int[] {1}, new int[] {0});
      victim.connect();
      victim.synchronizePose();
      ...
      for (RemoteArmorSlot slot : RemoteArmorSlot.values()) {
        RemoteItemStack item = new RemoteItemStack(slot.leatherItemId(), 1, 0);
        int source = find(victim.inventory(), item);
        require(source >= 36, "leather source absent");
        victim.equipLeatherArmor(source, slot);
      }
      require(victim.inventory().occupiedSlots() == 4, "victim armor count drifted");
      attacker.connect();
      attacker.synchronizePose();
      ...
      require(swordSlot >= 36, "diamond sword source absent");
      attacker.selectHeldSlot(swordSlot - 36);
      ...
      for (int step = 0; step < 80; step++) {
        victimAir = victim.moveAndObserve(0D, 0.3D, 0D, 1).resulting();
      ...
      strike = attacker.attackPlayer(victimName);
      worldline.test.WorldlineSmokeAwait.observe(victim, 2);
      hit = victim.awaitIncomingHit(18);
      require(strike.weaponId() == 276 && strike.hurtStatus() == 2
              && strike.target().equals(victimName) && hit.victim().equals(victimName)
              && hit.healthBefore() == 20 && hit.healthAfter() == 18 && hit.damage() == 2,
          "combat evidence drifted");
      worldline.test.WorldlineSmokeAwait.observe(attacker, 2);
      int swordDamage = attacker.inventory().slot(swordSlot).item().damage();
      require(swordDamage == 1, "diamond sword wear drifted: " + swordDamage);
      attacker.close();
      victim.close();
      awaitPlayers(server, 0);
      server.save();
      saved = server.player(victimName);
    ...
    System.out.println("WORLDLINE_M66_HIT=target=" + strike.targetEntityId()
        + ";health=" + hit.healthBefore() + "->" + hit.healthAfter() + ";damage=" + hit.damage()
        + ";saved=" + saved.health());
