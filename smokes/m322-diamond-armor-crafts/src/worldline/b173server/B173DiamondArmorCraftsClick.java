...
public final class B173DiamondArmorCraftsClick {
  ...
  private static int craft(B173PlayChannel channel, B173PlayInbound inbound, int windowId,
      int action, int diamondSlot, int dest, int[] grid, int resultId) throws IOException {
    ...
    for (int index = 0; index < grid.length; index++) {
      RemoteInventoryView after =
          replace(inbound.activeWindow().inventory(), grid[index], item(DIAMOND, 1));
  ...
  private static void requireDest(RemoteInventoryView personal, int dest, int diamondSlot) {
    if (dest < 9 || dest > 44 || dest == diamondSlot || dest == 5 || dest == 6 || dest == 7
