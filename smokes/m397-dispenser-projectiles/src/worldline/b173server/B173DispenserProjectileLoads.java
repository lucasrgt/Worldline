...
public final class B173DispenserProjectileLoads {
  ...
  public static void snowball(B173WireClient actor) {
    try {
      new B173DispenserProjectileLoads(actor).move(39, 0, SNOWBALL);
    } catch (IOException error) {
      throw new IllegalStateException("dispenser-projectile snowball load failed", error);
    }
  }

  public static void egg(B173WireClient actor) {
    try {
      new B173DispenserProjectileLoads(actor).move(40, 0, EGG);
    } catch (IOException error) {
      throw new IllegalStateException("dispenser-projectile egg load failed", error);
    }
  }
  ...
  private void move(int personalSlot, int ownedSlot, RemoteItemStack expected) throws IOException {
    ...
    if (active.descriptor().kind() != RemoteWindowKind.DISPENSER || before.size() != 45
      ...
      throw new IllegalStateException(
          "dispenser-projectile move requires occupied source and empty target");
    prepare(before, 2);
  ...
  private void prepare(RemoteInventoryView view, int actions) {
    ...
    if (action > 32767 - actions)
      throw new IllegalStateException("dispenser-projectile transaction counter exhausted");
  }
