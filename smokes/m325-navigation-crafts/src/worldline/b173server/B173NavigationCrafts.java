...
public final class B173NavigationCrafts {
  ...
  public static void apply(B173WireClient actor) {
    try {
      new B173NavigationCrafts(actor).run();
    } catch (IOException error) {
      throw new IllegalStateException("navigation crafts failed", error);
    }
  }
  ...
  private void run() throws IOException {
    ...
    require(!personal().slot(37).empty() && personal().slot(37).item().equals(COMPASS)
            ...
            && inbound.cursor() == null,
        "navigation crafted inventory drifted");
  }

  private void craft(int centerPersonal, int centerId, int ringPersonal, int ringId,
      int destPersonal, int[] ring, RemoteItemStack result) throws IOException {
    pickup(centerPersonal, centerId, 1);
    placeOne(5, null);
    storeLeftover(centerPersonal);
