...
public final class RouteRecoverySmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      ...
      PlayerPose predicted = new PlayerPose(
          initial.x() + .125D, initial.y(), initial.z(), initial.yaw(), initial.pitch());
      BlockPosition block = solid(before, predicted);
      ...
      require(outcomes.get(0).disposition() == MovementDisposition.UNCHALLENGED
              && outcomes.get(0).resulting().equals(predicted),
          "route first step drifted");
      ...
      require(outcomes.get(2).disposition() == MovementDisposition.UNCHALLENGED
              && close(route.finalPose().x(), predicted.x() + .125D),
          "route did not recover after correction");
      require(route.corrections() == 1, "route correction count drifted");
      after = worldline.test.WorldlineSmokeAwait.observe(client, 1);
      require(after.containsChunk(chunkX, chunkZ), "route lost cached chunk");
      client.close();
      ...
      require(close(player.x(), route.finalPose().x()) && close(player.y(), route.finalPose().y())
          ...
          "recovered route pose was not persisted");
    } finally {
      client.close();
      server.close();
    }
    System.out.println(
        "WORLDLINE_M36_API=movement-route,ordered-outcomes,correction-recovery,cache,persistence");
    ...
    System.out.println(
        "WORLDLINE_M36_PERSISTED=" + player.x() + "," + player.y() + "," + player.z());
