...
public final class BoatCurrentPushChannel {
  ...
  private BoatCurrentPushChannel(
      BlockPosition source, BlockPosition flow, BlockPosition south, int column, int fluid, int meta) {
    this.source = source;
    this.flow = flow;
    this.south = south;
    this.column = column;
  ...
  static BoatCurrentPushChannel build(B173WireClient actor, RemoteChunkSnapshot initial, int cx, int cz)
      throws Exception {
    ...
    for (int index = 0; index < floor.length; index++) {
      ...
      place(actor, south, BlockFace.UP, 1);
    }
    BlockPosition west = place(actor, top, BlockFace.WEST, 1);
    place(actor, west, BlockFace.UP, 1);
    place(actor, e4, BlockFace.UP, 1);
    ...
    BlockState flowing = worldline.test.WorldlineSmokeAwait.awaitBlock(actor, flow, new BlockState(8, 2), 80)
                             .blockAt(flow.x(), flow.y(), flow.z());
    return new BoatCurrentPushChannel(source, flow, BlockFace.UP.adjacent(BlockFace.SOUTH.adjacent(e2)), column,
  ...
  static void station(B173WireClient actor, double x, double y, double z) {
    ...
    PlayerPose here = actor.moveAndObserve(0D, 0D, 0D, 1).resulting();
    require((x - here.x()) * (x - here.x()) + (z - here.z()) * (z - here.z()) <= 2.25D && Math.abs(y - here.y()) <= 3D,
