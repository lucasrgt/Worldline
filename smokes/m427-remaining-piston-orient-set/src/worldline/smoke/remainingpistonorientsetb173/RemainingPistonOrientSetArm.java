...
public final class RemainingPistonOrientSetArm {
  static BlockPosition raise(B173WireClient a, RemoteChunkSnapshot initial, int cx, int cz,
    ...
    while (water(at(initial, BlockFace.UP.adjacent(top), cx, cz).legacyId())) {
      ...
      require(++column[0] <= 15, "water column exceeded remaining-piston-orient fixture");
    }
  ...
  static BlockPosition stack(B173WireClient a, BlockPosition support, int n) throws Exception {
    a.selectHeldSlot(0);
    BlockPosition p = support;
    for (int i = 0; i < n; i++)
      p = place(a, p, BlockFace.UP, 1);
    return p;
  }
  static BlockPosition piston(B173WireClient a, BlockPosition support, int slot, int id, int meta,
      float yaw) throws Exception {
    BlockPosition t = BlockFace.UP.adjacent(support);
    a.selectHeldSlot(slot);
    a.look(yaw, 0F);
    a.placeHeldBlock(support, BlockFace.UP);
    BlockState got = worldline.test.WorldlineSmokeAwait.observe(a, 5).blockAt(t.x(), t.y(), t.z());
    require(got.equals(new BlockState(id, meta)),
        "live piston " + id + ":" + meta + " facing drift: " + got + " at " + cell(t));
    return t;
  }
  ...
  static BlockPosition foundation(RemoteChunkSnapshot c, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic remaining-piston-orient foundation");
  }
  ...
  static void persist(
      RemoteChunkSnapshot after, int cx, int cz, BlockPosition p, int id, int meta, String label) {
    require(at(after, p, cx, cz).equals(new BlockState(id, meta)),
        label + " persist drift: " + at(after, p, cx, cz));
  }
