...
public final class ItemStackMergeSetSmoke {
    ...
    private static final String TRACE = "v1|server=official-b1.7.3|seed=17320110707|"
            ...
            + "wire=packet21x2-count1|oracle=item-stack-merge-absent-not-despawn-age|"
            + EVIDENCE;
    ...
    public static void main(String[] arguments) throws Exception {
        if (arguments.length != 5) throw new IllegalArgumentException(
                "usage: ItemStackMergeSetSmoke server.jar workspace port seed username");
        Path jar = Paths.get(arguments[0]), workspace = Paths.get(arguments[1]);
        int port = Integer.parseInt(arguments[2]);
        long seed = Long.parseLong(arguments[3]);
        String user = arguments[4];
        B173ItemStackMerge.require(seed == 17320110707L && user.equals("ItemMerge581")
                && user.length() <= 16, "item-stack-merge identity drift");
        ...
        try {
            ...
            B173ItemStackMerge.require(actor.awaitInventory().occupiedSlots() == 2,
                    "item-stack-merge inventory seed drift");
            actor.selectHeldSlot(0);
            actor.look(0F, 90F);
            actor.dropHeldItem();
            RemoteDroppedItem first = actor.awaitDroppedItem(STONE);
            B173ItemStackMerge.require(first.item().equals(STONE)
                    && first.item().count() == 1, "first Packet21 stone 1x1 absent");
            worldline.test.WorldlineSmokeAwait.awaitEntity(actor, actor::inventory,
                    view -> view.slot(36).empty(), "first dropped slot", 10);
            actor.selectHeldSlot(1);
            actor.dropHeldItem();
            RemoteDroppedItem second = worldline.test.WorldlineSmokeAwait.awaitEntity(
                    actor, () -> actor.peekDroppedItem(STONE),
                    drop -> drop != null && drop.entityId() != first.entityId(),
                    "second Packet21 stone", 20);
            B173ItemStackMerge.require(second.item().equals(STONE)
                    && second.item().count() == 1
                    && second.entityId() != first.entityId(),
                    "second Packet21 stone 1x1 absent");
            worldline.test.WorldlineSmokeAwait.awaitEntity(actor, actor::inventory,
                    view -> view.occupiedSlots() == 0, "both dropped slots", 10);
            double dx = second.x() - first.x(), dz = second.z() - first.z();
            B173ItemStackMerge.require(dx * dx + dz * dz <= 1D,
                    "dropped stones were not in contact range");
            ...
            B173ItemStackMerge.require(B173ItemStackMerge.live(actor, STONE) == 2
                    ...
                    && !B173ItemStackMerge.collected(actor, second.entityId())
                    && first.item().count() == 1 && second.item().count() == 1
                    && actor.inventory().occupiedSlots() == 0,
            ...
            System.out.println("WORLDLINE_M581_SIGNATURE="
                    + B173ItemStackMerge.sha(TRACE));
        } finally {
            actor.close();
            server.close();
        }
