...
public final class B173OreBlockCrafts {
    ...
    public static void apply(B173WireClient actor) {
        try { new B173OreBlockCrafts(actor).run(); }
        catch (IOException error) { throw new IllegalStateException("ore-block crafts failed", error); }
    }

    private void run() throws IOException {
        require(GOLD.legacyId() == 41 && IRON.legacyId() == 42 && DIAMOND.legacyId() == 57
                && LAPIS.legacyId() == 22, "ore-block result identity drifted");
    ...
    private void craft(int personal, RemoteItemStack ingredient, RemoteItemStack result) throws IOException {
        ...
        for (int index = 0; index < CELLS.length; index++) {
            ...
            if (index == CELLS.length - 1) after = replace(after, 0, result);
