...
public final class B173DispenserSetLoads {
    ...
    public static void planks(B173WireClient actor) {
        try { new B173DispenserSetLoads(actor).move(40, 1, PLANKS); }
        catch (IOException error) { throw new IllegalStateException("dispenser-set planks load failed", error); }
    }
    ...
    private void move(int personalSlot, int ownedSlot, RemoteItemStack expected) throws IOException {
        ...
        if (active.descriptor().kind() != RemoteWindowKind.DISPENSER || before.size() != 45
            ...
            throw new IllegalStateException("dispenser-set move requires occupied source and empty target");
    ...
    private void prepare(RemoteInventoryView view, int actions) {
        ...
        if (action > 32767 - actions) throw new IllegalStateException("dispenser-set transaction counter exhausted");
    }
