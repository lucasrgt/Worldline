  ...
  public static void main(String[] arguments) throws Exception {
    if (arguments.length != 9)
      throw new IllegalArgumentException(
          "usage: RedstoneWirePowerSmoke server.jar workspace port seed username chunkX chunkZ fixtureTicks signalTicks");
    ...
    try {
      ...
      wireOff = before.blockAt(local(wire.x(), chunkX), wire.y(), local(wire.z(), chunkZ));
      require(wireOff.equals(new BlockState(55, 0)), "wire powered before treatment");
      ...
      require(leverOn.legacyId() == 69 && leverOn.metadata() != leverOff.metadata()
              && wireOn.legacyId() == 55 && wireOn.metadata() > 0,
          "lever signal did not power wire: " + leverOn + " / " + wireOn);
      actor.close();
    ...
    require(delta.changed == 2, "signal changed unrelated states: " + delta);
    ...
    String trace = "v1|server=official-b1.7.3|seed=" + seed
        ...
        + "ticks|cause=packet15-lever-activate|effect=packet53-wire-power|observation=fresh-login-packet51|"
        + evidence + "|disconnect=clean";
    System.out.println("WORLDLINE_M116_SIGNAL=" + evidence);
    System.out.println("WORLDLINE_M116_TRACE=" + trace);
