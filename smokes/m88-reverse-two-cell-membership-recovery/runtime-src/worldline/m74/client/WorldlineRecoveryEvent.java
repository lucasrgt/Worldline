...
public final class WorldlineRecoveryEvent {
  ...
  public static void head(Minecraft client) {
    ...
    if (step < 4 && readyToSend()) {
      if (AFTER != 300 || GAP != 30
        ...
        throw new IllegalStateException("M88 runtime drift");
    ...
    if (step > 0) {
      int at = step - 1, index = indices[at], generation = at / 2 + 1, ty = y + index;
      if (step % 2 == 1)
  ...
  public static void tail() {
    ...
    if (events[at] >= 0)
      return;
    int index = indices[at], generation = at / 2 + 1, operation = at % 2 + 1,
