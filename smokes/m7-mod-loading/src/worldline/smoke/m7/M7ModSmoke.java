...
public final class M7ModSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        if ("run".equals(arguments[0]) && arguments.length == 3) run(jar, Integer.parseInt(arguments[2]));
        else if ("reject".equals(arguments[0]) && arguments.length == 2) reject(jar);
        else throw new IllegalArgumentException("invalid M7 smoke mode");
    }
    ...
    private static void run(Path jar, int expectedBlock) throws Exception {
        ...
        try (LoadedMod<B173Mod> loaded = ModLoader.load(jar, RUNTIME, API, B173Mod.class)) {
            ...
            try {
                ...
                runtime.installMod(loaded.instance()); runtime.tick();
