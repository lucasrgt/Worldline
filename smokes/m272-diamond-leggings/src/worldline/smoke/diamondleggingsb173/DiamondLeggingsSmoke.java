...
public final class DiamondLeggingsSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    RemoteArmorSlot legs = RemoteArmorSlot.LEGGINGS;
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(workspace, actorName, 4.5D, 70D, 4.5D, new int[] {0},
          new int[] {ITEM}, new int[] {1}, new int[] {0});
      ...
      require(actor.awaitInventory().occupiedSlots() == 1
              ...
              && actor.inventory().slot(WINDOW).empty(),
          "diamond-leggings seed drifted");
      observer.connect();
      ...
      require(actor.inventory().slot(WINDOW).item().equals(stack)
              ...
              && actor.inventory().slot(SOURCE).empty(),
          "diamond-leggings equip drifted");
      RemoteArmorPiece peer =
          observer.awaitPeerArmor(new RemoteArmorPiece(actorName, legs, ITEM, 0));
      require(peer.slot() == legs && peer.legacyId() == ITEM && peer.legacyId() != LEATHER
              && peer.slot().equipmentSlot() == PACKET5,
          "peer diamond-leggings Packet5 drifted");
      requireArmor(actor.inventory(), stack);
      actor.close();
      observer.close();
      ...
      requireArmor(restored.awaitInventory(), stack);
      witness.connect();
      ...
      require(server.player(actorName).inventoryItems() == 1,
          "restored diamond-leggings count drifted");
      String evidence = "window=" + WINDOW + ":" + ITEM + ",packet5=" + PACKET5 + ":" + ITEM
          + ",leather=" + LEATHER
          + ",distinct=true,actions=1,2,persisted=true,clients=4,disconnect=clean";
  ...
  private static void requireArmor(RemoteInventoryView view, RemoteItemStack stack) {
    require(!view.slot(WINDOW).empty() && view.slot(WINDOW).item().equals(stack)
            && view.slot(WINDOW).item().legacyId() != LEATHER && view.slot(SOURCE).empty()
            && view.occupiedSlots() == 1,
        "diamond-leggings window drifted");
  }
