...
public final class PlayerMovementSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        PlayerPose initial, target; ServerPlayerState player;
        try {
            server.boot(); client.connect();
            awaitPlayers(server, Collections.singletonList(username), 5000L);
            initial = client.synchronizePose(); target = client.moveBy(dx, dy, dz);
            require(close(target.x(), initial.x() + dx) && close(target.y(), initial.y() + dy)
                    && close(target.z(), initial.z() + dz), "client movement target drifted");
            Thread.sleep(300L); client.close();
            ...
            require(close(target.x(), player.x()) && close(target.y(), player.y())
                    && close(target.z(), player.z()), "official server did not persist target position");
        } finally { client.close(); server.close(); }
        System.out.println("WORLDLINE_M25_API=login,position-ack,move,logout,save,player-position");
        System.out.println("WORLDLINE_M25_SOURCE=" + B173WireClient.class.getProtectionDomain()
                .getCodeSource().getLocation());
        System.out.println("WORLDLINE_M25_INITIAL=" + pose(initial));
        System.out.println("WORLDLINE_M25_TARGET=" + pose(target));
        System.out.println("WORLDLINE_M25_PERSISTED=" + player.x() + "," + player.y() + "," + player.z());
