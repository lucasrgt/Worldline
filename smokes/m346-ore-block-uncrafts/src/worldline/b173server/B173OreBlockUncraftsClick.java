...
public final class B173OreBlockUncraftsClick {
  ...
  public static void gold(B173WireClient actor) {
    uncraft(actor, 1, 36, GOLD_BLOCK, GOLD_INGOTS);
  }
  public static void iron(B173WireClient actor) {
    uncraft(actor, 5, 37, IRON_BLOCK, IRON_INGOTS);
  }
  public static void diamond(B173WireClient actor) {
    uncraft(actor, 9, 38, DIAMOND_BLOCK, DIAMONDS);
  }
  public static void lapis(B173WireClient actor) {
    uncraft(actor, 13, 39, LAPIS_BLOCK, LAPIS);
  }
  ...
  private static void uncraft(
      B173WireClient actor, int action, int hotbar, RemoteItemStack block, RemoteItemStack result) {
    try {
      ...
      step(channel, inbound, action + 1, 1, null,
          with(with(inbound.inventory(), 1, block), 0, result), null);
      ...
      step(channel, inbound, action + 3, hotbar, null, with(inbound.inventory(), hotbar, result),
          null);
