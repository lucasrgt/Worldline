...
public final class OreBlockUncraftsSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        require(B173OreBlockUncraftsClick.GOLD_BLOCK.legacyId() == 41
                ...
                && B173OreBlockUncraftsClick.LAPIS.damage() == 4, "ore-block uncraft identities drifted");
        ...
        try {
            ...
            B173PlayerSeed.writeInventory(workspace, user, 4.5D, 72D, 4.5D,
                    new int[] {0, 1, 2, 3}, new int[] {41, 42, 57, 22}, new int[] {1, 1, 1, 1}, new int[] {0, 0, 0, 0});
            actor.connect(); actor.synchronizePose();
            RemoteInventoryView initial = actor.awaitInventory();
            require(initial.occupiedSlots() == 4 && initial.slot(36).item().equals(B173OreBlockUncraftsClick.GOLD_BLOCK)
                    ...
                    && B173OreBlockUncraftsClick.emptyCraft(initial), "ore-block seed drifted");
            ...
            require(actor.inventory().slot(36).item().equals(B173OreBlockUncraftsClick.GOLD_INGOTS)
                    ...
                    && B173OreBlockUncraftsClick.emptyCraft(actor.inventory()), "uncrafted gold 266 drifted");
            ...
            require(actor.inventory().slot(37).item().equals(B173OreBlockUncraftsClick.IRON_INGOTS)
                    ...
                    && B173OreBlockUncraftsClick.emptyCraft(actor.inventory()), "uncrafted iron 265 drifted");
            ...
            require(actor.inventory().slot(38).item().equals(B173OreBlockUncraftsClick.DIAMONDS)
                    ...
                    && B173OreBlockUncraftsClick.emptyCraft(actor.inventory()), "uncrafted diamond 264 drifted");
            B173OreBlockUncraftsClick.lapis(actor);
            requireStored(actor.inventory());
            actor.close(); awaitPlayers(server, 0); server.save();
            require(server.player(user).inventoryItems() == 4, "ore-block uncraft persistence count drifted");
    ...
    private static void requireStored(RemoteInventoryView view) {
        require(B173OreBlockUncraftsClick.stored(view)
                && view.slot(36).item().legacyId() == 266 && view.slot(37).item().legacyId() == 265
                && view.slot(38).item().legacyId() == 264 && view.slot(39).item().legacyId() == 351
                && view.slot(39).item().damage() == 4, "stored ore-block uncrafts 266+265+264+351 drifted"); }
