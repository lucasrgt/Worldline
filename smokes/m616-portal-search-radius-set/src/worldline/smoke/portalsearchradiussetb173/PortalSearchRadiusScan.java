...
final class PortalSearchRadiusScan {
  ...
  static PlayerPose enter(
      B173WireClient actor, PlayerPose pose, double x, double y, double z, int travel, int dimension) throws Exception {
    pose = go(actor, pose, x, y, z);
    worldline.test.WorldlineSmokeAwait.observe(actor, travel);
    PortalSearchRadiusKit.require(
        actor.awaitDimension(dimension) == dimension, "portal-search-radius Packet9 " + dimension + " absent");
  ...
  static Portal find(RemoteWorldView view, PlayerPose pose, int stayDimension) {
    int minX = Integer.MAX_VALUE, maxX = Integer.MIN_VALUE, minY = 999, maxY = -1, minZ = Integer.MAX_VALUE,
        maxZ = Integer.MIN_VALUE, count = 0;
    ...
    PortalSearchRadiusKit.require(
        count == 6 && maxY - minY == 2 && ((maxX - minX == 1 && maxZ == minZ) || (maxZ - minZ == 1 && maxX == minX)),
  ...
  static int near90(RemoteWorldView view, int originX, int originZ, int radius) {
    ...
    for (RemoteChunkSnapshot chunk : view.chunks()) {
      ...
      for (int x = 0; x < 16; x++)
        for (int z = 0; z < 16; z++)
          for (int y = 0; y < 128; y++)
            if (chunk.blockAt(x, y, z).legacyId() == 90 && Math.abs(baseX + x - originX) <= radius
                && Math.abs(baseZ + z - originZ) <= radius)
              count++;
    }
    return count;
  }
  ...
  static String prove(Portal existing, PlayerPose dest, RemoteWorldView world, int shift) {
    ...
    PortalSearchRadiusKit.require(
        sky(world.chunkAt(((int) Math.floor(dest.x())) >> 4, ((int) Math.floor(dest.z())) >> 4)) == 0,
        "search destination is not Nether");
    PortalSearchRadiusKit.require(
        same(arrived, existing) && arrived.count == 6, "search trip missed the existing Nether frame");
    PortalSearchRadiusKit.require(dist > PortalSearchRadiusKit.CREATE && dist <= PortalSearchRadiusKit.SEARCH,
    ...
    return "dimensions=0->-1,shift=" + shift + ",search=existing,radius=" + PortalSearchRadiusKit.SEARCH
        + ",create-window=" + PortalSearchRadiusKit.CREATE
