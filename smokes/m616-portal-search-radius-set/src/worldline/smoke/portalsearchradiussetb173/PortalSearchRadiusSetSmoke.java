...
public final class PortalSearchRadiusSetSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    if (arguments.length != 10)
      throw new IllegalArgumentException("usage: PortalSearchRadiusSetSmoke server.jar workspace port seed username "
    ...
    PortalSearchRadiusKit.require(
        seed == 17320110707L && user.equals("PortalSrch616") && user.length() <= 16 && shift >= 24 && shift <= 48,
    ...
    try {
      ...
      PortalSearchRadiusKit.require(actor.dimension() == 0 && actor.awaitInventory().occupiedSlots() == 3,
      ...
      pose = actor.moveAndObserve(0D, 0D, 0D, 1).resulting();
      pose = PortalSearchRadiusScan.go(actor, pose, first.enterX(), first.enterY(), first.enterZ());
      PortalSearchRadiusKit.light(actor, first, portalTicks);
      PortalSearchRadiusKit.require(actor.dimension() == 0, "source portal traveled during activation");
      pose = PortalSearchRadiusScan.enter(actor, pose, first.enterX(), first.enterY(), first.enterZ(), travelTicks, -1);
      ...
      RemoteChunkSnapshot netherChunk =
          nether.chunkAt(((int) Math.floor(pose.x())) >> 4, ((int) Math.floor(pose.z())) >> 4);
      PortalSearchRadiusKit.require(PortalSearchRadiusScan.sky(netherChunk) == 0, "generated Nether portal drift");
      ...
      actor =
          relogOverworld(actor, server, workspace, user, timeout, land.x() + 0.5D, land.y() + 4.5D, land.z() + 0.5D);
      pose = actor.moveAndObserve(0D, 0D, 0D, 1).resulting();
      ...
      pose = actor.moveAndObserve(0D, 0D, 0D, 1).resulting();
      pose = PortalSearchRadiusScan.go(actor, pose, source.enterX(), source.enterY(), source.enterZ());
      PortalSearchRadiusKit.light(actor, source, portalTicks);
      PortalSearchRadiusKit.require(actor.dimension() == 0, "offset portal traveled during activation");
      pose =
          PortalSearchRadiusScan.enter(actor, pose, source.enterX(), source.enterY(), source.enterZ(), travelTicks, -1);
      ...
      server.save();
      PortalSearchRadiusKit.require(
          server.player(user).dimension() == -1, "searched player dimension was not persisted");
  ...
  private static B173WireClient relogOverworld(B173WireClient actor, B173DedicatedServer server, Path workspace,
      String user, Duration timeout, double x, double y, double z) throws Exception {
    B173WireClient next = PortalSearchRadiusKit.relog(actor, server, workspace, user, timeout, x, y, z, 0);
    next.synchronizePose();
    PortalSearchRadiusKit.require(next.dimension() == 0 && next.awaitInventory().occupiedSlots() == 3,
