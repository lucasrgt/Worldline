...
final class PortalSearchRadiusKit {
    ...
    static BlockPosition farFoundation(RemoteChunkSnapshot chunk, int chunkX, int chunkZ) {
        for (int x = 4; x <= 10; x++) for (int z = 4; z <= 11; z++) for (int y = 120; y >= 40; y--) {
            int id = chunk.blockAt(x, y, z).legacyId();
            if ((id == 1 || id == 2 || id == 3 || id == 12 || id == 13 || id == 24)
                    && clear(chunk, x, y, z))
                return new BlockPosition(chunkX * 16 + x, y, chunkZ * 16 + z);
        }
    ...
    private static boolean clear(RemoteChunkSnapshot chunk, int x, int y, int z) {
        int column = 0;
        while (column < 15 && y + 1 + column < 128
                && water(chunk.blockAt(x, y + 1 + column, z).legacyId())) column++;
        int height = column + 6;
        if (y + height >= 128) return false;
        for (int dx = 0; dx < 4; dx++) for (int dy = 1; dy <= height; dy++) {
            int id = chunk.blockAt(x + dx, y + dy, z).legacyId();
            if (id != 0 && !water(id)) return false;
        }
        return true;
    }

    static Raised raise(B173WireClient actor, RemoteChunkSnapshot chunk, int chunkX, int chunkZ,
            BlockPosition anchor) throws Exception {
        int column = 0;
        actor.selectHeldSlot(0);
        while (water(chunk.blockAt(local(anchor.x(), chunkX), anchor.y() + 1,
                local(anchor.z(), chunkZ)).legacyId())) {
    ...
    static void light(B173WireClient actor, Frame frame, int settle) throws Exception {
        ...
        actor.useHeldItemOnBlock(new BlockPosition(frame.bottom.x() + 1, frame.bottom.y(),
                frame.bottom.z()), BlockFace.UP);
        RemoteWorldView active = worldline.test.WorldlineSmokeAwait.observe(actor, settle);
        int portals = 0;
        for (int y = 1; y <= 3; y++) for (int x = 1; x <= 2; x++) {
            BlockState state = active.blockAt(frame.bottom.x() + x, frame.bottom.y() + y,
                    frame.bottom.z());
    ...
    static B173WireClient relog(B173WireClient actor, B173DedicatedServer server, Path workspace,
            String user, Duration timeout, double x, double y, double z, int dimension)
            throws Exception {
        actor.close();
