...
public final class GlowstoneDustCraftsSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    require(B173GlowstoneDustCrafts.DUST4.legacyId() == 348
            && B173GlowstoneDustCrafts.GLOWSTONE.legacyId() == 89,
        "glowstone dust identities drifted");
    B173DedicatedServer server =
        new B173DedicatedServer(jar, workspace, port, seed, timeout, 3, true);
    B173WireClient actor = client(port, user, timeout), reader = null;
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 72D, 4.5D, new int[] {0},
          new int[] {348}, new int[] {4}, new int[] {0});
      actor.connect();
      actor.synchronizePose();
      RemoteInventoryView initial = actor.awaitInventory();
      require(initial.occupiedSlots() == 1
              && initial.slot(36).item().equals(B173GlowstoneDustCrafts.DUST4)
              && initial.slot(36).item().legacyId() == 348
              && B173GlowstoneDustCrafts.emptyCraft(initial),
      ...
      server.save();
      require(server.player(user).inventoryItems() == 1,
          "glowstone dust craft persistence count drifted");
  ...
  private static void requireStored(RemoteInventoryView view) {
    require(!view.slot(36).empty() && view.slot(36).item().equals(B173GlowstoneDustCrafts.GLOWSTONE)
        ...
        "stored glowstone 89 from dust 348 drifted");
  }
