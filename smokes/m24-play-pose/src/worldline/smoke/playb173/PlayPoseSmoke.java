  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      ...
      require(close(initial.x(), player.x()) && close(initial.y(), player.y())
              && close(initial.z(), player.z()),
          "acknowledged position did not persist");
      require(player.yaw() == yaw && player.pitch() == pitch, "requested rotation did not persist");
    } finally {
      client.close();
      server.close();
    }
    System.out.println("WORLDLINE_M24_API=login,spawn-time,position-ack,look,save,player-pose");
    System.out.println("WORLDLINE_M24_SOURCE="
        + B173WireClient.class.getProtectionDomain().getCodeSource().getLocation());
    System.out.println("WORLDLINE_M24_INITIAL=" + pose(initial));
    System.out.println("WORLDLINE_M24_PERSISTED=" + player.x() + "," + player.y() + "," + player.z()
