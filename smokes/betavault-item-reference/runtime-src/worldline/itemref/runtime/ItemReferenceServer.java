...
public final class ItemReferenceServer {
  ...
  private static void connected(PlayerPacketHandlerSetEvent event) {
    if (player != null)
      throw new IllegalStateException("item-reference smoke supports one client");
    player = event.player;
    ticks = 0;
  }
  ...
  private static void tick(GameTickEvent.End event) {
    ...
    try {
      if (phase.equals("create"))
        create(current, vault);
      else if (phase.equals("reload"))
        reload(current, vault);
      else
        throw new IllegalStateException("invalid item-reference phase");
      current.playerScreenHandler.sendContentUpdates();
    } finally {
      current.skipPacketSlotUpdates = false;
    }
  ...
  private static void create(ServerPlayerEntity player, BE_CellVault vault) {
    ...
    require(!stack.isItemEqual(distinct) && !stack.equals(distinct)
            && !ItemStack.areEqual(stack, distinct),
        "distinct cells compared equal");
  ...
  private static void reload(ServerPlayerEntity player, BE_CellVault vault) {
    ...
    System.out.println(
        "WORLDLINE_ITEMREF_RELOAD=PASS ref=" + reference.canonical() + " amount=100 nbt=preserved");
  }
