package worldline.smoke.falldamagesetb173;
...
/** Walks off two raised columns so Packet8 records strictly more damage from the taller fall. */
public final class FallDamageSetSmoke{
 private FallDamageSetSmoke(){}
 public static void main(String[]a)throws Exception{
  if(a.length!=7)throw new IllegalArgumentException("usage: FallDamageSetSmoke server.jar workspace port seed username chunkX chunkZ");
  ...
  require(seed==17320110707L&&user.equals("FallDmg461")&&user.length()<=16,"fall-damage-set identity drift");
  Duration timeout=Duration.ofSeconds(180);B173DedicatedServer server=new B173DedicatedServer(jar,workspace,port,seed,timeout,3,true);B173WireClient actor=new B173WireClient("127.0.0.1",port,user,timeout),reader=null;BlockPosition top,pad;int column,shortDrop,tallDrop,shortAfter,tallAfter;double airY;PlayerPose pose;
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1},new int[]{1,322},new int[]{64,2},new int[]{0,0});actor.connect();pose=actor.synchronizePose();require(actor.awaitInventory().occupiedSlots()==2&&actor.awaitHealth(20)==20,"fall-damage-set inventory or health drift");RemoteChunkSnapshot initial=actor.awaitRemoteChunk(cx,cz).chunkAt(cx,cz);top=foundation(initial,cx,cz);column=0;actor.selectHeldSlot(0);
   while(water(initial.blockAt(local(top.x(),cx),top.y()+1,local(top.z(),cz)).legacyId())){top=place(actor,top,BlockFace.UP,1);pose=actor.moveAndObserve(0D,1D,0D,1).resulting();require(++column<=15,"water column exceeded fall-damage-set fixture");}
   top=place(actor,top,BlockFace.UP,1);pose=actor.moveAndObserve(0D,1D,0D,1).resulting();column++;pad=place(actor,top,BlockFace.EAST,1);airY=pad.y()+3.0D;
   for(int n=0;n<8;n++){top=place(actor,top,BlockFace.UP,1);pose=actor.moveAndObserve(0D,1D,0D,1).resulting();column++;}
   pose=walkOff(actor,pose,top,pad);shortDrop=(int)Math.round(pose.y()-airY);require(shortDrop>=4&&over(pose,pad)&&actor.health()==20,"short-fall height drift drop="+shortDrop);
   pose=land(actor,pose,airY);shortAfter=B173Fall.awaitDrop(actor,20);require(shortAfter<20,"short-fall Packet8 drift "+shortAfter);
   actor.selectHeldSlot(1);actor.useSelectedItemInAir();heal(actor,20);actor.selectHeldSlot(0);
   actor.look(0F,70F);BlockPosition tall=place(actor,pad,BlockFace.NORTH,1);
   ...
   actor.close();awaitPlayers(server,0);server.save();require(server.player(user).health()==tallAfter,"persisted fall-damage health drift");
   reader=new B173WireClient("127.0.0.1",port,user,timeout);reader.connect();reader.synchronizePose();require(reader.awaitHealth(tallAfter)==tallAfter,"fresh-login fall-damage health drift");
   String evidence="column="+column+",support="+top.x()+":"+top.y()+":"+top.z()+":1:0,pad="+pad.x()+":"+pad.y()+":"+pad.z()+":1:0,short="+shortDrop+":20->"+shortAfter+",tall="+tallDrop+":20->"+tallAfter+",heal=322,status=2,taller=true,persisted=true,clients=2,disconnect=clean";
   String trace="v1|server=official-b1.7.3|seed="+seed+"|fixture=raised-stone+east-pad+drop"+shortDrop+"+drop"+tallDrop+"+golden-apple322|cause=packet13-ungrounded-walk-off+packet15-item322|wire=packet38-status2+packet8-health20->"+shortAfter+"/20->"+tallAfter+"|oracle=fall-damage-set-taller-hurts-more+fresh-login|"+evidence;
   System.out.println("WORLDLINE_M461_SET="+evidence);System.out.println("WORLDLINE_M461_TRACE="+trace);System.out.println("WORLDLINE_M461_SIGNATURE="+sha(trace));
