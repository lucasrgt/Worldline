...
public final class RsNorLatchSetSmoke{
 ...
 public static void main(String[]a)throws Exception{
  ...
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1,2,3,4},new int[]{1,356,331,76,69},new int[]{48,2,16,2,2},new int[]{0,0,0,0,0});actor.connect();actor.synchronizePose();require(actor.awaitInventory().occupiedSlots()==5,"rs-nor inventory drift");
   ...
   actor.selectHeldSlot(5);Thread.sleep(8000L);actor.awaitBlock(f.q,qOff);worldline.test.WorldlineSmokeAwait.awaitBlock(actor,f.qbar,barOn,5);
   f.armHold(actor);actor.selectHeldSlot(5);Thread.sleep(2000L);require(worldline.test.WorldlineSmokeAwait.observe(actor,10).blockAt(f.q.x(),f.q.y(),f.q.z()).equals(qOff)&&worldline.test.WorldlineSmokeAwait.observe(actor,1).blockAt(f.qbar.x(),f.qbar.y(),f.qbar.z()).equals(barOn),"hold arm raced the RESET pair");
   actor.moveAndObserve(2D,0D,0D,2);pulse(actor,f.set);worldline.test.WorldlineSmokeAwait.awaitBlock(actor,f.qbar,barOff,10);
 ...
 private static void pulse(B173WireClient a,BlockPosition lever)throws Exception{a.activateBlock(lever,BlockFace.UP);worldline.test.WorldlineSmokeAwait.observe(a,5);}
