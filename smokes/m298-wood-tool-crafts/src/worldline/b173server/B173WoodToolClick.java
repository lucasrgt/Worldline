...
public final class B173WoodToolClick {
  ...
  private RemoteItemStack apply(B173WireClient actor, int resultId, int[] planks, int[] sticks) {
    try {
      ...
      if (cursor != null) {
        put(channel, inbound, stickSlot, cursor);
        cursor = null;
      }
  ...
  private void put(B173PlayChannel channel, B173PlayInbound inbound, int combined,
      RemoteItemStack stack) throws IOException {
    RemoteInventoryView before = inbound.activeWindow().inventory(), personal = inbound.inventory();
  ...
  private void takeResult(B173PlayChannel channel, B173PlayInbound inbound, RemoteItemStack result,
    ...
    for (int slot : sticks)
      taken = replace(taken, slot, null);
    step(channel, inbound, 0, 0, result, taken, personal, personal, result,
        STAT + result.legacyId(), 1);
  }

  private void store(B173PlayChannel channel, B173PlayInbound inbound, int combined,
  ...
  private void step(B173PlayChannel channel, B173PlayInbound inbound, int slot, int button,
      ...
      RemoteInventoryView personalAfter, RemoteItemStack cursorAfter, int statisticId,
