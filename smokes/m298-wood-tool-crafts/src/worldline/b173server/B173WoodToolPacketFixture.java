...
public final class B173WoodToolPacketFixture {
  ...
  public static void verify() throws Exception {
    ...
    packet(input, 40, 0, 1, 280, 9);
    packet(input, 8, 1, 2, -1, 0);
    packet(input, 40, 0, 3, -1, 0);
    packet(input, 39, 0, 4, 5, 11);
    packet(input, 2, 1, 5, -1, 0);
    packet(input, 5, 1, 6, -1, 0);
    packet(input, 39, 0, 7, -1, 0);
    packet(input, 0, 0, 8, 268, 1);
  ...
  private static void packet(DataInputStream input, int slot, int button, int action, int item,
      int count) throws Exception {
    if (input.readUnsignedByte() != 102 || input.readUnsignedByte() != 1
        || input.readShort() != slot || input.readUnsignedByte() != button
        || input.readShort() != action || input.readBoolean() || input.readShort() != item)
