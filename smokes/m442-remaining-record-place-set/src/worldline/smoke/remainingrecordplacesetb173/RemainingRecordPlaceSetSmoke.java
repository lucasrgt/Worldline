package worldline.smoke.remainingrecordplacesetb173;
...
/** Places two official jukeboxes 84 and inserts discs 2256 and 2257 without Packet21 eject. */
public final class RemainingRecordPlaceSetSmoke {
  private RemainingRecordPlaceSetSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: RemainingRecordPlaceSetSmoke server.jar workspace port seed username chunkX chunkZ");
    ...
    require(seed == 17320110707L && user.equals("RecPlace442") && user.length() <= 16,
        "remaining-record-place-set identity drift");
    ...
    try {
      ...
      require(actor.awaitInventory().occupiedSlots() == 4,
          "remaining-record-place-set inventory drift");
      ...
      while (
        ...
        require(++column <= 15, "water column exceeded remaining-record-place-set fixture");
      }
      ...
      require(goldLive.equals(new BlockState(84, 1)) && greenLive.equals(new BlockState(84, 1))
              ...
              && actor.peekDroppedItem(new RemoteItemStack(2256, 1, 0)) == null
              && actor.peekDroppedItem(new RemoteItemStack(2257, 1, 0)) == null,
          "official remaining-record-place-set insert absent");
      actor.close();
      ...
      require(after.blockAt(local(gold.x(), cx), gold.y(), local(gold.z(), cz)).equals(goldLive)
              && after.blockAt(local(green.x(), cx), green.y(), local(green.z(), cz))
                  .equals(greenLive),
          "persisted remaining-record-place-set drift");
      String evidence = "column=" + column + ",support=" + cell(top, 1, 0) + "+" + cell(east, 1, 0)
          + ",place=84x2,jukebox=" + cell(gold, 84, goldLive.metadata()) + "+"
          + cell(green, 84, greenLive.metadata())
          + ",insert=2256->empty+2257->empty,play=" + wire(goldPlay) + "+" + wire(greenPlay)
          + ",eject=none,persisted=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=raised-stone+remaining-jukebox84-place-set|cause=packet15-item84-place+packet15-disc2256+packet15-disc2257|wire=packet61-instrument1005-pitch2256+packet61-instrument1005-pitch2257|oracle=official-remaining-record-place-set+fresh-login-jukebox84|"
          + evidence;
      System.out.println("WORLDLINE_M442_SET=" + evidence);
      System.out.println("WORLDLINE_M442_TRACE=" + trace);
      System.out.println("WORLDLINE_M442_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
  ...
  private static RemoteNoteEvent insert(B173WireClient a, BlockPosition box, int slot, int disc)
    ...
    require(live.equals(new BlockState(84, 1)) && played && a.inventory().slot(36 + slot).empty(),
        "official remaining-record insert absent: disc=" + disc + ",live=" + live
            + ",click=" + click);
    return click;
  }
  private static BlockPosition foundation(RemoteChunkSnapshot q, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic remaining-record-place-set foundation");
  }
