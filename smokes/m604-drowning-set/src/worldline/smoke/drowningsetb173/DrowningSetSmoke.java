...
public final class DrowningSetSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    require(seed == 17320110707L && user.equals("Drown604") && user.length() <= 16, "drowning-set identity drift");
  ...
  private static void run(B173DedicatedServer server, B173WireClient actor, Path workspace, String user, int chunkX,
    ...
    B173PlayerSeed.writeInventory(
        workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1}, new int[] {1, 9}, new int[] {64, 8}, new int[] {0, 0}, 20);
    actor.connect();
    ...
    int eye = WorldlineSmokeAwait.observe(actor, 5)
                  .blockAt((int) Math.floor(pose.x()), (int) Math.floor(pose.y() + 1.62D), (int) Math.floor(pose.z()))
                  .legacyId();
    require(water(WorldlineSmokeAwait.observe(actor, 1).blockAt(lower.x(), lower.y(), lower.z()).legacyId())
            && water(WorldlineSmokeAwait.observe(actor, 1).blockAt(upper.x(), upper.y(), upper.z()).legacyId())
            && water(eye) && actor.health() == 20,
        "pre-drown fixture drift");
    int after = WorldlineSmokeAwait.awaitEntity(actor, actor::health, h -> h.intValue() < 20, "drowning health", 800);
    require(after == 18, "drowning Packet8 health drift: 20->" + after);
    RemoteIncomingHit hit = actor.awaitIncomingHit(after);
    require(hit.healthBefore() == 20 && hit.healthAfter() == 18 && hit.damage() == 2, "drowning Packet38/8 drift");
    pose = actor.moveAndObserve(0D, 3D, 0D, 4).resulting();
    require(actor.health() == 18, "drowning death is m465 not hurt: " + actor.health());
    require(water(WorldlineSmokeAwait.observe(actor, 1).blockAt(upper.x(), upper.y(), upper.z()).legacyId()),
        "drowning left water 8/9 absent");
    actor.close();
    server.save();
    String evidence = "cause=drown,column=" + column + ",water=" + lower.x() + ":" + lower.y() + ":" + lower.z() + "+"
        + upper.x() + ":" + upper.y() + ":" + upper.z() + ",health=20->18,damage=2,status=2,packet8=18,clients=1,"
        + "disconnect=clean";
