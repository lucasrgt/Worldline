...
public final class B173BoatBreak {
  ...
  public static void attack(B173WireClient client, int entity) {
    try {
      ...
      if (entity < 0 || entity == local)
        throw new IllegalArgumentException("invalid boat target");
  ...
  public static RemoteDroppedItem wreckage(B173WireClient client, int[] before) {
    ...
    if (newer(boat, before[0]))
      return boat;
    RemoteDroppedItem plank = client.peekDroppedItem(PLANK);
    if (newer(plank, before[1]))
      return plank;
    RemoteDroppedItem stick = client.peekDroppedItem(STICK);
    return newer(stick, before[2]) ? stick : null;
  }

  public static RemoteDroppedItem attackUntilDrop(
      B173WireClient actor, B173WireClient observer, int boat) {
    ...
    if (found == null)
      throw new IllegalStateException("boat Packet21 wreckage absent after Packet7 attacks");
    RemoteDroppedItem peer = wreckage(observer, before);
    if (peer == null)
      peer = observer.awaitDroppedItem(found.item());
    if (!found.item().equals(peer.item()) || found.entityId() != peer.entityId())
      throw new IllegalStateException("peer boat wreckage drift");
    return found;
  }

  private static int id(RemoteDroppedItem item) {
    return item == null ? -1 : item.entityId();
  }
