...
public final class DoubleExtenderSetArm{
 ...
 private DoubleExtenderSetArm(BlockPosition r,BlockPosition f,BlockPosition p0,BlockPosition p1,BlockPosition p2,BlockPosition rl,BlockPosition fl){
  rear=r;front0=f;payload0=p0;payload1=p1;payload2=p2;rearLever=rl;frontLever=fl;
 }
 static DoubleExtenderSetArm build(B173WireClient a,RemoteChunkSnapshot initial,BlockPosition support,int cx,int cz)throws Exception{
  BlockPosition rear=BlockFace.UP.adjacent(support),front0=BlockFace.WEST.adjacent(rear),payload0=BlockFace.WEST.adjacent(front0);
  BlockPosition payload1=BlockFace.WEST.adjacent(payload0),payload2=BlockFace.WEST.adjacent(payload1);
  BlockPosition rearLever=BlockFace.EAST.adjacent(support),westPad=BlockFace.WEST.adjacent(support),frontPad=BlockFace.WEST.adjacent(westPad);
 ...
 static BlockPosition raise(B173WireClient a,RemoteChunkSnapshot initial,int cx,int cz,int[] column)throws Exception{
  ...
  while(water(at(initial,BlockFace.UP.adjacent(top),cx,cz).legacyId())){top=place(a,top,BlockFace.UP,1);a.moveAndObserve(0D,1D,0D,1);require(++column[0]<=15,"water column exceeded double-extender fixture");}
 ...
 static BlockPosition foundation(RemoteChunkSnapshot c,int cx,int cz){for(int x=4;x<=11;x++)for(int z=4;z<=11;z++)for(int y=126;y>=1;y--)if(c.blockAt(x,y,z).legacyId()==3&&water(c.blockAt(x,y+1,z).legacyId()))return new BlockPosition(cx*16+x,y,cz*16+z);throw new IllegalStateException("no deterministic double-extender foundation");}
 static BlockState at(RemoteChunkSnapshot c,BlockPosition p,int cx,int cz){return c.blockAt(p.x()-cx*16,p.y(),p.z()-cz*16);}
 static boolean air(RemoteChunkSnapshot c,BlockPosition p,int cx,int cz){return at(c,p,cx,cz).legacyId()==0;}
