    ...
    public static void main(String[] arguments) throws Exception {
        ...
        try {
            ...
            B173PlayerSeed.writeInventory(workspace, username, 4.5D, 60D, 4.5D,
                    new int[] {0, 1}, new int[] {1, 13}, new int[] {16, 1}, new int[] {0, 0});
            actor.connect();
            ...
            require(initial.blockAt(local(gravel.x(), chunkX), gravel.y(),
                    local(gravel.z(), chunkZ)).legacyId() == 0, "gravel target was not initial air");
            ...
            require(before.blockAt(local(support.x(), chunkX), support.y(), local(support.z(), chunkZ))
                    .equals(new BlockState(1, 0)), "stable gravel support fixture drift");
            require(before.blockAt(local(gravel.x(), chunkX), gravel.y(), local(gravel.z(), chunkZ))
                    .equals(new BlockState(13, 0)), "stable gravel 13:0 fixture drift");
            ...
            require(after.blockAt(local(support.x(), chunkX), support.y(), local(support.z(), chunkZ))
                    .equals(new BlockState(13, 0)), "fresh landed gravel 13:0 drift");
            require(after.blockAt(local(gravel.x(), chunkX), gravel.y(), local(gravel.z(), chunkZ))
                    .equals(new BlockState(0, 0)), "fresh cleared upper gravel drift");
            require(after.blockAt(local(support.x(), chunkX), support.y(), local(support.z(), chunkZ))
                    .legacyId() != 12, "landed sand instead of gravel");
        } finally {
            actor.close();
            if (reader != null) {
                reader.close();
        ...
        String evidence = "column=" + column + ",lower=" + cell(support) + ":1:0->13:0,upper="
                + cell(gravel) + ":13:0->0:0,entity-type=" + fall.type() + ",packet23=" + fall.type()
