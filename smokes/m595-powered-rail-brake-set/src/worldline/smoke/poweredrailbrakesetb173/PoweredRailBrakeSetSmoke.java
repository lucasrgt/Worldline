...
public final class PoweredRailBrakeSetSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    PoweredRailBrakeSetArm.require(seed == 17320110707L && user.equals("RailBrake595") && user.length() <= 16,
        "powered-rail-brake identity drift");
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2, 3, 4},
          new int[] {1, 27, 28, 328, 76}, new int[] {32, 8, 1, 1, 1}, new int[] {0, 0, 0, 0, 0});
      actor.connect();
      actor.synchronizePose();
      PoweredRailBrakeSetArm.require(actor.awaitInventory().occupiedSlots() == 5, "powered-rail-brake inventory drift");
      RemoteChunkSnapshot initial = actor.awaitRemoteChunk(chunkX, chunkZ).chunkAt(chunkX, chunkZ);
      int[] column = new int[1];
      PoweredRailBrakeSetArm arm =
          PoweredRailBrakeSetArm.place(actor, PoweredRailBrakeSetArm.raise(actor, initial, chunkX, chunkZ, column));
      ...
      PoweredRailBrakeSetArm.require(cart.type() == 10 && cart.throwerId() == 0 && cart.velocityX() == 0
              && cart.velocityY() == 0 && cart.velocityZ() == 0 && cart.fixedX() == arm.launch.x() * 32 + 16
              && cart.fixedY() == arm.launch.y() * 32 + 27 && cart.fixedZ() == arm.launch.z() * 32 + 16
      ...
      PoweredRailBrakeSetArm.require(PoweredRailBrakeSetArm.idleDetector(idle, arm.beyond)
              && PoweredRailBrakeSetArm.unpowered(idle, arm.launch) && PoweredRailBrakeSetArm.unpowered(idle, arm.mid),
      ...
      WorldlineSmokeAwait.awaitWorld(actor,
          world
          -> world.blockAt(arm.torch.x(), arm.torch.y(), arm.torch.z()).equals(new BlockState(76, 5))
              && PoweredRailBrakeSetArm.powered(world, arm.launch) && PoweredRailBrakeSetArm.powered(world, arm.mid)
      ...
      WorldlineSmokeAwait.awaitWorld(actor,
          world
          -> world.blockAt(arm.torch.x(), arm.torch.y(), arm.torch.z()).equals(new BlockState(0, 0))
              && PoweredRailBrakeSetArm.unpowered(world, arm.launch) && PoweredRailBrakeSetArm.unpowered(world, arm.mid)
              && PoweredRailBrakeSetArm.idleDetector(world, arm.beyond),
      ...
      PoweredRailBrakeSetArm.require(PoweredRailBrakeSetArm.idleDetector(held, arm.beyond)
              && PoweredRailBrakeSetArm.unpowered(held, arm.launch) && PoweredRailBrakeSetArm.unpowered(held, arm.mid),
      ...
      String evidence = "column=" + column[0] + ",support=" + PoweredRailBrakeSetArm.cell(arm.support) + ":1:0,wall="
          + PoweredRailBrakeSetArm.cell(arm.wall) + ":1:0,bumper=" + PoweredRailBrakeSetArm.cell(arm.bumper)
          + ":1:0,launch=" + PoweredRailBrakeSetArm.cell(arm.launch) + ":27:0->8->0,mid="
          + PoweredRailBrakeSetArm.cell(arm.mid) + ":27:0->8->0,beyond=" + PoweredRailBrakeSetArm.cell(arm.beyond)
          + ":28:0,cart=type10+thrower0+fixed" + cart.fixedX() + ":" + cart.fixedY() + ":" + cart.fixedZ()
