...
public final class PoweredRailBrakeSetArm {
    ...
    private PoweredRailBrakeSetArm(BlockPosition support, BlockPosition wall, BlockPosition bumper,
            BlockPosition launch, BlockPosition mid, BlockPosition beyond, BlockPosition torch) {
    ...
    static PoweredRailBrakeSetArm place(B173WireClient actor, BlockPosition top) throws Exception {
        ...
        require(launch.x() == mid.x() && mid.x() == beyond.x() && mid.z() == launch.z() + 1
                && beyond.z() == launch.z() + 2 && wall.z() == launch.z() - 1
                && bumper.z() == beyond.z() + 1, "north-south powered-rail-brake track drift");
        return new PoweredRailBrakeSetArm(top, wall, bumper, launch, mid, beyond,
    ...
    static void awaitPlayers(B173DedicatedServer server, int count) {
        new WorldlineAwait(40).awaitEntity(server::players,
                (List<String> players) -> players.size() == count, "player count");
    }
