...
public final class WorldlinePairMod {
    ...
    @EventListener private static void block(BlockRegistryEvent event) {
    ...
    @EventListener private static void blockEntity(BlockEntityRegisterEvent event) {
    ...
    @EventListener private static void messages(MessageListenerRegistryEvent event) {
        event.register(ACTIVATE, (player, packet) -> {
            if (player.world.isRemote) throw new IllegalStateException("M73 activation reached client");
            WorldlinePairServer.activate(player, packet.ints);
        });
        event.register(PLAN, (player, packet) -> {
            if (!player.world.isRemote) throw new IllegalStateException("M73 plan reached server");
            WorldlinePairSync.plan(packet.ints);
        });
        event.register(READY, (player, packet) -> {
            if (player.world.isRemote) throw new IllegalStateException("M73 ready reached client");
            WorldlinePairServer.ready(player, packet.ints);
        });
        event.register(SYNC, (player, packet) -> {
            if (!player.world.isRemote) throw new IllegalStateException("M73 sync reached server");
            if (packet.ints == null || packet.ints.length != 4) throw new IllegalStateException("invalid M73 sync shape");
            WorldlinePairSync.receive(player.world, packet.ints[0], packet.ints[1], packet.ints[2], packet.ints[3]);
        });
    }
}
