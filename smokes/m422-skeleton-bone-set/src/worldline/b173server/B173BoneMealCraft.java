...
public final class B173BoneMealCraft {
  ...
  public static void apply(B173WireClient actor) {
    try {
      ...
      if (slot < 9)
        throw new IllegalStateException("bone 352 absent from personal storage");
      mill(channel, inbound, slot, 1);
      if (inbound.cursor() != null || !same(inbound.inventory(), slot, MEAL)
          || !emptyCraft(inbound.inventory()))
  ...
  private static void mill(B173PlayChannel channel, B173PlayInbound inbound, int slot, int action)
    ...
    if (action != 4)
      throw new IllegalStateException("bone-meal action drift");
  }
