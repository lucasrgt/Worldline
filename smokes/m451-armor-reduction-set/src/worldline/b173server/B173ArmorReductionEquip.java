...
public final class B173ArmorReductionEquip {
    ...
    public void wear(B173WireClient actor, int[] items) {
        if (items == null || items.length != 4) throw new IllegalArgumentException("armor family must be four pieces");
        for (int index = 0; index < 4; index++) {
            int id = items[index], dest = 5 + index;
            if (id < 298 || id > 313) throw new IllegalArgumentException("armor id drifted");
            put(actor, id, dest);
        }
    ...
    public void strip(B173WireClient actor) {
        for (int dest = 5; dest <= 8; dest++)
            if (!actor.inventory().slot(dest).empty()) take(actor, dest);
        RemoteInventoryView view = actor.inventory();
        if (!view.slot(5).empty() || !view.slot(6).empty() || !view.slot(7).empty() || !view.slot(8).empty())
    ...
    private void put(B173WireClient actor, int id, int dest) {
        ...
        } catch (IOException error) { throw new IllegalStateException("armor equip failed", error); }
    }
    ...
    private void take(B173WireClient actor, int dest) {
        try {
            ...
            preflight(inbound);
            RemoteInventoryView before = inbound.inventory();
            if (before.slot(dest).empty()) return;
        ...
        } catch (IOException error) { throw new IllegalStateException("armor unequip failed", error); }
    }

    private void step(B173PlayChannel channel, B173PlayInbound inbound, int slot, RemoteItemStack predicted,
            RemoteInventoryView after, RemoteItemStack cursorAfter) throws IOException {
        if (action > 32767) throw new IllegalStateException("armor click counter exhausted");
        ...
        channel.output.flush(); inbound.awaitPersonalStep(); action++;
    }
    ...
    private static void preflight(B173PlayInbound inbound) {
        ...
        if (view.windowId() != 0 || view.size() != 45) throw new IllegalStateException("personal window drifted");
    }
    ...
    private static int emptySlot(RemoteInventoryView view) {
        for (int slot = 9; slot <= 44; slot++) if (view.slot(slot).empty()) return slot;
