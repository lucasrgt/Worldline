...
public final class ArmorReductionSetSmoke {
  ...
  public static void main(String[] a) throws Exception {
    ...
    require(seed == 17320110707L && user.equals("ArmorRed451") && user.length() <= 16,
    ...
    B173DedicatedServer server =
        B173DedicatedServer.difficulty(jar, workspace, port, seed, timeout, 2);
    ...
    try {
      ...
      actor =
          reseed(actor, server, workspace, user, port, timeout, pad, B173ArmorReductionAccess.IRON);
      RemoteIncomingHit iron = armedHit(server, actor, pad, B173ArmorReductionAccess.IRON);
      actor = reseed(
          actor, server, workspace, user, port, timeout, pad, B173ArmorReductionAccess.DIAMOND);
      ...
      require(bare.damage() > leather.damage() && bare.damage() > iron.damage()
          ...
          "armor Packet8 deltas were not strictly reduced " + bare.damage() + ">" + leather.damage()
              + ">" + iron.damage() + ">" + diamond.damage());
      actor.close();
      awaitPlayers(server, 0);
      server.save();
      String evidence = "column=" + pad.column + ",platform=7x7-48grass,spawner="
          + B173ArmorReductionAccess.cell(pad.spawner) + ",entityid=Zombie,mob=type54,night=14000"
          + ",unarmored=20->" + bare.healthAfter() + ":" + bare.damage() + ",leather=20->"
          + leather.healthAfter() + ":" + leather.damage() + ",iron=20->" + iron.healthAfter() + ":"
          + iron.damage() + ",diamond=20->" + diamond.healthAfter() + ":" + diamond.damage()
          + ",strict=" + bare.damage() + ">" + leather.damage() + ">" + iron.damage() + ">"
          + diamond.damage()
  ...
  private static Pad build(Path jar, Path workspace, int port, long seed, String user,
      Duration timeout, int cx, int cz) throws Exception {
    B173DedicatedServer server =
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D,
          ...
          new int[] {
              1, 2, 52, 322, 320, 298, 299, 300, 301, 306, 307, 308, 309, 310, 311, 312, 313},
  ...
  private static RemoteIncomingHit armedHit(
      B173DedicatedServer server, B173WireClient actor, Pad pad, int[] family) {
    ...
    require(
        zombie.legacyType() == 54 && zombie.legacyType() != 90, "zombie Packet24 identity drift");
    double[] at = {zombie.x(), zombie.y(), zombie.z()};
    if (family != null)
      new B173ArmorReductionEquip().wear(actor, family);
    B173ArmorReductionAccess.heal(actor);
    B173ArmorReductionAccess.settle(actor);
  ...
  private static B173WireClient reseed(B173WireClient actor, B173DedicatedServer server,
      Path workspace, String user, int port, Duration timeout, Pad pad, int[] armor)
      throws Exception {
    actor.close();
    awaitPlayers(server, 0);
    B173PlayerSeed.writeInventory(workspace, user, pad.top.x() + 0.5D, pad.top.y() + 1.0D,
        pad.top.z() + 0.5D, new int[] {0, 1, 2, 4, 5, 6, 7},
    ...
    require(next.awaitInventory().occupiedSlots() == 7 && next.awaitHealth(20) == 20,
        "reseed inventory drift");
    return next;
  }
