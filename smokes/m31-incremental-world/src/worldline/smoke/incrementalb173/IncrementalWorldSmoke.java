...
public final class IncrementalWorldSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        IncrementalRemoteWorldMultiplayerSession client = new B173WireClient(
                "127.0.0.1", port, username, timeout);
        BlockPosition target; BlockState before; RemoteWorldView after;
        try {
            server.boot(); server.operator(username); client.connect();
            PlayerPose pose = client.synchronizePose();
            RemoteWorldView initial = client.awaitRemoteChunk((int) Math.floor(pose.x()) >> 4,
                    (int) Math.floor(pose.z()) >> 4);
            target = target(initial, pose); before = initial.blockAt(target.x(), target.y(), target.z());
    ...
    private static BlockPosition requireTarget(RemoteWorldView world, PlayerPose pose) {
        BlockPosition stone = target(world, pose, true); if (stone != null) return stone;
        ...
        for (int y = centerY; y >= centerY - 5; y--) for (int radius = 0; radius <= 4; radius++) {
            for (int x = centerX - radius; x <= centerX + radius; x++) {
                for (int z = centerZ - radius; z <= centerZ + radius; z++) {
                    ...
                    if (dx * dx + dy * dy + dz * dz > 25.0D) continue;
