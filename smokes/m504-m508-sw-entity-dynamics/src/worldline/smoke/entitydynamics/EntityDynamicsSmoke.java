...
public final class EntityDynamicsSmoke {
  ...
  private static void run(CanonicalTrace t, String mode, int ticks) {
    EntityDynamicsBackend b = new EntityDynamicsBackend(SEED, mode);
    MinecraftRuntime r = new ControlledMinecraftRuntime(b);
    r.bootHeadless();
    r.loadWorld(WorldSource.at(Paths.get("memory", mode)));
    try {
      b.snapshot(t, mode + "-seed");
