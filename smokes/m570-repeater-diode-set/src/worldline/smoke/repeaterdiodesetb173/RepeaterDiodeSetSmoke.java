...
public final class RepeaterDiodeSetSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        RepeaterDiodeSetArm.require(user.equals("RptDiode570") && user.length() <= 16
                && seed == 17320110707L, "repeater-diode identity drift");
        ...
        try {
            ...
            RepeaterDiodeSetArm.require(actor.awaitInventory().occupiedSlots() == 4,
                    "repeater-diode inventory drift");
            ...
            RepeaterDiodeSetArm.require(settled.blockAt(arm.repeater.x(), arm.repeater.y(),
                    arm.repeater.z()).equals(new BlockState(93, 3))
                    && settled.blockAt(arm.in.x(), arm.in.y(), arm.in.z())
                            .equals(new BlockState(55, 0))
                    && settled.blockAt(arm.out.x(), arm.out.y(), arm.out.z())
                            .equals(new BlockState(55, 0)),
            ...
            String evidence = "column=" + column[0] + ",support="
                    + RepeaterDiodeSetArm.cell(arm.support) + ":1:0,repeater="
                    + RepeaterDiodeSetArm.cell(arm.repeater)
                    + ":93:3->94:3,facing=3,delay=1,reverse=rpt=93:3+in=55:0+out=55:15,"
                    + "forward=rpt=94:3+in=55:15+out=55:15,isolated=true,persisted=94:3,"
                    + "clients=2,disconnect=clean";
            String trace = "v1|server=official-b1.7.3|seed=" + seed
                    + "|fixture=raised-west-repeater93+in-dust55+out-dust55+south-levers|settle="
                    + fixture + "+" + signal
