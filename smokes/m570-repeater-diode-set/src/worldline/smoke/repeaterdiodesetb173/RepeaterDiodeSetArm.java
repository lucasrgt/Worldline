...
public final class RepeaterDiodeSetArm {
    ...
    private RepeaterDiodeSetArm(BlockPosition support, BlockPosition repeater, BlockPosition in,
            BlockPosition out, BlockPosition fwd, BlockPosition rev, BlockState fwdOff,
            BlockState revOff) {
    ...
    static RepeaterDiodeSetArm place(B173WireClient actor, RemoteChunkSnapshot initial,
            BlockPosition support, int chunkX, int chunkZ) throws Exception {
        ...
        require(actor.awaitBlock(repeater, new BlockState(93, 3))
                .blockAt(repeater.x(), repeater.y(), repeater.z()).equals(new BlockState(93, 3)),
    ...
    RemoteWorldView reverse(B173WireClient actor, int ticks) throws Exception {
        ...
        require(live.blockAt(rev.x(), rev.y(), rev.z()).equals(on(revOff))
                && live.blockAt(repeater.x(), repeater.y(), repeater.z())
                ...
                "reverse isolation absent: " + cell(live, repeater) + "/" + cell(live, in)
                        + "/" + cell(live, out) + "/" + cell(live, rev));
        actor.activateBlock(rev, BlockFace.UP);
    ...
    void persist(RemoteChunkSnapshot after, int chunkX, int chunkZ) {
        require(at(after, repeater, chunkX, chunkZ).equals(new BlockState(94, 3))
                ...
                && at(after, fwd, chunkX, chunkZ).equals(on(fwdOff))
                && at(after, rev, chunkX, chunkZ).equals(revOff),
    ...
    static void dust(B173WireClient actor, BlockPosition pad, BlockPosition target)
            throws Exception {
        ...
        require(worldline.test.WorldlineSmokeAwait.awaitBlock(actor, target, new BlockState(55, 0), 5)
                .blockAt(target.x(), target.y(), target.z()).equals(new BlockState(55, 0)),
                "unpowered dust drift at " + cell(target));
    }

    static BlockState lever(B173WireClient actor, BlockPosition pad, BlockPosition target, int meta)
            throws Exception {
        actor.placeHeldBlock(pad, BlockFace.SOUTH);
        BlockState live = worldline.test.WorldlineSmokeAwait.awaitBlock(actor, target,
                new BlockState(69, meta), 10).blockAt(target.x(), target.y(), target.z());
