...
public final class FallingSandSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    if (arguments.length != 9)
      throw new IllegalArgumentException(
          "usage: FallingSandSmoke server.jar workspace port seed username chunkX chunkZ fixtureTicks gravityTicks");
    ...
    int fixtureTicks = Integer.parseInt(arguments[7]),
        gravityTicks = Integer.parseInt(arguments[8]);
    ...
    try {
      ...
      while (water(
          initial.blockAt(local(support.x(), chunkX), support.y() + 1, local(support.z(), chunkZ))
              .legacyId())) {
        actor.placeHeldBlock(support, BlockFace.UP);
        support = BlockFace.UP.adjacent(support);
        actor.awaitBlock(support, new BlockState(1, 0));
      ...
      require(initial.blockAt(local(sand.x(), chunkX), sand.y(), local(sand.z(), chunkZ)).legacyId()
              == 0,
      ...
      require(before.blockAt(local(support.x(), chunkX), support.y(), local(support.z(), chunkZ))
              ...
              && before.blockAt(local(sand.x(), chunkX), sand.y(), local(sand.z(), chunkZ))
                  .equals(new BlockState(12, 0)),
          "stable sand fixture drift");
      ...
      cleared = live.blockAt(sand.x(), sand.y(), sand.z());
      require(opened.equals(new BlockState(0, 0)) && settled.equals(new BlockState(12, 0))
              && cleared.equals(new BlockState(0, 0)),
          "sand did not settle one block: " + opened + " / " + settled + " / " + cleared);
      actor.close();
      ...
      require(after.blockAt(local(support.x(), chunkX), support.y(), local(support.z(), chunkZ))
                  .equals(settled)
              && after.blockAt(local(sand.x(), chunkX), sand.y(), local(sand.z(), chunkZ))
                  .equals(cleared),
          "fresh settled sand drift");
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
    ...
    require(delta.changed == 2, "falling sand changed unrelated states: " + delta);
    String evidence = "column=" + column + ",lower=" + support.x() + ":" + support.y() + ":"
        + support.z() + ":1:0->12:0,upper=" + sand.x() + ":" + sand.y() + ":" + sand.z()
        + ":12:0->0:0,states=" + delta;
    String trace = "v1|server=official-b1.7.3|seed=" + seed
        + "|fixture=stone-column+supported-sand12|settle=" + fixtureTicks + "+" + gravityTicks
        + "ticks|cause=packet14-remove-support|confirmation=packet53-air|effect=official-falling-sand-settle|observation=live-packet53+fresh-login-packet51|"
        + evidence + "|disconnect=clean";
    System.out.println("WORLDLINE_M119_GRAVITY=" + evidence);
    System.out.println("WORLDLINE_M119_TRACE=" + trace);
    System.out.println("WORLDLINE_M119_SIGNATURE=" + sha256(trace));
  }
  private static BlockPosition foundation(RemoteChunkSnapshot chunk, int chunkX, int chunkZ) {
    ...
    throw new IllegalStateException("no deterministic sand foundation");
  }
