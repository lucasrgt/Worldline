...
public final class FurnaceCartPushSmoke {
    ...
    public static void main(String[] args) throws Exception {
        ...
        require(seed == 17320110707L && user.equals("FurnCart596") && user.length() <= 16,
                "furnace-cart-push identity drift");
    ...
    private static void run(B173DedicatedServer server, B173WireClient actor, Path workspace,
            String user, int port, int cx, int cz, Duration timeout) throws Exception {
        server.boot();
        B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D,
                new int[] {0, 1, 2, 3}, new int[] {1, 66, 343, 263},
        ...
        require(rail.x() == mid.x() && mid.x() == end.x() && mid.z() == rail.z() + 1
                && end.z() == mid.z() + 1 && wall.z() == rail.z() - 1
                && bumper.z() == end.z() + 1, "north-south rail66 track drift");
        ...
        require(cart.type() >= 10 && cart.type() <= 12 && cart.type() != 1
                && cart.throwerId() == 0 && cart.velocityX() == 0 && cart.velocityY() == 0
                && cart.velocityZ() == 0 && cart.fixedX() == rail.x() * 32 + 16
                && cart.fixedY() == rail.y() * 32 + 27 && cart.fixedZ() == rail.z() * 32 + 16
                && cart.fixedZ() != end.z() * 32 + 16, "furnace-cart Packet23 bounds drift");
        ...
        B173EntityVelocity push = WorldlineSmokeAwait.awaitCheckedEntity(actor,
                ...
                value -> value != null && value.fixedZ() > 0
                        && Math.abs(value.fixedZ()) >= Math.abs(value.fixedX()),
                "south Packet28", 80);
        ...
        try {
            ...
            require(after.blockAt(local(rail.x(), cx), rail.y(), local(rail.z(), cz))
                    ...
                    .equals(new BlockState(66, 0))
                    && after.blockAt(local(end.x(), cx), end.y(), local(end.z(), cz))
                    .equals(new BlockState(66, 0))
                    ...
                    .equals(new BlockState(1, 0)), "persisted furnace-cart-push drift");
            String evidence = "column=" + column + ",support=" + cell(top, 1)
                    + ",wall=" + cell(wall, 1) + ",bumper=" + cell(bumper, 1)
                    + ",rail=" + cell(rail, 66) + ",track=" + cell(mid, 66)
                    + ",end=" + cell(end, 66) + ",cart=type" + cart.type()
                    + "+thrower0+fixed" + cart.fixedX() + ":" + cart.fixedY() + ":"
                    + cart.fixedZ() + ",coal=263:1->0,unfueled-hold=idle,push=south"
            ...
            String trace = "v1|server=official-b1.7.3|seed=" + 17320110707L
                    ...
                    + "|oracle=unfueled-hold-idle+fueled-south-push-on-rail66-not-detector|"
                    + evidence;
