...
public final class DoorUpperBreakSetSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    if (arguments.length != 9) {
      throw new IllegalArgumentException("usage: DoorUpperBreakSetSmoke server.jar workspace port seed username chunkX "
          + "chunkZ fixtureTicks signalTicks");
    }
    ...
    require(
        seed == 17320110707L && user.equals("DoorUpper593") && user.length() <= 16, "door-upper-break identity drift");
    B173DedicatedServer server = new B173DedicatedServer(jar, workspace, port, seed, timeout, 3, true);
    B173WireClient actor = new B173WireClient("127.0.0.1", port, user, timeout);
    B173WireClient reader = null;
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2}, new int[] {1, 324, 258},
      ...
      String trace = "v1|server=official-b1.7.3|seed=" + seed + "|fixture=raised-stone+woodendoor64|settle=" + fixture
          + "+" + signal + "ticks|cause=packet15-item324-place+packet14-ironaxe258-upper"
  ...
  private static void awaitDoor(B173WireClient actor, BlockPosition lower, BlockPosition upper) throws Exception {
