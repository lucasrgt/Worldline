...
/** Eighteen accepted window-0 clicks that dye white wool 35:0 in the personal 2x2 grid. */
public final class B173DyeWoolCraft {
    private static final int[] WOOL_SLOTS = {36, 37, 38}, DYE_SLOTS = {39, 40, 41}, RESULTS = {14, 13, 11};
    ...
    private static final int[] CLICK_DAMAGE = {0, 0, 1, 0, 14, 0, 0, 0, 2, 0, 13, 0, 0, 0, 4, 0, 11, 0};

    private B173DyeWoolCraft() {}
    ...
    public static void verify() throws Exception {
        ...
        if (input.available() != 0 || bytes.size() != 207)
            throw new AssertionError("dyed-wool Packet102 sequence size drifted");
    }
    ...
    public static void apply(B173WireClient actor) {
        try {
            ...
            if (inbound.windowActive() || !inbound.cursorObserved() || inbound.cursor() != null)
                throw new IllegalStateException("dyed-wool craft preflight failed");
            int action = 1;
            for (int index = 0; index < RESULTS.length; index++)
                action = dye(channel, inbound, WOOL_SLOTS[index], DYE_SLOTS[index], RESULTS[index], action);
            if (action != 19 || inbound.cursor() != null) throw new IllegalStateException("dyed-wool action drift");
            RemoteInventoryView view = inbound.inventory();
            if (view.occupiedSlots() != 3 || !same(view, 36, 35, 14) || !same(view, 37, 35, 13)
                    || !same(view, 38, 35, 11) || !emptyCraft(view))
                throw new IllegalStateException("dyed-wool results drifted");
        } catch (IOException error) {
            throw new IllegalStateException("dyed-wool craft failed", error);
        }
    }
    ...
    private static int dye(B173PlayChannel channel, B173PlayInbound inbound, int woolSlot, int dyeSlot,
        ...
        if (woolSlot < 9 || dyeSlot < 9 || woolSlot > 44 || dyeSlot > 44 || inbound.cursor() != null
            ...
            throw new IllegalStateException("dyed-wool ingredients drifted");
        ...
        if (!same(inbound.inventory(), woolSlot, result) || !emptyCraft(inbound.inventory())
                || inbound.cursor() != null)
            throw new IllegalStateException("dyed-wool store drifted");
        return action;
    }
    ...
    private static void packet(DataInputStream input, int slot, int action, int item, int damage)
        ...
        if (input.readUnsignedByte() != 102 || input.readUnsignedByte() != 0 || input.readShort() != slot
            ...
            throw new AssertionError("dyed-wool Packet102 fields drifted");
        if (item >= 0 && (input.readUnsignedByte() != 1 || input.readShort() != damage))
            throw new AssertionError("dyed-wool Packet102 stack drifted");
    }
