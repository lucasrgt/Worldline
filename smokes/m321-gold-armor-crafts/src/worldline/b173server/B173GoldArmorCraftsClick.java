...
public final class B173GoldArmorCraftsClick {
    ...
    public static int[] apply(B173WireClient actor, int goldSlot, int[] dests) {
        try {
            ...
            if (window.descriptor().kind() != RemoteWindowKind.WORKBENCH || window.inventory().size() != 46
                    || dests == null || dests.length != 4 || goldSlot < 9 || goldSlot > 44)
    ...
    private static int craft(B173PlayChannel channel, B173PlayInbound inbound, int windowId, int action,
            int goldSlot, int dest, int[] grid, int resultId) throws IOException {
    ...
    private static void requireDest(RemoteInventoryView personal, int dest, int goldSlot) {
        if (dest < 9 || dest > 44 || dest == goldSlot || dest == 5 || dest == 6 || dest == 7 || dest == 8
