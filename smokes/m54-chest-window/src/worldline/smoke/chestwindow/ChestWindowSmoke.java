...
public final class ChestWindowSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      ...
      RemoteWorldView actorWorld = worldline.test.WorldlineSmokeAwait.awaitWorld(actor,
                          view
                          -> view.blockAt(target.x(), target.y(), target.z()).legacyId() == 54,
                          "actor chest placement", 5),
                      observerWorld = worldline.test.WorldlineSmokeAwait.awaitWorld(observer,
                          view
                          -> view.blockAt(target.x(), target.y(), target.z()).legacyId() == 54,
                          "observer chest placement", 5);
      ...
      require(actor.inventory().occupiedSlots() == 0, "placed chest did not leave actor inventory");
      ...
      require(descriptor.kind() == RemoteWindowKind.CHEST && "Chest".equals(descriptor.title())
              && descriptor.containerSlots() == 27,
          "chest descriptor drifted");
    ...
    System.out.println("WORLDLINE_M54_WINDOW=id=" + window.descriptor().windowId()
        + ";kind=" + window.descriptor().kind() + ";title=" + window.descriptor().title()
        + ";owned=" + window.descriptor().containerSlots() + ";total=" + window.inventory().size()
  ...
  private static PlayerPose acquire(ChestWindowMultiplayerSession client, String username) {
    ...
    }, inventory -> inventory.occupiedSlots() >= 1, "chest grant", 100);
