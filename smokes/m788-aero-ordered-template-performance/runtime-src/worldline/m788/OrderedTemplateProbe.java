...
public final class OrderedTemplateProbe {
    ...
    private static final String[] HASHES = new String[OrderedTemplateScene.CHECKPOINTS];
    private static final boolean[] CAPTURED = new boolean[OrderedTemplateScene.CHECKPOINTS];
    ...
    public static void beginScene() {
        if (sceneCleared || !OrderedTemplateState.fixtureActive()) return;
    ...
    public static void sample(Minecraft game) {
        if (!OrderedTemplateState.retaining() || started == 0L) return;
        ...
        int checkpoint = OrderedTemplateState.checkpoint();
        if (OrderedTemplateState.captureFrame() && !CAPTURED[checkpoint]) capture(game, checkpoint);
    }
    ...
    public static void flush(double cameraX, double cameraY, double cameraZ, long startedAt) {
        ...
        if (OrderedTemplateState.retaining()) {
            OrderedTemplatePageMetrics.recordFlush(compiledBefore);
        }
    }

    public static void finishDirect(long startedAt) { finishRender(startedAt); }

    public static void recordSubmission(int submitted) {
        if (OrderedTemplateState.retaining()) submittedMachines += submitted;
    }
    ...
    public static void write(File metrics, File frames, String name, int machines) throws Exception {
        ...
        try (PrintWriter out = new PrintWriter(new FileWriter(metrics))) {
            ...
            OrderedTemplatePageMetrics.write(out);
    ...
    private static void capture(Minecraft game, int checkpoint) {
        try {
            ...
            File directory = new File(System.getProperty("worldline.m788.framesDir"), arm);
            if (!directory.isDirectory() && !directory.mkdirs()) {
                throw new IllegalStateException("cannot create M788 frame directory");
            }
            ...
            throw new IllegalStateException("M788 framebuffer capture failed", error);
        }
    }
    ...
    private static void finishRender(long startedAt) {
        if (!OrderedTemplateState.retaining()) return;
