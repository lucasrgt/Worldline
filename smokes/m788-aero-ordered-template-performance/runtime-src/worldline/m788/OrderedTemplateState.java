...
/** Owns one deterministic ordered-template Cell Pages arm in a fresh client JVM. */
public final class OrderedTemplateState {
    public static final boolean ENABLED = Boolean.getBoolean("worldline.m788.enabled");
    private static final boolean PREPARE = Boolean.getBoolean("worldline.m788.prepare");
    private static final String ARM = System.getProperty("worldline.m788.arm", "prepare");
    private static final int TARGET_FRAMES = Integer.getInteger("worldline.m788.frames", 1200);
    private static final int WARM_FRAMES = Integer.getInteger("worldline.m788.warmFrames", 480);
    private static final int MAX_BLANK_CAPTURES =
        Integer.getInteger("worldline.m788.maxBlankCaptures", 24);
    private static final long MINIMUM_NS = Long.getLong("worldline.m788.minimumMillis", 20000L)
        * 1_000_000L;
    ...
    public static void drive(Minecraft game) {
        ...
        if (stage == 0 && game.world == null) {
            ...
            System.out.println("[WorldlineM788] start prepare=" + PREPARE + " arm=" + ARM);
            game.startGame("WorldlineAeroOrderedTemplate", "Worldline Aero Ordered Template",
                17320110788L);
            return;
        }
    ...
    public static void frame(Minecraft game) {
        ...
        if (!warmComplete()) {
            ...
            if (routeWarm < WARM_FRAMES) OrderedTemplateScene.place(game.player, routeWarm++);
            else {
                OrderedTemplateScene.place(game.player, 0);
                convergenceFrames++;
            }
            updateStablePages();
            if (convergenceFrames > 0 && convergenceFrames % 120 == 0) {
                System.out.println("[WorldlineM788] warm arm=" + ARM + " route=" + routeWarm
            ...
            require(convergenceFrames < 2400, "M788 page cache did not converge");
            return;
        }
        if (!armBegan) beginArm();
        long elapsed = System.nanoTime() - measurementStarted;
        if (retained >= TARGET_FRAMES && elapsed >= MINIMUM_NS
                && retained % OrderedTemplateScene.ROUTE_FRAMES == 0) {
            require(OrderedTemplateProbe.blankCaptures() <= MAX_BLANK_CAPTURES,
                "M788 blank framebuffer retry budget exhausted");
            if (OrderedTemplateProbe.captures() == OrderedTemplateScene.CHECKPOINTS) {
                finish(game);
                return;
            }
        }
        measuring = true;
        int route = retained % OrderedTemplateScene.ROUTE_FRAMES;
        checkpoint = route / OrderedTemplateScene.CAPTURE_STRIDE;
        capture = (route + 1) % OrderedTemplateScene.CAPTURE_STRIDE == 0;
        OrderedTemplateScene.place(game.player, route);
        retained++;
    }
    ...
    public static void renderFixture(double cameraX, double cameraY, double cameraZ) {
        ...
        OrderedTemplateProbe.beginScene();
        long renderStarted = OrderedTemplateProbe.beginRender();
        int submitted = 0;
        WorldlineM788Direct.discardQueued();
        ...
        try {
            for (Object value : game.world.blockEntities) {
                ...
                if (!(block instanceof MegaModelBlockEntity)
                        || !WorldlineM788Rehydrator.contains(block.x, block.y, block.z)) continue;
                if (templateArm()) {
                    ...
                    WorldlineM788Direct.draw(MegaModelBlockEntityRenderer.MODEL,
        ...
        if (templateArm()) OrderedTemplateProbe.flush(cameraX, cameraY, cameraZ, renderStarted);
        else OrderedTemplateProbe.finishDirect(renderStarted);
        OrderedTemplateProbe.recordSubmission(submitted);
    }
    ...
    private static void beginArm() {
        OrderedTemplateProbe.beginArm(ARM);
        measurementStarted = System.nanoTime();
        armBegan = true;
    }

    private static boolean templateArm() { return "template".equals(ARM); }
    ...
    private static void warmWorld(Minecraft game) {
        ...
        machines = WorldlineM788Rehydrator.count(world);
        if (worldWarm == 2 && machines != WorldlineM788Rehydrator.MACHINES) {
            WorldlineM788Rehydrator.rehydrate(world);
            machines = WorldlineM788Rehydrator.count(world);
        }
        WorldlineM788Rehydrator.order(world);
        if (machines < WorldlineM788Rehydrator.MACHINES && worldWarm < 1200) return;
        require(machines == WorldlineM788Rehydrator.MACHINES,
            "M788 fixture drift: " + machines);
        OrderedTemplateScene.place(game.player, 0);
        if (worldWarm < 40) return;
        if (PREPARE) {
            world.saveWithLoadingDisplay(true, null);
            System.out.println("[WorldlineM788] template-ready machines=" + machines);
            stage = 3;
            game.scheduleStop();
        } else stage = 2;
    }
    ...
    private static void finish(Minecraft game) {
        try {
            OrderedTemplateProbe.write(new File(System.getProperty("worldline.m788.metrics")),
                new File(System.getProperty("worldline.m788.framesFile")), ARM, machines);
        } catch (Exception error) {
            throw new IllegalStateException("M788 artifact write failed", error);
        }
        System.out.println("[WorldlineM788] capture-complete arm=" + ARM
            + " frames=" + OrderedTemplateProbe.frames() + " captures=" + OrderedTemplateProbe.captures());
        stage = 3;
        measuring = false;
        game.scheduleStop();
    }
