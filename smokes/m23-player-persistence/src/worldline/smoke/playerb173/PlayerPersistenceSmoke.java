...
public final class PlayerPersistenceSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      ...
      require(Double.isFinite(player.x()) && Double.isFinite(player.y())
              && Double.isFinite(player.z()) && player.y() > 0.0D,
          "persisted player position drifted");
    } finally {
      client.close();
      server.close();
    }
    System.out.println("WORLDLINE_M23_API=login,logout,save,player-state");
    System.out.println("WORLDLINE_M23_SOURCE="
        + B173DedicatedServer.class.getProtectionDomain().getCodeSource().getLocation());
    System.out.println(
        "WORLDLINE_M23_POSITION=" + player.x() + "," + player.y() + "," + player.z());
