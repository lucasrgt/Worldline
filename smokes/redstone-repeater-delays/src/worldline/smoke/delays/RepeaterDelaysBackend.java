...
final class RepeaterDelaysBackend implements GameBackend {
  ...
  void placeCircuit() {
    ...
    for (int index = 0; index < Z.length; index++) {
      require(current.setBlockAndMetadataWithNotify(
                  TORCH_X, Y, Z[index], Block.torchRedstoneActive.blockID, 5),
          "torch placement failed");
      require(current.setBlockAndMetadataWithNotify(
                  REPEATER_X, Y, Z[index], Block.redstoneRepeaterIdle.blockID, META[index]),
  ...
  void assertPlacedState() {
    for (int index = 0; index < Z.length; index++) {
      require(
          requireWorld().getBlockId(REPEATER_X, Y, Z[index]) == Block.redstoneRepeaterIdle.blockID,
