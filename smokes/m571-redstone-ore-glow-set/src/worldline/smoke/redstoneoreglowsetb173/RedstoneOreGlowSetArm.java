...
public final class RedstoneOreGlowSetArm {
  ...
  static void awaitOre(B173WireClient actor, BlockPosition ore, BlockState want, int polls, String label) {
    RemoteWorldView world = worldline.test.WorldlineSmokeAwait.awaitWorld(
        actor, view -> want.equals(view.blockAt(ore.x(), ore.y(), ore.z())), label, polls);
  ...
  static void stepOn(B173WireClient actor, BlockPosition ore) {
    ...
    worldline.test.WorldlineSmokeAwait.awaitCheckedEntity(actor, () -> {
      ...
      return worldline.test.WorldlineSmokeAwait.observe(actor, 1).blockAt(ore.x(), ore.y(), ore.z());
  ...
  static PlayerPose walk(B173WireClient actor, PlayerPose pose, double x, double y, double z) {
    ...
    throw new IllegalStateException("walk pose drift x=" + here.x() + " y=" + here.y() + " z=" + here.z());
  }
  ...
  static void awaitPlayers(B173DedicatedServer server, int count) {
    new worldline.test.WorldlineAwait(50).awaitEntity(
        server::players, players -> players.size() == count, "player count");
  }
