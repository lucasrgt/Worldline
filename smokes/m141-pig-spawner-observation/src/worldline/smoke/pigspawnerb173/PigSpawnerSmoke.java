package worldline.smoke.pigspawnerb173;
...
/** Places one official default-pig spawner and correlates Packet24 across two peers. */
public final class PigSpawnerSmoke {
  private PigSpawnerSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 8)
      throw new IllegalArgumentException(
          "usage: PigSpawnerSmoke server.jar workspace port seed actor observer chunkX chunkZ");
    ...
    BlockPosition foundation, top, spawner;
    int column;
    try {
      server.boot();
      ...
      require(actor.awaitInventory().occupiedSlots() == 3, "mob fixture inventory drift");
      ...
      while (
        ...
        column++;
        require(column <= 15, "water column exceeded mob fixture stack");
      }
      ...
      RemoteMobSpawn first = B173PigBehavior.spawn(actor), peer = B173PigBehavior.spawn(observer);
      require(first.equals(peer) && first.entityId() != actor.state().entityId()
              && first.entityId() != observer.state().entityId(),
          "peer mob identity drift");
      require(first.metadataEntries() == 2 && first.flags() == 0
              && Math.abs(first.x() - (spawner.x() + 0.5D)) <= 4.5D
              && Math.abs(first.y() - spawner.y()) <= 1.1D
              && Math.abs(first.z() - (spawner.z() + 0.5D)) <= 4.5D,
          "pig packet bounds drift: entries=" + first.metadataEntries() + ",flags=" + first.flags()
              + ",pose=" + first.x() + ":" + first.y() + ":" + first.z() + ",spawner=" + spawner);
      actor.close();
      observer.close();
      ...
      String evidence = "column=" + column + ",platform=7x7-48grass,spawner=" + spawner.x() + ":"
          + spawner.y() + ":" + spawner.z()
          + ":52:0,mob=type90+shared-positive-id+metadata2:flags0+within4x1x4,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=raised-7x7-grass-platform+default-spawner52|cause=official-spawner-delay-and-placement|wire=packet24-int+byte+3int+2byte+metadata|oracle=two-peer-identical-pig90-packet|"
          + evidence;
      System.out.println("WORLDLINE_M141_MOB=" + evidence);
      System.out.println("WORLDLINE_M141_TRACE=" + trace);
      System.out.println("WORLDLINE_M141_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      observer.close();
      server.close();
