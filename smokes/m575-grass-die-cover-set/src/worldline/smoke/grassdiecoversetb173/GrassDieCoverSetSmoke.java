...
public final class GrassDieCoverSetSmoke {
    ...
    public static void main(String[] a) throws Exception {
        if (a.length != 9) throw new IllegalArgumentException(
                "usage: GrassDieCoverSetSmoke server.jar workspace port seed username "
        ...
        GrassDieCoverSetArm.require(seed == 17320110707L && user.equals("GrassDie575")
                && user.length() <= 16 && window >= 1 && window <= 1200
                && windows >= 1 && windows <= 40, "grass-die-cover-set identity drift");
        ...
        try {
            ...
            B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D,
                    new int[] {0, 1}, new int[] {1, 2}, new int[] {64, 16}, new int[] {0, 0});
            actor.connect();
            actor.synchronizePose();
            GrassDieCoverSetArm.require(actor.awaitInventory().occupiedSlots() == 2,
                    "grass-die-cover inventory drift");
            ...
            worldline.test.WorldlineSmokeAwait.awaitWorld(actor,
                    v -> GrassDieCoverSetArm.id(v, center) == 2
                            && GrassDieCoverSetArm.id(v, covered) == 2
                            && GrassDieCoverSetArm.id(v, cover) == 1
                            && GrassDieCoverSetArm.id(v, grass[0]) == 2,
            ...
            String evidence = "column=" + column[0] + ",support="
                    + GrassDieCoverSetArm.token(top, 1, 0)
                    + ",grass-ring=8,source=2:0,exposed="
                    + GrassDieCoverSetArm.cells(exposed) + ",covered="
                    + GrassDieCoverSetArm.token(covered, 3, 0) + ",cover="
                    + GrassDieCoverSetArm.token(cover, 1, 0)
