...
public final class GrassDieCoverSetArm {
    ...
    static BlockPosition[] plant(B173WireClient a, BlockPosition[] support, int id)
            throws Exception {
    ...
    static void waitDie(B173WireClient a, BlockPosition[] grass, BlockPosition[] exposed,
            BlockPosition covered, int window, int windows) throws Exception {
        RemoteWorldView died = worldline.test.WorldlineSmokeAwait.awaitWorld(a,
                v -> id(v, covered) == 3, "covered grass die", window * windows);
        for (BlockPosition g : grass) require(id(died, g) == 2, "source grass 2 died during wait");
        for (BlockPosition p : exposed)
            require(id(died, p) == 2, "exposed grass 2 died during wait");
    }

    static void persist(RemoteChunkSnapshot after, int cx, int cz, BlockPosition[] grass,
            BlockPosition[] exposed, BlockPosition covered, BlockPosition cover) {
        for (BlockPosition g : grass)
            require(at(after, g, cx, cz).equals(new BlockState(2, 0)),
                    "source grass persist drift");
        for (BlockPosition p : exposed)
            require(at(after, p, cx, cz).equals(new BlockState(2, 0)),
                    "exposed grass persist drift");
        require(at(after, covered, cx, cz).equals(new BlockState(3, 0)),
                "covered dirt persist drift");
