...
public final class AeroPairedWireSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    String attackerName = arguments[4], victimName = arguments[5], observerName = arguments[6],
           arm = arguments[7];
    String trigger = arguments[8];
    require("control".equals(arm) || "event".equals(arm), "invalid arm");
    ...
    try {
      ...
      require(victim.awaitInventory().occupiedSlots() == 4, "victim inventory drifted");
      victim.look(0F, 90F);
      ...
      require(attacker.awaitInventory().occupiedSlots() == 1, "attacker inventory drifted");
      ...
      System.out.println("WORLDLINE_M71_WIRE_ARMED=arm=" + arm
          + ";attacker=" + attacker.state().entityId() + ";victim=" + victim.state().entityId());
      System.out.flush();
      await(control, "GO", victim, attacker);
      attacker.sendChat(trigger);
      System.out.println("WORLDLINE_M71_WIRE_TRIGGER=arm=" + arm);
      System.out.flush();
      if ("event".equals(arm)) {
        ...
        require(swing.entityId() == attacker.state().entityId()
            ...
            "event evidence drifted");
        System.out.println("WORLDLINE_M71_WIRE_EVENT=health=20->18;sword=0->1");
      } else {
        require(attacker.inventory().slot(sword).item().damage() == 0, "control weapon drifted");
        System.out.println("WORLDLINE_M71_WIRE_CONTROL=sword=0;combat-request=absent");
      }
      System.out.flush();
      await(control, "RELEASE", victim, attacker);
      attacker.close();
      victim.close();
  ...
  private static void await(BufferedReader control, String expected, PeerSwingSession victim,
      PeerSwingSession attacker) throws Exception {
    long end = System.currentTimeMillis() + 180000L;
    while (System.currentTimeMillis() < end) {
      if (control.ready()) {
        require(expected.equals(control.readLine()), expected + " drifted");
        return;
      }
      worldline.test.WorldlineSmokeAwait.observe(victim, 2);
      worldline.test.WorldlineSmokeAwait.observe(attacker, 2);
