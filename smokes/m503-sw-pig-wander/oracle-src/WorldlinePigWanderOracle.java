...
public final class WorldlinePigWanderOracle {
  ...
  private static void run(CanonicalTrace trace, String label, boolean caged) {
    dj world = new dj(
        new OracleMemorySaveHandler(SEED, "worldline-smoke", caged), "worldline-smoke", SEED, null);
    ...
    for (int tick = 1; tick <= TICKS; tick++) {
      ...
      if (tick % 5 == 0)
        snapshot(trace, label + "-tick" + tick, world, pig);
    }
  ...
  private static void snapshot(CanonicalTrace trace, String label, dj world, oc pig) {
    trace.record(label, world.m(), world.b.size(), (int) milli(pig.aP), (int) milli(pig.aQ),
        (int) milli(pig.aR), pig.bw, pig.bh ? 1 : 0);
  }
