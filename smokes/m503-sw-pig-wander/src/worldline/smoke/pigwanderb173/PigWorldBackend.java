...
final class PigWorldBackend implements GameBackend {
    ...
    PigWorldBackend(long seed, boolean caged) { this.seed = seed; this.caged = caged; }
    ...
    @Override public void close() { world = null; pig = null; }
    ...
    void snapshot(CanonicalTrace trace, String label) {
        ...
        trace.record(label, current.getWorldTime(), current.loadedEntityList.size(),
                (int) milli(pig.posX), (int) milli(pig.posY), (int) milli(pig.posZ),
                pig.ticksExisted, pig.isDead ? 1 : 0);
    }
