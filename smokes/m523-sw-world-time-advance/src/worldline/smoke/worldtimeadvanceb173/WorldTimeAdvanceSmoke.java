...
public final class WorldTimeAdvanceSmoke {
    ...
    public static void main(String[] a) throws Exception {
        ...
        Path jar = Paths.get(a[0]); Path workspace = Paths.get(a[1]);
        int port = Integer.parseInt(a[2]); long seed = Long.parseLong(a[3]);
        String user = a[4]; int heartbeats = Integer.parseInt(a[5]); long target = Long.parseLong(a[6]);
        Duration timeout = Duration.ofSeconds(120);
        require(target > Integer.MAX_VALUE && heartbeats >= 40, "invalid time fixture");

        B173DedicatedServer seedServer = new B173DedicatedServer(jar, workspace, port, seed, timeout, 3, true, true);
        try { seedServer.boot(); }
        finally { seedServer.close(); }
        ...
        B173DedicatedServer ticking = new B173DedicatedServer(jar, workspace, port, seed, timeout, 3, true, true);
        ...
        try {
            ...
            client.connect(); client.synchronizePose(); client.sustainTicks(heartbeats);
            client.close(); awaitPlayers(ticking, 0); ticking.save();
        } finally { client.close(); ticking.close(); }
        long advanced = B173LevelDatTime.read(level); long liveDelta = advanced - baseline;
        require(liveDelta >= heartbeats - 10L && liveDelta <= heartbeats + 200L,
                "heartbeat time delta outside bound: " + liveDelta);

        B173DedicatedServer control = new B173DedicatedServer(jar, workspace, port, seed, timeout, 3, true, true);
        try {
            control.boot();
            require(control.state().worldTime() == advanced, "second restart did not preserve time");
            control.save();
        } finally { control.close(); }
        long finalTime = B173LevelDatTime.read(level); long controlDelta = finalTime - advanced;
        require(controlDelta >= 0L && controlDelta < liveDelta, "no-heartbeat control was not smaller: " + controlDelta);

        String signal = "persisted=signed-long,restart=preserved,profile=overworld+nether,heartbeats=" + heartbeats
                + ",advance=bounded,no-heartbeat=smaller,save=clean,clients=1";
        String trace = "v1|server=official-b1.7.3|seed=" + seed + "|time=above-int32"
                + "|positive=protocol14-heartbeats-" + heartbeats + "|negative=reload-no-heartbeats|" + signal;
