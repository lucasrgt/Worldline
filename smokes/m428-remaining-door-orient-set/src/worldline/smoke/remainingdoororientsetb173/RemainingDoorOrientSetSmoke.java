...
public final class RemainingDoorOrientSetSmoke {
  ...
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: RemainingDoorOrientSetSmoke server.jar workspace port seed username chunkX chunkZ");
    Path jar = Paths.get(a[0]), workspace = Paths.get(a[1]);
    int port = Integer.parseInt(a[2]);
    long seed = Long.parseLong(a[3]);
    String user = a[4];
    int cx = Integer.parseInt(a[5]), cz = Integer.parseInt(a[6]);
    require(seed == 17320110707L && user.equals("DoorOrnt428") && user.length() <= 16,
        "remaining-door-orient-set identity drift");
    Duration timeout = Duration.ofSeconds(90);
    B173DedicatedServer server =
        new B173DedicatedServer(jar, workspace, port, seed, timeout, 3, true);
    B173WireClient actor = new B173WireClient("127.0.0.1", port, user, timeout), reader = null;
    BlockPosition top, east, south, pad1, pad2, pad3, face0, face1, face2, face3;
    int column;
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1},
          new int[] {1, 324}, new int[] {48, 4}, new int[] {0, 0});
      actor.connect();
      actor.synchronizePose();
      require(
          actor.awaitInventory().occupiedSlots() == 2, "remaining-door-orient-set inventory drift");
      ...
      while (
        ...
        require(++column <= 15, "water column exceeded remaining-door-orient-set fixture");
      }
      ...
      pad1 = place(actor, east, BlockFace.EAST, 1);
      south = place(actor, top, BlockFace.SOUTH, 1);
      pad2 = place(actor, south, BlockFace.SOUTH, 1);
      east = place(actor, pad2, BlockFace.EAST, 1);
      pad3 = place(actor, east, BlockFace.EAST, 1);
      actor.selectHeldSlot(1);
      face0 = door(actor, top, -90F, 0);
      face1 = door(actor, pad1, 0F, 1);
      face2 = door(actor, pad2, 90F, 2);
      face3 = door(actor, pad3, 180F, 3);
      RemoteWorldView live = worldline.test.WorldlineSmokeAwait.observe(actor, 5);
      require(halves(live, face0, 0) && halves(live, face1, 1) && halves(live, face2, 2)
              && halves(live, face3, 3),
          "live remaining-door-orient-set drift");
      actor.close();
      ...
      String evidence = "column=" + column + ",support=" + top.x() + ":" + top.y() + ":" + top.z()
          ...
          + ",look=-90+0+90+180,persisted=64:0/8+64:1/9+64:2/10+64:3/11,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=raised-stone+woodendoor64:0/8+64:1/9+64:2/10+64:3/11|cause=packet15-item324-look-90+0+90+180|wire=packet53-door64:0/8+64:1/9+64:2/10+64:3/11|oracle=remaining-door-hinge-face+fresh-login|"
          + evidence;
      System.out.println("WORLDLINE_M428_SET=" + evidence);
      System.out.println("WORLDLINE_M428_TRACE=" + trace);
      System.out.println("WORLDLINE_M428_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
  ...
  private static BlockPosition door(B173WireClient a, BlockPosition support, float yaw, int low)
      throws Exception {
    BlockPosition lower = BlockFace.UP.adjacent(support), upper = BlockFace.UP.adjacent(lower);
    a.look(yaw, 0F);
    worldline.test.WorldlineSmokeAwait.observe(a, 2);
    a.useHeldItemOnBlock(support, BlockFace.UP);
    awaitDoor(a, lower, upper, low, low + 8);
    return lower;
  }
  ...
  private static boolean halves(RemoteWorldView v, BlockPosition lower, int low) {
    return v.blockAt(lower.x(), lower.y(), lower.z()).equals(new BlockState(64, low))
        && v.blockAt(lower.x(), lower.y() + 1, lower.z()).equals(new BlockState(64, low + 8));
  }
  private static boolean persisted(
      RemoteChunkSnapshot q, BlockPosition lower, int cx, int cz, int low) {
    return q.blockAt(local(lower.x(), cx), lower.y(), local(lower.z(), cz))
               .equals(new BlockState(64, low))
        && q.blockAt(local(lower.x(), cx), lower.y() + 1, local(lower.z(), cz))
               .equals(new BlockState(64, low + 8));
  }
  private static String token(BlockPosition lower, int low) {
    return lower.x() + ":" + lower.y() + ":" + lower.z() + ":64:" + low + "/" + (low + 8);
  }
  private static BlockPosition foundation(RemoteChunkSnapshot q, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic remaining-door-orient-set foundation");
  }
