...
public final class FarmlandDrySetSmoke {
    ...
    public static void main(String[] a) throws Exception {
        if (a.length != 9) throw new IllegalArgumentException(
                "usage: FarmlandDrySetSmoke server.jar workspace port seed username "
        ...
        FarmlandDrySetArm.require(seed == 17320110707L && user.equals("FarmDry608")
                && user.length() <= 16 && window >= 1 && window <= 1200
                && windows >= 1 && windows <= 40, "farmland-dry-set identity drift");
        ...
        try {
            ...
            B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D,
                    new int[] {0, 1, 2}, new int[] {1, 3, 290},
                    new int[] {64, 1, 1}, new int[] {0, 0, 0});
            actor.connect();
            actor.synchronizePose();
            FarmlandDrySetArm.require(actor.awaitInventory().occupiedSlots() == 3,
                    "farmland-dry inventory drift");
            ...
            FarmlandDrySetArm.require(placed.blockAt(farm.x(), farm.y(), farm.z()).equals(dry)
                            && FarmlandDrySetArm.id(placed, cover) == 1,
            ...
            String evidence = "column=" + column[0] + ",support="
                    + FarmlandDrySetArm.token(top, 1, 0) + ",cell="
                    + FarmlandDrySetArm.token(farm, 3, 0) + ",cover="
                    + FarmlandDrySetArm.token(cover, 1, 0)
