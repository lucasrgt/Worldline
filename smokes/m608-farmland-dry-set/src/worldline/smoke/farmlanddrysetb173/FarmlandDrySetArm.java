...
public final class FarmlandDrySetArm {
    ...
    static void waitDry(B173WireClient a, BlockPosition farm, BlockPosition cover,
            int window, int windows) throws Exception {
        RemoteWorldView dried = worldline.test.WorldlineSmokeAwait.awaitWorld(a,
                v -> id(v, farm) == 3, "unhydrated farmland dry", window * windows);
        require(id(dried, cover) == 1, "rain roof 1 lost during wait");
    }

    static void persist(RemoteChunkSnapshot after, int cx, int cz, BlockPosition top,
            BlockPosition farm, BlockPosition cover, BlockState dirt) {
        require(at(after, top, cx, cz).equals(new BlockState(1, 0))
                        && at(after, farm, cx, cz).equals(dirt)
