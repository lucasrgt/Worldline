...
public final class RemainingBucketRestSetSmoke{
 ...
 public static void main(String[]a)throws Exception{
  ...
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1,2,3,4,5,6},new int[]{1,1,3,9,11,325,325},new int[]{64,64,2,1,1,1,1},new int[]{0,0,0,0,0,0,0});actor.connect();pose=actor.synchronizePose();require(actor.awaitInventory().occupiedSlots()==7,"remaining-bucket-rest-set inventory drift");
   ...
   actor.selectHeldSlot(7);pose=actor.moveAndObserve(0D,-2D,0D,2).resulting();actor.sustainTicks(fixtureTicks);
   require(open(actor,waterTarget).equals(air)&&open(actor,lavaTarget).equals(air),"horizontal air absent");
   RemoteWorldView live=actor.sustainTicks(flowTicks);waterFlow=live.blockAt(waterTarget.x(),waterTarget.y(),waterTarget.z());lavaFlow=live.blockAt(lavaTarget.x(),lavaTarget.y(),lavaTarget.z());
   ...
   RemoteWorldView waterReject=actor.sustainTicks(5);require(waterReject.blockAt(waterTarget.x(),waterTarget.y(),waterTarget.z()).equals(waterFlow)&&waterReject.blockAt(waterSource.x(),waterSource.y(),waterSource.z()).equals(stillWater)&&held(actor,5).equals(empty),"flowing water 8/9 pickup must reject");
   ...
   require(actor.sustainTicks(5).blockAt(waterSource.x(),waterSource.y(),waterSource.z()).equals(air)&&held(actor,5).equals(waterBucket),"source water 9:0 pickup drift");
   ...
   RemoteWorldView lavaReject=actor.sustainTicks(5);require(lavaReject.blockAt(lavaTarget.x(),lavaTarget.y(),lavaTarget.z()).equals(lavaFlow)&&lavaReject.blockAt(lavaSource.x(),lavaSource.y(),lavaSource.z()).equals(stillLava)&&held(actor,6).equals(empty)&&actor.health()==20,"flowing lava 10/11 pickup must reject");
   ...
   require(actor.sustainTicks(5).blockAt(lavaSource.x(),lavaSource.y(),lavaSource.z()).equals(air)&&held(actor,6).equals(lavaBucket)&&actor.health()==20,"source lava 11:0 pickup drift");
