...
public final class FireSpreadWoodSetSmoke {
  ...
  public static void main(String[] args) throws Exception {
    ...
    FireSpreadWoodSetArm.require(seed == 17320110707L && user.equals("FireWood606") && user.length() <= 16
            && window >= 1 && window <= 1200 && windows >= 1 && windows <= 40,
        "fire-spread-wood identity drift");
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2, 3, 4},
          new int[] {1, 87, 259, 5, 17}, new int[] {64, 1, 1, 16, 4}, new int[] {0, 0, 0, 0, 0});
      actor.connect();
      actor.synchronizePose();
      FireSpreadWoodSetArm.require(actor.awaitInventory().occupiedSlots() == 5, "fire-spread-wood inventory drift");
      ...
      FireSpreadWoodSetArm.require(FireSpreadWoodSetArm.id(placed, rack) == 87
              && FireSpreadWoodSetArm.id(placed, cover) == 1 && FireSpreadWoodSetArm.id(placed, fuels[0]) == 17
      ...
      String evidence = "column=" + column[0] + ",support=" + FireSpreadWoodSetArm.token(top, 1, 0) + ",rack="
          + FireSpreadWoodSetArm.token(rack, 87, 0) + ",flint=259,source-fire=" + FireSpreadWoodSetArm.cell(flame)
      ...
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          ...
          + "|oracle=live-source-fire51+adjacent-wood-plank-spread+fresh-login|" + evidence;
