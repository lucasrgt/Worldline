...
public final class FireSpreadWoodSetArm {
    ...
    static BlockPosition raise(B173WireClient actor, RemoteChunkSnapshot initial,
    ...
    static void waitSpread(B173WireClient actor, BlockPosition flame, BlockPosition[] fuels,
            int window, int windows) throws Exception {
        RemoteWorldView spread = WorldlineSmokeAwait.awaitWorld(actor,
                view -> id(view, flame) == 51 && anySpread(view, fuels),
                "wood plank fire spread", window * windows);
        ...
        while (fuelIndex < fuels.length) {
            ...
            require(value == 5 || value == 17 || value == 0 || value == 51,
                    "fuel cell drift during wait " + value);
            fuelIndex++;
        }
    }

    static void persist(RemoteChunkSnapshot after, int cx, int cz, BlockPosition rack,
            BlockPosition flame, BlockPosition cover, BlockPosition[] fuels) {
        require(at(after, rack, cx, cz).equals(new BlockState(87, 0)),
                "netherrack persist drift");
