...
final class M789Gate {
    private M789Gate() {}

    static M789Result evaluate(Path product, List<M789Pair> pairs,
                               List<M789Visual> visuals, Properties config) throws Exception {
        ...
        M789Hitch hitch = hitch(product, pairs, config);
        boolean performance = fpsRatio >= threshold(config, "fps.minimum.ratio")
            ...
            && hitch.passes();
        int size = pairs.size();
        return new M789Result(visualPass, workloadRepeatable, capturePass,
    ...
    private static Totals totals(List<M789Pair> pairs) {
        Totals value = new Totals();
        for (M789Pair pair : pairs) value.add(pair);
        return value;
    }

    private static boolean workloadRepeatable(List<M789Pair> pairs, double tolerance) {
    ...
    private static boolean qualifies(List<M789Pair> pairs, boolean baselineFirst,
                                      Properties config) {
    ...
    private static M789Hitch hitch(Path product, List<M789Pair> source,
        ...
        try (URLClassLoader loader = new URLClassLoader(new URL[] {product.toUri().toURL()}, null)) {
            ...
            for (M789Pair pair : source) evidence.add(pairMethod.invoke(null,
            ...
            return new M789Hitch(number(value, "baselineRatePpm"),
    ...
    private static double rate(M789Artifact value) {
        return (double) value.submittedMachines / value.frames;
    }
    private static double rebuildRate(M789Artifact value) {
