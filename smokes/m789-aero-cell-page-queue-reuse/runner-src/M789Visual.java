...
record M789Visual(String label, M789Artifact baseline, M789Artifact candidate,
                  long changedPixels, int maximumDelta, Set<Long> changedLocations) {
    static M789Visual compare(String label, M789Artifact baseline,
                              M789Artifact candidate) throws Exception {
        SmokeSupport.require(baseline.width == candidate.width && baseline.height == candidate.height
            && baseline.captures == candidate.captures, "M789 framebuffer shape diverged");
        long changed = 0L;
        int maximum = 0;
        ...
        return new M789Visual(label, baseline, candidate, changed, maximum, locations);
    }
