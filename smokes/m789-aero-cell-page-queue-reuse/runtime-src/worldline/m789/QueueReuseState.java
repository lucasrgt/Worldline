...
import aero.modellib.test.WorldlineM789Rehydrator;
import java.io.File;
...
public final class QueueReuseState {
    public static final boolean ENABLED = Boolean.getBoolean("worldline.m789.enabled");
    private static final boolean PREPARE = Boolean.getBoolean("worldline.m789.prepare");
    private static final String ARM = System.getProperty("worldline.m789.arm", "prepare");
    private static final int TARGET_FRAMES = Integer.getInteger("worldline.m789.frames", 1200);
    private static final int WARM_FRAMES = Integer.getInteger("worldline.m789.warmFrames", 480);
    private static final int MAX_BLANK_CAPTURES =
        Integer.getInteger("worldline.m789.maxBlankCaptures", 24);
    private static final long MINIMUM_NS = Long.getLong("worldline.m789.minimumMillis", 20000L)
        * 1_000_000L;
    ...
    public static void drive(Minecraft game) {
        ...
        if (stage == 0 && game.world == null) {
            ...
            System.out.println("[WorldlineM789] start prepare=" + PREPARE + " arm=" + ARM);
    ...
    public static void frame(Minecraft game) {
        ...
        if (!warmComplete()) {
            ...
            if (convergenceFrames > 0 && convergenceFrames % 120 == 0) {
                System.out.println("[WorldlineM789] warm arm=" + ARM + " route=" + routeWarm
            ...
            require(convergenceFrames < 2400, "M789 page cache did not converge");
            return;
        }
        ...
        if (retained >= TARGET_FRAMES && elapsed >= MINIMUM_NS
            ...
            require(QueueReuseProbe.blankCaptures() <= MAX_BLANK_CAPTURES,
                "M789 blank framebuffer retry budget exhausted");
    ...
    public static void renderFixture(double cameraX, double cameraY, double cameraZ) {
        ...
        int submitted = 0;
        WorldlineM789QueueReuse.discardQueued();
        ...
        try {
            for (Object value : game.world.blockEntities) {
                ...
                if (!(block instanceof MegaModelBlockEntity)
                        || !WorldlineM789Rehydrator.contains(block.x, block.y, block.z)) continue;
    ...
    private static void warmWorld(Minecraft game) {
        ...
        machines = WorldlineM789Rehydrator.count(world);
        if (worldWarm == 2 && machines != WorldlineM789Rehydrator.MACHINES) {
            WorldlineM789Rehydrator.rehydrate(world);
            machines = WorldlineM789Rehydrator.count(world);
        }
        WorldlineM789Rehydrator.order(world);
        if (machines < WorldlineM789Rehydrator.MACHINES && worldWarm < 1200) return;
        require(machines == WorldlineM789Rehydrator.MACHINES,
            "M789 fixture drift: " + machines);
        QueueReuseScene.place(game.player, 0);
        if (worldWarm < 40) return;
        if (PREPARE) {
            world.saveWithLoadingDisplay(true, null);
            System.out.println("[WorldlineM789] fixture-ready machines=" + machines);
            stage = 3;
            game.scheduleStop();
        } else stage = 2;
    }
    ...
    private static void finish(Minecraft game) {
        try {
            QueueReuseProbe.write(new File(System.getProperty("worldline.m789.metrics")),
                new File(System.getProperty("worldline.m789.framesFile")), ARM, machines);
        } catch (Exception error) {
            throw new IllegalStateException("M789 artifact write failed", error);
        }
        System.out.println("[WorldlineM789] capture-complete arm=" + ARM
