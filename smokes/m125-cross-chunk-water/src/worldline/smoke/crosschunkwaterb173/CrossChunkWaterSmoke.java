...
/** Opens a bounded official-water trench across one chunk seam. */
public final class CrossChunkWaterSmoke {
  private CrossChunkWaterSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 11)
      throw new IllegalArgumentException(
          "usage: CrossChunkWaterSmoke server.jar workspace port seed username sourceX neighborX chunkZ fixtureTicks flowTicks extra");
    Path jar = Paths.get(a[0]), workspace = Paths.get(a[1]);
    int port = Integer.parseInt(a[2]);
    long seed = Long.parseLong(a[3]);
    String user = a[4];
    int sourceX = Integer.parseInt(a[5]), neighborX = Integer.parseInt(a[6]),
        cz = Integer.parseInt(a[7]), fixtureTicks = Integer.parseInt(a[8]),
        flowTicks = Integer.parseInt(a[9]);
    require(neighborX == sourceX + 1 && "m125".equals(a[10]), "M125 seam descriptor drift");
    ...
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, user, sourceX * 16 + 15.5D, 60D, cz * 16 + 6.5D,
          new int[] {0, 1, 2}, new int[] {1, 3, 9}, new int[] {48, 1, 1}, new int[] {0, 0, 0});
      actor.connect();
      actor.synchronizePose();
      require(actor.awaitInventory().occupiedSlots() == 3, "seam trench inventory drift");
      RemoteWorldView initialWorld = actor.awaitRemoteChunk(sourceX, cz);
      worldline.test.WorldlineSmokeAwait.observe(actor, 20);
      initialWorld = actor.awaitRemoteChunk(neighborX, cz);
      RemoteChunkSnapshot initial = initialWorld.chunkAt(sourceX, cz);
      foundation = foundation(initial, sourceX, cz);
      top = foundation;
      column = 0;
      ...
      require(source.x() == sourceX * 16 + 15 && target.x() == neighborX * 16,
          "trench missed chunk seam");
      ...
      actor.awaitBlock(source, new BlockState(9, 0));
      RemoteWorldView filled = worldline.test.WorldlineSmokeAwait.observe(actor, 5);
      require(filled.blockAt(source.x(), source.y(), source.z()).equals(new BlockState(9, 0))
              && filled.blockAt(target.x(), target.y(), target.z()).equals(new BlockState(3, 0)),
          "seam water baseline drift");
      ...
      worldline.test.WorldlineSmokeAwait.observe(treatment, 20);
      RemoteWorldView before = treatment.awaitRemoteChunk(sourceX, cz);
      before = treatment.awaitRemoteChunk(neighborX, cz);
      left0 = before.chunkAt(sourceX, cz);
      right0 = before.chunkAt(neighborX, cz);
      ...
      settled = worldline.test.WorldlineSmokeAwait.observe(treatment, flowTicks)
                    .blockAt(target.x(), target.y(), target.z());
      require(opened.equals(new BlockState(0, 0)) && water(settled.legacyId()),
          "cross-chunk water absent");
      treatment.close();
      awaitPlayers(server, 0);
      server.save();
      ...
      RemoteWorldView after = reader.awaitRemoteChunk(sourceX, cz);
      after = reader.awaitRemoteChunk(neighborX, cz);
      left1 = after.chunkAt(sourceX, cz);
      right1 = after.chunkAt(neighborX, cz);
      require(left1.blockAt(15, source.y(), source.z() - cz * 16).equals(new BlockState(9, 0))
              && right1.blockAt(0, target.y(), target.z() - cz * 16).equals(settled),
          "fresh seam water drift");
    } finally {
      actor.close();
      if (treatment != null)
        treatment.close();
    ...
    require(left.changed == 0 && right.changed == 1,
        "cross-chunk water changed unrelated cells " + left + " / " + right);
    String evidence = "column=" + column + ",source=" + source.x() + ":" + source.y() + ":"
        + source.z() + ":9:0,target=" + target.x() + ":" + target.y() + ":" + target.z()
        + ":3:0->0:0->" + settled.legacyId() + ":" + settled.metadata() + ",left=" + left
        + ",right=" + right;
    String trace = "v1|server=official-b1.7.3|seed=" + seed
        + "|fixture=stone-trench-across-chunks+seeded-water9+dirt-gate3|seam=" + sourceX + ":"
        + neighborX + "," + cz + "|settle=" + fixtureTicks + "+" + flowTicks
        + "ticks|cause=packet14-open-neighbor-cell|confirmation=packet53-air|effect=official-horizontal-water-cross-chunk|observation=fresh-packet51-both-chunks|"
        + evidence + "|disconnect=clean";
    System.out.println("WORLDLINE_M125_WATER=" + evidence);
  ...
  private static BlockPosition foundation(RemoteChunkSnapshot q, int cx, int cz) {
    ...
    throw new IllegalStateException("no seam trench foundation");
  }
