...
/** Common server-safe registration for the exact M92 remove/restore protocol. */
public final class WorldlineRecoveryMod {
    ...
    @EventListener private static void messages(MessageListenerRegistryEvent event) {
        ...
        event.register(RESTORE, (player, packet) -> { if (!player.world.isRemote) throw new IllegalStateException("M92 restore reached server"); WorldlineRecoveryState.restore(packet.ints); });
    }
}
