...
public abstract class WorldlineRecoveryFrameMixin {
    @Shadow @Final private Minecraft client;@Inject(method="onFrameUpdate(F)V",at=@At("HEAD"))private void head(float d,CallbackInfo c){WorldlineRecoveryEvent.head(client);}
    @Inject(method="onFrameUpdate(F)V",at=@At("TAIL"))private void tail(float d,CallbackInfo c){WorldlineRecoveryEvent.tail();if(WorldlineFrameCensus.sealed()&&!WorldlineRecoveryFile.written())WorldlineRecoveryFile.write();}
}
