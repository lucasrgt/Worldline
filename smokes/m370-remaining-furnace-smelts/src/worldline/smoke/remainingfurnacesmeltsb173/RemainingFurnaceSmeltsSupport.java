...
/** Raised-stone pads and idle furnace 61:2 placement for remaining cactus, log, and clay. */
final class RemainingFurnaceSmeltsSupport {
  private RemainingFurnaceSmeltsSupport() {
  }
  ...
  static Raised raise(B173WireClient actor, RemoteChunkSnapshot chunk, int cx, int cz)
    ...
    while (water(chunk.blockAt(local(top.x(), cx), top.y() + 1, local(top.z(), cz)).legacyId())) {
      ...
      require(++column <= 15, "water column exceeded remaining-furnace-smelts fixture");
    }
  ...
  static BlockPosition foundation(RemoteChunkSnapshot chunk, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic remaining-furnace-smelts foundation");
  }
