...
public final class RemainingFurnaceSmeltsSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    RemoteItemStack cactus = new RemoteItemStack(81, 1, 0), log = new RemoteItemStack(17, 1, 0),
                    ...
                    green = new RemoteItemStack(351, 1, 2),
                    charcoal = new RemoteItemStack(263, 1, 1),
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D,
          new int[] {0, 1, 2, 3, 4, 5, 6, 7}, new int[] {1, 61, 81, 17, 337, 263, 263, 263},
      ...
      RemainingFurnaceSmeltsSupport.require(inventory.occupiedSlots() == 8
              && inventory.slot(38).item().equals(cactus) && inventory.slot(39).item().equals(log)
              && inventory.slot(40).item().equals(clay) && inventory.slot(41).item().equals(coal)
              && inventory.slot(42).item().equals(coal) && inventory.slot(43).item().equals(coal)
              && !inventory.slot(38).item().equals(new RemoteItemStack(15, 1, 0)),
          "remaining-furnace-smelts inventory drift");
      RemainingFurnaceSmeltsSupport.Raised raised = RemainingFurnaceSmeltsSupport.raise(
          actor, actor.awaitRemoteChunk(cx, cz).chunkAt(cx, cz), cx, cz);
      ...
      RemoteItemStack cactusOut =
          B173RemainingFurnaceSmelts.smelt(actor, cactusFurnace, 38, 41, cactus, green);
      RemoteItemStack logOut =
          B173RemainingFurnaceSmelts.smelt(actor, logFurnace, 39, 42, log, charcoal);
      RemoteItemStack clayOut =
          B173RemainingFurnaceSmelts.smelt(actor, clayFurnace, 40, 43, clay, brick);
      RemainingFurnaceSmeltsSupport.require(
          cactusOut.equals(green) && logOut.equals(charcoal) && clayOut.equals(brick),
          "compound remaining furnace outputs drifted");
      actor.close();
