    ...
    public static void main(String[] arguments) throws Exception {
        if (arguments.length != 9) {
            throw new IllegalArgumentException(
                    "usage: StickyHeadBreakSetSmoke server.jar workspace port seed username "
        ...
        StickyHeadBreakSetArm.require(seed == 17320110707L && user.equals("StkHeadBrk573")
                && user.length() <= 16, "sticky-head-break identity drift");
        ...
        try {
            ...
            B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D,
                    new int[] {0, 1, 2, 3}, new int[] {1, 29, 69, 257},
                    new int[] {32, 1, 1, 1}, new int[] {0, 0, 0, 0});
            actor.connect();
            actor.synchronizePose();
            actor.look(-90F, 0F);
            StickyHeadBreakSetArm.require(actor.awaitInventory().occupiedSlots() == 4,
                    "sticky-head-break inventory drift");
            ...
            RemoteChunkSnapshot after = reader.awaitRemoteChunk(chunkX, chunkZ)
                    .chunkAt(chunkX, chunkZ);
            arm.persist(after, chunkX, chunkZ);
            String evidence = "column=" + column[0]
                    + ",extend=29:4->12,head-break=34:12->0,base-left=29:12->0,piston="
                    + StickyHeadBreakSetArm.cell(arm.piston) + ":29:4->12->0,head="
                    + StickyHeadBreakSetArm.cell(arm.head) + ":1:0->34:12->0:0,pushed="
                    + StickyHeadBreakSetArm.cell(arm.pushed)
            ...
            String trace = "v1|server=official-b1.7.3|seed=" + seed
                    + "|fixture=sticky29-west-extended|settle=" + fixture + "+" + signal
                    + "ticks|cause=packet15-lever-activate+packet14-ironpick257-head34"
