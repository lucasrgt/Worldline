...
public final class GrassSpreadSetArm{
 static BlockPosition raise(B173WireClient a,RemoteChunkSnapshot initial,int cx,int cz,int[] column)throws Exception{
  ...
  while(water(at(initial,BlockFace.UP.adjacent(top),cx,cz).legacyId())){top=place(a,top,BlockFace.UP,1);a.moveAndObserve(0D,1D,0D,1);require(++column[0]<=15,"water column exceeded grass-spread fixture");}
 ...
 static BlockPosition[] ring(B173WireClient a,BlockPosition top)throws Exception{
  BlockPosition e=place(a,top,BlockFace.EAST,1),w=place(a,top,BlockFace.WEST,1),n=place(a,top,BlockFace.NORTH,1),s=place(a,top,BlockFace.SOUTH,1);
  return new BlockPosition[]{e,w,n,s,place(a,e,BlockFace.NORTH,1),place(a,e,BlockFace.SOUTH,1),place(a,w,BlockFace.NORTH,1),place(a,w,BlockFace.SOUTH,1)};
 }
 static BlockPosition[] plant(B173WireClient a,BlockPosition[] support,int id)throws Exception{BlockPosition[] out=new BlockPosition[support.length];for(int i=0;i<support.length;i++)out[i]=place(a,support[i],BlockFace.UP,id);return out;}
 ...
 static void persist(RemoteChunkSnapshot after,int cx,int cz,BlockPosition[] grass,BlockPosition[] lit,BlockPosition covered,BlockPosition cover){
  ...
  boolean spread=false;for(BlockPosition p:lit){int n=at(after,p,cx,cz).legacyId();require(n==2||n==3,"lit cell persist drift "+n);if(n==2)spread=true;}
  require(spread,"lit dirt 3->2 did not persist");
 }
 static String cells(BlockPosition[] p){StringBuilder s=new StringBuilder();for(int i=0;i<p.length;i++){if(i>0)s.append('+');s.append(cell(p[i]));}return s.toString();}
 static boolean anyGrass(RemoteWorldView v,BlockPosition[] lit){for(BlockPosition p:lit)if(id(v,p)==2)return true;return false;}
 static int id(RemoteWorldView v,BlockPosition p){return v.blockAt(p.x(),p.y(),p.z()).legacyId();}
