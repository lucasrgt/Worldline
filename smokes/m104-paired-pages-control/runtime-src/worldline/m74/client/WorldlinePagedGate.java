...
public final class WorldlinePagedGate {
    ...
    public static boolean prepare(Minecraft minecraft) { if (prepared || !WorldlinePagedBridge.armed()) return false;
        ...
        if (!WorldlineCensusProbe.mode().equals("present") || Aero_BECellRenderer.ENABLED!=pages || minecraft.options.fpsLimit != 0 || Aero_FramePacer.ENABLED || Aero_FramePacer.targetFps() != 0)
            throw new IllegalStateException("M104 runtime configuration drift"); WorldlinePagedBridge.align(); prepared = true; WorldlinePagedTimer.arm();
        System.out.println("[WorldlinePaged] armed marker=true pages="+pages+" fpsLimit=0 aeroFramePacing=false"); return true; }
}
