package worldline.smoke.woodendooropenb173;
...
/** Places official wooden door item 324 and toggles BlockDoor 64 open, then reloads the open halves. */
public final class WoodenDoorOpenSmoke {
  private WoodenDoorOpenSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: WoodenDoorOpenSmoke server.jar workspace port seed username chunkX chunkZ");
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1},
          new int[] {1, 324}, new int[] {32, 1}, new int[] {0, 0});
      actor.connect();
      ...
      while (
        ...
        require(++column <= 15, "water column exceeded door fixture");
      }
      ...
      require(after.blockAt(local(lower.x(), cx), lower.y(), local(lower.z(), cz))
                  .equals(new BlockState(64, 4))
              && after.blockAt(local(upper.x(), cx), upper.y(), local(upper.z(), cz))
                  .equals(new BlockState(64, 12)),
          "persisted open wooden door drift");
      String evidence = "column=" + column + ",support=" + top.x() + ":" + top.y() + ":" + top.z()
          + ":1:0,lower=" + lower.x() + ":" + lower.y() + ":" + lower.z()
          + ":64:0->4,upper=" + upper.x() + ":" + upper.y() + ":" + upper.z()
          + ":64:8->12,persisted=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=raised-stone+woodendoor64|cause=packet15-item324-place+empty-hand-packet15-open|wire=packet53-door64:0/8->4/12|oracle=woodendoor-open+fresh-login|"
          + evidence;
      System.out.println("WORLDLINE_M277_DOOR=" + evidence);
      System.out.println("WORLDLINE_M277_TRACE=" + trace);
      System.out.println("WORLDLINE_M277_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
