...
public final class WorkbenchWindowSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      ...
      require(replaceable(baseline.blockAt(target.x(), target.y(), target.z())),
          "workbench target was not replaceable");
      ...
      BlockState placed = actor.awaitBlock(target, new BlockState(58, 0))
                              .blockAt(target.x(), target.y(), target.z());
      worldline.test.WorldlineSmokeAwait.observe(actor, 5);
      require(placed.equals(new BlockState(58, 0)) && actor.inventory().occupiedSlots() == 0,
          "placed workbench state drifted");
      acquire(actor, actorName, 1, 1);
      ...
      require(actor.inventory().occupiedSlots() == 1
          ...
          "stone sentinel seed drifted");
      actor.selectHeldSlot(1);
      opened = actor.openWorkbench(target, BlockFace.UP);
      RemoteWindowDescriptor descriptor = opened.descriptor();
      RemoteInventoryView combined = opened.inventory();
      require(descriptor.kind() == RemoteWindowKind.WORKBENCH
              && "Crafting".equals(descriptor.title()) && descriptor.containerSlots() == 9
              && descriptor.playerTailOffset() == 10 && descriptor.totalSlots() == 46
              && combined.size() == 46,
          "workbench descriptor or shape drifted");
      for (int slot = 0; slot < 10; slot++)
        require(combined.slot(slot).empty(), "workbench result or matrix was not empty");
      require(combined.occupiedSlots() == 1 && combined.slot(37).item().equals(stone)
              && actor.inventory().slot(36).item().equals(stone),
          "workbench personal tail mapping drifted");
      closure = actor.closeWindow();
      require(closure.closedWindow().equals(opened) && closure.proofAction() == 1
              && closure.personalBefore().slot(36).item().equals(stone)
              && closure.personalAfter().equals(closure.personalBefore()),
          "workbench close proof drifted");
      actor.close();
      awaitPlayers(server, 0);
      server.save();
      require(server.player(actorName).inventoryItems() == 1,
          "workbench player inventory did not persist");
    } finally {
      actor.close();
      server.close();
    }
  ...
  private static PlayerPose acquire(WorkbenchSession client, String username, int id, int count) {
    ...
    }, inventory -> inventory.occupiedSlots() >= target, "workbench grant", 100);
