...
public final class WorldlineRecoveryState {
  ...
  public static synchronized void ack(int[] values) {
    if (values == null || values.length != 5 || values[4] < 1 || values[4] > 2)
      throw new IllegalStateException("invalid M89 ack");
    if (values[4] == 1) {
      if (removeAck)
        throw new IllegalStateException("duplicate M89 remove ack");
      x = values[0];
      y = values[1];
      z = values[2];
      root = values[3];
    ...
    if (!removeAck || restoreAck || values[0] != x || values[1] != y || values[2] != z
        || values[3] != root)
      throw new IllegalStateException("invalid M89 restore ack");
    restoreAck = true;
  }
  public static synchronized void restore(int[] values) {
    if (values == null || values.length != 4 || values[3] <= 0)
      throw new IllegalStateException("invalid M89 restore state");
    if (restoreReceived) {
      if (values[0] != x || values[1] != y || values[2] != z || values[3] != restoreNonce)
        throw new IllegalStateException("conflicting M89 restore state");
      return;
    }
    if (!removeAck || values[0] != x || values[1] != y || values[2] != z
        || values[3] != root * 100 + 5)
      throw new IllegalStateException("unbound M89 restore state");
  ...
  public static synchronized void apply(World world) {
    ...
    if (be.nonce() != 0 && be.nonce() != restoreNonce)
      throw new IllegalStateException("M89 restored nonce drift");
