...
public final class WorldlineRecoveryEvent {
    ...
    public static void head(Minecraft client) {
        if (!WorldlinePagedBridge.running() || WorldlinePagedBridge.sealed()) return; int x = WorldlineCensusSync.x(), y = WorldlineCensusSync.y(),
                z = WorldlineCensusSync.z() + 1, root = WorldlineCensusProbe.nonce(); WorldlineRecoveryState.apply(client.world);
        if (!removeRequested && WorldlinePagedBridge.count() >= AFTER) {
            if (AFTER != 300 || RESTORE_AFTER != 30 || !"8".equals(System.getProperty("aero.becell.rebuildsPerFrame"))) throw new IllegalStateException("M89 runtime drift");
        ...
        if (restoreRequested) { WorldlineRecoveryState.apply(client.world); blockRestored = client.world.getBlockId(x, y, z) == WorldlineCensusMod.block.id;
            beRestored = client.world.getBlockEntity(x, y, z) instanceof WorldlineCensusBlockEntity be && be.nonce() == root * 100 + 5; }
    }
    public static void tail() {
        int x = WorldlineCensusSync.x(), y = WorldlineCensusSync.y(), z = WorldlineCensusSync.z() + 1, root = WorldlineCensusProbe.nonce(), queued = Aero_BECellRenderer.queuedLastFrame();
