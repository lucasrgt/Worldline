...
public final class B173ArmorCraftsClick {
  ...
  public static int[] apply(B173WireClient actor, int ironSlot, int[] dests) {
    try {
      ...
      if (window.descriptor().kind() != RemoteWindowKind.WORKBENCH
          || window.inventory().size() != 46 || dests == null || dests.length != 4 || ironSlot < 9
          || ironSlot > 44)
  ...
  private static int craft(B173PlayChannel channel, B173PlayInbound inbound, int windowId,
      int action, int ironSlot, int dest, int[] grid, int resultId) throws IOException {
  ...
  private static void requireDest(RemoteInventoryView personal, int dest, int ironSlot) {
    if (dest < 9 || dest > 44 || dest == ironSlot || dest == 5 || dest == 6 || dest == 7
