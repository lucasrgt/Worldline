...
public final class RedstoneWireDepowerSmoke {
    ...
    public static void main(String[]arguments)throws Exception{
        if(arguments.length!=9)throw new IllegalArgumentException("usage: RedstoneWireDepowerSmoke server.jar workspace port seed username chunkX chunkZ fixtureTicks signalTicks");
        ...
        RemoteChunkSnapshot before,after;BlockPosition foundation,top,lever,wire;BlockState leverOn,leverOff,wireOn,wireOff;int column;
        try{server.boot();B173PlayerSeed.writeInventory(workspace,username,4.5D,60D,4.5D,new int[]{0,1,2},
            ...
            require(after.blockAt(local(lever.x(),chunkX),lever.y(),local(lever.z(),chunkZ)).equals(leverOff)
                    &&after.blockAt(local(wire.x(),chunkX),wire.y(),local(wire.z(),chunkZ)).equals(wireOff),"fresh depowered state drift");
        }finally{actor.close();if(reader!=null)reader.close();server.close();}
        StateDelta delta=delta(before,after);require(delta.changed==2,"depower changed unrelated states: "+delta);
        String evidence="column="+column+",lever="+lever.x()+":"+lever.y()+":"+lever.z()+":"+leverOn.metadata()+"->"+leverOff.metadata()
                +",wire="+wire.x()+":"+wire.y()+":"+wire.z()+":"+wireOn.metadata()+"->"+wireOff.metadata()+",states="+delta;
        String trace="v1|server=official-b1.7.3|seed="+seed+"|fixture=stone-column+lever69+dust331-wire55|settle="+fixtureTicks
                +"+"+signalTicks+"ticks|precondition=lever69:9+wire55:15|cause=packet15-lever-activate|effect=packet53-wire-depower|observation=fresh-login-packet51|"+evidence+"|disconnect=clean";
        System.out.println("WORLDLINE_M117_SIGNAL="+evidence);System.out.println("WORLDLINE_M117_TRACE="+trace);
