...
final class FurnaceSmeltInterruptClicks {
    ...
    static RemoteFurnaceSmelt control(B173WireClient actor, BlockPosition furnace, int inputSlot,
            int fuelSlot) throws Exception {
        ...
        FurnaceSmeltInterruptSupport.require(load.inputTakeAction() == 1 && load.inputStoreAction() == 2
                ...
                && load.input().equals(COBBLE) && load.fuel().equals(COAL)
                && actor.inventory().slot(inputSlot).empty()
                && actor.inventory().slot(fuelSlot).empty(), "control furnace load drifted");
        ...
        FurnaceSmeltInterruptSupport.require(smelt.output().equals(STONE) && !smelt.output().equals(IRON)
                ...
                && smelt.completionBurn() == 1401
                && smelt.window().inventory().slot(2).item().equals(STONE)
                && smelt.window().inventory().slot(0).empty()
                && smelt.window().inventory().slot(1).empty(), "control furnace smelt drifted");
        close(actor, smelt.window());
        return smelt;
    }

    static void input(B173WireClient actor, BlockPosition furnace, int inputSlot, int fuelSlot)
            throws Exception {
        open(actor, furnace, inputSlot, fuelSlot, COBBLE, COAL);
        RemoteFurnaceLoad load = actor.loadFurnace(inputSlot, fuelSlot);
        FurnaceSmeltInterruptSupport.require(load.input().equals(COBBLE) && load.fuel().equals(COAL),
                "input-interrupt load drifted");
        WorldlineSmokeAwait.awaitBlockMatching(actor, furnace,
                state -> state.legacyId() == 62, "burning furnace", 80);
        WorldlineSmokeAwait.awaitEntity(actor, () -> B173FurnaceInterrupt.window(actor),
                view -> view.slot(1).empty() && !view.slot(0).empty()
                        && view.slot(0).item().equals(COBBLE) && view.slot(2).empty(),
        ...
        FurnaceSmeltInterruptSupport.require(after.slot(2).empty()
                && actor.inventory().slot(inputSlot).item().equals(COBBLE),
                "input interrupt produced a completed output");
        close(actor, B173FurnaceInterrupt.active(actor));
    }

    static void fuel(B173WireClient actor, BlockPosition furnace, int inputSlot, int fuelSlot)
            throws Exception {
        open(actor, furnace, inputSlot, fuelSlot, COBBLE, COAL);
        ...
        FurnaceSmeltInterruptSupport.require(primed.slot(0).empty() && primed.slot(2).empty()
                && !primed.slot(1).empty() && primed.slot(1).item().equals(COAL)
        ...
        WorldlineSmokeAwait.observe(actor, 220);
        RemoteInventoryView after = B173FurnaceInterrupt.window(actor);
        FurnaceSmeltInterruptSupport.require(after.slot(2).empty() && after.slot(1).empty()
                && !after.slot(0).empty() && after.slot(0).item().equals(COBBLE)
                && actor.inventory().slot(fuelSlot).item().equals(COAL),
                "fuel interrupt produced a completed output");
        close(actor, B173FurnaceInterrupt.active(actor));
    }

    private static void open(B173WireClient actor, BlockPosition furnace, int inputSlot,
            int fuelSlot, RemoteItemStack input, RemoteItemStack fuel) throws Exception {
    ...
    private static void close(B173WireClient actor, RemoteContainerWindow window) {
        ...
        FurnaceSmeltInterruptSupport.require(closure.closedWindow().equals(window)
