...
/** Raised-stone pads and idle furnace 61:2 placement for interrupt arms. */
final class FurnaceSmeltInterruptSupport {
  private FurnaceSmeltInterruptSupport() {}
  ...
  static BlockPosition furnace(B173WireClient actor, BlockPosition support) throws Exception {
    ...
    require(idle.metadata() == 2 && !idle.equals(new BlockState(61, 0))
            && WorldlineSmokeAwait.observe(actor, 5).blockAt(target.x(), target.y(), target.z()).equals(idle),
        "idle furnace 61:2 drift");
    return target;
  }

  static Raised raise(B173WireClient actor, RemoteChunkSnapshot chunk, int cx, int cz) throws Exception {
    ...
    while (water(chunk.blockAt(local(top.x(), cx), top.y() + 1, local(top.z(), cz)).legacyId())) {
      ...
      require(++column <= 15, "water column exceeded furnace-smelt-interrupt fixture");
    }
  ...
  static BlockPosition foundation(RemoteChunkSnapshot chunk, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic furnace-smelt-interrupt foundation");
  }
