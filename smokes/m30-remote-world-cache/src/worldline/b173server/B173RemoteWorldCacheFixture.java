...
public final class B173RemoteWorldCacheFixture {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    cache.preChunk(event(-1, 2, true));
    cache.accept(chunk(-1, 2));
    cache.preChunk(event(3, -4, true));
    cache.accept(chunk(3, -4));
    require(cache.snapshot().loadedChunks() == 2 && cache.snapshot().containsChunk(-1, 2)
            && cache.snapshot().containsChunk(3, -4),
        "prechunk loads were not retained");
    ...
    try {
      cache.accept(chunk(-1, 2));
      throw new AssertionError("unqualified chunk was accepted");
    } catch (IOException expected) {
    }

    B173RemoteWorldCache bounded = new B173RemoteWorldCache();
    for (int index = 0; index < 256; index++)
      bounded.preChunk(event(index, 0, true));
    try {
      bounded.preChunk(event(256, 0, true));
      throw new AssertionError("cache bound was ignored");
    } catch (IOException expected) {
    }
  ...
  private static RemoteChunkSnapshot chunk(int x, int z) {
    return new RemoteChunkSnapshot(new RemoteChunkObservation(x * 16, 0, z * 16, 16, 128, 16, 1024),
        new byte[32768], new byte[16384], new byte[16384], new byte[16384]);
  }
