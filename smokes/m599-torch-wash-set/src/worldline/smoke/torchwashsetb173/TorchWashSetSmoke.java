...
public final class TorchWashSetSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        require(seed == 17320110707L && "TorchWash599".equals(user) && user.length() <= 16,
                "torch-wash identity drift");
        ...
        try {
            ...
            B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D,
                    new int[] {0, 1, 2, 3}, new int[] {1, 50, 9, 3},
            ...
            actor.moveAndObserve(top.x() + 0.5D - pose.x(), top.y() + 1.1D - pose.y(),
                    top.z() + 0.5D - pose.z(), 8);
            ...
            washed = worldline.test.WorldlineSmokeAwait.awaitBlockMatching(actor, torch,
                    state -> water(state.legacyId()), "washed torch water", 40);
            drop = actor.awaitDroppedItem(TORCH_ITEM);
            require(water(washed.legacyId()) && washed.legacyId() != 50
                    && drop.item().equals(TORCH_ITEM) && drop.item().count() == 1,
            ...
            RemoteChunkSnapshot after = reader.awaitRemoteChunk(chunkX, chunkZ)
                    .chunkAt(chunkX, chunkZ);
            BlockState persisted = after.blockAt(
                    local(torch.x(), chunkX), torch.y(), local(torch.z(), chunkZ));
            require(water(persisted.legacyId()) && persisted.legacyId() != 50,
                    "persisted washed torch drift");
            String evidence = "column=" + column + ",platform=3x3,support=" + cell(top, 1, 0)
                    + ",torch=" + cell(torch, 50, 5) + "->" + washed.legacyId() + ":"
                    + washed.metadata() + ",source=" + cell(source, 9, 0)
    ...
    private static BlockPosition place(B173WireClient actor, BlockPosition support, BlockFace face,
            BlockState state) throws Exception {
        BlockPosition target = face.adjacent(support);
        actor.placeHeldBlock(support, face);
        actor.awaitBlock(target, state);
        return target;
    }
