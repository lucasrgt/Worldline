...
public final class WorldlineRecoveryEvent {
    ...
    public static void head(Minecraft client) {
        if (!WorldlinePagedBridge.running() || WorldlinePagedBridge.sealed()) return; int x = WorldlineCensusSync.x(), y = WorldlineCensusSync.y() + 2,
        ...
        if (!removeRequested && WorldlinePagedBridge.count() >= AFTER) {
            if (AFTER != 300 || RESTORE_AFTER != 30 || !"8".equals(System.getProperty("aero.becell.rebuildsPerFrame"))) throw new IllegalStateException("M90 runtime drift");
        ...
        if (restoreRequested) { WorldlineRecoveryState.apply(client.world); blockRestored = client.world.getBlockId(x, y, z) == WorldlineCensusMod.block.id;
            beRestored = client.world.getBlockEntity(x, y, z) instanceof WorldlineCensusBlockEntity be && be.nonce() == root * 100 + 3; }
    }
    public static void tail() {
        int x = WorldlineCensusSync.x(), y = WorldlineCensusSync.y() + 2, z = WorldlineCensusSync.z(), root = WorldlineCensusProbe.nonce(), queued = Aero_BECellRenderer.queuedLastFrame();
