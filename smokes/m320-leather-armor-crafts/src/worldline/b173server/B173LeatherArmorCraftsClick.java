...
public final class B173LeatherArmorCraftsClick {
    ...
    public static int[] apply(B173WireClient actor, int leatherSlot, int[] dests) {
        try {
            ...
            if (window.descriptor().kind() != RemoteWindowKind.WORKBENCH || window.inventory().size() != 46
                    || dests == null || dests.length != 4 || leatherSlot < 9 || leatherSlot > 44)
    ...
    private static int craft(B173PlayChannel channel, B173PlayInbound inbound, int windowId, int action,
            int leatherSlot, int dest, int[] grid, int resultId) throws IOException {
        ...
        for (int index = 0; index < grid.length; index++) {
            RemoteInventoryView after = replace(inbound.activeWindow().inventory(), grid[index], item(LEATHER, 1));
    ...
    private static void requireDest(RemoteInventoryView personal, int dest, int leatherSlot) {
        if (dest < 9 || dest > 44 || dest == leatherSlot || dest == 5 || dest == 6 || dest == 7 || dest == 8
