...
public final class B173BasicCraftsClick {
    ...
    public static void planks(B173WireClient actor) {
        try {
            ...
            step(channel, inbound, 2, 1, 0, null, with(with(inbound.inventory(), 1, LOG), 0, PLANKS4), null);
            if (!same(inbound.inventory(), 0, PLANKS4))
                throw new IllegalStateException("plank result 5x4 absent from 2x2 log");
            step(channel, inbound, 3, 0, 0, PLANKS4, with(with(inbound.inventory(), 0, null), 1, null), PLANKS4);
    ...
    public static void sticks(B173WireClient actor) {
        try {
            ...
            step(channel, inbound, 7, 3, 1, null, with(with(inbound.inventory(), 3, PLANK), 0, STICKS), PLANKS2);
            if (!same(inbound.inventory(), 0, STICKS) || inbound.inventory().slot(0).item().legacyId() != 280)
            ...
            step(channel, inbound, 9, 0, 0, STICKS, with(with(with(inbound.inventory(), 0, null), 1, null), 3, null),
                    STICKS);
    ...
    public static void torches(B173WireClient actor) {
        try {
            ...
            step(channel, inbound, 14, 3, 1, null, with(with(inbound.inventory(), 3, STICK), 0, TORCHES), STICKS3);
            if (!same(inbound.inventory(), 0, TORCHES) || inbound.inventory().slot(0).item().legacyId() != 50)
            ...
            step(channel, inbound, 16, 0, 0, TORCHES, with(with(with(inbound.inventory(), 0, null), 1, null), 3, null),
                    TORCHES);
