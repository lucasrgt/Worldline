  ...
  public static void main(String[] a) throws Exception {
    if (a.length != 9)
      throw new IllegalArgumentException(
          "usage: ExtendedHeadBreakSetSmoke server.jar workspace port seed username chunkX chunkZ fixtureTicks signalTicks");
    ...
    ExtendedHeadBreakSetArm.require(
        seed == 17320110707L && user.equals("HeadBreak554") && user.length() <= 16,
        "extended-head-break identity drift");
    B173DedicatedServer server =
        new B173DedicatedServer(jar, workspace, port, seed, timeout, 3, true);
    B173WireClient actor = new B173WireClient("127.0.0.1", port, user, timeout), reader = null;
    ExtendedHeadBreakSetArm arm;
    int[] column = new int[1];
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2, 3},
          new int[] {1, 33, 69, 257}, new int[] {32, 1, 1, 1}, new int[] {0, 0, 0, 0});
      actor.connect();
      actor.synchronizePose();
      actor.look(-90F, 0F);
      ExtendedHeadBreakSetArm.require(
          actor.awaitInventory().occupiedSlots() == 4, "extended-head-break inventory drift");
      RemoteChunkSnapshot initial = actor.awaitRemoteChunk(cx, cz).chunkAt(cx, cz);
      BlockPosition top = ExtendedHeadBreakSetArm.raise(actor, initial, cx, cz, column);
      ...
      ExtendedHeadBreakSetArm.require(
          settled.blockAt(arm.piston.x(), arm.piston.y(), arm.piston.z())
      ...
      String evidence = "column=" + column[0] + ",extend=33:4->12,head-break=33:12->0,piston="
          ...
          + ":69:1->9,drops=packet21-33,persisted=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=piston33-west-extended|settle=" + fixture + "+" + signal
          + "ticks|cause=packet15-lever-activate+packet14-ironpick257-base|effect=official-extended-piston-base-break+head34-removed|observation=fresh-login-packet51|"
          + evidence;
      System.out.println("WORLDLINE_M554_SET=" + evidence);
      System.out.println("WORLDLINE_M554_TRACE=" + trace);
      System.out.println("WORLDLINE_M554_SIGNATURE=" + ExtendedHeadBreakSetArm.sha(trace));
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
