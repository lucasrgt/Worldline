...
public final class ExtendedHeadBreakSetArm{
 ...
 private ExtendedHeadBreakSetArm(BlockPosition s,BlockPosition p,BlockPosition h,BlockPosition u,BlockPosition l){support=s;piston=p;head=h;pushed=u;lever=l;}
 ...
 void persist(RemoteChunkSnapshot after,int cx,int cz){
  require(at(after,piston,cx,cz).equals(new BlockState(0,0))&&at(after,head,cx,cz).equals(new BlockState(0,0))&&at(after,pushed,cx,cz).equals(new BlockState(1,0))&&at(after,lever,cx,cz).equals(new BlockState(69,9)),"fresh extended-head-break leftover drift");
 }
 static BlockPosition raise(B173WireClient a,RemoteChunkSnapshot initial,int cx,int cz,int[] column)throws Exception{
  ...
  while(water(at(initial,BlockFace.UP.adjacent(top),cx,cz).legacyId())){top=place(a,top,BlockFace.UP,1);a.moveAndObserve(0D,1D,0D,1);require(++column[0]<=15,"water column exceeded extended-head-break fixture");}
 ...
 static BlockPosition foundation(RemoteChunkSnapshot c,int cx,int cz){for(int x=4;x<=11;x++)for(int z=4;z<=11;z++)for(int y=126;y>=1;y--)if(c.blockAt(x,y,z).legacyId()==3&&water(c.blockAt(x,y+1,z).legacyId()))return new BlockPosition(cx*16+x,y,cz*16+z);throw new IllegalStateException("no deterministic extended-head-break foundation");}
