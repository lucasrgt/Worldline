...
public final class CausalWaterFlowSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    BlockPosition source, target;
    BlockState opened, settled;
    try {
      server.boot();
      B173PlayerSeed.write(workspace, username, 4.5D, 58D, 4.5D);
      actor.connect();
      PlayerPose pose = actor.synchronizePose();
      before = actor.awaitRemoteChunk(chunkX, chunkZ).chunkAt(chunkX, chunkZ);
      source = source(before, chunkX, chunkZ);
      target = new BlockPosition(source.x(), source.y() - 1, source.z());
      pose = actor.moveAndObserve(target.x() + 0.5D - pose.x(), 0D, target.z() + 0.5D - pose.z(), 3)
                 .resulting();
      while (pose.y() > target.y() + 4D) {
        MovementOutcome move = actor.moveAndObserve(0D, -1D, 0D, 1);
        pose = move.resulting();
        require(
            !move.corrected() || pose.y() <= target.y() + 5D, "descent corrected above dig range");
      }
      Thread.sleep(1000L);
      actor.beginBreak(target);
      Thread.sleep(3000L);
      actor.finishBreak(target);
      opened = actor.awaitBlock(target, new BlockState(0, 0))
                   .blockAt(target.x(), target.y(), target.z());
      RemoteWorldView live = worldline.test.WorldlineSmokeAwait.observe(actor, settleTicks);
      settled = live.blockAt(target.x(), target.y(), target.z());
      ...
      require(after.blockAt(local(target.x(), chunkX), target.y(), local(target.z(), chunkZ))
                  .equals(settled),
    ...
    BlockState prior = before.blockAt(lx, target.y(), lz),
               sourceState = before.blockAt(lx, source.y(), lz);
    require(sourceState.legacyId() == 9 && prior.legacyId() == 3 && opened.legacyId() == 0
            && water(settled.legacyId()) && delta.changed > 0,
        "causal water fixture drift");
    String evidence = "source=" + source.x() + ":" + source.y() + ":" + source.z() + ":"
        + sourceState.legacyId() + ":" + sourceState.metadata() + ",opened=" + target.x() + ":"
        + target.y() + ":" + target.z() + ":" + prior.legacyId() + ":" + prior.metadata()
        + "->0:0->" + settled.legacyId() + ":" + settled.metadata() + ",states=" + delta;
    String trace = "v1|server=official-b1.7.3|seed=" + seed + "|chunk=" + chunkX + "," + chunkZ
        + "|cause=packet14-break-below-generated-water|confirmation=packet53-air|settle="
        + settleTicks + "ticks|observation=live-packet53+fresh-login-packet51|" + evidence
        + "|disconnect=clean";
    System.out.println("WORLDLINE_M114_WATER=" + evidence);
    System.out.println("WORLDLINE_M114_TRACE=" + trace);
    ...
    for (int x = 4; x <= 11; x++)
      for (int z = 4; z <= 11; z++)
        for (int y = 126; y >= 2; y--) {
          ...
          int below = chunk.blockAt(x, y - 1, z).legacyId(),
              floor = chunk.blockAt(x, y - 2, z).legacyId();
          if (id == 9 && below == 3 && floor == 3)
    ...
    for (int x = 0; x < 16; x++)
      for (int z = 0; z < 16; z++)
        for (int y = 0; y < 128; y++) {
          ...
          if (!a.equals(b)) {
            changed++;
            if (water(b.legacyId()) && !water(a.legacyId()))
              water++;
            if (b.legacyId() == 0)
              air++;
            row.clear();
  ...
  private static final class StateDelta {
    final int changed, water, air;
    final String hash;
    StateDelta(int c, int w, int a, String h) {
      changed = c;
      water = w;
      air = a;
      hash = h;
