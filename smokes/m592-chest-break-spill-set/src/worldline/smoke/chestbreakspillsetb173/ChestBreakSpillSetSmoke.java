...
public final class ChestBreakSpillSetSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      ...
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2, 3, 4},
          new int[] {1, 54, 4, 3, 286}, new int[] {32, 1, 1, 1, 1}, new int[] {0, 0, 0, 0, 0});
      actor.connect();
      actor.synchronizePose();
      require(actor.awaitInventory().occupiedSlots() == 5 && actor.inventory().slot(40).item().equals(AXE),
      ...
      require(window.descriptor().kind() == RemoteWindowKind.CHEST && "Chest".equals(window.descriptor().title())
              && window.descriptor().containerSlots() == 27 && window.inventory().size() == 63,
      ...
      require(loadCobble.stack().equals(COBBLE) && loadDirt.stack().equals(DIRT)
              && loadDirt.after().slot(0).item().equals(COBBLE) && loadDirt.after().slot(1).item().equals(DIRT),
      ...
      require(cobble.item().equals(COBBLE) && dirt.item().equals(DIRT) && chestDrop.item().equals(CHEST)
              && cobble.entityId() != dirt.entityId() && dirt.entityId() != chestDrop.entityId()
      ...
      require(after.blockAt(local(top.x(), cx), top.y(), local(top.z(), cz)).equals(new BlockState(1, 0))
              && after.blockAt(local(chest.x(), cx), chest.y(), local(chest.z(), cz)).equals(AIR),
          "persisted chest-break-spill air drift");
      String evidence = "column=" + column + ",support=" + cell(top, 1, 0) + ",chest=" + cell(chest, 54, 0)
          + "->0:0,load=4x1+3x1,spill=packet21-4x1+packet21-3x1,chest-drop=packet21-54"
          + ",persisted=air,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed + "|fixture=raised-stone+chest54+cobble4+dirt3"
  ...
  private static RemoteDroppedItem drop(B173WireClient actor, RemoteItemStack expected) {
    ...
    if (item == null)
      item = actor.awaitDroppedItem(expected);
