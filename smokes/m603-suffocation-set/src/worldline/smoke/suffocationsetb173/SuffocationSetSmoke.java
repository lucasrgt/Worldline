...
public final class SuffocationSetSmoke {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SuffocationSetSupport.require(seed == 17320110707L && user.equals("Suffocate603")
                && user.length() <= 16, "suffocation-set identity drift");
    ...
    private static void run(B173DedicatedServer server, B173WireClient actor, Path workspace,
        ...
        B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D,
                new int[] {0, 1}, new int[] {1, 12}, new int[] {64, 8}, new int[] {0, 0}, 20);
        actor.connect();
        PlayerPose pose = actor.synchronizePose();
        SuffocationSetSupport.require(actor.awaitInventory().occupiedSlots() == 2
                && actor.awaitHealth(20) == 20, "suffocation inventory or health drift");
        ...
        while (SuffocationSetSupport.water(initial.blockAt(
                SuffocationSetSupport.local(top.x(), chunkX), top.y() + 1,
                SuffocationSetSupport.local(top.z(), chunkZ)).legacyId())) {
        ...
        BlockPosition pad = SuffocationSetSupport.place(actor,
                SuffocationSetSupport.place(actor, top, BlockFace.SOUTH, 1), BlockFace.SOUTH, 1);
        ...
        SuffocationSetSupport.require(pose.y() >= pad.y() + 0.9D,
                "pad pose drift pose=" + pose.x() + "," + pose.y() + "," + pose.z());
        ...
        SuffocationSetSupport.require(hit.healthBefore() == 20 && hit.healthAfter() == 19
                && hit.damage() == 1, "suffocation Packet38/8 drift");
        SuffocationSetSupport.require(actor.health() == 19,
                "suffocation death is m465 not hurt: " + actor.health());
        SuffocationSetSupport.require(WorldlineSmokeAwait.observe(actor, 1)
                .blockAt(head.x(), head.y(), head.z()).legacyId() == 12,
                "suffocation left sand 12 absent");
        actor.close();
        server.save();
        String evidence = "cause=suffocate,column=" + column + ",head=" + head.x() + ":"
                + head.y() + ":" + head.z() + ":12:0,body=" + body.x() + ":" + body.y()
                + ":" + body.z() + ":12:0,health=20->19,packet8=19,status=2,clients=1,"
                + "disconnect=clean";
