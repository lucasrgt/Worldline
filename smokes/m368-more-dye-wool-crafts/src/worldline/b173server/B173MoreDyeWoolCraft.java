...
/** Eighteen accepted window-0 clicks that dye white wool 35:0 with yellow, orange, and pink. */
public final class B173MoreDyeWoolCraft {
    private static final int[] WOOL_SLOTS = {36, 37, 38}, DYE_SLOTS = {39, 40, 41}, RESULTS = {4, 1, 6};
    private static final int[] CLICK_SLOTS = {36, 1, 39, 2, 0, 36, 37, 1, 40, 2, 0, 37, 38, 1, 41, 2, 0, 38};
    private static final int[] CLICK_ITEMS = {35, -1, 351, -1, 35, -1, 35, -1, 351, -1, 35, -1, 35, -1, 351, -1, 35, -1};
    private static final int[] CLICK_DAMAGE = {0, 0, 11, 0, 4, 0, 0, 0, 14, 0, 1, 0, 0, 0, 9, 0, 6, 0};

    private B173MoreDyeWoolCraft() {}
    ...
    public static void verify() throws Exception {
        ...
        if (input.available() != 0 || bytes.size() != 207)
            throw new AssertionError("more dyed-wool Packet102 sequence size drifted");
    }
    ...
    public static void apply(B173WireClient actor) {
        try {
            ...
            if (inbound.windowActive() || !inbound.cursorObserved() || inbound.cursor() != null)
                throw new IllegalStateException("more dyed-wool craft preflight failed");
            int action = 1;
            for (int index = 0; index < RESULTS.length; index++)
                action = dye(channel, inbound, WOOL_SLOTS[index], DYE_SLOTS[index], RESULTS[index], action);
            if (action != 19 || inbound.cursor() != null) throw new IllegalStateException("more dyed-wool action drift");
            RemoteInventoryView view = inbound.inventory();
            if (view.occupiedSlots() != 3 || !same(view, 36, 35, 4) || !same(view, 37, 35, 1)
                    || !same(view, 38, 35, 6) || !emptyCraft(view))
                throw new IllegalStateException("more dyed-wool results drifted");
        } catch (IOException error) {
            throw new IllegalStateException("more dyed-wool craft failed", error);
        }
    }

    private static int dye(B173PlayChannel channel, B173PlayInbound inbound, int woolSlot, int dyeSlot,
            int resultDamage, int action) throws IOException {
        ...
        if (woolSlot < 9 || dyeSlot < 9 || woolSlot > 44 || dyeSlot > 44 || inbound.cursor() != null
            ...
            throw new IllegalStateException("more dyed-wool ingredients drifted");
        ...
        step(channel, inbound, dyeSlot, dye, replace(inbound.inventory(), dyeSlot, null), dye, action++);
        ...
        step(channel, inbound, woolSlot, null, replace(inbound.inventory(), woolSlot, result), null, action++);
        if (!same(inbound.inventory(), woolSlot, result) || !emptyCraft(inbound.inventory())
                || inbound.cursor() != null)
            throw new IllegalStateException("more dyed-wool store drifted");
        return action;
    }
    ...
    private static void packet(DataInputStream input, int slot, int action, int item, int damage)
        ...
        if (input.readUnsignedByte() != 102 || input.readUnsignedByte() != 0 || input.readShort() != slot
            ...
            throw new AssertionError("more dyed-wool Packet102 fields drifted");
        if (item >= 0 && (input.readUnsignedByte() != 1 || input.readShort() != damage))
            throw new AssertionError("more dyed-wool Packet102 stack drifted");
    }
