package worldline.smoke.crosschunkdoorb173;
...
/** Opens an official iron door from a lever placed across an east chunk seam. */
public final class CrossChunkIronDoorSmoke {
  private CrossChunkIronDoorSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 10)
      throw new IllegalArgumentException(
          "usage: CrossChunkIronDoorSmoke server.jar workspace port seed username doorX leverX chunkZ fixtureTicks signalTicks");
    ...
    String user = a[4];
    require(leverX == doorX + 1, "M128 requires east seam");
    ...
    try {
      ...
      actor.look(-90F, 0F);
      require(actor.awaitInventory().occupiedSlots() == 3, "seam door inventory drift");
      ...
      require(
          bottom.x() == doorX * 16 + 15 && lever.x() == leverX * 16, "door fixture missed seam");
      ...
      require(bottomOff.equals(new BlockState(71, 0)) && topOff.equals(new BlockState(71, 8)),
          "seam door off drift: " + bottomOff + " / " + topOff);
      ...
      require(leverOn.equals(new BlockState(69, 9)) && bottomOn.equals(new BlockState(71, 4))
          ...
          "cross-chunk door opening absent: " + leverOn + " / " + bottomOn + " / " + topOn);
      actor.close();
      ...
      left1 = after.chunkAt(doorX, cz);
      right1 = after.chunkAt(leverX, cz);
      require(left1.blockAt(15, bottom.y(), bottom.z() - cz * 16).equals(bottomOn)
              && left1.blockAt(15, top.y(), top.z() - cz * 16).equals(topOn)
              && right1.blockAt(0, lever.y(), lever.z() - cz * 16).equals(leverOn),
          "fresh seam door drift");
    } finally {
      actor.close();
      if (reader != null)
        reader.close();
    ...
    require(left.changed == 2 && right.changed == 1,
        "seam door changed unrelated states " + left + " / " + right);
    String evidence = "column=" + column + ",lever=" + lever.x() + ":" + lever.y() + ":" + lever.z()
        ...
        + ",top=" + topOff.metadata() + "->" + topOn.metadata() + ",left=" + left
        + ",right=" + right;
    String trace = "v1|server=official-b1.7.3|seed=" + seed
        ...
        + "ticks|cause=packet15-lever-activate-neighbor|effect=packet53-two-block-iron-door-open-source-chunk|observation=fresh-packet51-both-chunks|"
        + evidence + "|disconnect=clean";
    System.out.println("WORLDLINE_M128_DOOR=" + evidence);
    System.out.println("WORLDLINE_M128_TRACE=" + trace);
    System.out.println("WORLDLINE_M128_SIGNATURE=" + sha(trace));
  }
  private static BlockPosition foundation(RemoteChunkSnapshot q, int cx, int cz) {
    ...
    throw new IllegalStateException("no seam door foundation");
  }
