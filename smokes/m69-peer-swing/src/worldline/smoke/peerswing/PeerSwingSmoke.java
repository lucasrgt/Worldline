...
public final class PeerSwingSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    PeerSwingSession observer = client(port, observerName, timeout),
                     actor = client(port, actorName, timeout);
    ExecutorService executor = Executors.newSingleThreadExecutor();
    RemoteSwingRequest request;
    RemotePeerSwing swing;
    try {
      server.boot();
      server.save();
      ...
      B173PlayerSeed.writeInventory(workspace, actorName, x, y, z, new int[] {0}, new int[] {276},
          new int[] {1}, new int[] {0});
      ...
      request = actor.swingHeldItem();
      swing = pending.get(20, TimeUnit.SECONDS);
      require(request.username().equals(actorName) && request.entityId() == actor.state().entityId()
              ...
              && request.animation() == 1 && swing.animation() == 1,
          "swing evidence drifted");
      actor.close();
      observer.close();
      awaitPlayers(server, 0);
      server.save();
      require(server.player(actorName).inventoryItems() == 1, "actor sword persistence drifted");
    } finally {
    ...
    System.out.println(
        "WORLDLINE_M69_SWING=actor=packet18-animation1,peer=named-packet18-animation1,held=sword276:0");
