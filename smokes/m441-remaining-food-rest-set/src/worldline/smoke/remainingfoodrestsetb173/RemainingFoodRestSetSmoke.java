...
public final class RemainingFoodRestSetSmoke{
 ...
 private static void eat(B173WireClient a,int slot,RemoteItemStack item,RemoteItemStack leftover,int from,int to)throws Exception{a.selectHeldSlot(slot);require(a.health()==from&&a.inventory().slot(36+slot).item().equals(item),"pre-eat "+item.legacyId()+" drift health="+a.health());a.look(0F,0F);a.useSelectedItemInAir();worldline.test.WorldlineSmokeAwait.observe(a,20);require(a.awaitHealth(to)==to,"eat "+item.legacyId()+" health drift health="+a.health());worldline.test.WorldlineSmokeAwait.awaitEntity(a,a::inventory,v->leftover==null?v.slot(36+slot).empty():!v.slot(36+slot).empty()&&v.slot(36+slot).item().equals(leftover),"food slot transition",40);if(leftover==null)require(a.inventory().slot(36+slot).empty()&&a.health()==to,"eat "+item.legacyId()+" consume drift");else require(a.inventory().slot(36+slot).item().equals(leftover)&&a.health()==to,"eat "+item.legacyId()+" leftover drift");}
