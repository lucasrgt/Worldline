...
public final class MilkBucketSmoke{
 ...
 public static void main(String[]a)throws Exception{
  if(a.length!=7)throw new IllegalArgumentException("usage: MilkBucketSmoke server.jar workspace port seed username chunkX chunkZ");
  ...
  B173DedicatedServer server=new B173DedicatedServer(jar,workspace,port,seed,timeout,3,true);B173WireClient actor=new B173WireClient("127.0.0.1",port,user,timeout),reader=null;RemoteItemStack milk=new RemoteItemStack(335,1,0),empty=new RemoteItemStack(325,1,0);BlockPosition top,cell;int column;PlayerPose pose;
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1},new int[]{1,335},new int[]{48,1},new int[]{0,0},20);actor.connect();pose=actor.synchronizePose();require(actor.awaitInventory().occupiedSlots()==2&&actor.awaitInventory().slot(37).item().equals(milk),"milk inventory drift");require(actor.awaitHealth(20)==20,"seeded milk health drift");RemoteChunkSnapshot initial=actor.awaitRemoteChunk(cx,cz).chunkAt(cx,cz);top=foundation(initial,cx,cz);column=0;actor.selectHeldSlot(0);
 ...
 private static BlockPosition foundation(RemoteChunkSnapshot q,int cx,int cz){for(int x=4;x<=11;x++)for(int z=4;z<=11;z++)for(int y=126;y>=1;y--)if(q.blockAt(x,y,z).legacyId()==3&&water(q.blockAt(x,y+1,z).legacyId()))return new BlockPosition(cx*16+x,y,cz*16+z);throw new IllegalStateException("no deterministic milk foundation");}
