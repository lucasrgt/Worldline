...
/** Common server-safe registration for the exact M94 remove/restore protocol. */
public final class WorldlineRecoveryMod {
    ...
    @EventListener private static void messages(MessageListenerRegistryEvent event) {
        ...
        event.register(RESTORE, (player, packet) -> { if (!player.world.isRemote) throw new IllegalStateException("M94 restore reached server"); WorldlineRecoveryState.restore(packet.ints); });
    }
}
