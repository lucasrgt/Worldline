...
public final class WindowLifecycleSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    PersonalCraftingSession actor = client(port, actorName, timeout),
                            observer = client(port, observerName, timeout);
    RemoteContainerWindow window;
    RemoteWindowClosure closure;
    RemotePersonalTransaction take, restore;
    ...
    try {
      ...
      require(chest.legacyId() == 54
              && worldline.test.WorldlineSmokeAwait.observe(observer, 5)
                  ...
                  .equals(chest),
          "placed lifecycle chest diverged");
      window = actor.openChest(target, BlockFace.UP);
      require(window.inventory().size() == 63 && window.inventory().occupiedSlots() == 0,
          "lifecycle chest open drifted");
      closure = actor.closeWindow();
      require(closure.closedWindow().equals(window) && closure.proofAction() == 1
              && closure.personalBefore().equals(closure.personalAfter()),
          "personal-window closure proof drifted");
      ...
      restore = actor.clickPersonalSlot(36);
      worldline.test.WorldlineSmokeAwait.observe(actor, 5);
      observer.awaitPeerHeldItem(new RemoteHeldItem(actorName, 1, 0));
      require(take.actionId() == 2 && restore.actionId() == 3
              && restore.after().slot(36).item().equals(stone),
          "post-close personal transaction drifted");
      actor.close();
      observer.close();
    ...
    System.out.println("WORLDLINE_M58_WINDOW=id=" + window.descriptor().windowId()
        + ";proof=" + closure.proofAction() + ";actions=" + take.actionId() + ","
        + restore.actionId() + ";items=" + player.inventoryItems());
  ...
  private static PlayerPose settle(PersonalCraftingSession client) {
