...
import java.nio.charset.StandardCharsets;import java.nio.file.*;import java.security.MessageDigest;import java.time.Duration;import java.util.*;import worldline.api.*;import worldline.b173server.*;
...
public final class WaterCobbleSetSmoke{
 ...
 public static void main(String[]a)throws Exception{
  if(a.length!=9)throw new IllegalArgumentException("usage: WaterCobbleSetSmoke server.jar workspace port seed username chunkX chunkZ fixtureTicks flowTicks");
  ...
  require(seed==17320110707L&&user.equals("WatCob415")&&user.length()<=16,"water-cobble-set identity drift");
  ...
  BlockState air=new BlockState(0,0),cobble=new BlockState(4,0);int column;
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1,2,3,4},new int[]{1,1,3,11,9},new int[]{64,64,4,2,2},new int[]{0,0,0,0,0});actor.connect();PlayerPose pose=actor.synchronizePose();require(actor.awaitInventory().occupiedSlots()==5,"water-cobble-set inventory drift");
   RemoteChunkSnapshot initial=actor.awaitRemoteChunk(cx,cz).chunkAt(cx,cz);BlockPosition top=foundation(initial,cx,cz);column=0;actor.selectHeldSlot(0);
   while(water(initial.blockAt(local(top.x(),cx),top.y()+1,local(top.z(),cz)).legacyId())){top=place(actor,top,BlockFace.UP,1);pose=actor.moveAndObserve(0D,1D,0D,1).resulting();require(++column<=15,"water column exceeded water-cobble-set fixture");}for(int lift=0;lift<8;lift++){top=place(actor,top,BlockFace.UP,1);pose=actor.moveAndObserve(0D,1D,0D,1).resulting();column++;}
   actor.selectHeldSlot(1);BlockPosition row=top;for(int step=0;step<2;step++){row=place(actor,row,BlockFace.SOUTH,1);pose=actor.moveAndObserve(0D,0D,1D,1).resulting();}BlockPosition[] east=trench(actor,row);
   for(int step=0;step<2;step++){row=place(actor,row,BlockFace.SOUTH,1);pose=actor.moveAndObserve(0D,0D,1D,1).resulting();}BlockPosition[] south=trench(actor,row);
   ...
   require(peC.equals(cobble)&&psC.equals(cobble),"fresh cobble drift "+peC+" / "+psC);require(lavaSource(peS)&&lavaSource(psS),"fresh source drift "+peS+" / "+psS);require(water(peW.legacyId())&&water(psW.legacyId()),"fresh water drift "+peW+" / "+psW);
   String evidence="column="+column+",east-source="+east[0].x()+":"+east[0].y()+":"+east[0].z()+":11:0,east-flow="+east[1].x()+":"+east[1].y()+":"+east[1].z()+":3:0->0:0->"+eastFlow.legacyId()+":"+eastFlow.metadata()+"->4:0,east-water="+east[2].x()+":"+east[2].y()+":"+east[2].z()+":9:0,south-source="+south[0].x()+":"+south[0].y()+":"+south[0].z()+":11:0,south-flow="+south[1].x()+":"+south[1].y()+":"+south[1].z()+":3:0->0:0->"+southFlow.legacyId()+":"+southFlow.metadata()+"->4:0,south-water="+south[2].x()+":"+south[2].y()+":"+south[2].z()+":9:0,flowing-lava=10,moving=10:0,water=9,cobble=4,persisted=true,clients=2,disconnect=clean";
   String trace="v1|server=official-b1.7.3|seed="+seed+"|fixture=raised-stone-paired-trenches+seeded-still-lava11+dirt-gates3|settle="+fixtureTicks+"+"+flowTicks+"ticks|cause=packet14-open-flow-cells-then-packet15-still-water9-beside-flowing-lava|wire=packet53-air+packet53-flowing-lava10-as-11:2+packet53-water9+packet53-cobble4|oracle=two-cell-flowing-lava10-plus-water-to-cobble4-not-obsidian49|"+evidence;
   System.out.println("WORLDLINE_M415_SET="+evidence);System.out.println("WORLDLINE_M415_TRACE="+trace);System.out.println("WORLDLINE_M415_SIGNATURE="+sha(trace));
  }finally{actor.close();if(reader!=null)reader.close();try{server.close();}catch(RuntimeException e){if(e.getMessage()==null||(!e.getMessage().contains("pipe")&&!e.getMessage().contains("PIPE")&&!e.getMessage().contains("EOF")))throw e;}}
 }
 private static BlockPosition[] trench(B173WireClient a,BlockPosition top)throws Exception{
  BlockPosition west=place(a,top,BlockFace.WEST,1),east=place(a,top,BlockFace.EAST,1),east2=place(a,east,BlockFace.EAST,1),east3=place(a,east2,BlockFace.EAST,1);
  ...
  for(BlockPosition p:floor){int dx=p.x()-top.x(),dz=p.z()-top.z();if(dx==-1||dx==3||dz==-1||dz==1)place(a,p,BlockFace.UP,1);}
 ...
 private static BlockPosition foundation(RemoteChunkSnapshot q,int cx,int cz){for(int x=4;x<=11;x++)for(int z=4;z<=11;z++)for(int y=126;y>=1;y--)if(q.blockAt(x,y,z).legacyId()==3&&water(q.blockAt(x,y+1,z).legacyId()))return new BlockPosition(cx*16+x,y,cz*16+z);throw new IllegalStateException("no deterministic water-cobble-set foundation");}
