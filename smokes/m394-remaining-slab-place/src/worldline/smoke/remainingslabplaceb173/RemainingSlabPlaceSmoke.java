...
public final class RemainingSlabPlaceSmoke{
 ...
 public static void main(String[]a)throws Exception{
  ...
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1,2,3,4},new int[]{1,44,44,44,43},new int[]{32,1,1,1,1},new int[]{0,1,2,3,0});actor.connect();actor.synchronizePose();
   ...
   RemoteWorldView live=worldline.test.WorldlineSmokeAwait.observe(actor,5);require(live.blockAt(sandstone.x(),sandstone.y(),sandstone.z()).equals(ss)&&live.blockAt(wood.x(),wood.y(),wood.z()).equals(wd)&&live.blockAt(cobble.x(),cobble.y(),cobble.z()).equals(cb)&&live.blockAt(dbl.x(),dbl.y(),dbl.z()).equals(ds),"live remaining-slab-place drift");
