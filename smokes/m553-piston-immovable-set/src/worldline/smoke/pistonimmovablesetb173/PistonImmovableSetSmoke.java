...
public final class PistonImmovableSetSmoke{
 ...
 public static void main(String[]a)throws Exception{
  if(a.length!=9)throw new IllegalArgumentException("usage: PistonImmovableSetSmoke server.jar workspace port seed username chunkX chunkZ fixtureTicks signalTicks");
  ...
  PistonImmovableSetArm chest,furnace,spawner;int[] column=new int[1];
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1,2,3,4,5},new int[]{1,33,54,61,52,69},new int[]{32,3,1,1,1,3},new int[]{0,0,0,0,0,0});actor.connect();actor.synchronizePose();actor.look(-90F,0F);PistonImmovableSetArm.require(actor.awaitInventory().occupiedSlots()==6,"piston-immovable inventory drift");
   RemoteChunkSnapshot initial=actor.awaitRemoteChunk(cx,cz).chunkAt(cx,cz);BlockPosition top=PistonImmovableSetArm.raise(actor,initial,cx,cz,column);
   chest=PistonImmovableSetArm.place(actor,initial,top,cx,cz,1,2,54,5);
   actor.selectHeldSlot(0);actor.moveAndObserve(2D,0D,0D,2);BlockPosition furnaceSupport=PistonImmovableSetArm.padSouth(actor,top);
   ...
   actor.moveAndObserve(0D,0D,-4D,4);chest.pulse(actor,signal);actor.moveAndObserve(0D,0D,2D,2);furnace.pulse(actor,signal);actor.moveAndObserve(0D,0D,2D,2);spawner.pulse(actor,signal);
   actor.close();PistonImmovableSetArm.awaitPlayers(server,0);server.save();
   ...
   chest.persist(after,cx,cz);furnace.persist(after,cx,cz);spawner.persist(after,cx,cz);
   String evidence="column="+column[0]+",chest="+chest.token()+",furnace="+furnace.token()+",spawner="+spawner.token()+",piston33=4->4,chest-arm="+chest.arm()+",furnace-arm="+furnace.arm()+",spawner-arm="+spawner.arm()+",retracted=true,persisted=true,clients=2,disconnect=clean";
   String trace="v1|server=official-b1.7.3|seed="+seed+"|fixture=piston33-west+chest54+furnace61+spawner52|settle="+fixture+"+"+signal+"ticks|cause=packet15-lever-activate|effect=official-piston33-immovable-set|observation=fresh-login-packet51|"+evidence;
   System.out.println("WORLDLINE_M553_SET="+evidence);System.out.println("WORLDLINE_M553_TRACE="+trace);System.out.println("WORLDLINE_M553_SIGNATURE="+PistonImmovableSetArm.sha(trace));
