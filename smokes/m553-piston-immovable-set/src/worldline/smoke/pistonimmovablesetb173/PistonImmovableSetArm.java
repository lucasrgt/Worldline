...
public final class PistonImmovableSetArm {
  final BlockPosition support, piston, payload, destination, lever;
  final BlockState payloadState;
  private PistonImmovableSetArm(BlockPosition s, BlockPosition p, BlockPosition u, BlockPosition d,
      BlockPosition l, BlockState payloadState) {
    support = s;
    piston = p;
    payload = u;
    destination = d;
  ...
  static PistonImmovableSetArm place(B173WireClient a, RemoteChunkSnapshot initial,
      BlockPosition support, int cx, int cz, int pistonSlot, int payloadSlot, int payloadId,
      int leverSlot) throws Exception {
    BlockPosition piston = BlockFace.UP.adjacent(support),
                  ...
                  lever = BlockFace.EAST.adjacent(support);
    require(at(initial, piston, cx, cz).legacyId() == 0
        ...
        "piston 33 payload " + payloadId + " targets were not initial air");
    a.look(-90F, 0F);
    a.selectHeldSlot(pistonSlot);
    a.placeHeldBlock(support, BlockFace.UP);
    worldline.test.WorldlineSmokeAwait.awaitBlock(a, piston, new BlockState(33, 4), 5);
    a.selectHeldSlot(payloadSlot);
    a.placeHeldBlock(piston, BlockFace.WEST);
    BlockState payloadState = worldline.test.WorldlineSmokeAwait.awaitBlockMatching(
        a, payload, s -> s.legacyId() == payloadId, "immovable payload " + payloadId, 5);
    a.selectHeldSlot(leverSlot);
  ...
  RemoteWorldView pulse(B173WireClient a, int ticks) throws Exception {
    ...
    RemoteWorldView live = worldline.test.WorldlineSmokeAwait.awaitWorld(a,
        ...
        -> v.blockAt(lever.x(), lever.y(), lever.z()).equals(new BlockState(69, 9))
            ...
            && v.blockAt(destination.x(), destination.y(), destination.z())
                .equals(new BlockState(0, 0)),
        "immovable " + payloadState.legacyId() + " rejection", ticks);
    BlockState pistonLive = live.blockAt(piston.x(), piston.y(), piston.z()),
               payloadLive = live.blockAt(payload.x(), payload.y(), payload.z()),
               destLive = live.blockAt(destination.x(), destination.y(), destination.z());
    require(live.blockAt(lever.x(), lever.y(), lever.z()).equals(new BlockState(69, 9))
            && pistonLive.equals(new BlockState(33, 4)) && payloadLive.equals(payloadState)
            && destLive.equals(new BlockState(0, 0)),
        "immovable " + payloadState.legacyId()
            + " rejection absent: lever=" + live.blockAt(lever.x(), lever.y(), lever.z()) + "/"
            + pistonLive + "/" + payloadLive + "/" + destLive);
    return live;
  }
  void persist(RemoteChunkSnapshot after, int cx, int cz) {
    require(at(after, lever, cx, cz).equals(new BlockState(69, 9))
            && at(after, piston, cx, cz).equals(new BlockState(33, 4))
            && at(after, payload, cx, cz).equals(payloadState)
            && at(after, destination, cx, cz).equals(new BlockState(0, 0)),
        "fresh immovable " + payloadState.legacyId() + " rejection drift");
  }
  String token() {
    return payloadState.legacyId() + ":" + payloadState.metadata() + "->" + payloadState.legacyId()
        + ":" + payloadState.metadata();
  }
  String arm() {
    return cell(piston) + ":33:4->4,payload=" + cell(payload) + ":" + token()
        + ",dest=" + cell(destination) + ":0:0->0:0";
  }
  static BlockPosition raise(B173WireClient a, RemoteChunkSnapshot initial, int cx, int cz,
    ...
    while (water(at(initial, BlockFace.UP.adjacent(top), cx, cz).legacyId())) {
      ...
      require(++column[0] <= 15, "water column exceeded piston-immovable fixture");
    }
    top = place(a, top, BlockFace.UP, 1);
  ...
  static BlockPosition padSouth(B173WireClient a, BlockPosition from) throws Exception {
    a.selectHeldSlot(0);
    BlockPosition support = place(a, from, BlockFace.SOUTH, 1);
    support = place(a, support, BlockFace.SOUTH, 1);
    a.moveAndObserve(0D, 0D, 2D, 2);
    return support;
  }
  ...
  static BlockPosition foundation(RemoteChunkSnapshot c, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic piston-immovable foundation");
  }
