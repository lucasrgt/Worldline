...
public final class B173SpawnerDelay {
    ...
    private static byte[] replace(byte[] raw, short value) {
        ...
        int found = 0;
        int from = 0;
        while (from + NAME.length + 2 <= next.length) {
            int at = indexOf(next, from);
            if (at < 0) break;
        ...
        if (found < 1) throw new IllegalStateException("MobSpawner Delay absent");
        return next;
    }

    private static int indexOf(byte[] raw, int from) {
        int i = from;
        outer: while (i + NAME.length + 2 <= raw.length) {
            int j = 0;
            while (j < NAME.length) {
                if (raw[i + j] != NAME[j]) {
                    i++;
