...
public final class UndeadSunBurnSetSmoke {
    ...
    public static void main(String[] args) throws Exception {
        ...
        UndeadSunBurnSetSupport.require(seed == 17320110707L && user.equals("SunBurn619")
                && user.length() <= 16, "undead-sun-burn identity drift");
        int[] openColumn = new int[1];
        int[] coverColumn = new int[1];
        BlockPosition open = arm(jar, workspace.resolve("open"), port, seed, user, chunkX, chunkZ,
                false, openColumn);
        BlockPosition cover = arm(jar, workspace.resolve("cover"), port + 1, seed, user, chunkX,
                chunkZ, true, coverColumn);
        String evidence = "column=" + openColumn[0] + ",platform=7x7-48grass,open="
                + UndeadSunBurnSetSupport.cell(open) + ",cover="
                + UndeadSunBurnSetSupport.cell(cover)
    ...
    private static BlockPosition arm(Path jar, Path workspace, int port, long seed, String user,
            int chunkX, int chunkZ, boolean roofed, int[] column) throws Exception {
        Duration timeout = Duration.ofSeconds(180);
        BlockPosition spawner;
        B173DedicatedServer server = B173DedicatedServer.monsters(jar, workspace, port, seed,
                timeout, 3, true);
        B173WireClient actor = new B173WireClient("127.0.0.1", port, user, timeout);
        try {
            server.boot();
            B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D,
                    new int[] {0, 1, 2}, new int[] {1, 2, 52}, new int[] {64, 48, 1},
                    new int[] {0, 0, 0});
            actor.connect();
            actor.synchronizePose();
            UndeadSunBurnSetSupport.require(actor.awaitInventory().occupiedSlots() == 3,
                    "undead-sun-burn inventory drift");
            RemoteChunkSnapshot initial = actor.awaitRemoteChunk(chunkX, chunkZ)
                    .chunkAt(chunkX, chunkZ);
            BlockPosition top = UndeadSunBurnSetSupport.raise(actor, initial, chunkX, chunkZ,
                    column);
            UndeadSunBurnSetSupport.grassPlatform(actor, top);
            actor.selectHeldSlot(2);
            spawner = UndeadSunBurnSetSupport.place(actor, top, BlockFace.UP, 52);
            if (roofed) UndeadSunBurnSetSupport.centerRoof(actor, top);
        ...
        B173PlayerSeed.writeInventory(workspace, user, spawner.x() + 0.5D, spawner.y() + 1.1D,
                spawner.z() + 0.5D, new int[] {0, 1, 2}, new int[] {1, 2, 52},
    ...
    private static void observe(B173DedicatedServer server, B173WireClient actor,
            BlockPosition spawner, boolean roofed) throws Exception {
        server.boot();
        actor.connect();
        actor.synchronizePose();
        UndeadSunBurnSetSupport.require(actor.awaitInventory().occupiedSlots() >= 1,
                "undead-sun-burn reload inventory drift");
        server.setTime(14000L);
        RemoteMobSpawn spawn = UndeadSunBurnSetSupport.awaitPad(actor, spawner, roofed ? 2.1D : 3.5D);
        UndeadSunBurnSetSupport.require(spawn.legacyType() == 54
                && spawn.entityId() != actor.state().entityId() && spawn.legacyType() != 90
                && (spawn.flags() & 1) == 0, "undead Packet24 identity drift");
        WorldlineSmokeAwait.observe(actor, 10);
        UndeadSunBurnSetSupport.require(!B173UndeadSunBurn.onFire(actor, spawn),
                "undead Packet40 fire present at night");
        server.setTime(6000L);
        if (roofed) {
            WorldlineSmokeAwait.observe(actor, 80);
            UndeadSunBurnSetSupport.require(!B173UndeadSunBurn.onFire(actor, spawn),
                    "covered daylight Packet40 fire flag present");
        } else {
            B173UndeadSunBurn.awaitFire(actor, spawn);
            UndeadSunBurnSetSupport.require(B173UndeadSunBurn.onFire(actor, spawn),
                    "open daylight Packet40 fire flag absent");
        }
        actor.close();
        UndeadSunBurnSetSupport.awaitPlayers(server, 0);
        server.save();
