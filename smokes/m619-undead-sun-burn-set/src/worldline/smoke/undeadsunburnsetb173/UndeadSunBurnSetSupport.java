...
final class UndeadSunBurnSetSupport {
  ...
  static void centerRoof(B173WireClient actor, BlockPosition top) throws Exception {
    ...
    while (row < 5) {
      ...
      if (row > 0) {
        cursor = place(actor, new BlockPosition(start.x(), start.y(), start.z() + row - 1), BlockFace.SOUTH, 1);
      }
      int col = 0;
      while (col < 4) {
  ...
  private static void ring(B173WireClient actor, BlockPosition center, int radius, int id) throws Exception {
    int z = -radius + 1;
    while (z < radius) {
      place(actor, new BlockPosition(center.x() - radius + 1, center.y(), center.z() + z), BlockFace.WEST, id);
      place(actor, new BlockPosition(center.x() + radius - 1, center.y(), center.z() + z), BlockFace.EAST, id);
      z++;
    }
    int x = -radius + 1;
    while (x < radius) {
      place(actor, new BlockPosition(center.x() + x, center.y(), center.z() - radius + 1), BlockFace.NORTH, id);
      place(actor, new BlockPosition(center.x() + x, center.y(), center.z() + radius - 1), BlockFace.SOUTH, id);
      x++;
    }
    place(actor, new BlockPosition(center.x() - radius, center.y(), center.z() - radius + 1), BlockFace.NORTH, id);
    place(actor, new BlockPosition(center.x() - radius, center.y(), center.z() + radius - 1), BlockFace.SOUTH, id);
    place(actor, new BlockPosition(center.x() + radius, center.y(), center.z() - radius + 1), BlockFace.NORTH, id);
    place(actor, new BlockPosition(center.x() + radius, center.y(), center.z() + radius - 1), BlockFace.SOUTH, id);
  }
