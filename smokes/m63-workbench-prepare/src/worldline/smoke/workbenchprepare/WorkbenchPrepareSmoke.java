...
public final class WorkbenchPrepareSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        try {
            ...
            BlockPosition target = BlockFace.UP.adjacent(support); int workbench = find(actor.inventory(),
            ...
            actor.selectHeldSlot(1); require(actor.openWorkbench(target, BlockFace.UP).inventory().slot(37)
                    .item().equals(planks), "workbench plank mapping drifted");
            result = actor.prepareWorkbenchSlabs(36); RemoteItemStack one = new RemoteItemStack(5, 1, 0);
            require(result.takeAction() == 1 && result.firstAction() == 2 && result.secondAction() == 3
                    && result.thirdAction() == 4 && result.twoWide().slot(0).item()
                    .equals(new RemoteItemStack(72, 1, 0)) && result.prepared().slot(0).item()
                    .equals(new RemoteItemStack(44, 3, 2)) && result.prepared().slot(1).item().equals(one)
                    && result.prepared().slot(2).item().equals(one) && result.prepared().slot(3).item().equals(one)
                    && result.oneWide().slot(1).item().equals(one) && result.cursorAfterTake().count() == 3
                    && result.cursorAfterFirst().count() == 2 && result.cursorAfterSecond().count() == 1
                    && result.cursorEmptyAfterThird() && actor.inventory().equals(result.personalAfter()),
                    "workbench preparation drifted");
            boolean closeRejected = false; try { actor.closeWindow(); }
            catch (IllegalStateException expected) { closeRejected = true; }
        ...
        System.out.println("WORLDLINE_M63_PREPARE=actions=" + result.takeAction() + "," + result.firstAction()
                + "," + result.secondAction() + "," + result.thirdAction() + ";two="
