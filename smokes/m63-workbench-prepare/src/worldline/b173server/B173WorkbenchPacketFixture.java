...
public final class B173WorkbenchPacketFixture {
    ...
    public static void verify() throws Exception {
        ...
        for (int slot = 1; slot <= 3; slot++) B173ContainerPacket.write(output, 7, slot, 1, slot + 1, null);
        DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes.toByteArray()));
        for (int slot = 1; slot <= 3; slot++) if (input.readUnsignedByte() != 102
                || input.readUnsignedByte() != 7 || input.readShort() != slot
                || input.readUnsignedByte() != 1 || input.readShort() != slot + 1
                || input.readBoolean() || input.readShort() != -1)
