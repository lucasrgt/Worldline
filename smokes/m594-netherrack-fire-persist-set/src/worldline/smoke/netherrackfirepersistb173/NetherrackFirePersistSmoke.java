...
public final class NetherrackFirePersistSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2, 3}, new int[] {1, 87, 5, 259},
      ...
      require(id(early, rack) == 87 && id(early, netherFire) == 51, "netherrack fire lost before long window");
      ...
      require(cellId(after, chunkX, chunkZ, rack) == 87 && nether == 51 && plankCell != 51 && plankFlame != 51,
          "fresh login netherrack persist or plank expiry drift nether=" + nether + " planks=" + plankCell
              + " flame=" + plankFlame);
      String evidence = "column=" + column + ",support=" + cell(top) + ":1:0,rack=" + cell(rack)
          + ":87:0,flint=259,nether-fire=" + cell(netherFire) + ":51,planks=" + cell(planks) + ":5,plank-fire="
          + cell(plankFire) + ":expired,hold=2400,netherrack-persist=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed + "|fixture=raised-netherrack87+planks5+flintsteel259"
          + "|cause=packet15-item259+long-observation-window"
          + "|wire=packet53-fire51-netherrack-persist+plank-fire-expired"
          + "|oracle=netherrack-fire-persist-not-stone-ignition-not-support-extinguish|" + evidence;
  ...
  private static int cellId(RemoteChunkSnapshot chunk, int chunkX, int chunkZ, BlockPosition position) {
    return chunk.blockAt(local(position.x(), chunkX), position.y(), local(position.z(), chunkZ)).legacyId();
  }
