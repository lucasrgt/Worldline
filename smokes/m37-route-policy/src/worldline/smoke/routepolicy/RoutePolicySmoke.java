...
public final class RoutePolicySmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      ...
      route =
          client.moveRoute(Arrays.asList(new MovementStep(.125D, 0D, 0D, 5),
                               new MovementStep(block.x() + .5D - accepted.x(),
      ...
      require(after.containsChunk(chunkX, chunkZ), "stopped route lost cache");
      client.close();
      ...
      require(close(player.x(), accepted.x()) && close(player.y(), accepted.y())
              && close(player.z(), accepted.z()),
          "unexecuted third step changed persisted pose");
    } finally {
      client.close();
      server.close();
    }
    System.out.println(
        "WORLDLINE_M37_API=route-policy,stop-on-correction,no-retry,cache,persistence");
    ...
    System.out.println(
        "WORLDLINE_M37_PERSISTED=" + player.x() + "," + player.y() + "," + player.z());
