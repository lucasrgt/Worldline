...
public final class WorldlineLabSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    require(
        restored.fingerprint().equals(replayed.fingerprint()), "fresh restore and replay diverged");
    B173Comparison branches = B173Experiments.compare(
        checkpoint, runtime -> {}, runtime -> runtime.tap(B173Keys.SLOT_1 + 4), 1);
    ...
    CanonicalStateTrace trace = new CanonicalStateTrace(SEED, "checkpointTick", "events",
        "restoreSlot", "baselineSlot", "alternativeSlot", "guiSlots", "baseBlock65", "modBlock65");
    trace.record("lab", checkpoint.tick(), checkpoint.eventCount(), restored.selectedSlot(),
        branches.baseline().selectedSlot(), branches.alternative().selectedSlot(), slots, blocks[0],
        blocks[1]);
  ...
  private static int[] mod(B173Checkpoint checkpoint, Path jar) throws Exception {
    ...
    try {
      baseline.tick();
      base = baseline.observe().blockColumn()[1];
    } finally {
      baseline.close();
    }
    URL url = jar.toUri().toURL();
    try (URLClassLoader loader =
             new URLClassLoader(new URL[] {url}, B173Mod.class.getClassLoader())) {
      Class<? extends B173Mod> type =
          Class.forName("worldline.benchmark.ProbeMod", true, loader).asSubclass(B173Mod.class);
