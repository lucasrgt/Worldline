...
public final class InventoryObservationSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    RemoteInventoryView initial, updated;
    ...
    try {
      ...
      initial = client.awaitInventory();
      require(initial.windowId() == 0 && initial.size() == 45 && initial.occupiedSlots() == 0,
          "initial player inventory drifted");
      client.look(pose.yaw(), 90F);
      acquire(client, username, 1, 1);
      updated = client.inventory();
      ...
      require(updated.windowId() == 0 && updated.size() == 45 && updated.occupiedSlots() == 1
          ...
          "authoritative inventory delta drifted: window=" + updated.windowId()
              + ",size=" + updated.size() + ",occupied=" + updated.occupiedSlots()
              + ",slot36=" + (updated.slot(36).empty() ? "empty" : updated.slot(36).item()));
      client.close();
    ...
    System.out.println(
        "WORLDLINE_M48_API=inventory,immutable,window,slot-delta,server-authoritative");
    System.out.println("WORLDLINE_M48_INITIAL=window=" + initial.windowId()
        + ";slots=" + initial.size() + ";occupied=" + initial.occupiedSlots());
  ...
  private static void acquire(
      InventoryMultiplayerSession client, String username, int item, int occupied) {
