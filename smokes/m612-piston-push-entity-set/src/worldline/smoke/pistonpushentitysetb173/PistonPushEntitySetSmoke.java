...
public final class PistonPushEntitySetSmoke {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length != 9)
      throw new IllegalArgumentException("usage: PistonPushEntitySetSmoke server.jar workspace port seed username "
    ...
    PistonPushEntitySetArm.require(
        seed == 17320110707L && user.equals("PistEnt612") && user.length() <= 16, "piston-push-entity identity drift");
    ...
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2, 3}, new int[] {1, 33, 69, 4},
          new int[] {32, 1, 1, 1}, new int[] {0, 0, 0, 0});
      actor.connect();
      actor.synchronizePose();
      actor.look(-90F, 0F);
      PistonPushEntitySetArm.require(actor.awaitInventory().occupiedSlots() == 4, "piston-push-entity inventory drift");
      ...
      PistonPushEntitySetArm.require(
          actor.peekDroppedItem(COBBLE) != null && actor.peekDroppedItem(COBBLE).entityId() == spawn.entityId(),
      ...
      String evidence = "column=" + column[0] + ",extend=33:4->12,head=0:0->34:4,item=4x1:0,dx-west=true,piston="
          + PistonPushEntitySetArm.cell(arm.piston) + ":33:4->12,head-cell=" + PistonPushEntitySetArm.cell(arm.head)
          + ":0:0->34:4,pushed=" + PistonPushEntitySetArm.cell(arm.pushed)
          + ":0:0,persisted=true,clients=2,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed + "|fixture=piston33-west+air-head+cobble-item4+floors"
