...
public final class PistonPushEntitySetArm {
  ...
  private PistonPushEntitySetArm(BlockPosition support, BlockPosition piston, BlockPosition head, BlockPosition pushed,
      BlockPosition floor, BlockPosition landing, BlockPosition stand, BlockPosition lever, PlayerPose pose) {
  ...
  static PistonPushEntitySetArm place(B173WireClient actor, RemoteChunkSnapshot initial, int cx, int cz, int[] column)
      throws Exception {
    ...
    require(at(initial, piston, cx, cz).legacyId() == 0 && at(initial, head, cx, cz).legacyId() == 0
            && at(initial, pushed, cx, cz).legacyId() == 0 && at(initial, lever, cx, cz).legacyId() == 0,
  ...
  RemoteDroppedItem dropCobble(B173WireClient actor) throws Exception {
    ...
    require(spawn.item().equals(PistonPushEntitySetSmoke.COBBLE) && spawn.item().count() == 1 && spawn.x() >= head.x()
            && spawn.x() < head.x() + 1 && spawn.z() >= head.z() && spawn.z() < head.z() + 1,
  ...
  void installPiston(B173WireClient actor) throws Exception {
    ...
    BlockState placed = worldline.test.WorldlineSmokeAwait.awaitBlock(actor, piston, new BlockState(33, 4), 5)
                            .blockAt(piston.x(), piston.y(), piston.z());
    require(placed.equals(new BlockState(33, 4)), "west piston 33 absent: " + placed + " at " + cell(piston));
  ...
  RemoteWorldView extend(B173WireClient actor, int ticks) throws Exception {
    ...
    require(live.blockAt(lever.x(), lever.y(), lever.z()).equals(new BlockState(69, 9))
        ...
        "piston 33 entity-push extend absent: " + live.blockAt(piston.x(), piston.y(), piston.z()) + "/"
  ...
  static void displaced(RemoteDroppedItem spawn, RemoteDroppedItem after) {
    require(after.item().equals(PistonPushEntitySetSmoke.COBBLE) && after.item().count() == 1
            && after.x() <= spawn.x() - 0.5D && after.x() < spawn.x() && after.x() < Math.floor(spawn.x()),
  ...
  static PlayerPose walk(B173WireClient actor, PlayerPose pose, double x, double y, double z) {
    ...
    require((x - pose.x()) * (x - pose.x()) + (z - pose.z()) * (z - pose.z()) < 0.36D,
        "walk to " + x + ":" + y + ":" + z + " failed at " + pose.x() + ":" + pose.y() + ":" + pose.z());
    return pose;
  }
  ...
  static String fixed(RemoteDroppedItem item) {
    return Math.round(item.x() * 32D) + ":" + Math.round(item.y() * 32D) + ":" + Math.round(item.z() * 32D);
  }
