...
public final class RouteTerminationSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    MovementRouteExecution stopped, exhausted;
    ...
    try {
      ...
      stopped =
          client.moveRouteWithFallbackExecution(Arrays.asList(safe, blocked, later), event -> {
      ...
      exhausted = client.moveRouteWithFallbackExecution(
          Collections.singletonList(alternative(0D, 0D, .125D)), event -> {
            exhaustedEvents.add(event);
      ...
      require(after.containsChunk(chunkX, chunkZ), "summarized route lost cache");
      client.close();
      awaitPlayers(server, Collections.emptyList());
      server.save();
      player = server.player(username);
      PlayerPose finalPose = exhausted.result().finalPose();
    ...
    System.out.println(
        "WORLDLINE_M41_API=route-execution,immutable,terminal-event,exhausted,controller-stop");
    ...
    System.out.println(
        "WORLDLINE_M41_PERSISTED=" + player.x() + "," + player.y() + "," + player.z());
  ...
  private static boolean terminal(
      MovementRouteExecution execution, List<MovementRouteEvent> events, MovementAttemptKind kind) {
    MovementRouteEvent event = events.get(events.size() - 1);
    return execution.terminalEvent() == event && event.kind() == kind
        && event.outcome() == execution.result().outcomes().get(event.outcomeIndex());
  }
  private static String describe(MovementRouteEvent event) {
    return event.alternativeIndex() + ":" + event.outcomeIndex() + ":" + event.kind();
  }
