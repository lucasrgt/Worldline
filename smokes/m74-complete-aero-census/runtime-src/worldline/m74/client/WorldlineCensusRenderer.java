...
public final class WorldlineCensusRenderer extends BlockEntityRenderer {
  ...
  @Override
  public void render(BlockEntity raw, double x, double y, double z, float partialTick) {
    WorldlineCensusBlockEntity be = (WorldlineCensusBlockEntity) raw;
    if (be.world.getBlockId(be.x, be.y, be.z) != WorldlineCensusMod.block.id)
      throw new IllegalStateException("M74 rendered block drift");
    if (be.nonce() <= 0)
      return;
    Aero_BECellRenderer.queueAtRest(MODEL, "/terrain.png", be, x, y, z, 0F,
        be.world.method_1782(be.x, be.y + 1, be.z), Aero_RenderOptions.DEFAULT);
