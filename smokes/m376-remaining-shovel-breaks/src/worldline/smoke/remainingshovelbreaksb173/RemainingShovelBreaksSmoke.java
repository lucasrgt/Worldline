...
public final class RemainingShovelBreaksSmoke{
 private static final RemoteItemStack CLAY_BALL=new RemoteItemStack(337,1,0),SNOWBALL=new RemoteItemStack(332,1,0),SOUL=new RemoteItemStack(88,1,0),SHOVEL=new RemoteItemStack(284,1,0);
 private static final BlockState AIR=new BlockState(0,0);
 private RemainingShovelBreaksSmoke(){}
 public static void main(String[]a)throws Exception{
  if(a.length!=7)throw new IllegalArgumentException("usage: RemainingShovelBreaksSmoke server.jar workspace port seed username chunkX chunkZ");
  ...
  require(seed==17320110707L&&user.equals("Shovel376")&&user.length()<=16,"remaining-shovel-breaks identity drift");B173DedicatedServer server=new B173DedicatedServer(jar,workspace,port,seed,timeout,3,true);B173WireClient actor=new B173WireClient("127.0.0.1",port,user,timeout);BlockPosition top;int column;String clay,snow,snowblock,soulsand;
  try{server.boot();B173PlayerSeed.writeInventory(workspace,user,4.5D,60D,4.5D,new int[]{0,1,2,3,4,5},new int[]{1,82,78,80,88,284},new int[]{32,1,1,1,1,1},new int[]{0,0,0,0,0,0});actor.connect();actor.synchronizePose();RemoteInventoryView inventory=actor.awaitInventory();require(inventory.occupiedSlots()==6&&inventory.slot(41).item().equals(SHOVEL),"remaining-shovel-breaks inventory drift");RemoteChunkSnapshot initial=actor.awaitRemoteChunk(cx,cz).chunkAt(cx,cz);top=foundation(initial,cx,cz);column=0;actor.selectHeldSlot(0);
   while(water(initial.blockAt(local(top.x(),cx),top.y()+1,local(top.z(),cz)).legacyId())){top=place(actor,top,BlockFace.UP,1);actor.moveAndObserve(0D,1D,0D,1);require(++column<=15,"water column exceeded remaining-shovel-breaks fixture");}for(int lift=0;lift<8;lift++){top=place(actor,top,BlockFace.UP,1);actor.moveAndObserve(0D,1D,0D,1);column++;}
   clay=harvest(actor,top,1,5,82,CLAY_BALL);snow=harvest(actor,top,2,5,78,SNOWBALL);snowblock=harvest(actor,top,3,5,80,SNOWBALL);soulsand=harvest(actor,top,4,5,88,SOUL);
   String evidence="column="+column+",support="+top.x()+":"+top.y()+":"+top.z()+":1:0,"+clay+","+snow+","+snowblock+","+soulsand+",shovel=284,clients=1,disconnect=clean";String trace="v1|server=official-b1.7.3|seed="+seed+"|fixture=raised-stone+clay82+snow78+snowblock80+soulsand88|cause=packet14-goldshovel284|wire=packet53-air+packet21-id337+packet21-id332+packet21-id88|oracle=remaining-shovel-breaks-drops+cells-82-78-80-88-to-0|"+evidence;System.out.println("WORLDLINE_M376_SHOVEL="+evidence);System.out.println("WORLDLINE_M376_TRACE="+trace);System.out.println("WORLDLINE_M376_SIGNATURE="+sha(trace));
 ...
 private static BlockPosition foundation(RemoteChunkSnapshot q,int cx,int cz){for(int x=4;x<=11;x++)for(int z=4;z<=11;z++)for(int y=126;y>=1;y--)if(q.blockAt(x,y,z).legacyId()==3&&water(q.blockAt(x,y+1,z).legacyId()))return new BlockPosition(cx*16+x,y,cz*16+z);throw new IllegalStateException("no deterministic remaining-shovel-breaks foundation");}
