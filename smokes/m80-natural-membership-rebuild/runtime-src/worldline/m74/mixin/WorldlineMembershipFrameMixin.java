...
public abstract class WorldlineMembershipFrameMixin {
    @Shadow @Final private Minecraft client;@Inject(method="onFrameUpdate(F)V",at=@At("HEAD"))private void head(float d,CallbackInfo c){WorldlineMembershipEvent.head(client);}
    @Inject(method="onFrameUpdate(F)V",at=@At("TAIL"))private void tail(float d,CallbackInfo c){WorldlineMembershipEvent.tail();if(WorldlineFrameCensus.sealed()&&!WorldlineMembershipFile.written())WorldlineMembershipFile.write();}
}
