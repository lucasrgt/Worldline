...
public final class WorldlineMembershipMod {
    public static final Identifier CHANGE=Identifier.of(WorldlineCensusMod.NAMESPACE,"membership_change");static{EntrypointManager.registerLookup(MethodHandles.lookup());}public WorldlineMembershipMod(){}
    @EventListener private static void messages(MessageListenerRegistryEvent event){event.register(CHANGE,(player,packet)->{if(player.world.isRemote)WorldlineMembershipState.ack(packet.ints);else WorldlineMembershipServer.remove(player,packet.ints);});}
}
