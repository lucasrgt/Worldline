...
public final class WorldlinePagedTimer {
  ...
  static void arm() {
    if (armed)
      throw new IllegalStateException("duplicate M103 arm");
    if (!"false".equals(System.getProperty("aero.becell.pages"))
      ...
      throw new IllegalStateException("M103 runtime drift");
    armed = true;
  }
  public static void head() {
    if (!armed || WorldlinePagedBridge.sealed() || !WorldlinePagedBridge.running())
      return;
    if (capture)
      throw new IllegalStateException("M103 HEAD without TAIL");
  ...
  public static void tail() {
    ...
    if (rendererStart != 0 || queueStart != 0 || flushStart != 0)
      throw new IllegalStateException("unterminated M103 span");
    int i = WorldlinePagedBridge.count();
    if (i >= CAP || rendererCount > 0xffff || queueCount > 0xffff || flushCount > 0xffff)
      throw new IllegalStateException("M103 counter overflow");
  ...
  private static long begin(long old, String label) {
    if (old != 0)
      throw new IllegalStateException("recursive M103 " + label);
    return System.nanoTime();
  }
  private static long end(long start, String label) {
    if (start == 0)
      throw new IllegalStateException("unstarted M103 " + label);
    long v = System.nanoTime() - start;
    if (v < 0)
      throw new IllegalStateException("negative M103 " + label);
    return v;
  }
