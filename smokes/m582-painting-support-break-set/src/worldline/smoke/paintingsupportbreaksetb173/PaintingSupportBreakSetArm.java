...
public final class PaintingSupportBreakSetArm{
 ...
 static BlockPosition raise(B173WireClient a,RemoteChunkSnapshot initial,int cx,int cz,int[] column)throws Exception{
  ...
  while(water(at(initial,BlockFace.UP.adjacent(top),cx,cz).legacyId())){top=place(a,top,BlockFace.UP,1);a.moveAndObserve(0D,1D,0D,1);require(++column[0]<=15,"water column exceeded painting-support-break fixture");}
 ...
 static BlockPosition[] wall(B173WireClient a,BlockPosition top)throws Exception{
  BlockPosition support=place(a,top,BlockFace.EAST,1),south=place(a,support,BlockFace.UP,1),north=place(a,south,BlockFace.SOUTH,1),southTop=place(a,south,BlockFace.UP,1);place(a,north,BlockFace.UP,1);
  return new BlockPosition[]{south,southTop,north};
 }
 static void harvest(B173WireClient a,BlockPosition support)throws Exception{
  a.selectHeldSlot(2);a.beginBreak(support);worldline.test.WorldlineSmokeAwait.observe(a,20);a.finishBreak(support);a.awaitBlock(support,new BlockState(0,0));
 }
 static RemoteDroppedItem drop(B173WireClient a,RemoteItemStack expected){
  return worldline.test.WorldlineSmokeAwait.awaitEntity(a,()->a.peekDroppedItem(expected),value->value!=null&&value.item().equals(expected),"painting Packet21 drop",40);
 }
 static int gone(B173WireClient a,int entity){
  return worldline.test.WorldlineSmokeAwait.awaitEntity(a,()->B173PaintingAccess.peekDestroy(a,entity),value->value!=null,"painting Packet29 destroy",160).intValue();
 }
 ...
 static BlockPosition foundation(RemoteChunkSnapshot c,int cx,int cz){for(int x=4;x<=11;x++)for(int z=4;z<=11;z++)for(int y=126;y>=1;y--)if(c.blockAt(x,y,z).legacyId()==3&&water(c.blockAt(x,y+1,z).legacyId()))return new BlockPosition(cx*16+x,y,cz*16+z);throw new IllegalStateException("no deterministic painting-support-break foundation");}
