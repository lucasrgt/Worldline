...
public final class DroppedItemSpawnSmoke {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try {
      ...
      actor.connect();
      PlayerPose actorPose = actor.synchronizePose();
      ...
      requireSpawn(dropped, actorPose);
    ...
    System.out.println(
        "WORLDLINE_M51_EMPTY=local=" + after.slot(36).empty() + ";peer=" + empty.empty());
  ...
  private static void requireSpawn(RemoteDroppedItem item, PlayerPose actor) {
    ...
    require(
        item.entityId() > 0 && Math.abs(dx) <= 1D && dy >= -1D && dy <= 3D && Math.abs(dz) <= 1D,
        "dropped-item position was not near actor");
