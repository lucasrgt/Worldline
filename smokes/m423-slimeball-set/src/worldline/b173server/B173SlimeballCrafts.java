...
public final class B173SlimeballCrafts {
    ...
    public static int apply(B173WireClient actor) {
        try { return new B173SlimeballCrafts(actor).run(); }
        catch (IOException error) { throw new IllegalStateException("slimeball sticky craft failed", error); }
    }
    ...
    private int run() throws IOException {
        ...
        if (!has(STICKY)) throw new IllegalStateException("sticky piston 29 craft result drifted");
        return STICKY;
    }
    ...
    private void finish(RemoteItemStack result) throws IOException {
        ...
        int dest = empty();
        step(dest, 0, null, replace(window(), dest, result), replace(personal(), dest - 1, result), null);
    }
