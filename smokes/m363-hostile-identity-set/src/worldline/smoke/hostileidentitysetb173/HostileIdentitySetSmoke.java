package worldline.smoke.hostileidentitysetb173;
...
/** Places two official spawners, retargets EntityId to Zombie and Skeleton, and observes Packet24 types 54 and 51. */
public final class HostileIdentitySetSmoke {
  private HostileIdentitySetSmoke() {
  }
  public static void main(String[] a) throws Exception {
    if (a.length != 7)
      throw new IllegalArgumentException(
          "usage: HostileIdentitySetSmoke server.jar workspace port seed username chunkX chunkZ");
    ...
    try {
      ...
      require(actor.awaitInventory().occupiedSlots() == 4, "hostile-identity inventory drift");
      ...
      while (
        ...
        require(++column <= 15, "water column exceeded hostile-identity fixture");
      }
    ...
    try {
      ...
      actor.synchronizePose();
      require(
          actor.awaitInventory().occupiedSlots() >= 1, "hostile-identity reload inventory drift");
      server.setTime(14000L);
      RemoteMobSpawn zombie = actor.awaitMobSpawn(54), skeleton = actor.awaitMobSpawn(51);
      require(zombie.legacyType() == 54 && skeleton.legacyType() == 51
              ...
              && zombie.entityId() != skeleton.entityId(),
          "hostile Packet24 identity drift");
      require(zombie.legacyType() != 90 && skeleton.legacyType() != 90,
          "hostile identity collapsed to M141 pig");
      actor.close();
      awaitPlayers(server, 0);
      server.save();
      String evidence = "column=" + column + ",platform=7x7-48grass,spawners=" + cell(first) + "+"
          + cell(second)
          + ",entityid=Zombie+Skeleton,mobs=type54+type51,night=14000,clients=1,disconnect=clean";
      String trace = "v1|server=official-b1.7.3|seed=" + seed
          + "|fixture=raised-7x7-grass-platform+two-spawner52|cause=nbt-entityid-zombie+skeleton+time-14000|wire=packet24-type54+packet24-type51|oracle=two-hostile-packet24-identities|"
          + evidence;
      System.out.println("WORLDLINE_M363_HOSTILE=" + evidence);
      System.out.println("WORLDLINE_M363_TRACE=" + trace);
      System.out.println("WORLDLINE_M363_SIGNATURE=" + sha(trace));
    } finally {
      actor.close();
      server.close();
    }
  ...
  private static BlockPosition foundation(RemoteChunkSnapshot q, int cx, int cz) {
    ...
    throw new IllegalStateException("no deterministic hostile-identity foundation");
  }
