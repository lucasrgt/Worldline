...
public final class BowShotDurabilitySetSmoke {
  ...
  public static void main(String[] a) throws Exception {
    ...
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2}, new int[] {1, 261, 262},
          new int[] {32, 1, 1}, new int[] {0, 0, 0});
      actor.connect();
      actor.synchronizePose();
      require(actor.awaitInventory().occupiedSlots() == 3 && id(actor.inventory(), 37) == 261
              && damage(actor.inventory(), 37) == 0 && id(actor.inventory(), 38) == 262
              && count(actor.inventory(), 38) == 1,
      ...
      require(arrow.type() == 60 && (arrow.throwerId() == actor.state().entityId() || arrow.throwerId() == 0),
      ...
      require(held(after, 261).equals(bow) && find(after, 262) == null, "persisted held-stack bow durability drift");
      ...
      String trace = "v1|server=official-b1.7.3|seed=" + seed + "|fixture=raised-stone-platform+bow261+arrow262"
          + "|cause=packet15-air-bow261|wire=packet23-type60+packet103-" + bow.legacyId() + ":" + bow.damage()
  ...
  private static RemoteItemStack remaining(B173WireClient a) {
    return worldline.test.WorldlineSmokeAwait.awaitEntity(a,
        ()
            -> find(a.inventory(), 261),
        item -> item != null && item.legacyId() == 261 && item.count() == 1, "held 261 remaining after shot", 40);
  }
