...
public final class WorldlineFireSupportOracle {
    ...
    private static void run(CanonicalTrace trace, String label, int fixture) {
        dj world = new dj(new OracleFireMemorySaveHandler(SEED, "worldline-smoke"),
                "worldline-smoke", SEED, null);
        world.r.setSeed(SEED);
        preload(world);
        if (fixture == UNSUPPORTED) removeSupport(world);
        require(world.b(X, SUPPORT_Y + 1, Z, na.as.bn, 0),
                "fire placement did not change the cell");
        if (fixture == UNSUPPORTED) require(fireId(world) == 0,
                "unsupported fire survived placement");
        else require(fireId(world) == na.as.bn, "supported fire was not placed");
        snapshot(trace, label + "-seed", world);
        if (fixture == LOST) removeSupport(world);
        snapshot(trace, label + "-action", world);
        for (int tick = 1; tick <= TICKS; tick++) {
            world.h();
            snapshot(trace, label + "-tick" + tick, world);
        }
        if (fixture == RETAINED) require(supportId(world) == na.u.bn
                && fireId(world) == na.as.bn, "supported fire did not remain");
        else require(supportId(world) == 0 && fireId(world) == 0,
    ...
    private static void snapshot(CanonicalTrace trace, String label, dj world) {
        trace.record(label, world.m(), 0, new int[] {supportId(world),
                world.c(X, SUPPORT_Y, Z), fireId(world), world.c(X, SUPPORT_Y + 1, Z)});
    }
    private static int supportId(dj world) { return world.a(X, SUPPORT_Y, Z); }
    private static int fireId(dj world) { return world.a(X, SUPPORT_Y + 1, Z); }
