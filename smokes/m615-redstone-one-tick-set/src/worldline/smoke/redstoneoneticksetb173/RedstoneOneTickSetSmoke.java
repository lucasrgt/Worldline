...
public final class RedstoneOneTickSetSmoke {
  ...
  public static void main(String[] args) throws Exception {
    ...
    RedstoneOneTickSetArm.require(
        seed == 17320110707L && user.equals("OneTick615") && user.length() <= 16, "redstone-one-tick identity drift");
    ...
    try {
      server.boot();
      B173PlayerSeed.writeInventory(workspace, user, 4.5D, 60D, 4.5D, new int[] {0, 1, 2, 3}, new int[] {1, 29, 4, 69},
          new int[] {32, 1, 1, 1}, new int[] {0, 0, 0, 0});
      actor.connect();
      actor.synchronizePose();
      actor.look(-90F, 0F);
      RedstoneOneTickSetArm.require(actor.awaitInventory().occupiedSlots() == 4, "redstone-one-tick inventory drift");
      RemoteChunkSnapshot initial = actor.awaitRemoteChunk(cx, cz).chunkAt(cx, cz);
      RedstoneOneTickSetArm arm = RedstoneOneTickSetArm.place(actor, initial, cx, cz, column);
      arm.idle(worldline.test.WorldlineSmokeAwait.awaitBlock(actor, arm.piston, RedstoneOneTickSetArm.PISTON, 8),
          "one-tick idle drift");
      ...
      String trace = "v1|server=official-b1.7.3|seed=" + seed + "|fixture=west-sticky29+east-wall-lever69:1"
