...
public final class CorrelatedBatchSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        try {
            ...
            CorrelatedMovementRoutePlan first = new CorrelatedMovementRoutePlan(firstCorrelation,
                    Collections.singletonList(alternative(.125D, 0D, 0D)));
            CorrelatedMovementRoutePlan second = new CorrelatedMovementRoutePlan(secondCorrelation,
                    Collections.singletonList(alternative(0D, 0D, .125D)));
            batch = client.moveCorrelatedRouteBatch(Arrays.asList(first, second), event -> {
                events.add(event); return MovementRouteDirective.CONTINUE;
            ...
            require(batch.executions().size() == 1
                    ...
                    && batch.finalExecution().execution().termination() == MovementRouteTermination.EXHAUSTED,
            ...
            after = worldline.test.WorldlineSmokeAwait.observe(client,1); require(after.containsChunk(chunkX, chunkZ), "batch lost cache");
            ...
            require(close(player.x(), finalPose.x()) && close(player.y(), finalPose.y())
                    && close(player.z(), finalPose.z()), "batch pose was not persisted");
        } finally { client.close(); server.close(); }
        System.out.println("WORLDLINE_M43_API=correlated-batch,bounded,sequential,per-route-termination,stop");
        ...
        System.out.println("WORLDLINE_M43_PERSISTED=" + player.x() + "," + player.y() + "," + player.z());
